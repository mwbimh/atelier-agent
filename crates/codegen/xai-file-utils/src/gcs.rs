...
use atelier_auth::{AuthCredentialProvider, StaticAuthCredentialProvider};

use crate::storage_client::{Auth401AttributionCallback, StaticAtelierAuth, StorageClient};
...
/// `StaticAtelierAuth` carrying the inline user / deployment keys from
...
fn build_proxy_client_with_fallback(
    ...
    let provider = credentials.unwrap_or_else(|| {
        let mut creds = StaticAtelierAuth::new(Some(user_token.to_owned()));
