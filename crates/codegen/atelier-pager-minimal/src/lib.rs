...
//! # Wiring
//!
//! `atelier-pager` (the lib) does **not** depend on this crate — that would be
...
//! seam ([`atelier_pager::minimal_hook`]) of function pointers, and the
//! composition-root binary (`atelier-pager-bin`) calls [`install`] once at
...
/// Register the minimal-mode render hooks with `atelier-pager`.
///
...
/// subsequent calls are ignored (see [`atelier_pager::minimal_hook`]).
pub fn install() {
    atelier_pager::minimal_hook::install(atelier_pager::minimal_hook::MinimalHooks { draw });
}
