...
use atelier_pager::app::app_view::AppView;
...
pub fn pump_transcript(app: &mut AppView) {
    ...
    let appearance = super::commit::committed_appearance(&app.appearance);
    {
        ...
        let prev_thinking = atelier_pager::appearance::cache::load_show_thinking_blocks();
        ...
        atelier_pager::appearance::cache::set_show_thinking_blocks(prev_thinking);
    }
...
fn finish_transcript(app: &mut AppView, id: atelier_pager::app::agent::AgentId, out: String) {
    if out.is_empty() {
        if let Some(agent) = app.agents.get_mut(&id) {
            agent
                .scrollback
                .push_block(atelier_pager::scrollback::block::RenderBlock::system(
    ...
    let path =
        std::env::temp_dir().join(format!("atelier-transcript-{}.ansi", uuid::Uuid::new_v4()));
...
mod tests {
    use super::*;
    use atelier_pager::scrollback::block::RenderBlock;
