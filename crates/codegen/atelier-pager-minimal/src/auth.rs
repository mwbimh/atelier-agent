...
use atelier_pager::app::app_view::AuthState;
    ...
    match hint {
        MinimalAuthHint::SigningIn { url, code } => {
            y = put_line(
                ...
                y,
                bottom,
                Line::from(Span::styled("Sign in to Atelier", bold)),
            );
...
mod tests {
    ...
    fn device_user_code_parses_verification_url() {
        assert_eq!(
            device_user_code("https://accounts.atelier/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH")
        );
        assert_eq!(
            device_user_code("https://accounts.atelier/oauth2/device"),
            None
        );
    ...
    fn auth_hint_maps_auth_state() {
        use atelier_pager::app::app_view::AuthMode;

        // Device flow → SigningIn carrying the URL and the parsed code.
        let st = AuthState::Authenticating {
            request_seq: 1,
            handle: None,
            auth_url: Some("https://accounts.atelier/device?user_code=ABCD-EFGH".into()),
        ...
        match minimal_auth_hint(&st) {
            MinimalAuthHint::SigningIn { url, code } => {
                assert_eq!(
                    url.as_deref(),
                    Some("https://accounts.atelier/device?user_code=ABCD-EFGH")
                );
    ...
    fn render_auth_shows_url_and_code() {
        ...
        let hint = MinimalAuthHint::SigningIn {
            url: Some("https://accounts.atelier/device?user_code=ABCD-EFGH".into()),
        ...
        assert!(text.contains("Sign in to Atelier"), "header: {text:?}");
        assert!(text.contains("accounts.atelier/device"), "url: {text:?}");
