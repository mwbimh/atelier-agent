...
- **Using Atelier**
  ...
  - [SSH Passthrough](#ssh-passthrough-atelier-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`atelier inspect`)](#introspection)
...
- [Building with Atelier](#building-with-atelier) — headless API, ACP SDK integration
...
Credentials are stored in `~/.atelier/auth.json` and persist across sessions. Tokens expire after 7 days; Atelier will prompt you to re-authenticate when needed.
...
**3. Run `atelier`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.atelier/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Atelier is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
4. **stdout** → captured by Atelier and saved to `~/.atelier/auth.json` as the access token
5. exit 0 → success; exit non-zero → Atelier falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Atelier reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Atelier to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Atelier when to proactively refresh. Without it, Atelier assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Atelier captures this)
...
export ATELIER_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export ATELIER_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Atelier cannot detect expiry proactively and will only refresh after a 401.

The command is run via `sh -c`, so it can be a binary path, a shell script, or a pipeline.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with atelier.invalid". In headless mode (`atelier -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=atelier-cli")
...
When Atelier needs to refresh an expired token, it re-runs your binary with `ATELIER_AUTH_EXPIRED=1` set in the environment. Your binary can use this to take a faster silent-refresh path:
...
`ATELIER_AUTH_EXPIRED` is optional — if your binary ignores it, Atelier still works. It just runs the same flow for both login and refresh.

### Automatic Credential Refresh

Atelier supports automatic credential refresh for external auth providers and OIDC. When Atelier detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Atelier handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Atelier re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Atelier re-runs the binary and retries the request once.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Atelier silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Atelier refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export ATELIER_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `atelier login` before starting — Atelier runs your binary automatically on first launch. You _can_ run `atelier login` to explicitly hydrate `auth.json` ahead of time if you prefer.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Atelier tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
atelier --debug-file /tmp/atelier-auth.log -p "hello"
tail -f /tmp/atelier-auth.log
```
...
If you've authenticated with `atelier login`, you can use the stored credentials to call the CLI chat proxy directly via curl. The proxy requires specific headers that mirror what the atelier CLI sends internally:

```bash
curl -s -N -X POST "https://cli-chat-proxy.atelier.com/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $(jq -r '."https://accounts.atelier/sign-in".key' ~/.atelier/auth.json)" \
  -H "X-XAI-Token-Auth: atelier-cli" \
  -H "x-atelier-model-override: atelier-build" \
  -d '{
    "model": "atelier-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.atelier/auth.json` (set by `atelier login`)                                                                                                                              |
| `X-XAI-Token-Auth: atelier-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-atelier-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `atelier-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `atelier-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `atelier login` to refresh.
...
atelier --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
atelier --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
atelier --always-approve --prompt "Format all files"
```
...
Run Atelier non-interactively from the command line. Use headless mode when you need to:
...
atelier -p "Refactor the code" --deny "Edit(/etc/**)"
...
atelier --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
atelier -p "What does this project do?"

# Use a specific model
atelier -p "Optimize this function" -m atelier-build

# Get JSON output for parsing
atelier -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
atelier -p "Explain the architecture" --output-format streaming-json
...
atelier -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
atelier -p "Continue where we left off" -c

# Run in a different directory
atelier -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
atelier -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
atelier -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
git diff --staged | atelier -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  atelier -p "Migrate $file from CommonJS to ES modules." --always-approve
done
...
Run Atelier as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `atelier-build`)                           |
...
## SSH Passthrough (`atelier ssh`)

Use `atelier ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.

```bash
# Basic usage (same args as ssh)
atelier ssh user@host

# With SSH flags
atelier ssh -t user@host
atelier ssh -L 8080:localhost:8080 user@host

# With remote command
atelier ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `atelier ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Atelier can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `atelier -p` and parses JSON output.
...
class AtelierChat {
  ...
  async create(
    ...
    { model = "atelier-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("atelier", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await AtelierACPChat(cwd=".").init()
...
Atelier implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Atelier reads configuration from `~/.atelier/config.toml`. If the file doesn't exist, Atelier uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: ATELIER_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: ATELIER_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: ATELIER_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: ATELIER_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: ATELIER_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: ATELIER_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: ATELIER_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: ATELIER_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `ATELIER_TELEMETRY_BUILD_EVENTS_URL`, `ATELIER_TELEMETRY_BUILD_EVENTS_API_KEY`, and `ATELIER_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Atelier can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Atelier language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Atelier can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Atelier can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Atelier looks for server definitions in:

- project config: `<repo>/.atelier/lsp.json`
- user config: `~/.atelier/lsp.json`
...
- LSP tools are enabled (`ATELIER_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
ATELIER_LSP_TOOLS=1 atelier
```
...
If LSP tools are enabled but no usable server config is found, Atelier emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Atelier does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `ATELIER_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
With this config, `atelier` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Atelier reads these files and appends their contents to the system prompt.

Atelier scans for agent rules in this order:

1. `~/.atelier/` (global rules)
...
Within each directory, Atelier checks for these filenames:
...
Skills are reusable prompt packages that extend Atelier with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Atelier discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.atelier/skills/`           | Local | Highest  |
| `<repo_root>/.atelier/skills/` | Repo  | Medium   |
| `~/.atelier/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.atelier/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Atelier decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Atelier invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Atelier automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Atelier discovers agent definitions from `.atelier/agents/` (project), `~/.atelier/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `ATELIER_AGENT` env var
4. Default `atelier-build` agent
...
```bash
atelier --agent-profile ./my-agent.md
# or
export ATELIER_AGENT="my-custom-agent"
```
...
```bash
export ATELIER_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "atelier-build"              # route explore to a lighter model
```
...
# instructions_file = ".atelier/personas/concise.md"  # or load from file
```

Both are also discovered from `.atelier/roles/*.toml` and `.atelier/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Atelier with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.atelier/plugins/`            | Project |
| `~/.atelier/plugins/`          | User    |
...
Atelier discovers hooks from `.atelier/hooks/` in the project directory. Manage them with:
...
**Credential resolution order:** `api_key` → `env_key` → `XAI_API_KEY`. If neither `api_key` nor `env_key` is set, Atelier falls back to the global `XAI_API_KEY` environment variable.

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Atelier falls back to built-in defaults for known models.
...
[model.atelier-4.20-0309-reasoning]
temperature = 0.5
api_key = "sk-custom"
```

**How it works:** When you override a built-in model, Atelier starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `ATELIER_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `atelier models` output). You also need a `[model.*]` entry so Atelier knows
...
>
> [model.my-custom-model]              # 2. tell Atelier how to reach it
...
Point Atelier at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `ATELIER_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `ATELIER_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export ATELIER_MODELS_BASE_URL="https://api.acme.com/v1"
export XAI_API_KEY="xai-..."
atelier
```

Atelier fetches the model list from `{ATELIER_MODELS_BASE_URL}/models` on startup and sends inference requests to `ATELIER_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `ATELIER_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.atelier/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Atelier uses API key auth (`Authorization: Bearer`) instead of session auth. `atelier login` is not required — only the API key.

---

## MCP Servers

Extend Atelier's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.atelier/config.toml`:
...
MCP servers can also be configured per-project in `.atelier/config.toml`. Atelier walks from the current directory up to the git repo root, loading `.atelier/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.atelier/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.atelier/config.toml` | This repository   | ↑        |
| `<cwd>/.atelier/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.atelier/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.atelier/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.atelier/config.toml`. Other config sections (models, shortcuts, etc.) are only read from `~/.atelier/config.toml`.
...
> **Experimental:** requires `--experimental-memory` (or `ATELIER_MEMORY=1` / `[memory] enabled = true` in config).

Cross-session memory lets Atelier remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.atelier/memory/`:
- **Global** (`~/.atelier/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.atelier/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.atelier/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries
...
At the end of each session, Atelier saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.atelier/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Atelier searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
atelier memory edit --global

# Show memory statistics: file count, chunk count, and index size
atelier memory stats
```

### Configuration reference

Key options under `[memory]` in `~/.atelier/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.atelier/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Atelier emits the `atelier-shell-memory_injection`
...
Atelier can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.atelier/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.atelier/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.atelier/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.atelier/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.atelier/sandbox.toml` (global) or `.atelier/sandbox.toml` (per-project):
...
```bash
atelier --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire atelier process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Atelier logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.atelier/sandbox-events.jsonl`
...
Use `atelier inspect` to see everything Atelier discovers in the current directory:

```bash
atelier inspect          # human-readable output
atelier inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.atelier/skills/`, `~/.atelier/skills/`, plugins, and config paths
...
Atelier automatically discovers configuration from Claude Code directories alongside native `.atelier/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Atelier uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.atelier/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Atelier at runtime.

---

## Built-in Tools

Atelier includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `ATELIER_WEB_FETCH=1`.

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (SpaceXAI, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `atelier/company`).

---

## Session Persistence

Atelier automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.atelier/sessions/`, organized by URL-encoded working directory:
...
atelier -p "Remember: X=42" -s my-session
atelier -p "What is X?" -s my-session

# Resume existing session (errors if not found)
atelier -p "Continue" -r my-session
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.atelier/config.toml` | Configuration file                                  |
...
| `.atelier/config.toml`   | Project-scoped config (MCP servers)                 |
...
| `ATELIER_CLI_CHAT_PROXY_BASE_URL`  | Override the cli-chat-proxy URL (default: `https://cli-chat-proxy.atelier.com/v1`)                          |
| `ATELIER_MODELS_BASE_URL`          | Custom base URL for inference. Model list auto-fetched from `{base_url}/models` (see [Custom Models Endpoint](#custom-models-endpoint)) |
| `ATELIER_MODELS_LIST_URL`          | Override the model list URL if it differs from `{ATELIER_MODELS_BASE_URL}/models`                                              |
| `ATELIER_AUTH_PROVIDER_COMMAND`     | External auth binary (alternative to config file). See [External Auth Provider](#external-auth-provider) |
| `ATELIER_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `ATELIER_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `ATELIER_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `ATELIER_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `ATELIER_HOME`                     | Override config directory (default: `~/.atelier`)                                                           |
| `ATELIER_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `ATELIER_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `ATELIER_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `ATELIER_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `ATELIER_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `ATELIER_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `ATELIER_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `ATELIER_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `ATELIER_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `ATELIER_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.atelier/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `ATELIER_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `atelier` commands and flags.
...
atelier completions bash > ~/.local/share/bash-completion/completions/atelier
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Atelier-managed location):

```bash
mkdir -p ~/.atelier/completions/bash
atelier completions bash > ~/.atelier/completions/bash/atelier.bash
```
...
atelier completions zsh > ~/.zsh/completions/_atelier
```
...
```bash
mkdir -p ~/.atelier/completions/zsh
atelier completions zsh > ~/.atelier/completions/zsh/_atelier
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.atelier/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `atelier` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `atelier --debug` or `ATELIER_LOG_FILE` instead:

```bash
# Per-session debug log (~/.atelier/debug/<sessionId>.txt)
atelier --debug

# Log to a custom path
ATELIER_LOG_FILE=/tmp/atelier-debug.log atelier

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.atelier/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `ATELIER_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
ATELIER_LOG_FILE=/tmp/atelier-debug.log RUST_LOG="info,atelier_shell::auth=debug" atelier
```
...
atelier --debug-file /tmp/atelier-auth.log -p "hello"
grep "auth:" /tmp/atelier-auth.log
```
...
cat ~/.atelier/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.atelier/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.atelier/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
