...
- **atelier inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct atelier --minimal --resume command.
...
- **Atelier no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **atelier setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Atelier above the footer shortcuts.
...
- **atelier/pr/status** now reports whether an open PR is in the merge queue.
...
- **`atelier worktree ls`** now works as a short alias for `atelier worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.atelier/config.toml`.
...
- **Resumed atelier.invalid chats** now use the conversation's last model instead of the gateway default.
...
- **File links and media** now work for worktree sessions under ~/.atelier/worktrees/.
...
- **Terminal command cards** on atelier.invalid now correctly settle after foreground bash tasks.
...
- **atelier inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain atelier launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive atelier agent stdio without silent parse drops on session/* calls.
...
- **atelier --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- The "Managed by atelier.invalid" link in the Extensions modal is now clickable and underlined.
...
- **atelier --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **atelier update** on Windows no longer fails when a previous.old executable is still running.
...
- **atelier models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `atelier plugin marketplace add`.
...
- **Plan files** now default to.atelier/plan.md to match Atelier conventions.
...
- **--minimal** flag now shows in `atelier --help`.
...
- **Tool result previews** no longer paint opaque panels in `atelier --minimal`.
- **atelier wrap** now correctly handles quoted strings and shell aliases.
...
- **Atelier.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **Chat sessions show the atelier.invalid model catalog** in the picker.
...
- **`atelier update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `atelier wrap`.
...
- **atelier agent stdio** now checks for updates in the background like other modes.
...
- **atelier wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `atelier agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `atelier wrap`** to run any command with local clipboard support.
...
- **Fixed `atelier agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or atelier-desktop.
...
- You can now pass --json-schema to atelier -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `ATELIER_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **atelier -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in atelier -p and atelier agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `atelier` after running `atelier update`.
- Headless (`atelier -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`atelier plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **atelier sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **atelier quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, atelier sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, ATELIER_HUNK_TRACKER, or config.
...
- **Atelier** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`atelier dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`atelier mcp add`** now accepts positional arguments (e.g. `atelier mcp add filesystem -- npx...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`atelier update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.atelier.
...
- **MCP servers menu** and slash commands now work when starting atelier outside a project directory.
...
- **atelier --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `atelier --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `atelier update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`atelier --debug`** now produces per-session log files under ~/.atelier/debug/ even with a leader process.
...
- **`atelier update`** now restarts any older running leader so all clients get the new binary.
...
- **`atelier login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `atelier agent --leader` without forced interactive login or timeouts.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the ATELIER_IMAGE_GEN_HARNESS env var.
...
- **atelier inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via ATELIER_CURSOR_MCPS_ENABLED / ATELIER_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Atelier and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the atelier-build model instead of falling back to the active session model.
...
- **image_gen** now uses the higher-quality atelier-imagine-image-quality model.
...
- Memory system: /remember command, note modal with raw/enhanced preview, atelier/memory/rewrite ACP extension, Ctrl+F fullscreen toggle for /memory modal.
...
- **Version output** now shows [alpha] or [stable] in `--version`, banners, `/session-info`, and `atelier inspect`.
...
- **New `/export` slash command** and `atelier export` CLI command let you save or copy conversation transcripts as Markdown.
...
- **New `atelier plugin` commands** let you manage plugins and marketplaces directly from the terminal.
...
- **New `atelier plugin` commands** let you manage plugins and marketplaces directly from the terminal.
- New `/export` command and `atelier export` let you save or copy conversation transcripts as Markdown.
...
- **Atelier** now reminds itself to take action in very long conversations while still obeying instructions like "just plan".
...
- **Atelier** now reminds itself to execute plans in long conversations while strictly following instructions like "just plan".
...
- **Added `atelier logout` subcommand** to sign out from the terminal without launching the TUI.
...
- **API-key users** no longer see unavailable models such as atelier-build that 404 on selection.
- **Image and video generation** now recover better from network issues and large response bodies.
- **Improved error messages** when hitting a subscription wall with an API key set, suggesting `atelier logout`.
...
- **Web search and X search** can now execute server-side with full session persistence when ATELIER_BACKEND_SEARCH is enabled.
...
- **Goal mode** is now opt-in via config.toml or the ATELIER_GOAL environment variable.
...
- **Atelier CLI** now runs natively on Intel Macs via x86_64 builds.
...
- **Paywall** now lifts automatically after subscribing via atelier login or external purchase without needing to re-login.
...
- **Model list** no longer gets stuck on only atelier-build after waking from sleep on macOS.
...
- **Goal mode** is now opt-in via `[features] goal = true` in config.toml or the ATELIER_GOAL environment variable.
...
- **New `atelier logout` command** clears your cached login session directly from the terminal.
...
- **Fixed model list** getting stuck on only atelier-build after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **API key users** no longer see unavailable models like atelier-build in the model list.
- **Image and video generation** now handles slow or interrupted network responses more reliably without generic decode errors.
- **Fixed error messages** when hitting a subscription wall with an API key set, now suggesting `atelier logout`.
- **Fixed a crash** on exit when using MCP servers that require token refresh.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on Atelier.
...
- **`atelier sessions search`** now finds local and remote sessions correctly, and **`atelier -r <id>`** restores remote sessions reliably.
...
- **AI shell command suggestions** enabled via ATELIER_SUGGESTIONS_AI=1 environment variable.
...
- **Fixed git and bash commands** failing in Atelier Desktop on Windows.
...
- **`atelier -w my-worktree`** creates a custom-named worktree folder.
...
- **`/usage manage`** now opens billing page; **`atelier login`** enforces tiers correctly after refresh.
...
- **Legacy web login** no longer treated as first-party xAI account for sharing and billing; prompts **`atelier login`** upgrade.
...
- **Large tool outputs** shortened when sent to Atelier, full text saved to file in session folder.
...
- **Legacy authentication** removed; run **`atelier login`** to upgrade to OAuth.
...
- **Legacy web login** no longer treated as premium xAI account; run **`atelier login`** instead.
...
- **New `atelier-build-v2` agent** available in config.toml for prompt/tool testing.
...
- **Fixed `atelier sessions list`** auth failures.
...
- **New `atelier-build-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **`atelier sessions list`** now works reliably after login.
...
- **Added atelier memory clear** CLI command with scope selection and confirmation.
...
- **Computer Hub integration** into atelier-shell
...
- **atelier memory clear command** with scope and confirmation
...
- **Best-of-N mode** for atelier-build
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like atelier-build.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like atelier-build.
...
- **atelier-build model** now defaults to Chat Completions API.
...
- **Shell commands now work** in Atelier Desktop on Windows.
...
- **New `atelier-build-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `atelier import`** resumes Claude Code sessions with `atelier import <id/path>`.
- **New `atelier-build-orchestrator`** coordinates subagents for complex builds.
...
- **Fixed `atelier share`** failing with backend ZodError due to duplicate request headers.
...
- **New `atelier login --devbox`** option for signing in from headless devbox environments without browser access.
...
- **New `atelier mcp doctor`** diagnoses MCP servers and suggests fixes.
...
- **New `atelier mcp doctor`** diagnoses MCP server configuration and connectivity.
...
- **After plan approval**, Atelier suggests `/implement` for structured coding when available.

## Bug Fixes

- **Model lists and pickers** now show only models accessible via your `ATELIER_CODE_XAI_API_KEY`.
...
- **`atelier models`** and TUI picker now show team-specific models when **`ATELIER_CODE_XAI_API_KEY`** is set.
...
- **Native Windows support** now available experimentally for running Atelier without WSL.
...
- **Clearer login error messages** now suggest `atelier login`, API key env var, or config.toml.
...
- **Native Windows support** (opt-in) lets you run Atelier CLI directly without WSL, with MCP server compatibility.
...
- **Authentication errors** now clearly suggest `atelier login`, env var, or config.toml api_key.
- **Fixed false-positive loop warnings** for different shell commands during edit-test cycles.
...
- **Import Claude settings** from `.claude/` into `.atelier/config.toml` via welcome screen ('i') or `/import-claude`.
- **Skills reload automatically** when editing/saving SKILL.md files without restarting `atelier`.
...
- **Improved `atelier login`** with automatic code delivery from browser consent page.
...
- **New `atelier trace`** command uploads or exports session data for debugging.
...
- **New `atelier trace`** command exports or uploads session data for debugging.
...
- **`atelier models`** now lists available models and the default.
...
- **`atelier models`** command now lists available models and authentication status.
...
- **X Premium and Premium+** tiers now grant access to Atelier.
...
- **Model picker** now displays config key names like `atelier-build` for aliased models.
...
- **Welcome screen** shows **SuperAtelier upsell** without active subscription.
...
- **Welcome screen** restricts actions and upsells SuperAtelier without subscription.
...
- **SuperAtelier upsell** shown on welcome screen without subscription.
...
- **`atelier sessions search`** now finds sessions by content with scores and snippets.
...
- **`atelier sessions search`** now supports **full-text queries** across chat content and tools.
- **Session picker** now combines fuzzy matching with **deep content search** results.
- **Unified logs** now saved to `~/.atelier/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Auth.json hot-reload** fixed for atelier login in other terminals.
...
- **Auto-update restart** now launches the correct new version.
- **Auth.json hot-reload** now detects `atelier login` from other terminals.
...
- **External `atelier login`** now hot-reloads auth without app restart.
...
- **External `atelier login`** now hot-reloads auth without restarting.
...
- **New `atelier ssh`** command enables clipboard copy from remote sessions in Apple Terminal.
...
- **`atelier login --device-auth`** enables login from SSH, Docker, and other headless environments without port forwarding.
...
- **Added** `atelier login --device-auth` **flag** for headless environments like SSH and Docker.
...
- **Auth info extension** returns login method ID and email via atelier/auth/info ACP method.
...
- **Startup announcements** emits atelier/announcements/refreshed ACP notification on agent init.
...
- **Bundled agents** discovered from ~/.atelier/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.atelier/managed_config.toml/requirements.toml with `atelier inspect` and `atelier setup`.
- **web_fetch tool** enabled via [features] web_fetch in config.toml alongside env/remote settings.
- **atelier login defaults to OAuth** instead of legacy relay; use --legacy or ATELIER_OAUTH_ENABLED=0 for old behavior.
...
- **Automatic cleanup** removes old atelier/atelier binaries post-update, keeping current + previous.
...
- **Automatic cleanup** removes old atelier/atelier binaries post-update, keeping only current and previous.
...
- **Per-hook enable/disable toggles** via ~/.atelier/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **Parallel tool dispatch** via ATELIER_PARALLEL_TOOL_DISPATCH=1 runs concurrent Phase 2 execution.
...
- **Git stash-all** via atelier/git/stash ACP extension wired to desktop Changes panel.
...
- **OAuth login option** via --oauth/--legacy flags, ATELIER_OAUTH_ENABLED env, feature flag (legacy default).
...
- **Adds live PR status display** in desktop changes panel via new ACP atelier/pr/status extension.
...
- **Restores web search functionality** by reinjecting required proxy headers like x-atelier-client-version.
...
- **AtelierNight default theme** with **runtime color quantization** ensures correct rendering across truecolor, 256-color, and 16-color terminals.
...
- **Legacy relay auth flow** via `atelier login --legacy` restores pre-OIDC accounts.x.ai token exchange.
...
- **OIDC OAuth replaces relay auth**; removed `--auth-signin-url`/`exchange-code-url`/`redirect-target` flags, run `atelier login` to migrate.

## Features

- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.atelier/lsp.json`.
...
- **Web search model override** via CLI, config.toml or env defaults to atelier-4-1-fast-reasoning.
...
- **Image generation tool** gated behind a server-side flag or `ATELIER_IMAGE_GEN=1` env var with session auth.
- **Dynamic nested skill discovery** in `.atelier/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.atelier/config.toml before existing checks.
...
- **Suppresses config warning** for [desktop] section owned by atelier-desktop using opaque serde sink field.
...
- **Skills discovery expands** to.agents/skills directories alongside.atelier and.claude.
...
- **OIDC OAuth2 replaces legacy relay login** for `atelier login`; add `--legacy` flag for old flow and remove `--auth-signin-url` etc. CLI flags (migrate by deleting auth.json).
- **Adds `atelier login --legacy`** fallback; removes `--auth-signin-url` etc flags (use env/config defaults, no migration needed for most users).

## Features

- **ACP git extensions** (`atelier/git/info`, `/branches`, `/checkout`) enable cloud-local git ops without subprocesses.
...
- **ACP hooks/plugins listing** (`atelier/hooks/list`, `/plugins/list`) exposes loaded configs for pager modals.
- **Image generation tool** via xAI Imagine API saves to session/images/ folder.
- **Management actions for hooks/plugins** via atelier/{hooks,plugins}/action with live modal refresh.
...
- **Dynamic discovery of nested skills** from.atelier/skills/ subdirs at runtime via reminders.
- **PR status extension (atelier/pr/status)** fetches branch PR state, title, number, and URL via gh CLI.
...
- **Silent [desktop] config handling** consumes atelier-desktop section without spurious 'unrecognized key' warnings.
...
- **Question-answer panel** in pager with **AtelierBuildPlanNoSubagents** mode excluding subagent tools.
...
- **Discover skills from**.claude/skills directories alongside.atelier/skills.
...
- **Codex grep_files uses** shared rg_path() matching atelier_build pattern.
...
- **Auto-injected managed MCPs** from atelier.invalid for WebLogin, deduped with config.toml opt-out.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP atelier/auth/logout** removes scopes from ~/.atelier/auth.json.
- **Custom npm registry** via config.toml or ATELIER_NPM_REGISTRY respects enterprise.npmrc.
...
- **Help skill reads ~/.atelier/config.toml** to answer MCP server and model configuration queries.
- **Deployment keys supported** on all API proxy endpoints including storage and sessions.
- **Public install-atelier.sh script** supports deployment keys and channels without VPN.
...
- **Bundled /help skill** extracts README.md to ~/.atelier/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.atelier/personas/*.toml with fail-closed resolution.
...
- **Question-answer panel** adds pager UI for interactive permissions and subagent-free AtelierBuildPlan agent.
...
- **Auto-injects managed MCPs** from atelier.invalid into CLI/TUI for WebLogin users.
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.atelier/auth.json.
- **Discovers skills from.claude/skills** alongside.atelier/skills across local, repo, and user directories.
...
- **Codex grep_files uses shared rg_path** matching atelier_build and opencode patterns.
...
- **Full-text search across past sessions** via atelier/session/search ACP method with FTS5 indexing and workspace filtering.
- **ACP resume session in worktree** via atelier/git/worktree/resume_session for programmatic clients.
...
- **Configurable bash tool params** like timeouts injected from config.toml `[toolset.bash]` into AtelierBuild actors.

## Bug Fixes

- **Precise io::ErrorKind mapping** from ACP file errors enables consistent NotFound and PermissionDenied handling.
- **Silent atelier installation** hides internal details from install and auto-update user output.
...
- **SSE stream parsing** handles flat error format from Atelier proxy without deserialization failure.
...
- **Post-A/B session hooks** via ATELIER_AB_POST_SESSION_HOOK capture worktree changes.
...
- **Enhanced announcement UX** adds paging, prev/next commands, and `ATELIER_DEV_ANNOUNCEMENTS` override.
...
- **Optional auth for feedback** enables unauthenticated submissions with `ATELIER_USER_METADATA`.
- **Plan mode tools** add `enter_plan_mode`, `exit_plan_mode`, and `ask_user_question` with notifications.
- **Auto-installs atelier** alongside atelier during internal and GitHub release updates.
...
- **Independent feedback gating** via ATELIER_FEEDBACK_ENABLED separates it from telemetry controls.
...
- **Atelier 4.20 default model** with CLI/env/config/remote-settings overrides for catalog models.
...
- **`atelier completions` subcommand** generates bash/zsh completion scripts, with fast-path exit before any network or auth warmup.
...
- **use_tool works in all server contexts** by dispatching through ToolCallContext instead of session-actor interception, fixing silent failures in atelier-tools-server.
...
- **Custom skill path discovery** now works end-to-end — paths added via `atelier/skills/add` or config.toml are included in agent skill lookup.
...
- **`atelier memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boost in hybrid search.
...
- **User-defined subagents** via .atelier/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **Session token routing fixed** for default models — `atelier login` users no longer get 400 errors on proxy-routed models.
...
- **Restored remote sessions create distinct local children** with parent tracking, preventing identity reuse and duplicate restores on repeated `atelier -r`.
...
- **Faster startup before code navigation** by lazily building indexes only for web clients that advertise atelier/codeNavigation.
...
- **Shell completions** via `atelier completions <shell>`, leader CLI, and stdio reconnect replay.
- **User-defined subagents** via.atelier/agents/*.md with config toggles.
...
- **Full-text search across sessions** via new `atelier/session/search` ACP extension with FTS5 indexing.
...
- **Disables web search tool** via `--disable-web-search` flag or `ATELIER_DISABLE_WEB_SEARCH=1` for benchmarks.
- **hashline_read tool** annotates file output with anchors like `LINE:LOCAL:CONTEXT→CONTENT`.
- **Atelier 4.20 default model** with CLI/env/config/remote-settings overrides for web search and summaries.
- **Unauthenticated feedback submission** via optional tokens and `ATELIER_USER_METADATA` env.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.atelier/hooks/ with deny-wins trust controls.
- **Independent feedback flag** gates /feedback and popups separately from telemetry via ATELIER_FEEDBACK_ENABLED.
- **Leader CLI** adds `atelier leader list/info/profile/kill/dev` for discovery and CPU profiling.
...
- **ACP `atelier/subagent/list_running`** queries live progress for all running subagents of parent session.
- **Bash tool params from config.toml** override schema defaults like `timeout_secs` for AtelierBuild.
- **Live subagent progress** in TUI tasks panel via ACP polling and push notifications.
- **ACP extension resumes sessions in worktrees** via single call matching `atelier -w -r` CLI flow.
...
- **Custom skills now discoverable** via config.toml paths and `atelier/skills/add` in system prompts.
...
- **SSE parsing handles flat errors** from Atelier proxy alongside OpenAI-standard nested format.
...
- **Silent atelier installation** hides implementation details from install/update output.
...
- **Subagent support** gated behind `--subagents` flag, `ATELIER_SUBAGENTS` env var, config.toml, or remote settings — disabled by default.
...
- **`atelier memory reindex` and `doctor` CLI commands** for index maintenance, plus access-frequency boosting for retrieved memory chunks.
...
- **Cross-devbox session restore** via `atelier sessions list/search` CLI subcommands and hardened `atelier -r` with upload-ordering safety, cwd-scoped lookups, and staged-vs-unstaged correctness.
...
- **Kill background tasks by task_id** via new `atelier/task/kill` ACP extension method.
...
- **On-demand memory flush** via `/flush` command and `atelier/memory/flush` extension with concurrent-flush prevention.
- **Tool state export/import** via GetToolState RPC and initial_tool_state_json, enabling session warm-start and cloning.
- **`ATELIER_MEMORY=0` force-disables memory** regardless of config.toml or remote settings; CLI flag still overrides.
...
- **Headless mode (`atelier -p`) exit panic** fixed by flushing telemetry before runtime teardown.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.atelier/bin symlink at spawn time.
...
- **Mid-session MCP server toggling** via atelier/session/update_mcp_servers extension method with optimistic rollback.
- **Sandbox profile configurable via ATELIER_SANDBOX env variable**, defaulting to workspace profile on devboxes.
- **Agent version exposed in InitializeResponse** metadata for relay and client version discovery.
- **`atelier worktree` subcommand** for listing, inspecting, removing, and garbage-collecting session worktrees.
...
- **ACP file operations** were silently bypassing the client filesystem backend, breaking ask-mode write rejection in AtelierCode.
...
- **Reliable CLI updates** now refresh atelier-latest during installs, ensuring symlink-based setups launch the newly installed binary in fresh shells.
...
- **Agent environment metadata becomes queryable** by merging ATELIER_AGENT_METADATA into initialize responses and persisting the full blob in backend agent records.
...
- **Prompt introspection from the CLI** is available via `atelier prompt`, with JSON or section output and persisted session prompt_context snapshots.
...
- **Installation diagnostics** help troubleshoot conflicting atelier binaries by listing canonical and invoked paths, versions, update status, and optional JSON output.
...
- **npm installs now use a canonical Atelier binary path** via postinstall copy to `~/.atelier/bin/atelier`, preventing installer conflicts and version confusion.
