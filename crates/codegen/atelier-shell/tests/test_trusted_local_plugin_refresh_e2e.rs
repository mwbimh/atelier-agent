...
//! 4. Smoke-validate session JSON under `$ATELIER_HOME/sessions/` after exit.
//!
//! Requires a built `atelier` binary (`ATELIER_BINARY` or cargo-built pager) for the
//! ignored headless test.
//!
//! ```bash
//! cargo test -p atelier-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p atelier-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use atelier_agent::plugins::SharedPluginRegistryHandle;
use atelier_agent::plugins::discovery::DiscoveryConfig;
use atelier_agent::plugins::git_install::{InstallSource, install_from_source};
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _atelier_guard = EnvVarGuard::set("ATELIER_HOME", &atelier_home);
    ...
    let mut registry = InstallRegistry::empty(atelier_home.join("installed-plugins"));
    ...
    assert_eq!(
        plugin.scope,
        atelier_agent::plugins::PluginScope::CliOverride
    );
...
#[tokio::test]
#[ignore = "requires pre-built atelier binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    // The spawned binary gets HOME/ATELIER_HOME via `cmd.env` below; this global env
    // is only for the in-process post-run discovery assertion (which resolves the
    // registry via atelier_home()). `#[serial]` keeps it from racing other tests.
    let _home_guard = EnvVarGuard::set("HOME", &home);
    let _atelier_guard = EnvVarGuard::set("ATELIER_HOME", &atelier_home);
    ...
    // Smoke: session storage under ATELIER_HOME/sessions has JSON artifacts.
