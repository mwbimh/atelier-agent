...
//!
//! Each test spawns a real `atelier agent stdio` process against a mock
//! inference server and asserts on the persisted `summary.json`.
//!
//! Run locally:
//! ```bash
//! cargo test -p atelier-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.atelier/sessions/` and
...
fn read_summary(home: &std::path::Path, session_id: &str) -> serde_json::Value {
    let sessions_root = home.join(".atelier").join("sessions");
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let atelier_dir = home.path().join(".atelier");
        std::fs::create_dir_all(&atelier_dir).expect("create .atelier dir");
        std::fs::write(
...
"#,
        ...
        let client = AtelierStdioClient::spawn_with_home(&server, workdir.path(), home).await;
