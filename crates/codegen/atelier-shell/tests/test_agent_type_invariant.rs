...
//! - Session resume
//!
//! Each test spawns a real `atelier agent stdio` process, speaks the full ACP
...
//! Run locally:
//! ```bash
//! cargo test -p atelier-shell --test test_agent_type_invariant -- --ignored
//! ```
...
fn invalidate_models_cache(home: &std::path::Path) {
    let cache = home.join(".atelier").join("models_cache.json");
    if cache.exists() {
        std::fs::remove_file(&cache).expect("failed to delete models_cache.json");
    }
}
/// Start a mock server with two models:
/// - `default-model`: no agent_type (→ defaults to "atelier-build")
...
/// - `model-a`: no agent_type (→ "atelier-build")
/// - `model-b`: no agent_type (→ "atelier-build")
...
/// `atelier-build` harness. The system prompt sent to the LLM should contain
/// the atelier-build identity string.
#[tokio::test]
#[ignore]
async fn test_default_model_uses_atelier_build_harness() {
    with_local_set(|| async {
        ...
        assert!(
            sys_prompt.contains("Atelier") || sys_prompt.contains("atelier"),
            "default model should use atelier-build harness\nsystem prompt preview: {}",
...
async fn test_session_resume_preserves_harness() {
    with_local_set(|| async {
        ...
        drop(writer);
        let reader =
            AtelierStdioClient::spawn_with_home(&server, workdir.path(), shared_home).await;
        ...
        let original_has_atelier =
            original_sys_prompt.contains("Atelier") || original_sys_prompt.contains("atelier");
        let resumed_has_atelier =
            resumed_sys_prompt.contains("Atelier") || resumed_sys_prompt.contains("atelier");
        assert_eq!(
            original_has_atelier,
            resumed_has_atelier,
            "resumed session should use the same harness as the original\n\
             original identity markers: atelier={original_has_atelier}\n\
             resumed identity markers: atelier={resumed_has_atelier}\n\
...
/// default to `"atelier-build"`. This exercises the serde default.
#[tokio::test]
#[ignore]
async fn test_model_without_agent_type_defaults_to_atelier_build() {
    with_local_set(|| async {
            ...
            assert!(
                sys_prompt.contains("Atelier") || sys_prompt.contains("atelier"),
                "model without agent_type should default to atelier-build harness\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
}
/// The `ATELIER_AGENT` escape hatch should override the model's agent_type.
/// Setting `ATELIER_AGENT=atelier-build` with an alternate-agent model should use
/// atelier-build harness.
#[tokio::test]
#[ignore]
async fn test_atelier_agent_env_overrides_model_agent_type() {
    with_local_set(|| async {
            ...
            cmd.args(["agent", "stdio"])
                ...
                .kill_on_drop(true);
            atelier_test_support::env::test_env_cmd_tokio(
                &mut cmd,
                &server.url(),
                home.path(),
            );
            cmd.env("ATELIER_AGENT", "atelier-build");
            let mut child = cmd.spawn().expect("spawn atelier");
            ...
            assert!(
                sys_prompt.contains("Atelier") || sys_prompt.contains("atelier"),
                "ATELIER_AGENT=atelier-build should override cursor model's agent_type\nsystem prompt preview: {}",
                & sys_prompt[..sys_prompt.len().min(500)]
            );
        })
        .await;
