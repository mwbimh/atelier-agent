//! Integration test for `_atelier/session/update` notifications.
//!
//! This test verifies that:
//! 1. Atelier extension session notifications (e.g., diff_review) can be sent via ext_notification
...
use atelier_shell::extensions::notification::{
    DiffContent, SessionNotification, SessionUpdate as ExtensionSessionUpdate,
};
use atelier_shell::session::info::Info as SessionInfo;
use atelier_shell::session::persistence::default_model_id;
use atelier_shell::session::storage::{JsonlStorageAdapter, SessionUpdate, StorageAdapter};

/// Test that Atelier extension session notifications round-trip through storage correctly.
#[tokio::test]
async fn test_extension_session_notification_storage_roundtrip() {
    ...
    let extension_notification = SessionNotification {
        session_id: session_id.clone(),
        update: ExtensionSessionUpdate::DiffReview {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Extension(Box::new(extension_notification.clone())),
        )
        .await
        .unwrap();
...
/// must survive `updates.jsonl` and reload as a replayable `_atelier/session/update`.
#[tokio::test]
async fn test_turn_completed_round_trips_through_storage() {
    ...
    adapter
        .append_update(
            &info,
            &SessionUpdate::Extension(Box::new(extension_notification)),
        )
        .await
        .unwrap();
...
/// Test that totalTokens can be extracted from both ACP and extension notifications.
#[tokio::test]
async fn test_extract_total_tokens_from_mixed_updates() {
    ...
    // Add extension notification with totalTokens
    let extension_notification = SessionNotification {
        session_id: session_id.clone(),
        update: ExtensionSessionUpdate::DiffReview { content: vec![] },
...
#[test]
fn test_extension_session_notification_serialization() {
