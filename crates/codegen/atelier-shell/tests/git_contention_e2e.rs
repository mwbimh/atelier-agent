...
//!
//! With the fs-watch machinery on (`atelier/hunkTracker` + `atelier/gitHeadChanged`
...
//!
//! Knobs: ATELIER_PERF_GIT_FILES (default 300), ATELIER_PERF_GIT_PICKS (default 6).
...
async fn run_storm(
    ...
    let files = env_usize("ATELIER_PERF_GIT_FILES", 300);
    let picks = env_usize("ATELIER_PERF_GIT_PICKS", 6);
...
fn git_rebase_refresh_storm_e2e() {
    ...
    let server = mock_rt
        ...
        .expect("mock server");
    let atelier_home = TempDir::new().expect("atelier home");
    ...
    let on = agent_rt.block_on(run_storm(
        &server,
        Some(json!({
            "atelier/hunkTracker": { "mode": "agent_only" },
