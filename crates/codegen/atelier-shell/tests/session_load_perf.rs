...
//! `ATELIER_PERF_SESSION_SRC=/path/to/<session-dir>`.
//!
//! Run:
//!   cargo test -p atelier-shell --test session_load_perf -- --nocapture
//!   cargo test -p atelier-shell --test session_load_perf full_session_load_e2e -- --ignored --nocapture
...
use atelier_shell::session::storage::{
    JsonlStorageAdapter, StorageAdapter, load_updates_for_replay_at,
};
use atelier_workspace::session::file_state::{FileSnapshot, FlexiblePath, RewindPoint};
...
impl GenOpts {
    fn from_env() -> Self {
        ...
        let scale = g("ATELIER_PERF_SCALE", 1).max(1);
        Self {
            turns: g("ATELIER_PERF_TURNS", 80) * scale,
            acu_per_turn: g("ATELIER_PERF_ACU_PER_TURN", 15),
            catalog_commands: g("ATELIER_PERF_CATALOG_COMMANDS", 64),
            catalog_desc_len: g("ATELIER_PERF_CATALOG_DESC_LEN", 320),
            agent_chunks_per_turn: g("ATELIER_PERF_AGENT_CHUNKS_PER_TURN", 8),
            agent_chunk_len: g("ATELIER_PERF_AGENT_CHUNK_LEN", 2000),
            rewind_points: g("ATELIER_PERF_REWIND_POINTS", 60) * scale,
            files_per_rewind: g("ATELIER_PERF_FILES_PER_REWIND", 40),
            file_content_len: g("ATELIER_PERF_FILE_CONTENT_LEN", 8000),
        }
    }
}
...
/// `Info` and the session directory path. Uses `ATELIER_PERF_SESSION_SRC` if set
...
async fn prepare_session(root: &Path, cwd: &Path, opts: &GenOpts) -> (Info, PathBuf) {
    ...
    if let Ok(src) = std::env::var("ATELIER_PERF_SESSION_SRC") {
...
/// `--ignored` (optionally `ATELIER_PERF_SESSION_SRC=...`) to get the numbers.
...
fn generate_or_restore_rewind(path: &Path, opts: &GenOpts) {
    if std::env::var("ATELIER_PERF_SESSION_SRC").is_ok() {
        return;
    }
...
async fn full_session_load_e2e() {
    ...
    let cwd = TempDir::new().unwrap();
    let opts = GenOpts::from_env();
    let instr_log = atelier_home.path().join("instr.jsonl");

    // SAFETY: single-threaded current-thread runtime; set before any agent code
    // reads these process-globals (atelier_home()/instrumentation mode are OnceLock).
    unsafe {
        std::env::set_var("ATELIER_HOME", atelier_home.path());
        std::env::set_var("ATELIER_INSTRUMENTATION", "log");
        std::env::set_var("ATELIER_INSTRUMENTATION_LOG", &instr_log);
        std::env::set_var("ATELIER_CLI_CHAT_PROXY_BASE_URL", server.url());
        std::env::set_var("ATELIER_XAI_API_BASE_URL", server.url());
    ...
    let _ = tracing_subscriber::registry()
        .with(atelier_shell::instrumentation::layer::<Registry>())
        .try_init();

    let (info, dir) = prepare_session(atelier_home.path(), cwd.path(), &opts).await;
    let updates_path = dir.join("updates.jsonl");
    let rewind_path = dir.join("rewind_points.jsonl");
    eprintln!(
    ...
    local
        .run_until(async move {
            ...
            let _ = atelier_shell::instrumentation::finalize();
