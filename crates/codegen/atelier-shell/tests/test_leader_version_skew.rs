...
//! - `ATELIER_BINARY_LEADER` — the binary that elects the initial leader
//!   (typically the latest released stable, e.g. fetched from
//!   `https://storage.googleapis.com/atelier-build-public-artifacts/cli/atelier-<ver>-linux-x86_64`).
//! - `ATELIER_BINARY_CLIENT` — the second client (typically a freshly built main).
//!
...
//!
//! ```bash
//! ATELIER_BINARY_LEADER=/path/to/atelier-old ATELIER_BINARY_CLIENT=/path/to/atelier-new \
//!   cargo test -p atelier-shell --test test_leader_version_skew -- --ignored --nocapture
//! ```
...
use atelier_test_support::leader::{
...
fn skew_binaries() -> Option<(std::path::PathBuf, std::path::PathBuf)> {
    ...
    if old == new {
        eprintln!(
            "SKIP: ATELIER_BINARY_LEADER/ATELIER_BINARY_CLIENT resolve to the same binary ({})",
            old.display()
        );
        return None;
    }
...
fn sandbox_unified_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".atelier").join("logs").join("unified.jsonl"))
        .unwrap_or_default()
}
...
#[tokio::test]
#[ignore = "two-binary version-skew test; set ATELIER_BINARY_LEADER/ATELIER_BINARY_CLIENT and run with --ignored"]
...
async fn old_client_adopts_new_leader_and_still_functions() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            // atelier/leader/version_mismatch notification's server-side warn).
...
/// `atelier update`'s relaunch signal against a REAL old leader: connect,
/// require `relaunch_v1`, send `RelaunchForUpdate`, and the leader exits so
/// the surviving client re-elects. Mirrors the private
/// `signal_leaders_to_relaunch` in `atelier-pager-bin/src/main.rs` (which is
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let control = LeaderClient::connect(
                home.path().join(".atelier").join("leader.sock"),
                "atelier-update",
                ClientMode::Stdio,
...
async fn eviction_leaves_single_leader_and_single_auth_owner() {
    ...
    tokio::task::LocalSet::new()
        .run_until(async {
            ...
            let auth_path = home.path().join(".atelier").join("auth.json");
