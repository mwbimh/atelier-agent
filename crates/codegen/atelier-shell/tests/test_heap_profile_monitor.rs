...
use atelier_shell::auth::{AtelierAuth, AtelierComConfig, AuthManager, AuthMode};
use atelier_shell::heap_profile::{
...
use atelier_test_support::{EnvGuard, MockInferenceServer};
use atelier_version::TEST_VERSION_ENV;
...
fn seed_auth_json(home: &Path, token: &str) {
    let scope = AtelierComConfig::default().auth_scope();
...
fn assert_storage_auth(uploads: &[atelier_test_support::mock_server::StorageUpload]) {
    for u in uploads {
        assert_eq!(
...
impl Harness {
    async fn start(remote: RemoteSettings) -> Self {
        ...
        let auth = Arc::new(AuthManager::new(home.path(), AtelierComConfig::default()));
