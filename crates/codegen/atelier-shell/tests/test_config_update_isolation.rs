...
/// All tests share this as ATELIER_HOME (the `OnceLock` in atelier-config
...
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("ATELIER_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = atelier_shell::config::load_effective_config().unwrap();
    let effective_cfg = atelier_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    atelier_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = atelier_shell::util::config::load_config_from_toml(&user_toml);
    ...
    assert_eq!(user_cfg.models.default.as_deref(), Some("atelier-3"));
}
