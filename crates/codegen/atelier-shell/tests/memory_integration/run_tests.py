...
Launches atelier agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/atelier-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/atelier-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/atelier-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    ATELIER_BINARY=/path/to/atelier-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a atelier agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated atelier environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.atelier/auth.json   (copied from real home)
      ~/.atelier/memory/     (pre-populated by tests)
      ~/.atelier/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="atelier-memtest-")
        ...
        # Create .atelier dirs in fake home
        self.atelier_home = os.path.join(self.fake_home, ".atelier")
        self.memory_dir = os.path.join(self.atelier_home, "memory")
        self.logs_dir = os.path.join(self.atelier_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.atelier/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.atelier_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.atelier/config.toml (where the agent reads config)."""
        with open(os.path.join(self.atelier_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("ATELIER_BINARY", "atelier")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}atelier binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
        env["HOME"] = self.fake_home
        env["ATELIER_MEMORY"] = "1"
        env["RUST_LOG"] = "warn"
        # Disable leader mode
        env["ATELIER_NO_LEADER"] = "1"
        if extra_env:
...
def _default_config() -> str:
    embed_model = os.environ.get("ATELIER_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"ATELIER_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """ATELIER_MEMORY=1 env var enables memory even without config."""
    section("13. ATELIER_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"ATELIER_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with ATELIER_MEMORY=1")
        else:
            fail("no index despite ATELIER_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.atelier/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("ATELIER_BINARY", "atelier")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["ATELIER_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["ATELIER_NO_LEADER"] = "1"
...
def build_binary():
    """Build atelier-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "atelier-pager")

    # Find rg for ATELIER_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["ATELIER_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building atelier-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "atelier-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "ATELIER_BINARY" in os.environ:
        binary = os.environ["ATELIER_BINARY"]
        print(f"Binary: {binary} (from $ATELIER_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["ATELIER_BINARY"] = binary
