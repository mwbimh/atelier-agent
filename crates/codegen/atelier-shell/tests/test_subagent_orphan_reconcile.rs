...
//!
//! This test spawns a real `atelier agent stdio` process, seeds an orphaned
...
//! ```bash
//! cargo test -p atelier-shell --test test_subagent_orphan_reconcile -- --ignored
//! ```
...
async fn resume_reconciles_orphaned_running_subagent() {
    with_local_set(|| async {
        ...
        // leaves behind.
        // AtelierStdioClient sets HOME=<temp>; the binary uses <HOME>/.atelier as ATELIER_HOME.
        let atelier_home = shared_home.path().join(".atelier");
        let session_dir = locate_session_dir(&atelier_home, session_id.0.as_ref());
