...
async fn mcp_tool_grant_persists_and_short_circuits_next_request() {
    run_actor_test(ClientType::AtelierPager, |handle, gw, cwd| async move {
...
async fn fallback_client_plain_allow_always_persists_mcp_tool() {
    // Regression: Generic / AtelierWeb / Extension clients
...
async fn dont_ask_policy_denies_without_prompting() {
    use atelier_workspace::permission::types::{PermissionConfig, PromptPolicy};
