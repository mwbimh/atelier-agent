pub mod atelier_auth_credentials;
pub mod config;
pub mod hooks;

// The foundation utilities live in `atelier-shell-base` (upstream of this
// crate so they build in parallel). Re-exported at the original paths so
// existing `crate::util::…` / `atelier_shell::util::…` users compile
// unchanged.
pub use atelier_shell_base::util::*;
