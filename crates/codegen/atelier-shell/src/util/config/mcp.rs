...
use atelier_tools::types::compat::{CompatConfig, CompatConfigToml};
...
pub use atelier_mcp::oauth_config::{McpOAuthConfig, McpOAuthConfigMap};
// MCP server config value types extracted to `atelier-config-types` (config
// dependency inversion); re-exported so `crate::util::config::*` paths keep working.
pub use atelier_config_types::{McpJsonOAuthBlock, McpServerConfig, McpServerTransportConfig};
...
pub use atelier_config_types::{McpConfig, RelaySyncConfig};
// Worktree-pool config value type extracted; re-exported to keep paths stable.
pub use atelier_config_types::PoolConfig;
...
/// Walks from cwd up to the git repo root checking `.atelier/config.toml` at each level.
/// Project-scoped `.atelier/config.toml` entries override global `~/.atelier/config.toml`
...
/// `.atelier/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.atelier/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.atelier/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.atelier/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .atelier/config.toml"
                );
...
///
/// Uses the top-level `disabled_mcp_servers` array in `~/.atelier/config.toml`.
...
/// Upsert an MCP server entry in `~/.atelier/config.toml`.
///
...
/// config file, e.g. a project-scoped `.atelier/config.toml`.
...
/// Delete an MCP server entry from `~/.atelier/config.toml`.
///
...
/// config file, e.g. a project-scoped `.atelier/config.toml`. OAuth credential
...
pub async fn delete_mcp_server_config_at(
    ...
    if let Ok(mut cred_store) = atelier_mcp::credentials::McpCredentialStore::load_default() {
...
/// project-scoped `.atelier/config.toml` files.
...
// `.mcp.json` discovery moved to `atelier-workspace` (client-side, shared with
// the folder-trust gate); re-exported so `crate::util::config::*` paths keep working.
pub use atelier_workspace::project_config::{
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.atelier/config.toml` files from `cwd` up to the
...
fn config_path() -> PathBuf {
    crate::util::atelier_home::atelier_home().join("config.toml")
}

/// Path to the user-level config file (`~/.atelier/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.atelier/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".atelier").join("config.toml")
}
...
/// Session registry synchronization is not supported by Atelier.
///
/// The old implementation read `[cli] session_registry`, which allowed a
/// local config or managed setting to re-enable the vendor session registry.
/// Keep this helper as a compatibility seam for callers while making the
/// privacy boundary unconditional.
...
mod tests {
    ...
    #[test]
    fn session_registry_config_is_ignored() {
        let root: TomlValue = toml::from_str("[cli]\nsession_registry = true\n").unwrap();

        assert_eq!(session_registry_from_toml_opt(&root), None);
    }
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.atelier/skills", "~/.atelier/skills/special/SKILL.md"]
ignore = ["~/.atelier/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.atelier/skills", "~/.atelier/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.atelier/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${ATELIER_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
