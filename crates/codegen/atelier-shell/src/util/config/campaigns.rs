...
pub fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_atelier_home() else {
        return;
    };
...
fn dismiss_campaign_ids_at(
    ...
    // several atelier processes share `$ATELIER_HOME`; the in-process mutex alone would
...
/// `ATELIER_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` = none; beats
...
pub fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("ATELIER_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid ATELIER_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `ATELIER_CAMPAIGNS_OVERRIDE` (replaces all
...
fn resolve_dismissable_campaigns() -> Vec<CampaignEntry> {
    ...
    match ConfigLayers::load() {
        Ok(layers) => filter_active_campaigns(
            merge_campaign_entries(&layers.campaign_source_slices(&[])),
            &dismissed,
        ),
        Err(e) => {
            tracing::warn!(error = %e, "campaigns: layer load failed; no campaigns available for dismiss bookkeeping");
            Vec::new()
        }
    }
}
...
/// `ATELIER_CAMPAIGNS_OVERRIDE`. For one-shot CLI entrypoints that never fetch
...
mod tests {
    use super::*;
    use atelier_config::ConfigLayers;
    ...
    /// `ATELIER_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the
    ...
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("ATELIER_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `ATELIER_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*: the
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("ATELIER_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// made while `ATELIER_CAMPAIGNS=0` must still record the dismissal, or a
    /// later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::set(
            "ATELIER_CAMPAIGNS_OVERRIDE",
            r#"[{"id":"dismiss-during-kill-switch","models":{"default":"m"}}]"#,
        );
        let _kill = EnvGuard::set("ATELIER_CAMPAIGNS", "0");
    ...
    /// `ATELIER_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing — even
    /// layer + remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("ATELIER_CAMPAIGNS_OVERRIDE", "[]");
