...
/// [`atelier_config::env_bool`] parser (`ATELIER_TOOLS_FIND_BFS` /
/// `ATELIER_TOOLS_GREP_UGREP`, plus the `ATELIER_FIND_BFS` / `ATELIER_GREP_UGREP` aliases).
///
/// Pass the **merged** requirements ([`crate::config::load_merged_requirements`])
/// so an org policy in any requirements layer — not only
/// `~/.atelier/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a
/// [`atelier_tools::computer::local::SearchShadowConfig`] on the local terminal
/// backend.
pub fn resolve_search_tools_enabled(
    ...
    let disable = atelier_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = atelier_config::env_bool(primary).or_else(|| atelier_config::env_bool(alias));
    ...
    (
        resolve("ATELIER_TOOLS_FIND_BFS", "ATELIER_FIND_BFS", "find_bfs"),
        resolve(
            "ATELIER_TOOLS_GREP_UGREP",
            "ATELIER_GREP_UGREP",
            "grep_ugrep",
        ),
    )
...
const ENV_PERSISTENT_SHELL: &str = "ATELIER_PERSISTENT_SHELL";
...
mod persistent_local_shell_tests {
    ...
    // ATELIER_PERSISTENT_SHELL is process-global (the documented kill-switch a dev
...
/// the shared [`atelier_config::env_bool`] via `BoolFlag`). The secs env var
/// lives in the tools crate (`RESPONSE_TIMEOUT_ENV`), parsed once there.
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "ATELIER_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
///
/// Precedence: requirements > env (`ATELIER_ASK_USER_QUESTION_TIMEOUT_ENABLED`)
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            atelier_tools::implementations::atelier_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}
...
///
/// Precedence: requirements > env (`ATELIER_ASK_USER_QUESTION_TIMEOUT_SECS`,
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        atelier_tools::implementations::atelier_build::ask_user_question::response_timeout_env_secs(
        ),
...
mod ask_user_question_timeout_tests {
    ...
    use atelier_tools::implementations::atelier_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = atelier_tools::implementations::atelier_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes ATELIER_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
