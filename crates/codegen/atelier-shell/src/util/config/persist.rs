...
/// Process-wide write lock for `~/.atelier/config.toml`.
///
...
mod tests {
    ...
    fn merge_section_updates_modeled_fields_preserving_unmodeled() {
        ...
        ui.insert(
            ...
            TomlValue::String("atelierday".into()),
        );
        table.insert("ui".into(), TomlValue::Table(ui));
...
"#;
        ...
        assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("atelierday"));
...
"#;
        ...
        cfg.models.default = Some("atelier-4".to_string());
    ...
    fn models_config_serializes_only_some_fields() {
        let m = crate::agent::config::ModelsConfig {
            default: Some("atelier-3".to_string()),
            ..Default::default()
        };
        ...
        if let TomlValue::Table(t) = v {
            ...
            assert_eq!(t.get("default").and_then(|x| x.as_str()), Some("atelier-3"));
        } else {
    ...
    fn merge_section_models_only_updates_set_fields_preserves_others() {
        ...
        let cfg = crate::agent::config::ModelsConfig {
            default: Some("atelier-new".to_string()),
            ..Default::default()
        };
    ...
    fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
        ...
        let root: TomlValue = toml::from_str(original).unwrap();
        let mut cfg = load_config_from_toml(&root);
        cfg.models.default = Some("atelier-persisted".to_string());
        ...
        assert_eq!(
            reloaded.models.default.as_deref(),
            Some("atelier-persisted")
        );
        let cfg2 = crate::agent::config::Config::new_from_toml_cfg(&reloaded_root)
            .expect("new_from_toml_cfg");
        assert_eq!(cfg2.models.default.as_deref(), Some("atelier-persisted"));
    }
    ...
    mod resolve_auto_compact {
        ...
        const TEST_MODEL: &str = "atelier-4.5";
        const OTHER_MODEL: &str = "atelier-4.3";

        /// Serialize tests that mutate `ATELIER_AUTO_COMPACT_THRESHOLD_PERCENT`.
    ...
    fn settings_helpers_target_correct_ui_fields() {
        ...
        let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("atelierday".to_string()));
        assert_eq!(cfg.ui.auto_light_theme, Some("atelierday".to_string()));
