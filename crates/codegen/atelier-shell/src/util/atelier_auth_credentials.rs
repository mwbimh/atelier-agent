...
use std::sync::Arc;
/// Credentials for authenticating with atelier backend services.
///
...
/// User token (xAI users) sends `Bearer` + `X-XAI-Token-Auth: atelier-cli`.
/// Deployment key takes precedence when both are present.
#[derive(Clone)]
pub struct AtelierAuthCredentials {
...
impl std::fmt::Debug for AtelierAuthCredentials {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("AtelierAuthCredentials")
            .field(
                "user_token",
...
impl AtelierAuthCredentials {
    ...
    pub fn auth_error_hint(&self) -> &'static str {
        if self.deployment_key.is_some() {
            "Your ATELIER_DEPLOYMENT_KEY is invalid or expired. Please contact a team admin."
        } else if self.user_token.is_some() {
            "Your auth token is invalid or expired. Run `atelier login` to re-authenticate."
        } else {
            "Not authenticated."
        }
    }
    ...
    pub fn resolve(&self) -> AtelierAuthCredentials {
    ...
    /// the bearer.
    pub async fn resolve_async(&self) -> AtelierAuthCredentials {
...
impl atelier_auth::HttpAuth for AtelierAuthCredentials {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        AtelierAuthCredentials::apply(self, builder, base_url)
    }
}
#[cfg(test)]
mod tests {
    ...
    fn resolve_returns_token_when_not_expired() {
        ...
        let creds = AtelierAuthCredentials::new(None).with_auth_manager(mgr);
    ...
    fn resolve_returns_token_during_early_invalidation_window() {
        ...
        let creds = AtelierAuthCredentials::new(None).with_auth_manager(mgr.clone());
    ...
    fn resolve_returns_static_token_when_no_auth_manager() {
        let creds = AtelierAuthCredentials::new(Some("static-token".into()));
