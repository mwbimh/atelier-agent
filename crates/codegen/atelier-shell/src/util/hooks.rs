...
use atelier_hooks::discovery::HookSource;
...
pub fn discover_hook_source_paths(
    ...
    // sources. Native .atelier/hooks/ directories are still scanned (they hold
    ...
    // user_atelier_home() is None when no home resolves, so inspect lists the same
    // sources a live session loads, instead of a cwd-relative .atelier.
    ...
    if let Some(ref atelier) = atelier {
        global.push(atelier.join("hooks"));
    }

    let custom_paths: Vec<PathBuf> = atelier
        .as_ref()
    ...
    if let Some(root) = git_root {
        ...
        project.push(root.join(".atelier").join("hooks"));
        if !skip_cursor {
...
pub fn discover_hooks(
    ...
    atelier_hooks::discovery::HookRegistry,
    Vec<atelier_hooks::error::HookError>,
) {
