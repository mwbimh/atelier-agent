...
pub struct ContextInfo {
    ...
    /// always matches the actual trigger (e.g. 65 for atelier-build in remote settings).
...
pub struct SessionInfoData {
    /// Agent definition name for this session (e.g. `atelier-build`).
...
pub fn is_coding_model_slug(model: &str) -> bool {
    matches!(model, "atelier-build" | "atelier-4.5")
}
...
/// Full wire response for `atelier/session/info`.
///
...
mod tests {
    ...
    fn should_show_model_fingerprint_truth_table() {
        ...
        assert!(should_show_model_fingerprint(false, "atelier-build"));
        assert!(should_show_model_fingerprint(false, "atelier-4.5"));
