...
use atelier_sampling_types::truncate_bytes;
...
use atelier_tools::types::compat::CompatConfig;
use atelier_tools::types::output::{
    BashOutput, ReadFileOutput, ToolOutput as ToolsToolOutput, ToolRunResult,
};
use atelier_workspace::file_system::CodebaseIndexManager;
use atelier_workspace::permission::{
    AccessKind, ClientType, Decision, PermissionEvent, PermissionHandle,
};
use atelier_workspace::session::file_state::{FileStateHandle, FileStateTracker};
...
pub(crate) use spawn::*;
/// Client-registered hook gates (the `atelier/hooks/run` reverse request).
mod hooks;
...
pub(crate) struct SessionActor {
    ...
    /// `atelier-sampler`. Threaded into every `SamplerConfig`
    /// reconstructed by `reconstruct_full_config`. `None` when the
    /// session was spawned without an `AuthManager` (BYOK direct
    /// mode, test fixtures).
    pub(crate) attribution_callback: Option<atelier_sampler::SharedAttributionCallback>,
    ...
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<atelier_sampling_types::CompactionAtTokens>>,
    ...
    /// sampler itself sends the matching `x-atelier-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<atelier_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) agent: std::cell::RefCell<atelier_agent::Agent>,
    /// Dedup slot for `atelier/git_head_changed`, shared with the fs-watch
    /// `GitHead` consumer (see `git_head_dedup_key`).
    pub(crate) last_reported_branch: Arc<parking_lot::Mutex<Option<String>>>,
    /// Client opted into `atelier/gitHeadChanged`. When false (headless/SDK),
    ...
    /// Kill-switch (`ATELIER_GOAL_USE_CURRENT_MODEL_ONLY` / `[features]
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<atelier_tools::implementations::atelier_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) mcp_announced_servers:
        Mutex<HashMap<String, atelier_tools::implementations::search_tool::ServerFingerprint>>,
    ...
    pub(crate) hook_registry:
        std::cell::RefCell<Option<Arc<atelier_hooks::discovery::HookRegistry>>>,
    /// Client hooks from `session/new` `_meta["atelier/hooks"]`; gated in
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<atelier_agent::plugins::PluginRegistry>>>,
    /// Shared handle to the agent-level plugin registry.
    /// Used by `/plugins reload` to trigger a rebuild that new sessions see.
    pub(crate) plugin_registry_handle: Option<atelier_agent::plugins::SharedPluginRegistryHandle>,
    ...
    /// Handle to the per-session `atelier-sampler` actor.
    ///
    ...
    pub(crate) sampler_handle: atelier_sampler::SamplerHandle,
    /// Cached recipe for constructing this session's [`atelier_agent::Agent`].
    ///
    /// Populated once at session spawn and then reused by
    /// `handle_rebuild_agent_for_definition` to build a fresh `Agent`
    /// (system prompt, [`atelier_tools::bridge::ToolBridge`], tool
    ...
    fn build_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == atelier_tools::implementations::atelier_build::SCHEDULER_CREATE_TOOL_NAME
            }),
...
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<atelier_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering, `atelier prompt --json`
/// inspection, and post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &atelier_agent::PromptContext) {
...
) -> Option<atelier_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use atelier_tools::types::output::{MCPOutput, ToolOutput};
    use atelier_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl atelier_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
...
mod tool_meta_stamp_tests {
    //! Pin the `atelier/tool` stamps on the harness emission paths: the early
    ...
    use atelier_tools::tool_taxonomy::TOOL_META_KEY;
    use atelier_workspace::permission::PermissionCommand;
    use tokio::sync::mpsc;
    ...
    /// The `atelier/tool` object from an event's `_meta`, if present.
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "AtelierBuild:read_file".to_string(),
                    )])
                    .await,
                );
                ...
                let t = tool_meta(early.as_ref()).expect("early ToolCall carries atelier/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["namespace"], "atelier_build");
                assert!(t.get("input").is_none(), "identity-only before parse");
                let refined = refined.expect("refinement ToolCallUpdate emitted");
                let t = tool_meta(refined.as_ref()).expect("refinement carries atelier/tool");
        ...
        local
            .run_until(async {
                ...
                let t = tool_meta(update.meta.as_ref())
                    .expect("permission-request ToolCallUpdate carries atelier/tool");
                assert_eq!(t["name"], "read_file");
                assert_eq!(t["kind"], "read");
                assert_eq!(t["input"]["path"], "/tmp/stamp.txt");
            })
...
mod managed_gateway_tool_tests {
    ...
    use atelier_tools::types::output::{MCPOutput, ToolOutput};
    use atelier_tools::types::tool::{ToolKind, ToolNamespace};
    use atelier_tools::types::tool_metadata::ToolMetadata;
    #[derive(Debug)]
    struct FixtureMcpTool;
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<atelier_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
