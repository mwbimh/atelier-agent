...
pub enum SessionCommand {
    ...
    /// rather than the agent-level default (e.g. `atelier-4.5`).
    ///
    /// Keeps the existing base_url, api_key, and other config — only changes
    /// the `model` field sent in the `x-atelier-model-override` header and merges
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<atelier_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.atelier/hooks`,
    ...
    /// Out-of-band history repair (`atelier/session/repair`): fix tool-pairing
    ...
    /// mid-session via the `atelier/session/update_mcp_servers` extension method).
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<atelier_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        task_id: String,
        respond_to: oneshot::Sender<Result<atelier_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<atelier_tools::types::TaskSnapshot>>>,
    },
    ...
    PluginsList {
        respond_to: oneshot::Sender<Option<std::sync::Arc<atelier_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// serialized mailbox; the rebroadcast of `atelier/queue/changed` is the
    ...
    /// `atelier/queue/changed` is the truth signal for every attached client.
    ...
    /// tool-free model call (default `atelier-build-0.1` when available via
    ...
    /// `atelier-build` model, low temperature, and capped output tokens.
    RewriteMemoryNote {
        raw_text: String,
    ...
    Interject {
        ...
        /// `atelier/session/interjection` so the originating pager can dedup its
    ...
    TakeHarnessTraceTurns {
        respond_to:
            oneshot::Sender<Vec<Vec<atelier_sampling_types::conversation::ConversationItem>>>,
    },
