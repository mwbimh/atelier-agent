...
/// Dedup key for `atelier/git_head_changed`, shared by the watcher's `GitHead`
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> atelier_workspace::file_system::FileIndexDelta {
    use atelier_workspace::file_system::FileIndexDelta;
...
pub(crate) struct CapabilityInputs {
    ...
    pub hunk_tracking: bool,
    pub code_nav: bool,
    /// `atelier/gitHeadChanged`; opt-in (absent => off).
...
impl ClientNotify {
    async fn on_change(&self, paths: &[PathBuf], kind: FsEventKind) {
        ...
        match self.mode {
            ClientFsMode::Events => {
                // Present-tense strings are the `atelier/fs_notify` wire protocol;
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = atelier_workspace::session::git::get_branch(&self.cwd).await;
        let commit = atelier_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = atelier_workspace::session::git::get_worktree_info(&self.cwd).await;
...
impl FsWatchPlan {
    pub(crate) fn build(caps: FsWatchCapabilities, deps: FsWatchDeps) -> Self {
        ...
        let hunk = (caps.hunk_tracking && deps.hunk_tracking_enabled).then(|| {
            let git_root = atelier_workspace::session::git::find_git_root_from_path(&deps.cwd).ok();
            HunkTracking {
