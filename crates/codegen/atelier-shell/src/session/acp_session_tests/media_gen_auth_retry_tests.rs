...
fn ok_result(text: &str) -> Result<ToolRunResult, atelier_tool_runtime::ToolError> {
    Ok(ToolRunResult {
...
fn err(msg: &str) -> Result<ToolRunResult, atelier_tool_runtime::ToolError> {
...
fn http_err(status: u16, msg: &str) -> Result<ToolRunResult, atelier_tool_runtime::ToolError> {
    Err(atelier_tool_runtime::ToolError::new(
        atelier_tool_runtime::ToolErrorKind::Custom,
        msg.to_owned(),
    )
...
fn is_auth_tool_error_classification() {
    ...
    let cases: Vec<(bool, atelier_tool_runtime::ToolError)> = vec![
        ...
        (
            true,
            atelier_tool_runtime::ToolError::new(
                atelier_tool_runtime::ToolErrorKind::Custom,
        ...
        // the inference path's gate in atelier-sampling-types/src/error.rs:
        ...
        (
            true,
            atelier_tool_runtime::ToolError::invalid_arguments(r#"{"error":"invalid_token"}"#),
        ),
        // Fallback path: case-insensitive "unauthorized" anywhere in
        // the message body.
        (
            true,
            atelier_tool_runtime::ToolError::invalid_arguments("UNAUTHORIZED"),
        ),
        ...
        (
            ...
            atelier_tool_runtime::ToolError::not_found(
                atelier_tool_protocol::ToolId::new("image_gen").expect("valid"),
        ...
        (
            false,
            atelier_tool_runtime::ToolError::invalid_arguments("request id req_401abc failed"),
        ),
    ];
