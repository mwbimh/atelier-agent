...
//!
//! These tests verify the parallel dispatch path (ATELIER_PARALLEL_TOOL_DISPATCH):
...
/// JSON key. The compat toolset input types use `path`, and atelier_build's
...
#[test]
fn lock_path_for_args_matches_atelier_build_file_path() {
    // atelier_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_atelier_build_target_file() {
    // atelier_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_atelier_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch (e.g. atelier_build search_replace + StrReplace
    ...
    assert_eq!(lock_path_for_args(&atelier), lock_path_for_args(&compat));
}
