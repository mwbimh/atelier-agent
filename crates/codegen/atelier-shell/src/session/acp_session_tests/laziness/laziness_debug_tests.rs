...
fn assistant_with_reasoning_items(
    ...
    if !reasoning_text.is_empty() {
        out.push(ConversationItem::Reasoning(
            atelier_sampling_types::rs::ReasoningItem {
                id: String::new(),
                summary: vec![atelier_sampling_types::rs::SummaryPart::SummaryText(
                    atelier_sampling_types::rs::SummaryTextContent {
...
fn flatten_skips_reasoning_when_encrypted_only() {
    ...
    let items = vec![
        ConversationItem::Reasoning(atelier_sampling_types::rs::ReasoningItem {
            id: String::new(),
            summary: vec![],
            content: None,
...
fn synthetic_user_text(
    text: &str,
    reason: atelier_sampling_types::SyntheticReason,
) -> ConversationItem {
    ConversationItem::User(UserItem {
        content: vec![ContentPart::Text { text: text.into() }],
...
fn window_assistant_text_pin_skips_empty_assistant_turns() {
    ...
    let empty_asst = ConversationItem::Assistant(atelier_sampling_types::AssistantItem {
...
fn sample_line() -> LazinessDebugLogLine {
    LazinessDebugLogLine {
        ...
        model_id: "atelier-4.5".to_string(),
        items_sent: 28,
