...
/// shape, so it also covers the laxer atelier ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use atelier_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use atelier_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // atelier backend => strip_reasoning=false; the over-budget branch strips anyway.
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = atelier_sampling_types::build_messages_request(&req);
