...
fn auth_manager_with_refresher(
    ...
    let am = Arc::new(AuthManager::new(dir.path(), AtelierComConfig::default()));
    am.hot_swap(AtelierAuth {
...
/// attribution.
fn auth_error() -> atelier_sampler::SamplingErrorInfo {
    atelier_sampler::SamplingErrorInfo {
        kind: atelier_sampler::SamplingErrorKind::Auth,
...
fn model_not_found_error() -> atelier_sampler::SamplingErrorInfo {
    atelier_sampler::SamplingErrorInfo {
            kind: atelier_sampler::SamplingErrorKind::Api,
            message: "API error (status 404 Not Found): The model atelier-build does not exist or your team does not have access".into(),
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "hint must mention `atelier logout`, got: {msg}"
            );
            assert!(
                msg.contains("atelier login"),
                "hint must mention `atelier login`, got: {msg}"
            );
            assert!(
...
fn unauthorized_401_error() -> atelier_sampler::SamplingErrorInfo {
    atelier_sampler::SamplingErrorInfo {
            kind: atelier_sampler::SamplingErrorKind::Api,
            message: "Unauthorized (401) from https://cli-chat-proxy.atelier.com/v1/responses: {\"error\":\"Invalid or expired credentials (auth_kind=bearer, x_xai_token_auth=atelier-cli, upstream=Unauthenticated, reason=no auth context)\"}".into(),
            status_code: Some(401),
            is_retryable: false,
            retry_after_secs: None,
            model_metadata: None,
