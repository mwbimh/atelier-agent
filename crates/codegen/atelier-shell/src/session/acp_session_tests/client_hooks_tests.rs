...
/// `atelier/hooks/run` request is answered with a deny and `run_pre_tool_use_client_hook`
    ...
    local
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            // Answer the atelier/hooks/run reverse request with a deny; ack the UI
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                atelier_tools::registry::types::ToolConfig::for_tool::<
                    atelier_tools::implementations::use_tool::UseTool,
                >(),
            ])
            .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        atelier_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
                    callback_ids: vec!["cb_0".to_string()],
                    timeout: None,
                }],
...
/// post-tool event is observed as a fire-and-forget `atelier/hooks/event` notification.
#[tokio::test(flavor = "current_thread")]
async fn post_tool_use_and_failure_never_double_fire() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_atelier_build_agent_with_todo().await;
    ...
    local
        .run_until(async {
            ...
            *actor.client_hooks.borrow_mut() = client_hooks;
            ...
            // Answer the reverse atelier/hooks/run request with a deny carrying a reason;
