...
#[cfg(test)]
pub(crate) async fn test_agent_default() -> atelier_agent::Agent {
...
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> atelier_agent::Agent {
    use atelier_tools::implementations::atelier_build::update_goal::UpdateGoalTool;
    use atelier_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
/// Atelier-build agent with the real `TodoWriteTool` (id `todo_write`, kind
/// `Plan`) registered, so `tool_for_kind(ToolKind::Plan)` resolves through the
/// live toolset instead of the literal fallback.
#[cfg(test)]
pub(crate) async fn test_atelier_build_agent_with_todo() -> atelier_agent::Agent {
    use atelier_tools::implementations::atelier_build::todo::TodoWriteTool;
    use atelier_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> atelier_agent::Agent {
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<atelier_tools::registry::types::ToolConfig>,
) -> atelier_agent::Agent {
    test_agent_from_config(
        atelier_tools::registry::types::ToolServerConfig {
            tools,
            behavior_preset: None,
        },
        atelier_agent::AgentDefinition::default_atelier_build(),
        std::sync::Arc::new(atelier_tools::computer::local::LocalTerminalBackend::new()),
    )
    .await
}
#[cfg(test)]
async fn test_agent_from_config(
    ...
    definition: atelier_agent::AgentDefinition,
    backend: std::sync::Arc<dyn atelier_tools::computer::types::TerminalBackend>,
) -> atelier_agent::Agent {
    use atelier_tools::computer::local::LocalFs;
    use atelier_tools::computer::types::AsyncFileSystem;
    ...
    atelier_agent::Agent::new(
        definition,
        atelier_agent::PromptContext::default(),
        String::new(),
        tool_bridge,
        atelier_agent::ReminderPolicy::default(),
        atelier_agent::CompactionPolicy::default(),
        vec![],
        false,
    )
}
...
pub(crate) async fn create_test_actor_ex(
    ...
    let actor = SessionActor {
        ...
        permissions: atelier_workspace::permission::PermissionHandle::allow_all(),
        tool_context,
