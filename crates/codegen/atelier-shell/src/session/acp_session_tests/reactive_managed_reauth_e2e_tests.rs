...
//! `atelier/mcp/server_status` pushes that clients consuming only `server_status`
...
//! handshake against rmcp 2.1 in `atelier-mcp/tests/repro_sse_flood.rs`): a
...
use atelier_mcp::servers::{ClientStateKind, HttpConfig, McpClient};
use axum::body::Body;
...
const MANAGED: &str = "atelier_com_testconnector";
...
/// Drain all `atelier/mcp/server_status` pushes currently queued on `gw_rx`.
...
async fn entry_b_routes_auth_rejection_but_not_policy_denial() {
    use atelier_mcp::servers::{is_auth_rejection_message, parse_mcp_tool_name};

    // Managed-prefix gate: only `atelier_com_*` tools enter entry-B.
