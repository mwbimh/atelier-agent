...
//!
//! Tests mutate `ATELIER_GOAL_CLASSIFIER` so they carry `serial`.
...
use atelier_tools::implementations::atelier_build::update_goal::UpdateGoalInput;
...
async fn answer_summarizer(
    ...
    req: Box<atelier_tools::implementations::atelier_build::task::types::SubagentRequest>,
) {
    match behaviour {
...
fn configure_goal_actor(
    ...
    actor.tool_context.cwd =
        atelier_paths::AbsPathBuf::new(tmp.path().to_path_buf()).expect("abs cwd");
