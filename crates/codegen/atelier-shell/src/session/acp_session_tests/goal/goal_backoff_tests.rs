    ...
    local
        .run_until(async {
            ...
            *actor.goal_update_rx.borrow_mut() = Some(rx);
            tx.send(
                atelier_tools::implementations::atelier_build::update_goal::envelope_for_test(
...
async fn drain_goal_updates_completes_after_blocked_does_not_leak_pause_message() {
    ...
    local
            .run_until(async {
                ...
                let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
                *actor.goal_update_rx.borrow_mut() = Some(rx);
                tx.send(atelier_tools::implementations::atelier_build::update_goal::envelope_for_test(atelier_tools::implementations::atelier_build::update_goal::UpdateGoalInput {
                        completed: None,
                        message: None,
...
-> atelier_tools::implementations::atelier_build::update_goal::UpdateGoalEnvelope {
    let input = atelier_tools::implementations::atelier_build::update_goal::UpdateGoalInput {
        completed: Some(true),
        message: None,
        blocked_reason: None,
    };
...
async fn subagent_spawn_captures_effective_model_id() {
    ...
    local
        .run_until(async {
            ...
            assert_eq!(model.as_deref(), Some("atelier-4.5"));
        })
        .await;
}
...
async fn goal_tokens_by_model_breaks_down_active_goal_records() {
    ...
    local
        .run_until(async {
            let actor = make_test_actor_with_active_goal().await;
            insert_record_with_model(&actor, "a", Some("test-goal"), 0, 100, Some("atelier-4"));
            insert_record_with_model(&actor, "b", Some("test-goal"), 0, 400, Some("atelier-3"));
            ...
            insert_record_with_model(&actor, "d", Some("other-goal"), 0, 999, Some("atelier-3"));
            ...
            // finished/in-flight split in goal_tokens). If it leaked, atelier-4
            // would be 800 and sort first.
            insert_record_with_model(&actor, "e", Some("test-goal"), 0, 700, Some("atelier-4"));
            actor
                .subagent_token_records
                .lock()
                .get_mut("e")
            ...
            assert_eq!(
                out,
                vec![
                    ("atelier-3".to_owned(), 400),
                    ("atelier-4".to_owned(), 100),
