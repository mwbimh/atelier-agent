...
/// Local skill is always listable, so it renders into the atelier markdown skill
/// catalog when the pending baseline is drained for a different agent.
fn regression_skill() -> atelier_tools::implementations::skills::types::SkillInfo {
...
/// Regression: a zero-turn agent rebuild INTO a atelier/Default agent
/// must re-inject the baseline skill `<system-reminder>`. `initialize()`
/// is otherwise the only place skills are surfaced for the atelier agent, so
    ...
    local
        .run_until(async {
            ...
            assert!(
                ...
                "reminder must carry the atelier skill catalog header:\n{text}",
            );
            assert!(
