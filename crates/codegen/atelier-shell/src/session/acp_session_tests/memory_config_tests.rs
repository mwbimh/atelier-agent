...
async fn create_injection_ready_actor(
    initial_conversation: Vec<atelier_sampling_types::ConversationItem>,
) -> SessionActor {
    ...
    local
        .run_until(async {
            let actor = create_injection_ready_actor(vec![
                atelier_sampling_types::ConversationItem::system("You are a helpful assistant."),
                atelier_sampling_types::ConversationItem::user(
            ...
            assert!(
                reminder.contains(atelier_chat_state::MEMORY_CONTEXT_OPEN_TAG),
        ...
        .run_until(async {
            let persisted_block =
                crate::session::helpers::memory_context::format_memory_reminder(&[
                    atelier_tools::types::memory_backend::MemorySearchResult {
                ...
                .unwrap();
            let actor = create_injection_ready_actor(vec![
                atelier_sampling_types::ConversationItem::system(format!(
