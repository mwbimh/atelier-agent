...
/// user's next queued prompt) and rebroadcasts `atelier/queue/changed`. The
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.push_assistant_response(
                ConversationItem::assistant_tool_calls(vec![atelier_sampling_types::ToolCall {
                    id: "call-1".into(),
    ...
    local
        .run_until(async {
            ...
            let (sampler_event_tx, _sampler_event_rx) = tokio::sync::mpsc::unbounded_channel::<
                atelier_sampler::SamplingEvent,
            >();
            let sampler_handle = atelier_sampler::SamplerActor::spawn(
                cfg,
                atelier_sampler::RetryPolicy::default(),
                sampler_event_tx,
            );
            let (gateway_tx, _gateway_rx) = tokio::sync::mpsc::unbounded_channel::<
                xai_acp_lib::AcpClientMessage,
            ...
            let request = ConversationRequest {
                items: vec![
                    ConversationItem::User(atelier_sampling_types::UserItem { content :
                    vec![atelier_sampling_types::ContentPart::Text { text : "hi".into(),
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use atelier_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(
                    ...
                    matches!(p, atelier_sampling_types::ContentPart::Text { text } if
