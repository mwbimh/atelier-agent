...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: atelier_tools::implementations::atelier_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"atelier-build-plan"`)
...
mod tests {
    ...
    fn planner_template_default_render_preserves_wording_and_has_no_placeholders() {
        ...
        // (atelier-build) tool names; guards against accidental wording drift.
    ...
    fn planner_prompt_pins_minimal_honest_path_for_unobservable_behavior() {
        let rendered = RoleToolNames::inherit_defaults().apply(GOAL_PLANNER_PROMPT_TEMPLATE);
        assert!(rendered.contains("MINIMAL honest path"));
        assert!(rendered.contains("verifier will then\n  rightly call theater"));
    }
