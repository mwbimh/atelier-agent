...
pub async fn fork_session(
    ...
    tracing::info!(
        ...
        "FORK_COPY: session data copied locally"
    );
...
mod tests {
    ...
    #[test]
    fn fork_writeback_is_disabled() {
        // Fork state is intentionally local-only in the private build.
        const FORK_WRITEBACK_ENABLED: bool = false;
        assert!(!FORK_WRITEBACK_ENABLED);
    }
    ...
    fn test_fork_session_request_serialization() {
        ...
        assert_eq!(deserialized.new_model_id, Some("atelier-3".to_string()));
    }
