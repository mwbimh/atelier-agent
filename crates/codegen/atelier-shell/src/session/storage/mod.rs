...
/// Like [`load_updates_for_replay`], but resolves the session under a specific atelier home.
...
mod tests {
    ...
    fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
        ...
        let old_xai = r#"{"timestamp":2,"method":"_atelier/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
        let raw = format!("{a1}\n{old_xai}\n");
        ...
        let new_xai = r#"{"timestamp":2,"method":"_atelier/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
        let raw = format!("{a1}\n{new_xai}\n");
        let prepared = prepare_replay_lines(&raw, Some("ev1"));
        assert!(!prepared.mark_replay);
        assert_eq!(prepared.lines.len(), 1);
    ...
    fn prepare_replay_reports_spawn_without_finish() {
        let spawn = |id: &str, child: &str| {
            format!(
                r#"{{"method":"_atelier/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
            )
        };
        let finish = |id: &str| {
            format!(
                r#"{{"method":"_atelier/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
            )
        };
