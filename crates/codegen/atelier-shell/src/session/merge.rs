...
//!
//! Used by both the ACP `atelier/session/list` handler and the `atelier sessions`
...
pub async fn fetch_merged(
    ...
    let repo_urls_fut = async {
        cwd.map(|c| {
            atelier_workspace::session::git::resolve_normalized_remote_urls(std::path::Path::new(c))
        })
        .unwrap_or_default()
    };
