...
pub(super) async fn run_session(
    ...
    if !session.startup_hints.is_subagent && liveness_watchers_enabled {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<atelier_mcp::servers::McpClientEvent>();
        {
    ...
    loop {
        tokio::select! {
            ...
            } else { tracing::debug!(target : atelier_telemetry::memory_log::TARGET,
            ...
            atelier_telemetry::memory_log::TARGET, "MEMORY_DREAM_CHECK: timer fired");
            ...
            .fire_hook(atelier_hooks::event::HookEventName::SessionEnd, None,
            atelier_hooks::event::HookPayload::SessionEnd { reason : "channel_closed"
            ...
            atelier_hooks::dispatcher::dispatch_non_blocking(& registry,
            atelier_hooks::event::HookEventName::SessionEnd, & envelope, & ctx,). await;
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(atelier_hooks::event::HookEventName::Stop,
            None, atelier_hooks::event::HookPayload::Stop { reason : "channel_closed"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            atelier_hooks::dispatcher::dispatch_non_blocking(& registry,
            atelier_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
            ...
            atelier_telemetry::memory_log::TARGET, result = ? result, tool_searches =
            ...
            atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::SessionEnded
            ...
            .push_capped(atelier_tools::implementations::atelier_build::task::types::MonitorEventNotification
            ...
            atelier_agent::prompt::skills::list_skills_with_plugins(Some(& cwd), &
            ...
            .send(atelier_mcp::servers::McpClientEvent::ConfigDiff { added : diff.added
            ...
            .forward_fire_and_forget(acp::ExtNotification::new("atelier/mcp/tools_changed",
            ...
            session.fire_hook(atelier_hooks::event::HookEventName::SessionStart, None,
            atelier_hooks::event::HookPayload::SessionStart { source, model_id : None,
            agent_type : None, },); if let Some(registry) = session.hook_registry
            .borrow().clone() { let ctx = session.hook_run_ctx(); let results =
            atelier_hooks::dispatcher::dispatch_non_blocking(& registry,
            atelier_hooks::event::HookEventName::SessionStart, & envelope, & ctx,).
            ...
            .fire_hook(atelier_hooks::event::HookEventName::SessionEnd, None,
            atelier_hooks::event::HookPayload::SessionEnd { reason : "shutdown"
            .to_string(), turn_count : None, tool_call_count : None, },); if let
            Some(registry) = session.hook_registry.borrow().clone() { let ctx = session
            .hook_run_ctx(); let results =
            atelier_hooks::dispatcher::dispatch_non_blocking(& registry,
            ...
            session.send_hook_execution("session_end", None, None, & results). await; }
            let envelope = session.fire_hook(atelier_hooks::event::HookEventName::Stop,
            None, atelier_hooks::event::HookPayload::Stop { reason : "shutdown"
            .to_string(), },); if let Some(registry) = session.hook_registry.borrow()
            .clone() { let ctx = session.hook_run_ctx(); let results =
            atelier_hooks::dispatcher::dispatch_non_blocking(& registry,
            atelier_hooks::event::HookEventName::Stop, & envelope, & ctx,). await;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[atelier_sampling_types::ConversationItem],
    turn_number: usize,
