...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Atelier-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
///   `AccessKind::Edit` and is plan-file-only (same as atelier edits). Deleting
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &atelier_tools::implementations::atelier_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
impl SessionActor {
    /// Merge the canonical `atelier/tool` identity envelope into a tool-call
    ...
    pub(super) fn stamp_tool_meta(
        ...
        atelier_tools::normalization::merge_tool_meta(
            &toolset,
        ...
        while let Some((idx, mut result)) = dispatch_rx.recv().await {
            ...
            let tool_loop = match result {
                Ok(tool_result) => {
                    ...
                    post_tool_use_result = self
                        .hook_event_active(atelier_hooks::event::HookEventName::PostToolUse)
                        .then(|| {
                ...
                Err(err) => {
                    ...
                    if self
                        .hook_event_active(atelier_hooks::event::HookEventName::PostToolUseFailure)
                    {
                        let raw_input: serde_json::Value =
                            serde_json::from_str(&prepared.raw_arguments)
                                .unwrap_or(serde_json::Value::Null);
                        let (tool_input_value, tool_input_truncated) =
                            atelier_hooks::event::truncate_payload(raw_input);
                        let hook_tool_name = prepared.hook_tool_name();
                        self.dispatch_hook(
                            atelier_hooks::event::HookEventName::PostToolUseFailure,
                            atelier_hooks::event::HookPayload::PostToolUseFailure {
            ...
            if let Some(tool_result_value) = post_tool_use_result {
                ...
                let (tool_result_val, tool_result_truncated) =
                    atelier_hooks::event::truncate_payload(tool_result_value);
                let hook_tool_name = prepared.hook_tool_name();
                self.dispatch_hook(
                    atelier_hooks::event::HookEventName::PostToolUse,
                    atelier_hooks::event::HookPayload::PostToolUse {
                        tool_name: hook_tool_name.to_owned(),
                        tool_use_id: prepared.call_id.clone(),
                        tool_input: tool_input_value,
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        if self.hook_event_active(atelier_hooks::event::HookEventName::PreToolUse) {
            ...
            let envelope = self.make_hook_envelope(
                ...
                None,
                atelier_hooks::event::HookPayload::PreToolUse {
            ...
            if let Some(registry) = hook_registry_snapshot {
                ...
                let pre_result =
                    atelier_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx)
                        .await;
                ...
                .await;
                if let atelier_hooks::result::HookDecision::Deny { reason, hook_name } =
                    pre_result.decision
                {
                    return Ok(Err(self
                        .deny_tool(
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                atelier_workspace::permission::AccessKind::Read(p) => (
                    atelier_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                atelier_workspace::permission::AccessKind::Edit(p) => {
                    (atelier_telemetry::events::AccessKind::Edit, p.clone())
                }
                atelier_workspace::permission::AccessKind::Bash(cmd) => {
                    (atelier_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                atelier_workspace::permission::AccessKind::Grep { path, glob } => (
                    atelier_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                atelier_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (atelier_telemetry::events::AccessKind::Mcp, name.clone())
                }
                atelier_workspace::permission::AccessKind::WebFetch(u) => {
                    (atelier_telemetry::events::AccessKind::Web, u.clone())
                }
                atelier_workspace::permission::AccessKind::WebSearch(q) => {
                    (atelier_telemetry::events::AccessKind::Web, q.clone())
                }
            };
            ...
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::events::PermissionPrompted {
                    tool_name: call.function.name.clone(),
            ...
            let (decision_outcome, _reject_reason) = match &decision {
                Decision::Allow | Decision::Ask => {
                    (atelier_telemetry::events::PermissionOutcome::Allow, None)
                }
                Decision::Reject(reason) | Decision::PolicyDeny(reason) => (
                    atelier_telemetry::events::PermissionOutcome::Deny,
                    Some(reason.to_string()),
                ),
                Decision::Cancelled => (
                    atelier_telemetry::events::PermissionOutcome::Cancelled,
                    None,
                ),
                Decision::FollowupMessage(_) => {
                    (atelier_telemetry::events::PermissionOutcome::Followup, None)
                }
            };
            tracing::info_span!(
                "tool.decision", tool_name = % call.function.name, tool_use_id = % call
            ...
            .in_scope(|| {});
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::events::PermissionDecisionPayload {
                    tool_name: call.function.name.clone(),
                    access_kind: telemetry_access_kind,
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        atelier_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        atelier_hooks::event::HookEventName::PermissionDenied,
                        atelier_hooks::event::HookPayload::PermissionDenied {
                            tool_name: resolved_tool_name.clone(),
    ...
    /// Issue the `atelier/exit_plan_mode` reverse-request and await the user's
    ...
    pub(super) async fn request_plan_approval(
        ...
        atelier_tools::implementations::atelier_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use atelier_tools::implementations::atelier_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                atelier_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
    ...
    async fn send_tool_call_start(
        ...
        let (title, kind, locations, content) = match tool_call_input {
            ...
            ToolInput::ReadFile(read_file) => (
                ...
                vec![
                    acp::ToolCallLocation::new(read_file.path).line(
                        atelier_tools::normalization::norm_offset_i64(read_file.offset)
                            .map(|l| l as u32),
                    ),
                ],
                Vec::new(),
            ...
            ToolInput::Skill(skill) => {
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::SkillDispatched {
    ...
    /// `atelier_tools::reminders::task_completion::consumed_completion_ids`,
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use atelier_telemetry::enums::PrCreationSource;
        use atelier_tools::util::git_detect;
        match &result.output {
            atelier_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    atelier_telemetry::session_ctx::log_event(
                        atelier_telemetry::events::PrMerged {},
                    );
                }
            }
            atelier_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: atelier_tools::util::git_detect::PrRef,
        source: atelier_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        let consumed_ids =
            atelier_tools::reminders::task_completion::consumed_completion_ids(&result.output);
        if !consumed_ids.is_empty() {
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                atelier_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let atelier_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                atelier_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | atelier_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            atelier_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            atelier_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
    ...
    /// Translate one [`atelier_sampler::SamplingEvent`] from the
    ...
    pub(crate) async fn handle_sampling_event(
        self: &Arc<Self>,
        event: atelier_sampler::SamplingEvent,
    ) {
        use atelier_sampler::{SamplingChannel, SamplingEvent};
        match event {
...
fn execute_tool_call_parts(
    ...
    let display = atelier_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod plan_mode_edit_gate_tests {
    ...
    use atelier_workspace::permission::AccessKind;
    ...
    fn search_replace(path: &str) -> ToolInput {
        use atelier_tools::implementations::atelier_build::search_replace::SearchReplaceInput;
    ...
    fn write(path: &str) -> ToolInput {
        use atelier_tools::implementations::opencode::write::WriteInput;
        ToolInput::Write(WriteInput {
            file_path: path.into(),
            content: "x".into(),
        })
    }
    /// Atelier edit tools are plan-file-only while plan mode is active — the
    /// enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn atelier_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "atelier tools get no markdown exception — plan file only"
        );
    }
    ...
    fn apply_patch_rejected_in_plan_mode() {
        use atelier_tools::implementations::codex::apply_patch::ApplyPatchInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use atelier_tools::implementations::atelier_build::exit_plan_mode::ExitPlanModeExtResponse;
