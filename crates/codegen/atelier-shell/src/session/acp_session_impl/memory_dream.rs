...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        &self,
        bridge: &atelier_tools::bridge::ToolBridge,
    ) -> Result<(), String> {
        ...
        bridge
            .register_mcp_tools(
                ...
                atelier_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                atelier_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        atelier_telemetry::session_ctx::log_event(
            atelier_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_atelier_conv_id: Some(session_id.clone()),
            x_atelier_req_id: Some(format!("xai-dream-{}", uuid::Uuid::new_v4())),
            x_atelier_session_id: Some(session_id),
    ...
    pub(super) async fn run_memory_flush(
        ...
        let result = async {
            ...
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            let request = ConversationRequest {
                ...
                model: Some(model),
                x_atelier_conv_id: Some(session_id.clone()),
                x_atelier_req_id: Some(format!("xai-flush-{}", uuid::Uuid::new_v4())),
                x_atelier_session_id: Some(session_id.clone()),
                x_atelier_agent_id: Some(atelier_telemetry::id::agent_id()),
                ..Default::default()
            };
        ...
        tracing::info!(target: atelier_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        atelier_telemetry::session_ctx::log_event(
            atelier_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => atelier_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => atelier_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => atelier_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => atelier_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::MemoryFlushed {
    ...
    /// LLM call using the `atelier-build` model.
    ///
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let request = ConversationRequest {
            items,
            tools: vec![],
            model: Some("atelier-build".to_owned()),
        ...
        let result = match sampling_client.api_backend() {
            crate::sampling::ApiBackend::ChatCompletions => {
                ...
                let events =
                    atelier_sampler::stream_chat_completions(raw, meta, request_id, idle_timeout);
                atelier_sampler::collect_response(events).await
            }
            crate::sampling::ApiBackend::Responses => {
                let (raw, meta, doom_loop) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = atelier_sampler::stream_responses(
                    raw,
                    meta,
                    request_id,
                    idle_timeout,
            ...
            crate::sampling::ApiBackend::Messages => {
                let (raw, meta) = sampling_client
                    ...
                    .await
                    .map_err(|e| format!("rewrite stream failed: {e}"))?;
                let events = atelier_sampler::stream_messages(raw, meta, request_id, idle_timeout);
                atelier_sampler::collect_response(events).await
            }
        };
