...
/// bookkeeping (rewind and file state) and built-in file tools therefore use
/// the same worker connection when the session is worker-backed.
#[derive(Clone)]
struct WorkspaceFsToolAdapter {
    fs: atelier_workspace::file_system::AsyncFsWrapper,
}

impl WorkspaceFsToolAdapter {
    fn new(fs: atelier_workspace::file_system::AsyncFsWrapper) -> Self {
        Self { fs }
    }
}

#[async_trait::async_trait]
impl atelier_tools::computer::types::AsyncFileSystem for WorkspaceFsToolAdapter {
    async fn read_file(
        &self,
        path: &std::path::Path,
...
fn drop_cli_catchall_allows(
    rules: Vec<atelier_workspace::permission::types::PermissionRule>,
    policy_block: Option<&'static str>,
) -> (
    Vec<atelier_workspace::permission::types::PermissionRule>,
    Vec<atelier_workspace::permission::types::PermissionRule>,
) {
    ...
    for rule in rules {
        if atelier_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
mod cli_catchall_drop_tests {
    ...
    use atelier_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use atelier_workspace::permission::rules::parse_permission_rule;
    use atelier_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    web_search_sampling_config: Option<atelier_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: atelier_tools::implementations::atelier_build::deploy_app::AppBuilderDeployerConfig,
    ...
    hook_registry_override: Option<std::sync::Arc<atelier_hooks::discovery::HookRegistry>>,
    ...
    parent_scheduler_handle: Option<
        atelier_tools::implementations::atelier_build::scheduler::types::SchedulerHandle,
    >,
    ...
    atelier_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(atelier_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            atelier_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
            )
            .await;
        let yolo_pin = atelier_workspace::permission::resolution::yolo_disabled_by_policy();
            ...
            && !dropped_catchalls.is_empty()
        {
            ...
            if startup_hints.non_interactive {
                eprintln!("atelier: --allow catch-all ignored: {reason}");
            }
        }
        if !cli_permission_rules.is_empty() {
            match &mut permission_config {
                ...
                None => {
                    permission_config =
                        Some(atelier_workspace::permission::types::PermissionConfig::new(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(atelier_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if atelier_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    atelier_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
                ...
                .map(|t| {
                    std::sync::Arc::new(t)
                        as std::sync::Arc<
                            dyn atelier_workspace::permission::PermissionHookTransport,
                        >
                });
        ...
        let (permissions, permission_events_rx) =
            atelier_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        atelier_tools::implementations::WebSearchConfig::Disabled
    } else if let Some(cfg) = web_search_sampling_config {
        if let Some(api_key) = cfg.api_key {
            atelier_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("ATELIER_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    if let Some(cw) = context_window_override {
        tracing::warn!(
            ...
            "ATELIER_DEBUG_CONTEXT_WINDOW override active"
        );
    }
    ...
    let auto_wake_delivered =
        atelier_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        atelier_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        atelier_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
    ...
    let terminal_backend: std::sync::Arc<dyn atelier_tools::computer::types::TerminalBackend> =
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn atelier_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn atelier_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(atelier_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(WorkspaceFsToolAdapter::new(tool_context.fs.clone()))
        };
    ...
    let compaction_policy = atelier_agent::CompactionPolicy {
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn atelier_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        if let Err(e) = storage.ensure_initialized() {
            tracing::warn!(
                target : atelier_telemetry::memory_log::TARGET, error = % e,
        ...
        }
        {
            ...
            tokio::task::spawn_blocking(move || match gc_storage.gc(gc_max_age) {
                Ok(removed) if removed > 0 => {
                    tracing::info!(
                        target : atelier_telemetry::memory_log::TARGET, removed,
        ...
        let params = crate::session::memory::MemoryBackendParams {
            ...
            auth_credentials: auth_manager.as_ref().map(|am| {
                ...
                ) as std::sync::Arc<dyn atelier_auth::AuthCredentialProvider>
            }),
        };
        ...
        atelier_telemetry::session_ctx::log_event(
            atelier_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
    ...
    let mcp_state = {
        ...
        if !acp_mcp_servers.is_empty() {
            ...
            tracing::info!(
                ...
                "Registered in-process SDK MCP servers (atelier/mcp/sdk_call)"
            );
        }
    ...
    let resolved_task_output =
        atelier_tools::reminders::task_completion::resolve_task_output_tool_name(
            agent.tool_bridge(),
        )
        .await;
    let resolved_read =
        atelier_tools::reminders::task_completion::resolve_read_tool_name(agent.tool_bridge())
            .await;
    ...
    tool_context.task_output_tool_name = resolved_task_output.unwrap_or_else(|| {
        atelier_tools::reminders::task_completion::DEFAULT_TASK_OUTPUT_TOOL.to_string()
    });
    let scheduler_handle_for_handle = {
        let toolset = agent.tool_bridge().toolset();
        let res = toolset.resources.lock().await;
        res.get::<atelier_tools::implementations::atelier_build::scheduler::types::SchedulerHandle>(
        )
        .cloned()
    };
    ...
    let sampler_retry_policy = atelier_sampler::RetryPolicy {
        max_retries: max_retries.unwrap_or(5),
        rate_limit_retry_threshold: 2,
    };
    let (sampler_event_tx, sampler_event_rx) =
        tokio::sync::mpsc::unbounded_channel::<atelier_sampler::SamplingEvent>();
    let sampler_handle = atelier_sampler::SamplerActor::spawn(
    ...
    let mut hook_discovery_errors: Vec<atelier_hooks::error::HookError> = Vec::new();
    let built_hook_registry: Option<Arc<atelier_hooks::discovery::HookRegistry>> =
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        max_turns,
        max_retries: atelier_sampler::resolve_max_retries(max_retries),
        ...
        vcs_kind: {
            ...
            match atelier_workspace::session::git::discover_git_root(root) {
                atelier_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                    atelier_workspace::session::git::detect_vcs_kind(&git_root)
                }
                _ => atelier_workspace::session::git::VcsKind::None,
            }
        },
    ...
    }
    {
        ...
        session
            ...
            .borrow()
            .tool_bridge()
            .update_resource(atelier_tools::types::resources::PlanFilePath(plan_path))
            .await;
    }
    ...
    session
        ...
        .tool_bridge()
        .update_resource(
            atelier_tools::implementations::atelier_build::update_goal::GoalUpdateHandle(
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                tracing::info!(
                    target : atelier_telemetry::memory_log::TARGET, files = files.len(),
                ...
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use atelier_tools::implementations::atelier_build::ask_user_question::AskUserQuestionMode;
    ...
    let telemetry_ctx = atelier_telemetry::session_ctx::TelemetryCtx::new(
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            atelier_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    tokio::task::spawn_local(async move {
        atelier_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
...
> {
    let (init_tx, init_rx) = tokio::sync::oneshot::channel::<
        Result<SessionInitResult, atelier_agent::AgentBuildError>,
    >();
