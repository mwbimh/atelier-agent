...
impl SessionActor {
        ...
        match &notification.update {
            ExtensionSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    atelier_hooks::event::HookEventName::SubagentStart,
                    None,
                    atelier_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = atelier_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        atelier_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
            ...
            ExtensionSessionUpdate::SubagentFinished {
                ...
                let envelope = self.fire_hook(
                    atelier_hooks::event::HookEventName::SubagentEnd,
                    None,
                    atelier_hooks::event::HookPayload::SubagentStop {
                        subagent_id: subagent_id.clone(),
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            atelier_hooks::event::HookEventName::Notification,
            None,
            atelier_hooks::event::HookPayload::Notification {
