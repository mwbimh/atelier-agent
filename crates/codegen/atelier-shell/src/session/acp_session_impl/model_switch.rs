...
impl SessionActor {
    ...
    async fn apply_sampling_config(
        ...
        *self.image_generation_endpoint.borrow_mut() =
        ...
        self.chat_state_handle
            .update_sampling_config(atelier_sampling_types::SamplingConfig {
                ...
                if let ConversationItem::System(sys) = item {
                    if use_concise {
                        sys.content = std::sync::Arc::<str>::from(
                            atelier_agent::prompt::template::COMPACT_SYSTEM_PROMPT,
                        );
                    } else {
                        sys.content =
    ...
    /// [`atelier_agent::AgentDefinition`], replaces `self.agent`,
