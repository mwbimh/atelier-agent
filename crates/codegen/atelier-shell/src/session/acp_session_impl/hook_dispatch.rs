...
) -> atelier_tool_protocol::turn_hook::TurnHookOutcome {
...
) -> atelier_tool_protocol::session_event::ToolCallOutcome {
...
pub(super) fn notification_hook_for_update(
    ...
    match update {
        ExtensionSessionUpdate::DiffReview { .. } => Some((
            "permission_prompt".into(),
            Some("Diff review requested".into()),
            None,
            Some("info".into()),
        )),
        ExtensionSessionUpdate::AutoRecoveryExhausted { error, .. } => Some((
            "agent_error".into(),
            Some(error.clone()),
            None,
            Some("error".into()),
        )),
        ExtensionSessionUpdate::RetryState(RetryState::Exhausted { reason, .. }) => Some((
            "agent_error".into(),
            Some(reason.clone()),
            None,
            Some("error".into()),
        )),
        ExtensionSessionUpdate::RetryState(RetryState::Failed { message, .. }) => Some((
            "agent_error".into(),
            Some(message.clone()),
            None,
            Some("error".into()),
...
impl SessionActor {
    pub(super) fn hook_run_ctx(&self) -> atelier_hooks::runner::RunContext<'_> {
        atelier_hooks::runner::RunContext {
    ...
    pub(super) async fn send_hook_annotation(&self, message: &str) {
        self.send_extension_notification(ExtensionSessionUpdate::HookAnnotation {
    ...
    pub(super) async fn send_hook_execution(
        ...
        results: &[atelier_hooks::result::HookRunResult],
    ) {
        if results.is_empty()
            || results
                .iter()
                .all(|r| matches!(r, atelier_hooks::result::HookRunResult::Skipped { .. }))
        {
            return;
        }
        use crate::extensions::notification::{HookRunEntryDto, HookRunStatusDto};
        use atelier_hooks::result::HookRunResult;
        ...
        self.send_extension_notification(ExtensionSessionUpdate::HookExecution {
    ...
    pub(super) async fn dispatch_hook(
        &self,
        event: atelier_hooks::event::HookEventName,
        payload: atelier_hooks::event::HookPayload,
        prompt_id: Option<&str>,
        tool_name: Option<&str>,
    ) {
        ...
        let results =
            atelier_hooks::dispatcher::dispatch_non_blocking(&registry, event, &envelope, &ctx)
                .await;
    ...
    pub(super) async fn emit_hook_executed_telemetry(
        ...
        for r in results {
            let (hook_name, elapsed, outcome) = match r {
                atelier_hooks::result::HookRunResult::Success {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    atelier_telemetry::events::HookOutcome::Success,
                ),
                atelier_hooks::result::HookRunResult::Failed {
                    hook_name, elapsed, ..
                } => (
                    hook_name,
                    elapsed,
                    atelier_telemetry::events::HookOutcome::Error,
                ),
                atelier_hooks::result::HookRunResult::Skipped { .. } => continue,
            };
            atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::HookExecuted {
...
mod notification_hook_filter_tests {
    ...
    fn hook_execution_does_not_fire_notification_hook() {
        let update = ExtensionSessionUpdate::HookExecution {
    ...
    fn hook_annotation_does_not_fire_notification_hook() {
        let update = ExtensionSessionUpdate::HookAnnotation {
    ...
    fn retry_in_progress_does_not_fire_notification_hook() {
        let update = ExtensionSessionUpdate::RetryState(RetryState::Retrying {
            attempt: 1,
            max_retries: 3,
            reason: "timeout".into(),
        });
    ...
    fn feedback_request_does_not_fire_notification_hook() {
        let update = ExtensionSessionUpdate::FeedbackRequest(FeedbackRequestNotification {
    ...
    fn diff_review_fires_permission_prompt() {
        let update = ExtensionSessionUpdate::DiffReview { content: vec![] };
    ...
    fn task_completed_does_not_fire_via_filter() {
        let update = ExtensionSessionUpdate::TaskCompleted {
            task_snapshot: atelier_tools::types::TaskSnapshot {
                task_id: "task-1".into(),
