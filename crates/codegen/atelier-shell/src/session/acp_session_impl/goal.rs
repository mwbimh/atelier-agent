...
impl SessionActor {
    ...
    pub(super) async fn drain_goal_updates_with_extra(
        ...
        extra: Vec<atelier_tools::implementations::atelier_build::update_goal::UpdateGoalEnvelope>,
    ) {
    ...
    pub(crate) async fn resolve_goal_role_override(
        ...
        //    silently run atelier-build flavor. Non-strict names (atelier-build
        //    family) run the default flavor and pass; unresolvable names fall
        //    through to the describe `Unknown` arm below.
        if atelier_agent::config::is_strict_harness_agent_type(&pair.agent_type)
    ...
    /// agrees with `from_summary` — `search_replace` on the default atelier-build
    /// host, not the literal `write`). The `{TOOLSET_TOOLS}` block is empty on
    /// this path (no per-role toolset enumeration); on the default atelier-build
    ...
    pub(super) async fn prune_prior_goal_continuation_directives(&self) {
        use atelier_sampling_types::conversation::SyntheticReason;
