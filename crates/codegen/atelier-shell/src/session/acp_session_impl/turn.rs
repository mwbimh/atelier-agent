...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &atelier_tools::implementations::atelier_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
impl SessionActor {
    ...
    pub(super) async fn handle_prompt(
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let prompt_blocks = match slash_commands::resolve(
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = atelier_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        atelier_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    atelier_telemetry::session_ctx::log_event(
                        atelier_telemetry::events::SlashCommandUsed {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        atelier_telemetry::session_ctx::log_event(
                            atelier_telemetry::events::PluginUsed {
        ...
        atelier_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
        ...
        *self.tool_context.prompt_index.lock().await = current_prompt_index;
        ...
        let (query, extra_images) = if !self.is_cursor_harness() {
            let extraction = atelier_tools::util::base64_images::extract_base64_images(query);
        ...
        self.tool_bridge_handle()
            .update_resource(atelier_tools::types::resources::AttachedImages(
                attached_image_refs,
            ))
            .await;
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) => {
                ...
                .await;
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::TurnCompleted {
                        outcome: atelier_telemetry::events::Outcome::Completed,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::TurnCompleted {
                        outcome: atelier_telemetry::events::Outcome::Cancelled,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                atelier_telemetry::session_ctx::log_session_event(
                    atelier_telemetry::events::ApiError {
                ...
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::TurnCompleted {
                        outcome: atelier_telemetry::events::Outcome::Error,
                        duration_ms: turn_duration_ms,
                        tool_call_count: turn_tool_count,
                        model_id: turn_model_id,
                        cancellation_category: None,
                ...
                self.dispatch_hook(
                    atelier_hooks::event::HookEventName::StopFailure,
                    atelier_hooks::event::HookPayload::StopFailure {
                        error: format!("{err}"),
                    },
                    Some(prompt_id),
                    None,
        ...
        self.dispatch_hook(
            atelier_hooks::event::HookEventName::Stop,
            atelier_hooks::event::HookPayload::Stop {
    ...
    /// monitor events only via the structured `atelier/monitor_event` channel.
    pub(crate) async fn inject_pending_monitor_events(&self) {
        let Some(buffer) = &self.tool_context.monitor_event_buffer else {
            return;
        };
        let mine = atelier_tools::implementations::atelier_build::task::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = atelier_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = atelier_tools::reminders::wrap_reminder(&body);
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        use atelier_tools::types::memory_backend::MemoryBackend as _;
        ...
        tracing::info!(
            target : atelier_telemetry::memory_log::TARGET, configured_min_score,
            "MEMORY_INJECT_SEARCH: results={result_count}"
        );
        atelier_telemetry::session_ctx::log_event(
            atelier_telemetry::memory_telemetry::MemoryInjection {
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<atelier_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            request.x_atelier_session_id = Some(self.session_info.id.to_string());
            request.x_atelier_turn_idx =
                Some(self.chat_state_handle.get_prompt_index().await.to_string());
            request.x_atelier_agent_id = Some(atelier_telemetry::id::agent_id());
            if request.x_atelier_deployment_id.is_none() {
                request.x_atelier_deployment_id = crate::managed_config::resolve_deployment_id(
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            let turn_refused =
                stop_reason == Some(atelier_sampling_types::StopReason::ContentFilter);
            ...
            for item in response.items {
                match item {
                    atelier_sampling_types::ConversationItem::Assistant(_) => {
...
mod user_echo_broadcast_tests {
    ...
    /// the text from the `atelier/session/interjection` broadcast, so a live
