...
impl SessionActor {
    ...
    /// Emit per-server `atelier/mcp/tools_changed` notifications.
    ///
    ...
    /// Handle explicit auth trigger from the client (atelier/mcp/auth_trigger).
    ///
    ...
    pub(super) async fn handle_mcp_auth_trigger(&self, server_name: &str) -> Result<(), String> {
        if server_name.starts_with(crate::session::managed_mcp::MANAGED_MCP_PREFIX) {
            return Err("To authenticate, visit atelier.invalid".to_string());
        }
        let client = {
    ...
    /// or removed it from `~/.atelier/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.atelier/config.toml` + the project
    /// `.atelier/config.toml`) on every call. The auto-restart task
    /// calls this at most:
    ...
    /// `AtelierClientHandler`, and the liveness watcher is armed —
    ...
    ///
    /// The `AtelierClientHandler` constructed inside `try_handshake`
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(atelier_mcp::servers::McpClientEvent::ToolsChanged {
                server: server.to_string(),
            });
        }
        let arc_client = std::sync::Arc::new(new_client);
        let _ = arc_client
            .arm_liveness_watcher(atelier_mcp::liveness::DEFAULT_POLL_INTERVAL)
            .await;
        {
            let mut mcp_state = self.mcp_state.lock().await;
            mcp_state
    ...
    pub(super) async fn ensure_mcp_tools_initialized(&self) {
        ...
        if mcp_server_configs.is_empty() && !has_acp {
            ...
            if mcp_state.generation() == generation {
                ...
                self.events
                    .emit(atelier_runtime_events::events::Event::McpInitCancelled {
        ...
        }
        {
            ...
            let managed_count = mcp_server_configs
                ...
                .count() as u32;
            self.events.emit(
                atelier_runtime_events::events::Event::McpManagedConfigResult {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(atelier_runtime_events::events::Event::McpServerFailed {
                            server_name: sname,
        ...
        tokio::task::spawn_local(async move {
            ...
            for client in mcp_clients.iter() {
                ...
                futs.push(async move {
                    ...
                    ew.emit(atelier_runtime_events::events::Event::McpServerStarting {
            ...
            > = std::collections::HashMap::new();
            {
                ...
                if mcp_state.generation() != generation {
                    ...
                    event_writer.emit(atelier_runtime_events::events::Event::McpInitCancelled {
                        reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                    });
                    return;
                }
                    ...
                    match result {
                        Ok((server_name, registrations, elapsed, timeout_sec)) => {
                            ...
                            for reg in registrations {
                                ...
                                if reg.model_visible {
                                        ...
                                        .await
                                    {
                                        ...
                                        event_writer
                                            .emit(atelier_runtime_events::events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                            ...
                            let transport_enum = match server_transport_map
                                ...
                                .unwrap_or("unknown")
                            {
                                "stdio" => atelier_telemetry::events::McpTransport::Stdio,
                                "sse" => atelier_telemetry::events::McpTransport::Sse,
                                _ => atelier_telemetry::events::McpTransport::Http,
                            };
                            atelier_telemetry::session_ctx::log_event(
                                atelier_telemetry::events::McpServerConnected {
                                    server_name: server_name.clone(),
                                    tool_count,
                            ...
                            let transport_str = server_transport_map
                                ...
                                .unwrap_or("unknown");
                            event_writer.emit(
                                atelier_runtime_events::events::Event::McpServerConnected {
                                    server_name: server_name.clone(),
                        ...
                        Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                            ...
                            let error_type_label = match error_cat {
                                atelier_runtime_events::events::McpErrorCategory::AuthRequired => {
                                    atelier_telemetry::events::McpErrorType::Auth
                                }
                                atelier_runtime_events::events::McpErrorCategory::Timeout => {
                                    atelier_telemetry::events::McpErrorType::Timeout
                                }
                                _ => atelier_telemetry::events::McpErrorType::HandshakeFailed,
                            };
                            atelier_telemetry::session_ctx::log_event(
                            ...
                            event_writer.emit(
                                atelier_runtime_events::events::Event::McpServerFailed {
                                    server_name: server_name.clone(),
                            ...
                            let detail = (!needs_auth).then(|| {
                                atelier_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                    .into_owned()
                            });
                ...
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::McpInitCompleted {
                ...
                event_writer.emit(atelier_runtime_events::events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<atelier_tools::types::tool_index::ServerSummary> {
        use atelier_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            atelier_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
        if let Some(hint) = self.rendered_mcp_hint().await {
            text.push_str(&hint);
        }
