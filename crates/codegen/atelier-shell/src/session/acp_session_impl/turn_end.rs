...
impl SessionActor {
    ...
    /// Emit `atelier/git_head_changed` after an edit/shell command that may have
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch) = tokio::join!(
            atelier_workspace::session::git::get_worktree_info(cwd),
            atelier_workspace::session::git::get_branch(cwd),
        );
        ...
        let params = atelier_workspace::session::git::GitHeadChanged {
        ...
        if let Ok(raw) = serde_json::value::to_raw_value(&params) {
            let notification = acp::ExtNotification::new("atelier/git_head_changed", raw.into());
            self.notifications
                .gateway
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<atelier_tools::implementations::atelier_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) async fn handle_completion(&self, prompt_id: String, result: PromptTurnResult) {
        ...
        if became_idle {
            ...
            self.agent
                ...
                .tool_bridge()
                .update_resource(
                    atelier_tools::implementations::atelier_build::task::types::CurrentPromptIdResource(
                        String::new(),
                    ),
                )
                .await;
        ...
        // replayed `_atelier/session/update` rail so a viewer that re-attaches
    ...
    /// persists + forwards via `send_extension_notification`. Retiring
    ...
    pub(super) async fn emit_turn_completed(
        ...
        self.send_extension_notification_with_extra_meta(
    ...
    pub(super) fn classify_install_error(
        err: &atelier_agent::plugins::install_registry::InstallError,
    ) -> String {
