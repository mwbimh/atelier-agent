...
use super::*;
/// Partition the AGENTS.md / Claude.md / `.atelier/rules/*.md` files returned
/// by `read_agents_config_with_paths` into "workspace" (cwd / repo root /
/// extra workspace user dir) and "user" (`~/.atelier/`, `~/.claude/`,
/// `~/.atelier/bundled/`) buckets, mirroring the split between
...
fn partition_rules_by_scope(
    files: Vec<atelier_agent::prompt::agents_md::AgentConfigFile>,
) -> (
    Vec<atelier_agent::prompt::user_message::RuleEntry>,
    Vec<atelier_agent::prompt::user_message::RuleEntry>,
) {
    let home = dirs::home_dir().map(|p| p.to_string_lossy().to_string());
    let user_prefixes: Vec<String> = match home {
        Some(h) => vec![format!("{h}/.atelier/"), format!("{h}/.claude/")],
        None => vec![],
    };
    let mut workspace = Vec::new();
    let mut user = Vec::new();
    for f in files {
        let entry = atelier_agent::prompt::user_message::RuleEntry::from(f);
...
/// end for atelier ordering), so a failed offload never
...
impl SessionActor {
        ...
        if drop_startup_skill_reminder {
            conversation.retain(|item| {
                !matches!(
                    ...
                    Some(atelier_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    async fn build_templated_user_message(
        &self,
        cwd: &std::path::Path,
        template: atelier_agent::prompt::user_message::UserMessageTemplate,
    ) -> Option<String> {
        use atelier_agent::prompt::agents_md::read_agents_config_with_paths;
        use atelier_agent::prompt::user_message::UserMessageContext;
    ...
    async fn gather_vcs_for_prefix(
        ...
        use atelier_workspace::file_system::{git_status_short, jj_status};
        use atelier_workspace::session::git::VcsKind;
    ...
    ) -> Vec<atelier_agent::prompt::user_message::McpServerEntry> {
        use atelier_agent::prompt::user_message::McpServerEntry;
    ...
    pub(super) async fn transcribe_user_images(
        ...
        let client = atelier_sampler::SamplingClient::new(sampler_config).map_err(|e| {
