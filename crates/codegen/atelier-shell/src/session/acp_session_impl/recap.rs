...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(&self, question: &str) -> Result<String, String> {
        ...
        let request = ConversationRequest {
            ...
            temperature: None,
            x_atelier_conv_id: Some(btw_session_id.clone()),
            x_atelier_req_id: Some(format!("xai-btw-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model.clone()),
            temperature: None,
            x_atelier_conv_id: Some(x_atelier_conv_id.clone()),
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_atelier_req_id: x_atelier_req_id.to_owned(),
            x_atelier_conv_id: x_atelier_conv_id.to_owned(),
            strip_reasoning,
    ...
    /// (`ATELIER_PROMPT_SUGGESTIONS_MODEL`) > `[models] prompt_suggestion`
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// (config.toml) > remote `prompt_suggestion_model` (remote settings) >
    /// [`prompt_suggest::DEFAULT_SUGGEST_MODEL`] (`atelier-build-0.1`). Every
    ...
    /// `atelier-build-0.1` for OAuth users) the request is **skipped
    ...
    /// a reasoning model's response, and some models (e.g. `atelier-build`)
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            temperature: None,
            x_atelier_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_atelier_req_id: Some(format!("xai-promptsuggest-{}", uuid::Uuid::new_v4())),
            x_atelier_session_id: Some(self.session_info.id.to_string()),
            x_atelier_agent_id: Some(atelier_telemetry::id::agent_id()),
            ..Default::default()
        };
        ...
        tracing::debug!(
            raw_preview = %atelier_tools::util::truncate_str(raw.trim(), 60),
