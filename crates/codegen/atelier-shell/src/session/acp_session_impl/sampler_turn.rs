...
/// `SamplingError::is_auth_error` in atelier-sampling-types: 403 is
...
impl SessionActor {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl atelier_sampler::BearerResolver for AuthManagerBearerResolver {
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<atelier_workspace::permission::ClassifierMessage>,
            ...
            while let Some((messages, respond_to)) = rx.recv().await {
                let result = async {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            atelier_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            atelier_workspace::permission::ClassifierMessageRole::User => {
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            atelier_workspace::permission::classifier_output_json_schema(),
                        ),
                        reasoning_effort: classifier_reasoning_effort,
                        x_atelier_conv_id: Some(session_id.clone()),
                        x_atelier_req_id: Some(format!("xai-perm-auto-{}", uuid::Uuid::new_v4())),
        ...
        let clf =
            atelier_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    pub(super) async fn resolve_aux_sampler_config(
        &self,
        slug: &str,
    ) -> Option<atelier_sampler::SamplerConfig> {
        let creds = self.chat_state_handle.get_credentials().await;
        let session_key = self
            .auth_manager
            .as_ref()
        ...
        let disable_api_key_auth = self
            .auth_manager
            .as_ref()
            .map(|am| am.atelier_com_config().api_key_auth_disabled())
            .unwrap_or(false);
    ...
    async fn resolve_auto_classifier_sampler(
        &self,
        slug: &str,
    ) -> Option<(atelier_sampler::SamplingClient, String)> {
        ...
        let client = atelier_sampler::SamplingClient::new(cfg)
            .map_err(|e| {
                tracing::warn!(
                    error = % e,
    ...
    pub(crate) async fn prepare_chat_completion(
        ...
        let sampling_client =
            atelier_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
        Ok(sampling_client)
    }
    ...
    /// `atelier-sampler` instead of constructing a new
    /// `OaiCompatClient`.
    ///
    ...
    pub(crate) async fn handle_sampling_failure(
        self: &Arc<Self>,
        error: atelier_sampler::SamplingErrorInfo,
    ) -> Result<SamplerFailureRecovery, acp::Error> {
        use atelier_sampler::SamplingErrorKind;
        ...
        let client_version = atelier_version::VERSION;
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `atelier logout` then `atelier login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if atelier_sampling_types::is_context_length_error(&error.message) {
            "context_length"
        } else {
            error.kind.as_str()
        };
    ...
    pub(crate) async fn run_turn_via_sampler(
            ...
            .await
        {
            ...
            Err(rich_err) => {
                self.turn_stream_drained.lock().take();
                let info = atelier_sampler::SamplingErrorInfo::from(&rich_err);
