...
pub struct SessionHandle {
    ...
    /// `atelier/codeNavigation.enabled`.  Stored per-session so that in leader
    ...
    /// Exposed so the `atelier/toggle_plan_mode` handler can toggle plan mode
    ...
    /// Set via `atelier/debug/arm_auto_compact`.
    pub force_compact: std::sync::Arc<std::sync::atomic::AtomicBool>,
    pub permission_handle: atelier_workspace::permission::PermissionHandle,
    ...
    pub terminal_backend:
        Option<std::sync::Arc<dyn atelier_tools::computer::types::TerminalBackend>>,
...
impl SessionHandle {
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<atelier_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<atelier_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
