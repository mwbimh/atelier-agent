//! atelier-build's L5 wiring onto the shared full-replace engine
//! (`atelier_compaction::code_compaction`).
//!
//! The shared engine drives the sample → retry → degenerate/failure
//! classification loop via [`sample_full_replace_summary`](atelier_compaction::sample_full_replace_summary);
//! this module adapts atelier-build's transport and telemetry to its two seams:
//!
...
//! [`FullReplaceError`](atelier_compaction::FullReplaceError).
...
/// [`CompactionSampler`] for atelier-build's full-replace pass.
///
...
/// else the structured atelier-build prompt), so the shared `CompactionPrompt`
/// the engine passes is ignored — the engine builds the atelier-build prompt,
...
impl CompactionSampler for ShellCompactionSampler {
    ...
    async fn sample_compaction(
        ...
        // message (compat short vs structured atelier-build), ignoring the shared
...
/// Map atelier-build's [`CompactFailure`] onto the shared engine's
...
/// [`FullReplaceObserver`] that reproduces atelier-build's per-attempt telemetry:
    ...
    fn on_attempt(&self, _attempt: u32, outcome: &FullReplaceAttemptOutcome<'_>) {
        ...
        match outcome {
            ...
            FullReplaceAttemptOutcome::Degenerate {
                ...
                if *will_retry {
                    atelier_telemetry::session_ctx::log_event(CompactionRetryDegraded {
