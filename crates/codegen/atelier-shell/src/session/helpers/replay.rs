...
use crate::extensions::notification::{
    CompactionCheckpointFile, CompactionCheckpointInfo, SessionUpdate as ExtensionSessionUpdate,
};
...
pub fn find_latest_compaction_checkpoint(
    ...
    for line in raw_contents.lines() {
        ...
        if env.method != Some("_atelier/session/update") {
            continue;
        }
        ...
        >(raw_params.get())
            && let ExtensionSessionUpdate::CompactionCheckpoint(info) = notification.update
        {
            latest = Some(*info);
        }
    }
        ...
        match update {
            SessionUpdate::Extension(notification) => {
                match &notification.update {
                    ExtensionSessionUpdate::CompactionCheckpoint(info) => {
                        return self.handle_checkpoint(info, session_dir);
                    }
                    ExtensionSessionUpdate::RewindMarker {
                        target_prompt_index,
                        ..
                    } => {
                    ...
                    // Other extension notifications are informational — skip them.
                    _ => {}
                }
            }
...
mod tests {
    ...
    fn make_rewind_marker(target: usize) -> SessionUpdate {
        SessionUpdate::Extension(Box::new(ExtensionNotification {
    ...
    fn make_checkpoint(
        ...
        SessionUpdate::Extension(Box::new(ExtensionNotification {
            session_id: acp::SessionId::new("test"),
            update: ExtensionSessionUpdate::CompactionCheckpoint(Box::new(
