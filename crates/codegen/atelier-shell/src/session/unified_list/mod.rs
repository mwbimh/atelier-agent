...
pub fn conversations_lane_enabled() -> bool {
    ...
    std::env::var("ATELIER_SESSION_LIST_CONVERSATIONS")
        .ok()
        .is_some_and(|v| {
            !matches!(
...
pub fn force_kind_chat(req: &mut ListReq) {
    ...
    let mut filters = match meta.remove("atelier/facetFilters") {
        Some(serde_json::Value::Object(map)) => map,
        _ => serde_json::Map::new(),
    };
    filters.insert(
...
pub struct ExtListResponseMeta {
    #[serde(rename = "atelier/facets")]
    pub facets: FacetSummary,
    #[serde(rename = "atelier/partial")]
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            source: "local".into(),
            model_id: Some("atelier-build".into()),
            num_messages: 7,
    ...
    fn ext_superset_preserves_every_legacy_field_and_adds_title_and_meta() {
        ...
        assert_eq!(value["_meta"]["atelier/session"]["kind"], "build");
    ...
    fn parsed_meta_reads_facet_filters_query_and_limit() {
        let meta = serde_json::json!(
            { "atelier/facetFilters" : { "kind" : ["build"], "starred" : true },
            "atelier/query" : "antelope", "atelier/limit" : 5, }
        );
    ...
    fn forced_kind_preserves_other_facets() {
        let mut req = ListReq {
            meta: Some(serde_json::json!(
                { "atelier/facetFilters" : { "kind" : ["build"], "starred" : [true],
                "workspace" : ["w1"] }, "atelier/query" : "antelope", "atelier/limit" : 5,
                }
            )),
            ..ListReq::default()
        };
    ...
    async fn forced_kind_serves_conversations_only() {
        ...
        .await;
        let _env = atelier_test_support::EnvGuard::set(
            "ATELIER_CONVERSATIONS_BASE_URL",
            format!("http://{addr}"),
        );
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = atelier_test_support::EnvGuard::unset("ATELIER_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on =
                atelier_test_support::EnvGuard::set("ATELIER_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off =
                atelier_test_support::EnvGuard::set("ATELIER_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        use crate::agent::chat_modes::ATELIER_CHAT_MODE_ENV;
        let _chat_off = atelier_test_support::EnvGuard::unset(ATELIER_CHAT_MODE_ENV);
        let _desktop_off =
            atelier_test_support::EnvGuard::unset("ATELIER_SESSION_LIST_CONVERSATIONS");
        assert!(
        ...
        {
            let _chat = atelier_test_support::EnvGuard::set(ATELIER_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        let raw = serde_json::json!(
            { "_meta" : { "atelier/facetFilters" : { "kind" : ["build"], "starred" : [true]
            } }, }
        )
        .to_string();
        {
            let _off = atelier_test_support::EnvGuard::unset(ATELIER_CHAT_MODE_ENV);
        ...
        {
            let _on = atelier_test_support::EnvGuard::set(ATELIER_CHAT_MODE_ENV, "1");
            let req = parse_list_req(&raw).expect("parse");
            let parsed = ParsedMeta::parse(req.meta.as_ref());
    ...
    /// Wire pin for the cross-crate `atelier/partial` envelope the pager parses:
