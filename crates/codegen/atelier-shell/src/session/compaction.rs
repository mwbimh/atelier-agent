...
fn prefire_lead_percent() -> u64 {
    std::env::var("ATELIER_PREFIRE_LEAD_PERCENT")
        .ok()
...
mod two_pass_prefire_helper_tests {
    ...
    fn prefire_lead_percent_defaults_to_10() {
        unsafe { std::env::remove_var("ATELIER_PREFIRE_LEAD_PERCENT") };
...
impl SessionActor {
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("ATELIER_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target : "two_pass",
                "two_pass: DEBUG ATELIER_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
...
impl SessionActor {
    ...
    pub(crate) async fn run_compact(
        ...
        if let Err(e) = self
            .run_compact_inner(
                user_context,
                None,
                atelier_telemetry::events::CompactionTrigger::Manual,
            )
            .await
        {
            let span = tracing::Span::current();
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::events::AutoCompactSuppressed {
    ...
    async fn run_compact_inner(
        ...
        let trigger_str = match trigger {
            atelier_telemetry::events::CompactionTrigger::Manual => "manual",
            atelier_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = atelier_telemetry::events::CompactionScope::begin(
            trigger,
            tokens_before,
            context_window,
            model_id.clone(),
        ...
        self.dispatch_hook(
            atelier_hooks::event::HookEventName::PreCompact,
            atelier_hooks::event::HookPayload::PreCompact {
        ...
        let compaction_tools: Vec<atelier_sampling_types::ToolSpec> = effective_tool_defs
            ...
            .collect();
        let compaction_hosted_tools: Vec<atelier_sampling_types::HostedTool> = if use_backend_search
        {
        ...
        let fr_config = atelier_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            ...
            .await
            {
                ...
                Err(atelier_compaction::FullReplaceError::NothingToCompact) => {
                    last_error = Some(
                        acp::Error::internal_error().data("compact failed: nothing to compact"),
                    );
                    break;
                }
                Err(atelier_compaction::FullReplaceError::EmptyResponse) => {
                ...
                Err(atelier_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            atelier_telemetry::session_ctx::log_event(
                                atelier_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                atelier_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                atelier_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn atelier_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn atelier_tools::types::memory_backend::MemoryBackend)
            };
        ...
        if let Some(ref recovery_backend) = memory_backend_impl {
            ...
            if n > 0 {
                ...
                tracing::debug!(
                    target : atelier_telemetry::memory_log::TARGET, count = n,
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        atelier_sampling_types::ContentPart::Text { text } => {
        ...
        self.dispatch_hook(
            atelier_hooks::event::HookEventName::PostCompact,
            atelier_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &atelier_sampler::SamplingErrorInfo,
    ) -> bool {
        if self
            .compaction
            .auto_compact_suppressed
    ...
    pub(crate) async fn run_compact_only(
        ...
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::AutoCompactFired {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                atelier_telemetry::events::CompactionTrigger::Auto,
            )
            .await;
...
mod inline_auto_compact_flow_tests {
    ...
    use atelier_workspace::file_system::MockFs;
    use atelier_workspace::permission::PermissionHandle;
    ...
    async fn create_test_actor(
        ...
        let actor = SessionActor {
            ...
            vcs_kind: atelier_workspace::session::git::VcsKind::Git,
            ...
            sampler_handle: atelier_sampler::SamplerHandle::noop(),
    ...
    fn classify_suppress_reason_maps_error_text() {
        ...
        assert_eq!(
            classify("API error (status 402 Payment Required): Atelier usage balance exhausted"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
            classify("Atelier usage limit reached"),
            SuppressReason::CreditBlock
        );
        assert_eq!(
    ...
    fn api_error_with_context_window(context_window: u64) -> atelier_sampler::SamplingErrorInfo {
        atelier_sampler::SamplingErrorInfo {
            kind: atelier_sampler::SamplingErrorKind::Api,
