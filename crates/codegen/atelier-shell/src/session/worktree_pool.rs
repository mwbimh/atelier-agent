...
//!   under `~/.atelier/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.atelier/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.atelier/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.atelier/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.atelier/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.atelier/`.
...
/// `CleanAll` mode. Placed under `~/.atelier/worktree_pool/<instance_id>/<uuid>/`.
