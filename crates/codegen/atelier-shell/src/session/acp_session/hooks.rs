...
//!
//! Hooks registered at `session/new` (`_meta["atelier/hooks"]`) come in two flavors,
//! both matched by the agent ([`atelier_hooks::matcher::HookMatcher`], shared with
//! file hooks):
//! - **`PreToolUse` gate**: an awaited reverse *request* `atelier/hooks/run`; a `deny`
//!   blocks the tool.
//! - **All other events**: fire-and-forget *notifications* `atelier/hooks/event`,
...
use atelier_hooks::event::{HookEventEnvelope, HookEventName, HookPayload};
use atelier_telemetry::events::ClientHookGateOutcome;
...
const HOOK_EVENT_METHOD: &str = "atelier/hooks/event";
const HOOK_RUN_METHOD: &str = "atelier/hooks/run";

/// Default per-callback bound for a client's `atelier/hooks/run` reply; on timeout the gate
...
/// Outcome of the `atelier/hooks/run` reverse request, before interpreting it as a
    ...
    match outcome {
        ReverseOutcome::Responded(raw) => {
            match serde_json::from_str::<ClientHookResponse>(raw.get()) {
                Ok(resp) => {
                    ...
                    let label = match resp.decision {
                        ...
                        ClientHookDecision::Other => {
                            tracing::warn!(
                                "atelier/hooks/run returned an unknown decision value; failing open"
                            );
                ...
                Err(err) => {
                    tracing::warn!(%err, "malformed atelier/hooks/run response; failing open");
                    (
        ...
        ReverseOutcome::Transport(err) => {
            tracing::warn!(%err, "atelier/hooks/run transport error (no client wired?); failing open");
            (
                ClientHookResponse::default(),
        ...
        ReverseOutcome::Timeout => {
            tracing::warn!("atelier/hooks/run timed out; failing open");
            (
                ClientHookResponse::default(),
...
impl SessionActor {
    ...
    pub(super) async fn deny_tool(
        ...
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::HookBlocked {
    ...
    /// `atelier/hooks/run` once per matching callback with the shared `envelope` (the
    ...
    pub(super) async fn run_pre_tool_use_client_hook(
        ...
        let tool_name = atelier_hooks::dispatcher::extract_tool_name(envelope)
            ...
            .map(|(callback_id, timeout)| {
                ...
                async move {
                    ...
                    atelier_telemetry::session_ctx::log_event(
                        atelier_telemetry::events::ClientHookGate {
    ...
    /// Issue one `atelier/hooks/run` reverse request, bounded by a per-callback `timeout`.
    ...
    /// `atelier/hooks/event` notification to each matching registered callback.
...
mod tests {
    ...
    async fn hook_event_active_inert_vs_active() {
        use atelier_hooks::event::HookEventName;
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    atelier_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.hook_event_active(HookEventName::Stop));
