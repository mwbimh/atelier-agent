...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(atelier_version::VERSION.to_string());
    s
}

/// Pipeline: persist → strip → submit → telemetry (telemetry fields captured
/// before strip). Callers merge `ATELIER_USER_METADATA` and set
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if telemetry_enabled {
        ...
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// ratings). Gated by `ATELIER_FEEDBACK_ENABLED`.
    pub feedback_enabled: bool,
    /// Whether session analytics (signal sync, turn deltas) are enabled.
    /// Gated by `ATELIER_TELEMETRY_ENABLED`. These are analytics data that
...
impl FeedbackManager {
    ...
    /// `atelier/debug/trigger_feedback` ACP extension method to exercise
