...
//!
//! Receives [`atelier_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers
//!   ([`atelier_mcp::liveness`]),
//! - the [`atelier_mcp::servers::AtelierClientHandler`] (server-pushed
...
//! `atelier/mcp/server_status` and the payload schema defined by
...
pub const SERVER_STATUS_METHOD: &str = "atelier/mcp/server_status";
...
pub struct McpServerStatusPayload {
    ...
    /// MCP server name (`atelier_com_linear`, `github`, ...).
    pub name: String,
    /// `managed` (sourced from cli-chat-proxy / `atelier_com_` prefix)
    /// or `local` (user `.atelier/config.toml`).
...
pub enum McpServerStatus {
    /// Client is in [`atelier_mcp::servers::ClientStateKind::Ready`]
...
/// [`atelier_mcp::servers::McpClientEventKind`]). Used as the
...
pub fn build_payload(
    ...
    let (status, reason, detail) = match (kind, event) {
        ...
        // NeedsAuth ("visit atelier.invalid"), not a generic Unavailable, so a
        ...
        (McpClientEventKind::HandshakeFailed, McpClientEvent::HandshakeFailed { reason, .. })
            if source == McpServerSource::Managed
                && atelier_mcp::servers::is_auth_rejection_message(reason) =>
        {
            (
...
/// 3. `flush_window` — emit ACP `atelier/mcp/server_status` per
///    surviving entry.
...
mod tests {
    ...
    fn managed_handshake_non_auth_stays_unavailable() {
        for reason in ["403 Forbidden", "cli-chat-proxy returned 502"] {
            ...
            let ev = McpClientEvent::HandshakeFailed {
                server: "atelier_com_slack".to_string(),
    ...
    /// Contract: managed server names (starting with `atelier_com_`)
    /// are classified as `Managed`; everything else as `Local`.
    #[test]
    fn classify_source_uses_managed_prefix() {
        assert_eq!(
            classify_source("atelier_com_linear"),
            McpServerSource::Managed
        );
    ...
    async fn dispatcher_drops_dead_clients_on_transport_closed() {
        use atelier_mcp::servers::{McpClient, McpState};
    ...
    fn recoverable_http_servers_excludes_managed_stdio_and_disabled() {
        let configs = vec![
            ...
            http_cfg("atelier_com_slack"), // managed
