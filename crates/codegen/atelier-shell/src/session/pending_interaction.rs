...
use crate::extensions::notification::{
    SessionNotification, SessionUpdate as ExtensionSessionUpdate,
};
...
pub enum PendingKind {
    ...
    Question,
    /// `atelier/exit_plan_mode` plan approval.
    PlanApproval,
}

/// Whether a blocking plan-approval reverse-request is parked in `pending`.
///
/// The resume re-park issues `atelier/exit_plan_mode` from a detached task
...
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: ExtensionSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            ExtensionSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                ExtensionSessionUpdate::InteractionResolved {
