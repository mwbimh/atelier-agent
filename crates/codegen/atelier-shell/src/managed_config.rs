...
fn remove_managed_config_files(home: &std::path::Path) {
    ...
    for name in [
        "managed_config.toml",
        "requirements.toml",
        atelier_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
/// would just 401).
fn eligible_team_principal(auth: AtelierAuth) -> Option<AtelierAuth> {
...
/// config is a atelier.invalid feature with one atelier.invalid auth.
fn read_active_team_auth() -> Option<AtelierAuth> {
    let home = crate::util::atelier_home::atelier_home();
...
/// files (TUI tick vs `atelier login` vs prefetch). `None` on contention — the
...
#[derive(Clone, Copy)]
enum SyncBudget {
    /// Background loop and explicit `atelier setup`; runs retries to completion.
    Standard,
...
/// Base is 1s; `ATELIER_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("ATELIER_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
...
/// Map a `reqwest` send failure to a `ManagedConfigError` via the shared `atelier-http` classifier.
...
/// Override with `ATELIER_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("ATELIER_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
pub(crate) fn spawn_sync(cancel: tokio_util::sync::CancellationToken) {
    let _ = cancel;
    // Managed configuration is a vendor control plane and is intentionally
    // removed from Atelier.  Keeping this entry point as a no-op lets the
    // bootstrap lifecycle remain unchanged without starting a timer or
    // creating a hidden network path.
    tracing::debug!("managed config sync disabled");
}

/// Deployment id reported for `deployment_key` on chat requests, credential
/// snapshots, and OTel: the **server** AtelierBuildDeployment UUID (the id
...
/// Resolve deployment key from `ATELIER_DEPLOYMENT_KEY` env var, then config files.
...
/// Whether managed config fetching is enabled.
///
/// Managed configuration is a vendor control plane and is removed from the
/// private build. No environment variable, local config, or explicit command
/// may re-enable it.
pub fn is_fetch_enabled() -> bool {
    // Managed configuration is a vendor control plane. Atelier must remain
    // local-first, so no environment variable or config layer may re-enable
    // this network path.
    false
}

/// Fetch managed config + requirements and write to `~/.atelier/`, trying the
...
struct SyncOutcome {
    ...
    /// `atelier setup` doesn't misreport a lock skip as "no config".
    served: bool,
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<AtelierAuth>,
...
/// read-only `atelier setup --json` both build on this.
...
fn apply_fetched(
    ...
    let verified = if atelier_config::signed_policy::verification_active() {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(atelier_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        atelier_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
    }
...
/// Post-login hook for `atelier login` and the ACP/TUI authenticate flow: clear any
...
pub async fn post_login_sync(authenticated: Option<AtelierAuth>) -> ManagedConfigSync {
    clear_orphan();
    if !is_fetch_enabled() {
...
/// Whether a credential exists that `atelier setup` could install config for.
...
pub async fn ensure_managed_policy_present(
    ...
    let team = tokio::time::timeout(SESSION_START_AUTH_DEADLINE, auth_manager.auth())
        ...
        .ok()
        .and_then(Result::ok)
        .filter(AtelierAuth::is_team_principal);
    if !has_principal() {
        return;
    }
...
/// `atelier setup`); ceasing to serve `fail_closed` rolls back.
pub fn managed_policy_gate() -> Result<(), String> {
    // Managed policy is part of the removed vendor control plane. The private
    // runtime must not inspect legacy auth state while deciding whether to
    // start, and no remote policy is available to enforce here.
    return Ok(());

    // Skip under the lib unit-test build only: `bootstrap` reaches this without a staged
    // `ATELIER_HOME` and would flake on the dev machine's real marker/auth. The pure decision
...
/// self-healing class as deleting the user-writable files outright; /etc/atelier and MDM are unaffected.
...
/// Outcome of the `atelier setup` sync. The caller renders it — CLI presentation
/// and exit codes stay out of the library.
#[derive(Debug)]
pub enum SetupOutcome {
    /// Config was written to `~/.atelier`.
    Installed,
...
/// Result of `atelier setup --json`: what the server serves for the current
/// principal, verbatim. `managed_config` may embed the enforced deployment key,
/// exactly as `atelier setup` would write it to disk.
...
pub struct SetupReport {
    ...
    /// TOML documents exactly as `atelier setup` would install them.
    pub managed_config: Option<String>,
    pub requirements: Option<String>,
    pub fail_closed: bool,
}

/// Fetches the report behind `atelier setup --json` without writing anything:
/// no artifacts, no signature sidecar, no sync marker.
pub async fn fetch_setup_report() -> Result<SetupReport, ManagedConfigError> {
    Err(ManagedConfigError::Disabled)
}

/// Run the `atelier setup` sync for the current principal. The caller must check
/// [`has_principal`] first and render the no-principal guidance.
pub async fn run_setup() -> SetupOutcome {
    SetupOutcome::Failed(ManagedConfigError::Disabled)
