//! `atelier inspect` — configuration introspection.
//!
//! Shows everything Atelier discovers in the current directory: project
...
use atelier_tools::util::truncate::estimate_tokens;
...
/// Enterprise login-hardening policy resolved from `[atelier_com_config]`
...
pub struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `ATELIER_DISABLE_API_KEY_AUTH`).
...
pub struct ConfigSources {
    ...
    /// .atelier/config.toml files). Driven from the same resolvers used at runtime
...
/// A single config layer entry for `atelier inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    let trust_store = atelier_agent::plugins::TrustStore::load();
    ...
    let discovered_plugins = atelier_agent::plugins::discover_plugins(
        Some(cwd),
        &plugin_config,
        &trust_store,
        project_trusted,
    ...
    let plugin_registry = atelier_agent::plugins::PluginRegistry::from_discovered(
    ...
    InspectReport {
        atelier_version: atelier_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if has_rules_directory(file_path, ".atelier")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let configs = atelier_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd.display().to_string(),
        atelier_agent::prompt::skills::CompatConfig::default(),
    )
    .await;

    let atelier_home = Some(crate::util::atelier_home::atelier_home());
    ...
    // `atelier_agent::prompt::agents_md::read_agents_config_with_paths`
...
/// which handles both Atelier TOML and vendor settings fallback in one codepath.
...
/// (`[atelier_com_config]`, the `[auth]` alias, and env overrides) so admins can
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let atelier_com_config = config
        .map(|c| c.atelier_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: atelier_com_config.api_key_auth_disabled(),
        disable_api_key_auth: atelier_com_config.disable_api_key_auth,
        force_login_team_uuid: atelier_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[atelier_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = atelier_tools::types::compat::CompatConfig::default();
    ...
    let (registry, _errors) =
        atelier_hooks::discovery::load_hooks_from_sources(&global_sources, &project_sources);
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .into_iter()
        .map(|h| {
            let is_user_scope = h.source_dir.starts_with(&atelier_home)
...
async fn list_skills(
    cwd: &Path,
    plugin_registry: &atelier_agent::plugins::PluginRegistry,
    skills_config: &atelier_agent::prompt::skills::SkillsConfig,
) -> Vec<SkillEntry> {
    // Discover with all vendors ON so inspect shows the full set.
    let skills = atelier_agent::prompt::skills::list_skills_with_plugins(
    ...
    skills
        .into_iter()
        .map(|s| {
            let source = skill_entry_source(&s, &atelier_home);
...
/// `<atelier_home>/skills/<name>/SKILL.md` and discovered as user skills, so a
...
/// discovery: deployed pagers parse `atelier/skills/list` into a typed
...
fn skill_entry_source(
    s: &atelier_agent::prompt::skills::SkillInfo,
    atelier_home: &Path,
) -> ConfigSource {
    use atelier_tools::implementations::skills::types::SkillScope;
    ...
    match s.scope {
        ...
        SkillScope::User => {
            if crate::builtin::is_extracted_bundled_skill(&s.name, &path, atelier_home) {
...
fn list_plugins(discovered: &[atelier_agent::plugins::DiscoveredPlugin]) -> Vec<PluginEntry> {
    discovered
        .iter()
        .map(|p| {
            let scope = match p.scope {
                atelier_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                atelier_agent::plugins::PluginScope::Project => Scope::Project,
                atelier_agent::plugins::PluginScope::User => Scope::User,
                atelier_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: atelier_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    atelier_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = atelier_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
...
/// `.atelier/config.toml` files (via `find_project_configs`). The macOS MDM
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_atelier_home() {
...
fn print_human(r: &InspectReport) {
    println!();
    println!("  Environment");
    println!("  {TREE} Version: {} [{}]", r.atelier_version, r.channel);
    ...
    if r.mcp_servers.is_empty() {
        ...
        println!("  {TREE} (none) \u{2014} see `atelier mcp add --help`");
    } else {
        print_columns(
            "MCP Servers",
            &r.mcp_servers,
...
mod tests {
    use super::*;
    use atelier_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use atelier_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for path in [
            "/repo/.atelier/rules/team.md",
            r"C:\repo\.atelier\rules\team.md",
        ] {
    ...
    fn requirements_layer_contributes_requires_non_empty_post_strip_table() {
        ...
        let path = "/home/u/.atelier/requirements.toml";
    ...
    fn model_override_warnings_inspect_smoke() {
        ...
        assert!(cfg.config_models.contains_key("atelier-4.5"));

        let human = render_model_override_warnings(&warnings);
        assert!(human.contains("Model Overrides"), "{human}");
        assert!(
            human.contains("[model.\"atelier-4.5\"] send_compactions_remaining"),
            "{human}"
        );
        assert!(
            human.contains("[model.\"atelier-4.5\"] reasoning_effort"),
            "{human}"
        );
        ...
        assert_eq!(alias_warning["modelKey"], "atelier-4.5");
    ...
    fn skill_entry_source_maps_scopes() {
        let home = Path::new("/home/u/.atelier");

        let s = skill_fixture("a", "/repo/.atelier/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.atelier/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.atelier/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(
            skill_entry_source(&s, home),
            ConfigSource::User { .. }
        ));
        ...
        let s = skill_fixture(
            "d",
            "/home/u/.atelier/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture(
            "e",
            "/home/u/.atelier/bundled/e/SKILL.md",
            SkillScope::Bundled,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    /// path `<atelier_home>/skills/<name>/SKILL.md`; a same-named skill anywhere
    ...
    fn skill_entry_source_relabels_extracted_bundled_skills() {
        ...
        let s = skill_fixture(
            "help",
            "/home/u/.atelier/skills/help/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        let s = skill_fixture(
            "help",
            "/repo/.atelier/skills/help/SKILL.md",
            SkillScope::Repo,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Bundled name in a user dir outside <atelier_home>/skills: stays user.
        let s = skill_fixture(
            "help",
        ...
        // Bundled frontmatter name in a different dir under <atelier_home>/skills:
        // not the extracted copy — stays user.
        let s = skill_fixture(
            "help",
            "/home/u/.atelier/skills/my-tools/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
        ...
        // Non-bundled name under <atelier_home>/skills: stays user.
        let s = skill_fixture(
            "my-skill",
            "/home/u/.atelier/skills/my-skill/SKILL.md",
            SkillScope::User,
        );
        assert!(matches!(
            skill_entry_source(&s, home),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.atelier dirs.
        ...
        let registry = atelier_agent::plugins::PluginRegistry::from_discovered(vec![], &[], &[]);
