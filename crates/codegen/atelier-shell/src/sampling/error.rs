...
//!
//! The canonical error types now live in `atelier_sampling_types::error`.
//! This module re-exports them and adds `map_sampling_err_to_acp` which
//! depends on `agent_client_protocol::Error` (a atelier-shell dependency).

// Re-export everything from the standalone crate.
pub use atelier_sampling_types::error::*;
...
/// API key / team rate-limit copy. Personal atelier.invalid upgrades do not raise API
/// team limits; admins purchase credits or a higher spend-based tier.
/// See https://docs.atelier/developers/rate-limits#rate-limit-tiers
pub const RATE_LIMITED_USER_MESSAGE_API_KEY: &str = "You\u{2019}ve hit your team\u{2019}s API rate limit. Ask a team admin to purchase more credits for higher limits, or try again later. See https://docs.atelier/developers/rate-limits#rate-limit-tiers";
...
/// This stays in atelier-shell because it depends on `agent_client_protocol::Error`.
    ...
    match err {
        ...
        SamplingError::MaxTokensTruncation => {
            acp::Error::internal_error().data(terminal_error_data(
                err.to_string(),
                None,
                atelier_sampler::SamplingErrorKind::MaxTokensTruncation,
            ))
        }
...
pub fn terminal_error_data(
    ...
    kind: atelier_sampler::SamplingErrorKind,
) -> serde_json::Value {
    if kind != atelier_sampler::SamplingErrorKind::MaxTokensTruncation {
...
pub fn stop_reason_for_turn_error(err: &acp::Error) -> &'static str {
    let is_max_tokens = err
        ...
        .and_then(|v| v.as_str())
        .is_some_and(|k| k == atelier_sampler::SamplingErrorKind::MaxTokensTruncation.as_str());
...
mod tests {
    ...
    fn rate_limited_user_message_oauth_vs_api_key() {
        ...
        assert!(
            RATE_LIMITED_USER_MESSAGE_API_KEY
                .contains("https://docs.atelier/developers/rate-limits#rate-limit-tiers")
        );
