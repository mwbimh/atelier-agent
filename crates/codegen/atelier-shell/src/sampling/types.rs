// Re-export all types from the standalone atelier-sampling-types crate.
// This keeps all existing `crate::sampling::types::*` imports working.
pub use atelier_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` previously lived
// here. They were moved into `atelier-sampling-types::types` (and
// are re-exported above via the wildcard) so the new
// `atelier-sampler` crate can reference them without a circular
// dep on `atelier-shell`.

// Tests for the types now live in atelier-sampling-types crate.

use atelier_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `atelier-sampling-types` or
/// `atelier-tools`) so neither of those crates needs to depend on
