...
pub use self::types::*;
pub use atelier_sampler::ApiBackend;
pub use atelier_sampler::SamplingClient as Client;

// Re-export async-openai Responses API types under `rs` namespace
pub use async_openai::types::responses as rs;

// ---------------------------------------------------------------------------
// atelier-sampler re-exports
// ---------------------------------------------------------------------------
//
// The actual streaming / retry / HTTP-client logic lives in the
// `atelier-sampler` crate. We re-export the public surface here so
...
// `atelier_sampler::*`. The shell-side `sampling::client::Config`
