//! Leader-follower IPC architecture for atelier-shell.
//!
...
//! │  └────────────────────────┬────────────────────────────────┘│
//! └───────────────────────────┼──────────────────────────────────┘
//!                             │ IPC (Unix socket at ~/.atelier/leader.sock)
//!         ┌───────────────────┼───────────────────┐
//!         ▼                   ▼                   ▼
//! ┌───────────────┐   ┌───────────────┐   ┌───────────────┐
...
//!
//! ```ignore
//! use atelier_shell::leader::{connect_or_spawn, ClientCapabilities, ClientMode};
//!
...
mod transport;
use crate::env::AtelierBuildEnvironment;
...
const CLIENT_LEADER_VERSION: &str = atelier_version::VERSION;
...
/// Maximum reconnection attempts for bounded mode (headless/`atelier -p`).
...
pub struct LeaderDescriptor {
    ...
    pub environment: Option<AtelierBuildEnvironment>,
...
pub enum LeaderTarget {
    Environment(AtelierBuildEnvironment),
    WsUrl(String),
    Pid(u32),
}
fn known_environment_for_ws_url(ws_url: &str) -> Option<AtelierBuildEnvironment> {
    let environments: &[AtelierBuildEnvironment] = &[AtelierBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
fn known_environment_for_suffix(ws_url_suffix: &str) -> Option<AtelierBuildEnvironment> {
    let environments: &[AtelierBuildEnvironment] = &[AtelierBuildEnvironment::Production];
    environments
        .iter()
        .copied()
...
pub async fn discover_leaders() -> Vec<LeaderDescriptor> {
    discover_leaders_in(&crate::util::atelier_home::atelier_home()).await
}
                ...
                Ok(()) => {
                    ...
                    crate::unified_log::warn(
                        ...
                        Some(serde_json::json!(
                            ...
                            "reason" : reason, "killer_ver" : atelier_version::VERSION,
                            }
                        )),
                    );
                }
...
impl From<&crate::auth::AtelierComConfig> for LeaderEnvUrls {
    fn from(c: &crate::auth::AtelierComConfig) -> Self {
        Self {
            atelier_ws_url: c.atelier_ws_url.clone(),
            atelier_ws_origin: c.atelier_ws_origin.clone(),
        }
    }
}
...
pub enum ReconnectPolicy {
    ...
    /// Suitable for headless/`atelier -p` where hanging forever is unacceptable.
...
///     "atelier-tui", ClientMode::Stdio, env_urls, caps, status_tx,
/// );
///
...
///
/// The `env_urls.atelier_ws_url` determines which leader instance to connect to.
/// Different WS URLs get different leader processes (via hashed socket paths).
///
/// # Arguments
///
/// * `client_type` - Identifier for the client type (e.g., "atelier-tui", "vscode")
...
pub async fn connect_or_spawn(
    ...
    let mut lock = LeaderLock::new(&env_urls.atelier_ws_url);
...
fn spawn_leader_subprocess(env_urls: &LeaderEnvUrls) -> Result<u32, ConnectionError> {
    ...
    cmd.arg("--atelier-ws-url").arg(&env_urls.atelier_ws_url);
    cmd.arg("--atelier-ws-origin")
        .arg(&env_urls.atelier_ws_origin);
    ...
    for key in [
        "ATELIER_DEBUG_LOG",
        "ATELIER_HOOKS_LOG",
        "ATELIER_LOG_SAMPLING",
        "ATELIER_INSTRUMENTATION",
    ] {
    ...
    let log_path = crate::util::atelier_home::atelier_home().join("leader.log");
...
mod tests {
    ...
    async fn reconnect_against_hung_leader_exhausts_attempts_without_respawn() {
        ...
        let env_urls = LeaderEnvUrls {
            atelier_ws_url: "wss://test.invalid".into(),
            atelier_ws_origin: "https://test.invalid".into(),
        };
