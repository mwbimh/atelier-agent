...
pub enum ClientMode {
    /// Headless mode (atelier agent, atelier agent headless) - uses websocket relay.
    /// Leader connects to websocket relay once and forwards messages.
    Headless,
    /// Stdio mode (atelier agent stdio, atelier -p) - uses local IPC.
...
pub struct ClientCapabilities {
    ...
    /// Whether this client has advertised `atelier/codeNavigation.enabled`.
...
pub struct LeaderCapabilities {
    ...
    /// (driven by `atelier update`). Old leaders default to `false`, so a new
...
pub enum ControlCommand {
    ...
    /// `atelier update`). The leader stops admitting new turns, waits a bounded
    ...
    ///
    /// `to_version` is the version `atelier update` just installed; the leader uses
...
mod tests {
    ...
    fn workspace_status_payload_roundtrip() {
        let payload = ControlPayload::WorkspaceStatus {
            ...
            uptime_ms: 4200,
            active_tool_calls: 2,
            sessions: vec!["atelier-a".into(), "atelier-b".into()],
            pid: 4242,
        };
