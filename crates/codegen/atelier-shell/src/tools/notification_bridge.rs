//! Notification bridge: translates `atelier-tools` `ToolNotification` events
//! into `atelier-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
use atelier_workspace::session::file_state::FileStateTracker;
...
pub struct NotificationBridgeConfig {
    ...
    /// `atelier/incrementalBashOutput` capability.
    ...
    /// next-turn exit reminder. Atelier-build leaves this `false` — its
...
async fn emit_current_mode_update(
    ...
    mode: atelier_tools::types::SessionMode,
) {
...
async fn handle_notification(
    ...
    match notification {
        ...
        ToolNotification::BashExecutionBackgrounded(bg) => {
            ...
            // Forward as atelier/task_backgrounded ExtNotification so the TUI can
            ...
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("atelier/task_backgrounded", params.into());
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            let is_monitor =
                task_snapshot.kind == atelier_tools::computer::types::TaskKind::Monitor;
            ...
            // stdout. The pager still receives atelier/task_completed below for UI.
            ...
            if task_snapshot.block_waited || task_snapshot.explicitly_killed {
                ...
                let body = if is_monitor {
                    atelier_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    atelier_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = atelier_tools::reminders::wrap_reminder(&body);
            ...
            if let Some(params) = params {
                let notification: acp::ExtNotification =
                    acp::ExtNotification::new("atelier/task_completed", params.into());
                config.gateway.forward_fire_and_forget(notification);
            }
        ...
        ToolNotification::PlanModeEntered(entered) => {
            ...
            if activated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
                ...
                emit_current_mode_update(config, atelier_tools::types::SessionMode::Plan).await;
            }
            tracing::info!(
        ...
        ToolNotification::PlanModeExited(exited) => {
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                emit_current_mode_update(config, atelier_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
        ...
        ToolNotification::ScheduledTaskRemoved(removed) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "atelier/scheduled_task_deleted",
                        params.into(),
                    ));
            }
        }
        ...
        ToolNotification::ScheduledTaskCreated(created) => {
                ...
                .and_then(|v| serde_json::value::to_raw_value(&v))
            {
                config
                    .gateway
                    .forward_fire_and_forget(acp::ExtNotification::new(
                        "atelier/scheduled_task_created",
                        params.into(),
                    ));
            }
        }
...
mod tests {
    ...
    /// drain it). The pager's `atelier/task_completed` notification still fires.
    #[tokio::test]
    async fn bash_task_completed_suppresses_auto_wake_during_goal_loop() {
        ...
        assert!(
            found_ext,
            "atelier/task_completed ExtNotification must still be sent for UI"
        );
    }
    ...
    async fn monitor_event_skipped_after_task_completed_auto_wake() {
        ...
        handle_notification(
            &config,
            ToolNotification::MonitorEvent(atelier_tools::notification::types::MonitorEvent {
    ...
    async fn scheduled_task_created_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskCreated(
            atelier_tools::notification::types::ScheduledTaskCreated {
                task_id: "loop-1".into(),
    ...
    async fn bash_output_chunk_persists_and_broadcasts_one_event_id() {
        ...
        let notification = ToolNotification::BashOutputChunk(
            atelier_tools::notification::types::BashOutputChunk {
                base: atelier_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_removed_is_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskRemoved(
            atelier_tools::notification::types::ScheduledTaskRemoved {
                task_id: "loop-1".into(),
            },
        );
        let mut offsets = HashMap::new();
    ...
    async fn task_backgrounded_persisted_line_is_stamped() {
        ...
        let notification = ToolNotification::BashExecutionBackgrounded(
            atelier_tools::notification::types::BashExecutionBackgrounded {
                base: atelier_tools::notification::types::BashNotificationBase {
    ...
    async fn scheduled_task_fired_is_not_persisted() {
        ...
        let notification = ToolNotification::ScheduledTaskFired(
            atelier_tools::notification::types::ScheduledTaskFired {
                task_id: "loop-1".into(),
                prompt: "check deploy".into(),
                human_schedule: "every 5 minutes".into(),
                next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    async fn legacy_monitor_event_without_owner_is_injected() {
        // Legacy / non-atelier-build backends record no owner; such events must
    ...
    async fn plan_mode_exited_emits_current_mode_update_default() {
        ...
        *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
        ...
        let notification =
            ToolNotification::PlanModeExited(atelier_tools::notification::types::PlanModeExited {
    ...
    /// Default (atelier) polarity: the exit_plan_mode tool result is the model's
    ...
    async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
        ...
        let notification =
            ToolNotification::PlanModeExited(atelier_tools::notification::types::PlanModeExited {
                tool_call_id: "tc-exit-atelier".into(),
                plan_content: Some("- step 1".into()),
                plan_file_path: "/tmp/test-session/plan.md".into(),
            });
    ...
    async fn plan_mode_entered_emits_current_mode_update_plan() {
        ...
        let notification = ToolNotification::PlanModeEntered(
            atelier_tools::notification::types::PlanModeEntered {
