use crate::models;
use atelier_sampler::SamplerConfig;
use atelier_tools::implementations::atelier_build;
use atelier_tools::registry::types::ToolConfig;
use serde::{Deserialize, Serialize};

/// Production atelier-build foreground command-timeout ceiling (seconds). The
...
impl BashToolConfig {
    ...
    pub fn to_bash_params_json(
        ...
        // production atelier-build opts up to 10h by sending it explicitly
...
pub struct WebFetchToolConfig {
    ...
    /// this URL. Resolution: TOML > `ATELIER_WEB_FETCH_PROXY` env > remote settings > None.
...
impl WebFetchToolConfig {
    ...
    pub fn resolve_params(
    ...
    ) -> atelier_tools::implementations::atelier_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("ATELIER_WEB_FETCH_PROXY"))
...
/// from `atelier_tools::registry::types::ToolsetConfig` which holds
...
///
/// Selects between the standard `AtelierBuild` toolset (`read_file`,
/// `search_replace`, `grep`) and the anchor-based `AtelierBuildHashline`
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "AtelierBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "AtelierBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"AtelierBuild:read_file"));
        assert!(ids.contains(&"AtelierBuild:search_replace"));
        assert!(ids.contains(&"AtelierBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"AtelierBuildHashline:hashline_read"));
        assert!(ids.contains(&"AtelierBuildHashline:hashline_edit"));
        assert!(ids.contains(&"AtelierBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            atelier_agent::config::AgentDefinition::plan(),
            atelier_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "AtelierBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"AtelierBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"AtelierBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"AtelierBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production atelier-build must set the 10h foreground ceiling"
        );
    }
