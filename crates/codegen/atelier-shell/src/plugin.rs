...
use atelier_agent::plugins::discovery::PluginScope;
use atelier_agent::plugins::git_install::{self, UpdateStatus};
use atelier_agent::plugins::install_registry::{
...
/// When `keep_data` is true, `~/.atelier/plugin-data/<id>/` is preserved.
...
fn marketplace_root_for_provenance(
    provenance: &atelier_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `atelier plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `atelier plugin install ./{name}`, or add a \
                     source with `atelier plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `atelier plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `atelier plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
fn filter_sources_by_allowlist(
    ...
    allowlist: &atelier_workspace::permission::resolution::MarketplaceAllowlist,
...
/// `~/.atelier/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user atelier via user_atelier_home() (None when no home resolves) and
    // home separately, so removal still runs from $ATELIER_HOME when no home dir
    // exists, and never touches a cwd-relative .atelier.
    let home = dirs::home_dir();
    let atelier = atelier_config::user_atelier_home();

    let mut settings_candidates: Vec<std::path::PathBuf> = Vec::new();
    if let Some(ref atelier) = atelier {
        settings_candidates.push(atelier.join("settings.local.json"));
        settings_candidates.push(atelier.join("settings.json"));
    }
    ...
    if let Some(ref atelier) = atelier {
        known_candidates.push(atelier.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("atelier plugin install ./sentry"), "{msg}");
        assert!(msg.contains("atelier plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("atelier plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    fn marketplace_allowlist(
        urls: &[&str],
    ) -> atelier_workspace::permission::resolution::MarketplaceAllowlist {
        atelier_workspace::permission::resolution::MarketplaceAllowlist {
    ...
    fn filter_sources_by_allowlist_unrestricted_passes_everything() {
        let allowlist = atelier_workspace::permission::resolution::MarketplaceAllowlist::default();
        let sources = vec![
