//! `atelier/billing` extension handler.
//!
//! Fetches the authenticated user's Atelier billing configuration
...
/// Current billing configuration for Atelier coding credits.
///
/// Carries both the newer credits-config fields (`credit_usage_percent`,
/// `current_period`) and the deprecated `AtelierBuildBillingConfig` fields
...
/// against both the new `GetAtelierCreditsConfig` and the legacy
/// `GetAtelierBuildBillingConfig` backend responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// (`GetAtelierCreditsConfig.prepaid_balance`); absent in the legacy billing
    /// shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (shared weekly/monthly
    /// pool). From `AtelierCreditsConfig.is_unified_billing_user`, which billing
    /// sets from remote settings `unified_consumer_billing_enabled`. `None` when
    /// absent (legacy `GetAtelierBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/atelier/credits` + auto-topup-rule).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BillingConfigResponse {
    ...
    /// User-friendly subscription tier name (e.g. "SuperAtelier Heavy").
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    return Err(acp::Error::invalid_params()
        .data("Billing and subscription services are disabled in Atelier."));
...
///
/// Keeps history to a count + the most recent period so `~/.atelier/logs/unified.jsonl`
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Billing data requires auth with atelier.invalid. Run `atelier login` to authenticate.",
    )?;
    ...
    // forwards to the backend `GetAtelierCreditsConfig`.
    ...
    let credits_resp = crate::http::shared_client()
        ...
        .header(
            "X-XAI-Token-Auth",
            crate::auth::AtelierComConfig::default().token_header,
        )
    ...
    // Every prompt / /usage / poll path hits `atelier/billing`; log the fetched
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    let auth = super::auth_gate::require_xai_auth(
        ...
        "Auto top-up data requires auth with atelier.invalid. Run `atelier login` to authenticate.",
    )?;
...
mod tests {
    ...
    fn billing_unified_log_ctx_includes_credits_and_collapses_history() {
        ...
        assert_eq!(ctx["subscriptionTier"], "SuperAtelier");
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetAtelierCreditsConfig` response: percentage-based usage,
        ...
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_ATELIER_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
