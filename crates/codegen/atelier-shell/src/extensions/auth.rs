//! `atelier/auth/*` and legacy `atelier/{get,set}ApiKey` extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    return Err(acp::Error::invalid_params()
        .data("Vendor authentication is disabled in Atelier; configure a local Provider."));

    match args.method.as_ref() {
        "atelier/auth/getBearerToken" => handle_get_bearer_token(agent).await,
        "atelier/getApiKey" => handle_get_api_key(),
        "atelier/setApiKey" => handle_set_api_key(args),
        "atelier/auth/submit_code" => handle_submit_code(agent, args),
        "atelier/auth/get_url" => handle_get_url(agent).await,
        "atelier/auth/logout" => handle_logout(agent, args).await,
        "atelier/auth/info" => handle_info(agent),
        "atelier/auth/check_subscription" => handle_check_subscription(agent).await,
...
fn handle_set_api_key(args: &acp::ExtRequest) -> ExtResult {
    ...
    if let Some(k) = key {
        if k.is_empty() {
            crate::auth::clear_api_key(&atelier_home)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
            unsafe { std::env::remove_var("XAI_API_KEY") };
        } else {
            crate::auth::store_api_key(&atelier_home, k)
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
            // SAFETY: ext_method is single-threaded per agent
...
fn handle_info(agent: &MvpAgent) -> ExtResult {
    ...
    struct AuthInfoResponse {
        ...
        /// `atelier-asset://` URL resolved by the Electron protocol handler,
    ...
    // Return a atelier-asset:// URL that the Electron renderer resolves at
    ...
    let profile_image_url = match raw_asset_id.as_deref().filter(|k| !k.is_empty()) {
        ...
        Some(key) => Some(format!("atelier-asset:///{key}")),
        None => None,
    };
