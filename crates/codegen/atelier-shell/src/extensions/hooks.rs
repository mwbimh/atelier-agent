//! `atelier/hooks/*` extension handlers.
//!
...
pub fn hook_spec_to_info(spec: &atelier_hooks::config::HookSpec) -> HookInfo {
    ...
    HookInfo {
        ...
        disabled: atelier_hooks::trust::is_hook_disabled(&spec.name),
    }
}

// Wire types for client-registered hooks (`atelier/hooks/run`); the gate that uses
...
/// for both the `atelier/hooks/run` request (gate) and the `atelier/hooks/event` notification
...
/// Response payload for `atelier/hooks/run` (client to agent). `Default` (used on
...
/// Parse client hooks from `session/new` `_meta["atelier/hooks"]`, shaped
...
pub(crate) fn parse_client_hooks(meta: Option<&acp::Meta>) -> ClientHooks {
    ...
    let Some(map) = meta
        .and_then(|m| m.get("atelier/hooks"))
    ...
    for (event_name, value) in map {
        ...
        let Ok(event) = HookEventName::deserialize(de) else {
            tracing::warn!(event = %event_name, "ignoring unknown atelier/hooks event");
            continue;
        };
        let Some(array) = value.as_array() else {
            tracing::warn!(event = %event_name, "atelier/hooks event value is not an array; skipping");
            continue;
        };
...
/// clear) when the request meta carries `atelier/hooks`, else `None` so a reconnect that
...
pub(crate) fn reconnect_client_hooks(meta: Option<&acp::Meta>) -> Option<ClientHooks> {
    meta.and_then(|m| m.get("atelier/hooks"))
...
fn parse_hook_group(event: HookEventName, value: &serde_json::Value) -> Option<ClientHookGroup> {
    ...
    let group = WireGroup::deserialize(value)
        .inspect_err(|err| tracing::warn!(%event, %err, "ignoring malformed atelier/hooks group"))
        .ok()?;
    if group.hook_callback_ids.is_empty() {
        tracing::warn!(%event, "ignoring atelier/hooks group with no hookCallbackIds");
        return None;
    }
    ...
    let matcher = match group.matcher.as_deref() {
        ...
        Some(pattern) => match HookMatcher::new(pattern) {
            ...
            Err(err) => {
                tracing::warn!(%event, pattern, %err, "ignoring atelier/hooks group with invalid matcher");
                return None;
            }
        },
    };
...
mod tests {
    ...
    fn parse_client_hooks_canonicalizes_subagent_alias() {
        let meta = serde_json::json!({
            "atelier/hooks": { "SubagentEnd": [{ "hookCallbackIds": ["cb"] }] }
        });
        let hooks = parse_client_hooks(meta.as_object());
    ...
    /// Reconnect refresh applies hooks only when the load meta carries `atelier/hooks`:
    ...
    fn reconnect_client_hooks_only_when_key_present() {
        ...
        let cleared =
            reconnect_client_hooks(serde_json::json!({ "atelier/hooks": {} }).as_object());
        assert!(cleared.is_some_and(|h| h.is_empty()));

        let set = reconnect_client_hooks(
            serde_json::json!({
                "atelier/hooks": { "PreToolUse": [{ "hookCallbackIds": ["cb"] }] }
            })
            .as_object(),
        );
    ...
    fn client_hook_dispatch_serializes_envelope() {
        use atelier_hooks::event::{HookEventEnvelope, HookPayload};
