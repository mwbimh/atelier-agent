//! `atelier/feedback`, `atelier/feedback/dismiss`, `atelier/btw`, and `atelier/review/*`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "atelier/feedback" | "atelier/feedback/dismiss" => {
            Err(acp::Error::internal_error().data("Feedback collection is disabled in Atelier."))
        }
        m if m.starts_with("atelier/review") => Err(acp::Error::internal_error()
            .data("Cloud review event uploads are disabled in Atelier.")),
        _ => Err(acp::Error::method_not_found()),
    }
}

/// Handle `atelier/btw` -- a side question that doesn't interrupt the current turn.
...
async fn handle_feedback(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if !agent.cfg.borrow().is_feedback_enabled() {
        return Err(acp::Error::internal_error().data(
            "Feedback is disabled. To enable, set ATELIER_FEEDBACK_ENABLED=true or \
...
///
/// Methods:
/// - `atelier/review/comment`: record a new inline code comment to cloud storage
/// - `atelier/review/comment/delete`: record a tombstone event for a deleted comment
async fn handle_review(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "atelier/review/comment" => {
