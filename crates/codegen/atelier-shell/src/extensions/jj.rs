//! Jujutsu extension handlers — delegates to [`atelier_workspace::session::jj`].

use agent_client_protocol as acp;

use super::{Empty, ExtResult, to_ext_response, to_ext_response_partial};
use atelier_workspace::session::git::{CommitData, StageData};
use atelier_workspace::session::jj;

/// Handle a `atelier/git/*` method for a jj-colocated repo.
///
...
pub async fn try_handle(
    ...
    match method {
        "atelier/git/status" => Some(to_ext_response(jj::status(git_root).await)),
        "atelier/git/info" => Some(to_ext_response(jj::info(git_root).await)),
        // git HEAD points at `@-` in a colocated repo; route to jj so we report
        // the working-copy commit (`@`), consistent with `status`/`info`.
        "atelier/git/current_commit" => Some(to_ext_response(jj::current_commit(git_root).await)),
        "atelier/git/branches" => Some(to_ext_response(jj::list_bookmarks(git_root).await)),

        // jj has no staging area — stage/unstage are no-ops
        "atelier/git/stage" => Some(to_ext_response(Ok(StageData { paths: Vec::new() }))),
        "atelier/git/stage/content" | "atelier/git/unstage" => Some(to_ext_response(Ok(Empty {}))),
        ...
        "atelier/git/checkout" => Some(Err(acp::Error::invalid_params()
            .data("checkout is not supported in jj repos; use `jj new` or `jj edit`"))),
        "atelier/git/stash" => Some(Err(acp::Error::invalid_params()
