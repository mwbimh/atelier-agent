//! ACP extension handler for bulk session updates (`atelier/session/updates`).
//!
...
//! the update type (`"session/update"` for ACP, `"_atelier/session/update"` for
//! Atelier extensions) and extract the notification payload from `params`.
...
/// Send updates as chunked `_atelier/session/updates/chunk` notifications.
...
mod tests {
    ...
    fn dummy_gateway() -> atelier_acp_runtime::AcpAgentGatewaySender {
    ...
    fn make_request(
        ...
        acp::ExtRequest::new("atelier/session/updates", std::sync::Arc::from(raw))
    }
    ...
    async fn handle_tail_request_with_rewind_returns_only_live_timeline() {
        ...
        std::fs::write(
            ...
            [
                ...
                r#"{"timestamp":5,"method":"_atelier/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":1}}}"#,
    ...
    async fn handle_falls_back_to_id_lookup_for_divergent_cwd() {
        ...
        // Clean up the dir written under the real atelier home.
        let _ = std::fs::remove_dir_all(&child_dir);
    }

    fn capturing_gateway() -> (
        atelier_acp_runtime::AcpAgentGatewaySender,
        tokio::sync::mpsc::UnboundedReceiver<atelier_acp_runtime::AcpClientMessage>,
    ) {
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        (atelier_acp_runtime::AcpAgentGatewaySender::new(tx), rx)
    }
    ...
    fn xai_rewind(target: usize) -> String {
        format!(
            r#"{{"timestamp":0,"method":"_atelier/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"rewind_marker","target_prompt_index":{target}}}}}}}"#
        )
    }
