//! `atelier/memory/flush`, `atelier/memory/rewrite`, and `atelier/compact_conversation`
//! extension handlers.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        m if m.starts_with("atelier/compact_conversation") => handle_compact(agent, args).await,
        "atelier/memory/flush" => handle_flush(agent, args).await,
        "atelier/memory/rewrite" => handle_rewrite(agent, args).await,
