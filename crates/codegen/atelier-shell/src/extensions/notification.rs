...
pub enum SessionUpdate {
    ...
    ///
    /// Emitted by the `atelier/recap` ext method: on demand via the `/recap`
    ...
    /// outcome. Rides the persisted `_atelier/session/update` rail (unlike the
    /// fire-and-forget `atelier/session/prompt_complete` notification), so a
...
/// detailed migration guidance (`atelier logout` / `atelier login`) in the
...
pub struct CompactionRequestFile {
    ...
    /// Note: this is a per-artifact ID, not the model API's `x_atelier_req_id`
    ...
    /// or `"detailed"` (10-section structured prompt for atelier-build and similar agents).
...
pub struct RecapRequestFile {
    ...
    /// API's `x_atelier_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
...
mod tests {
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.atelier/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.atelier/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.atelier/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.atelier/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.atelier/memory/MEMORY.md".into(),
    ...
    fn make_goal_updated_full() -> SessionUpdate {
        SessionUpdate::GoalUpdated {
            ...
            live_tokens_by_model: vec![("atelier-4".into(), 6_000), ("atelier-3".into(), 4_000)],
    ...
    fn goal_updated_serializes_snake_case_tag() {
        ...
        assert_eq!(json["live_tokens_by_model"][0][0], "atelier-4");
    ...
    fn model_changed_serializes_snake_case_with_optional_effort() {
        ...
        assert_eq!(json["model_id"], "atelier-4");
        ...
        assert_eq!(json["sessionUpdate"], "model_changed");
        assert_eq!(json["model_id"], "atelier-3");
        assert!(
    ...
    fn model_changed_envelope_carries_session_id_at_top_level() {
        ...
        assert_eq!(json["update"]["model_id"], "atelier-4");
    }
