...
//! - `atelier/internal/reload_models`          model list hot-reload from config.toml
//! - `atelier/internal/reload_models_cache`    model catalog hot-reload from disk cache
//! - `atelier/internal/auth_cleared`           auth hot-clear cleanup
//! - `atelier/plugins/reload`                  rebuild shared plugin registry
//! - `atelier/commands/list`                   list slash commands
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        ...
        "atelier/internal/reload_skills" => handle_reload_skills(agent),
        "atelier/internal/reload_models" => handle_reload_models(agent),
        "atelier/internal/reload_models_cache" => handle_reload_models_cache(agent),
        "atelier/internal/auth_cleared" => handle_auth_cleared(agent),
        "atelier/plugins/reload" => handle_plugins_reload(agent).await,
        "atelier/commands/list" => handle_commands_list(agent, args).await,
...
/// hot-reload watcher when `<cwd>/.atelier/config.toml`,
...
/// Hot-reload the model catalog from `~/.atelier/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // that `atelier/commands/list` (the pull used by atelier-desktop after session
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .atelier/plugins under it, plus
