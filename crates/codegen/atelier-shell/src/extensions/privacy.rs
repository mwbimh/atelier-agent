//! `atelier/privacy/setCodingDataRetention` extension handler.
//!
//! Disabled in vendorless Atelier. The legacy proxy implementation remains
//! below for build compatibility, but the public entry point rejects it before
//! parsing parameters, resolving auth, or issuing network requests.
...
const PRIVACY_EXTENSION_ENABLED: bool = false;
const PRIVACY_DISABLED_MESSAGE: &str =
    "Privacy extension is disabled in Atelier; vendor privacy sync is unavailable.";

fn is_privacy_method(method: &str) -> bool {
    method == "atelier/privacy/setCodingDataRetention"
}

fn disabled_error() -> acp::Error {
    acp::Error::internal_error().data(PRIVACY_DISABLED_MESSAGE)
}

#[tracing::instrument(skip_all, fields(method = %args.method))]
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    if is_privacy_method(args.method.as_ref()) {
        if !PRIVACY_EXTENSION_ENABLED {
            return Err(disabled_error());
        }
        return handle_set(agent, args).await;
    }
...
async fn handle_set(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let auth = agent.auth_manager.auth().await.map_err(|e| {
        ...
        acp::Error::auth_required()
            .data("Authentication required. Run `atelier login` to re-authenticate.")
    })?;

    let proxy_url = agent.cfg.borrow().endpoints.proxy_url();
    let url = format!("{proxy_url}/privacy/coding-data-retention");
    let token_header = agent.auth_manager.atelier_com_config().token_header.clone();

    let body = serde_json::json!({
        "codingDataRetentionOptOut": params.coding_data_retention_opt_out,
    });

    let provider: std::sync::Arc<dyn atelier_auth::AuthCredentialProvider> = std::sync::Arc::new(
...
mod tests {
    ...
    #[test]
    fn privacy_proxy_entry_is_hard_disabled() {
        assert!(!PRIVACY_EXTENSION_ENABLED);
        assert!(is_privacy_method("atelier/privacy/setCodingDataRetention"));
        assert!(!is_privacy_method("atelier/btw"));
        assert!(PRIVACY_DISABLED_MESSAGE.contains("disabled"));
    }
}
