...
//!
//! - `atelier/mcp/list` — list available MCP servers (agent-scoped or session-annotated)
//! - `atelier/mcp/call` — invoke an MCP tool directly, outside the LLM loop
//! - `atelier/mcp/servers_updated` — notification pushed when managed configs resolve
//! - `atelier/mcp/server_status` — per-server delta pushed by the
...
pub mod mcp_methods {
    ...
    pub const PREFIX: &str = "atelier/mcp/";

    pub const LIST: &str = "atelier/mcp/list";
    pub const READ_RESOURCE: &str = "atelier/mcp/read_resource";
    pub const AUTH_STATUS: &str = "atelier/mcp/auth_status";
    pub const AUTH_TRIGGER: &str = "atelier/mcp/auth_trigger";
    pub const TOGGLE: &str = "atelier/mcp/toggle";
    pub const TOGGLE_TOOL: &str = "atelier/mcp/toggle_tool";
    pub const UPSERT: &str = "atelier/mcp/upsert";
    pub const DELETE: &str = "atelier/mcp/delete";

    pub const SERVERS_UPDATED: &str = "atelier/mcp/servers_updated";
    pub const TOOLS_CHANGED: &str = "atelier/mcp/tools_changed";
    pub const INIT_PROGRESS: &str = "atelier/mcp/init_progress";
}
...
// Re-export the `atelier/mcp/server_status` schema +
// method constant from the dispatcher module so external callers
// have a single import point alongside the other `atelier/mcp/*`
// types.
//
...
pub async fn notify_servers_updated(
    ...
    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
        ...
        tracing::info!("Sent atelier/mcp/servers_updated notification to client");
    }
}

// ── Dispatch ────────────────────────────────────────────────────────

/// Inbound `atelier/mcp/*` methods this agent services, resolved from the wire string.
///
...
//
// Implements the `McpResourceProvider` trait from atelier-tools so that
// `ListMcpResources` / `FetchMcpResource` tools can access MCP
// servers without depending on `atelier-mcp` directly.
...
impl atelier_tools::types::resources::McpResourceProvider for McpStateResourceProvider {
    async fn list_resources(
        &self,
        server: Option<String>,
    ) -> Result<Vec<atelier_tools::types::resources::McpResourceInfo>, String> {
            ...
            match mcp_service.list_all_resources().await {
                Ok(all_resources) => {
                    for r in all_resources {
                        resources.push(atelier_tools::types::resources::McpResourceInfo {
                            uri: r.uri.clone(),
    ...
    async fn read_resource(
        ...
        server: String,
        uri: String,
    ) -> Result<atelier_tools::types::resources::McpResourceReadResult, String> {
        let client = {
            let state = self.0.lock().await;
            Arc::clone(
                state
        ...
        match first {
            rmcp::model::ResourceContents::TextResourceContents {
                ...
                text,
                ..
            } => Ok(atelier_tools::types::resources::McpResourceReadResult {
                uri: content_uri,
                name: None,
                description: None,
                mime_type,
                content: Some(atelier_tools::types::resources::McpResourceContent::Text(
                    text,
                )),
            }),
            rmcp::model::ResourceContents::BlobResourceContents {
                ...
                description: None,
                mime_type,
                content: Some(atelier_tools::types::resources::McpResourceContent::Blob(
                    blob.into_bytes(),
                )),
            }),
...
mod tests {
    ...
    /// The emit-only reverse method (`atelier/mcp/sdk_call`) shares the `atelier/mcp/`
    ...
    fn inbound_sdk_call_has_no_forward_route() {
        ...
        assert_eq!(
            ...
            None,
            "inbound atelier/mcp/sdk_call must not resolve to a forward handler"
        );
    ...
    fn test_mcp_list_response_serialization() {
        let resp = McpListResponse {
            servers: vec![
                McpServerEntry {
                    name: "atelier_com_linear".to_string(),
                    display_name: None,
                    source: McpServerSource::Managed,
                    config: McpServerConfig::Http {
                        ...
                        scope_name: Some("Atelier CLI".to_string()),
                    },
                    source_label: None,
                    session: None,
                },
        ...
        assert_eq!(json["servers"][0]["scopeName"], "Atelier CLI");
    ...
    fn disabled_managed_http_rows_keep_non_gateway_placeholder_config() {
        let entry = disabled_server_placeholder_entry("atelier_com_slack");
    ...
    fn disabled_placeholders_match_handle_list_catalog_relationships() {
        ...
        append_disabled_like_handle_list(
            &mut servers,
            &["atelier_com_slack", "atelier_mcp_linear"],
            true,
        );
        assert!(
            servers.is_empty(),
        ...
        let mut servers = build_mcp_catalog_with_gateway_tools(&[], &[], None, &Default::default());
        append_disabled_like_handle_list(&mut servers, &["atelier_com_slack"], false);
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].name, "atelier_com_slack");
    ...
    fn test_disabled_session_state_serialization() {
        let entry = McpServerEntry {
            name: "atelier_com_slack".to_string(),
            display_name: None,
            source: McpServerSource::Managed,
            source_label: None,
            config: McpServerConfig::Http {
