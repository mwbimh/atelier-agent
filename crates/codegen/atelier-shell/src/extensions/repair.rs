//! `atelier/session/repair` — out-of-band recovery for sessions bricked by
...
/// Response payload for `atelier/session/repair`.
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    match args.method.as_ref() {
        "atelier/session/repair" => handle_session_repair(agent, args).await,
...
/// back atomically. `atelier_root` is injectable for tests.
async fn repair_on_disk(
    atelier_root: &std::path::Path,
    session_id: &str,
    dry_run: bool,
) -> ExtResult {
    let summary = crate::session::persistence::find_summary_by_session_id_in_root(
        session_id,
        &atelier_root.join("sessions"),
    )
    .ok_or_else(|| {
    ...
    let storage = JsonlStorageAdapter::with_root(atelier_root.to_path_buf());
