//! `atelier/share_session` extension handler.
//!
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    Err(acp::Error::invalid_params()
        .data("Session sharing is disabled in Atelier; sessions remain local."))
}
...
) -> Result<crate::auth::AtelierAuth, acp::Error> {
    super::auth_gate::require_xai_auth(
        auth_manager,
        "Authentication required to share session",
        "Share session is disabled. Run `atelier login` to authenticate.",
    )
}
...
mod tests {
    ...
    #[test]
    fn share_rejects_non_xai_auth_with_actionable_atelier_login_message() {
        let dir = tempdir().expect("tempdir");
        let mgr = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            AtelierComConfig::default(),
