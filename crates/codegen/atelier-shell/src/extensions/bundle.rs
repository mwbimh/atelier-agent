    ...
    match args.method.as_ref() {
        "atelier/bundle/sync" => {
            ...
            Err(acp::Error::invalid_params()
                .data("Remote bundle synchronization is disabled in Atelier."))
        }
...
mod tests {
    ...
    fn test_auth() -> crate::auth::AtelierAuth {
    ...
    fn status_only_reports_bundled_cache_not_higher_priority_sources() {
        ...
        std::fs::create_dir_all(project_root.join(".atelier/personas")).unwrap();
        std::fs::create_dir_all(project_root.join(".atelier/roles")).unwrap();
        std::fs::write(
            project_root.join(".atelier/personas/researcher.toml"),
            "instructions = \"project persona\"\n",
        )
        .unwrap();
        std::fs::write(
            project_root.join(".atelier/roles/reviewer.toml"),
