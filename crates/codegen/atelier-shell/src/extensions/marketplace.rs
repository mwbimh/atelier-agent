//! `atelier/marketplace/*` extension handlers.
//!
...
fn load_filtered_marketplace_sources() -> Vec<atelier_plugin_marketplace::MarketplaceSource> {
...
pub async fn handle(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    let _ = (agent, args);
    return Err(
        acp::Error::invalid_params().data("Remote marketplace services are disabled in Atelier.")
    );

    match args.method.as_ref() {
        "atelier/marketplace/list" => handle_list(agent, args).await,
        "atelier/marketplace/action" => handle_action(agent, args).await,
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                atelier_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
                atelier_plugin_marketplace::SourceKind::Local { path } => {
            ...
            for source in &sources {
                if let Some(ref filter) = source_url_or_path {
                    let identity = match &source.kind {
                        ...
                        atelier_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
                    };
                    if &identity != filter {
                        continue;
                    }
                }
                if let atelier_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
                    let cache_root = atelier_plugin_marketplace::git::default_cache_root();
                    if let Err(e) = atelier_plugin_marketplace::git::force_sync_source_cache(
                        url,
                        branch.as_deref(),
                        &cache_root,
                    ) {
...
async fn handle_update(
    ...
    let source_identity = |s: &atelier_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
            atelier_plugin_marketplace::SourceKind::Local { path } => path.display().to_string(),
            atelier_plugin_marketplace::SourceKind::Git { url, .. } => url.clone(),
        }
    };
    ...
    let plugin_path =
        match atelier_plugin_marketplace::MarketplaceRelativePath::parse(plugin_relative_path) {
            Ok(path) => path,
            Err(e) => {
                return ActionOutcome {
    ...
    let marketplace_root = match &source.kind {
        ...
        atelier_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = atelier_plugin_marketplace::git::default_cache_root();
            match atelier_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                atelier_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = atelier_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = atelier_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    match update_result {
        ...
        Err(atelier_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
...
async fn handle_install(
    ...
    if let Some((remote_url, remote_ref, remote_sha, remote_subdir)) = remote_entry {
        ...
        let marketplace_root = match &source.kind {
            ...
            atelier_plugin_marketplace::SourceKind::Git { url, branch } => {
                ...
                match atelier_plugin_marketplace::git::sync_source_cache_with_mode(
                    ...
                    branch.as_deref(),
                    &cache_root,
                    atelier_plugin_marketplace::git::SyncMode::UseTtl,
                ) {
                    Ok(lease) => {
        ...
        let plugin_path = match atelier_plugin_marketplace::MarketplaceRelativePath::parse(
            plugin_relative_path,
        ) {
            Ok(path) => path,
            Err(e) => {
...
async fn auto_install_defaults(
    agent: &MvpAgent,
    sources: &[atelier_plugin_marketplace::MarketplaceSource],
    ...
    for (source, scan) in sources.iter().zip(results.iter()) {
        ...
        let mut reg = atelier_agent::plugins::install_registry::InstallRegistry::load();
        ...
        if let Some((existing_key, _)) = existing {
            ...
            let _ = reg.save();
            reg = atelier_agent::plugins::install_registry::InstallRegistry::load();
        }
...
fn scan_source(
    source: &atelier_plugin_marketplace::MarketplaceSource,
    ...
    let scan = atelier_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = atelier_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match atelier_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: atelier_plugin_marketplace::MarketplaceEntry,
...
async fn handle_add_source(url: &str) -> atelier_hooks_plugins_types::ActionOutcome {
    ...
    let allowlist =
        &atelier_workspace::permission::resolution::managed_settings().marketplace_allowlist;
    ...
    let config_path = atelier_config::atelier_home().join("config.toml");
...
/// Acquire an advisory exclusive `flock` on `<atelier_home>/.config-init.lock`,
...
fn acquire_init_lock(atelier_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(atelier_home);
    let lock_path = atelier_home.join(".config-init.lock");
...
pub fn ensure_official_marketplace_source(atelier_home: &std::path::Path) {
    ...
    let _lock = match acquire_init_lock(atelier_home) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(
                error = %e,
                path = %atelier_home.join(".config-init.lock").display(),
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<atelier_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        atelier_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            atelier_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = atelier_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
