...
use atelier_agent::prompt::skills::{
...
fn discover_auto_sources(cwd: &str, skills: &[SkillInfo]) -> Vec<(String, usize)> {
    ...
    for subdir in &subdirs {
        try_add_source(atelier_home.join(subdir), None);
    }
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        "atelier/skills/add" => {
            ...
            .await
            {
                atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::SkillAdded {
                    added_count: 0,
                    total_skills: 0,
                    success: false,
                });
        ...
        "atelier/skills/remove" => {
            ...
            .await
            {
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::SkillRemoved { success: false },
                );
            ...
            atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::SkillRemoved {
                success: true,
            });
