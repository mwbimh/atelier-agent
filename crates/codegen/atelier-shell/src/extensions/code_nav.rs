...
//! using the atelier-codebase-graph index.
//!
//! ## Extension Methods
//!
//! | Method | Description |
//! |--------|-------------|
//! | `atelier/code/goto-definition` | Definition location(s) for symbol at position |
//! | `atelier/code/goto-references` | Reference location(s) for symbol at position |
//! | `atelier/code/find-definitions` | All definitions of a symbol by name |
//! | `atelier/code/find-references` | All references to a symbol by name |
//! | `atelier/code/status` | Indexing status |
...
/// Reason string for the `atelier/code/status` response.
///
...
fn to_code_nav_ext_response(resp: atelier_workspace::workspace_ops::CodeNavResponse) -> ExtResult {
    let symbol = resp
        .locations
        .first()
...
fn eligibility_error(reason: CodeNavEligibility) -> acp::Error {
    let msg = match reason {
        CodeNavEligibility::ClientNotWeb => {
            "code navigation is currently only enabled for atelier-web clients"
        }
        CodeNavEligibility::CapabilityNotAdvertised => {
            "client must advertise atelier/codeNavigation.enabled to use code navigation"
        }
