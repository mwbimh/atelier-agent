...
fn is_path_like(s: &str) -> bool {
    s.contains('/') || (cfg!(windows) && s.contains('\\')) || s == "~"
}
...
mod tests {
    ...
    fn path_like_matrix() {
        ...
        #[cfg(windows)]
        assert!(is_path_like(r"C:\Users\atelier"));
    }
    ...
    async fn suggest_var_prefix_lists_expansion_inserts_raw() {
        ...
        let _env = atelier_test_support::EnvGuard::set("ATELIER_SUGGEST_TEST_DIR", tmp.path());
        let ctx = SuggestContext::new(
            "cat $ATELIER_SUGGEST_TEST_DIR/no".into(),
            "cat $ATELIER_SUGGEST_TEST_DIR/no".len(),
            "/ignored".into(),
        );
        let results = FilePathProvider.suggest(&ctx).await;
        assert_eq!(results.len(), 1);
        assert_eq!(
            results[0].token_text.as_deref(),
            Some("$ATELIER_SUGGEST_TEST_DIR/notes.md")
        );
    }
