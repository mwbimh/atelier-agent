...
use atelier_tools::types::{KillOutcome, TaskSnapshot};
...
/// Wire DTO for the `atelier/task/kill` ext request.
///
/// `pub` (with both serde directions) so ACP clients (atelier-pager) build
...
/// Wire DTO for the `atelier/task/kill` ext response payload (nested under
...
/// Wire DTO for the `atelier/subagent/cancel` ext request.
///
/// `pub` (with both serde directions) so ACP clients (atelier-pager) build
...
/// Wire DTO for the `atelier/subagent/cancel` response payload (under `result` in
...
/// Handle `atelier/scheduler/*` extension methods.
...
/// Handle `atelier/subagent/*` extension methods.
...
mod tests {
    ...
    // ── atelier/subagent/cancel outcome wire DTO ──────────────────────────
