...
//!       [--output out.jsonl] \
//!       [--model atelier-4.5] \
//!       [--api-base-url https://api.atelier/v1] \
//!       [--api-key <key> | $ATELIER_API_KEY] \
//!       [--min-confidence 0.7] \
...
use atelier_shell::trace_classifier::{RunArgs, run, validate_min_confidence};
use clap::Parser;
...
struct Cli {
    ...
    /// has access to.
    #[arg(long, default_value = "atelier-4.5")]
    model: String,

    /// Sampler base URL.
    #[arg(long, default_value = "https://api.atelier/v1")]
    api_base_url: String,

    /// API key. Overrides `$ATELIER_API_KEY` when set.
    #[arg(long)]
...
mod tests {
    ...
    fn cli_parses_minimal_args() {
        ...
        assert_eq!(cli.api_base_url, "https://api.atelier/v1");
    ...
    fn cli_defaults_match_documented_values() {
        ...
        assert_eq!(by_id("model"), vec!["atelier-4.5"]);
        assert_eq!(by_id("api_base_url"), vec!["https://api.atelier/v1"]);
