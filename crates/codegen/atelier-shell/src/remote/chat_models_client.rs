//! atelier.invalid chat-product model catalog (`POST /rest/modes`) — the models
//! atelier-web's chat picker shows, distinct from the CLI `/v1/models` build
...
const ATELIER_WEB_URL: &str = "https://atelier.invalid";
...
pub enum ChatModelsError {
    #[error("no atelier.invalid credentials")]
    NoAuth,
...
impl ChatModelsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("ATELIER_MODES_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
                std::env::var("ATELIER_CONVERSATIONS_BASE_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .or_else(|| {
                std::env::var("ATELIER_CODE_WEB_URL")
                    .ok()
                    .filter(|s| !s.is_empty())
            })
            .unwrap_or_else(|| ATELIER_WEB_URL.to_string());
        Self {
    ...
    /// Gated only on a valid atelier.invalid bearer — deliberately NOT `is_xai_auth()`
    ...
    pub async fn list_modes(&self, locale: &str) -> Result<ListModesResponse, ChatModelsError> {
        ...
        let mut builder = self
            ...
            .header(
                "X-XAI-Token-Auth",
                self.auth.atelier_com_config().token_header.clone(),
            )
