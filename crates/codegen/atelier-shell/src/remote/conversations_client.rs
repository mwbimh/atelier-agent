...
use crate::auth::{AtelierAuth, AuthManager};
...
/// Body for `PUT /rest/app-chat/conversations/{id}` (atelier-web `chatUpdateConversation`).
...
impl ConversationsClient {
    pub fn new(auth: Arc<AuthManager>) -> Self {
        let base_url = std::env::var("ATELIER_CONVERSATIONS_BASE_URL")
            .ok()
            .filter(|s| !s.is_empty())
            .or_else(|| {
    ...
    async fn require_xai_auth(&self) -> Result<AtelierAuth, ConvError> {
