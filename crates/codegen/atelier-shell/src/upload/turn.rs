...
pub(crate) use atelier_telemetry::session_metrics::TraceUploadReason;
...
pub(crate) fn parse_agent_profile_from_meta(
    ...
    if value.is_object() {
        return match atelier_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = % def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return atelier_agent::discovery::by_name(name);
    }
    tracing::warn!(
...
mod tests {
    ...
    fn parse_ask_user_question_returns_none_when_absent() {
        let meta = serde_json::json!({ "agentProfile" : "atelier-build-plan" });
