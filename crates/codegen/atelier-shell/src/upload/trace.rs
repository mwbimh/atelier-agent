...
fn record_upload_failure(ctx: &PromptTraceContext, f: UploadFailure<'_>) {
    ...
    if level == UploadFailureLogLevel::Warn {
        atelier_telemetry::unified_log::warn(&msg, sid, log_ctx);
    } else {
        atelier_telemetry::unified_log::error(&msg, sid, log_ctx);
    }
}
...
pub(crate) async fn upload_plugin_state(
    ctx: &PromptTraceContext,
    registry: Option<&atelier_agent::plugins::PluginRegistry>,
) {
...
pub(crate) async fn upload_unified_log(ctx: &PromptTraceContext, wait: UploadWait) {
    ...
    let log_bytes = match tokio::task::spawn_blocking(move || {
        atelier_telemetry::unified_log::snapshot_session_log(&session_id)
    })
    .await
    {
        Ok(Some(bytes)) => bytes,
    ...
    .await;
    let full_log_bytes =
        tokio::task::spawn_blocking(atelier_telemetry::unified_log::snapshot_log).await;
    let user_id = ctx
        .auth_manager
        .current_or_expired()
        .map(|a| a.user_id)
...
impl TraceExportSource for DynamicResolver {
    ...
    fn proxy_credentials(&self) -> Option<Arc<dyn atelier_auth::AuthCredentialProvider>> {
...
pub(crate) fn spawn_startup_spill_reconcile(
    ...
    tokio::spawn(async move {
        match queue {
            Some(queue) => {
                let report =
                    atelier_workspace::recovery::run_startup_recovery(&atelier_home, &queue).await;
            ...
            None => {
                let purged = tokio::task::spawn_blocking(move || {
                    atelier_workspace::recovery::purge_spilled_items(&atelier_home)
                })
                .await;
                match purged {
                    Ok(removed) => {
...
pub(crate) fn spawn_purge_stale_upload_scratch() {
    {
        ...
        let dir = crate::util::atelier_home::atelier_home()
            .join("upload_queue")
            .join("scratch");
...
pub(crate) fn spawn_upload_queue(
    ...
    let queue = UploadQueue::spawn(atelier_home, resolver, UploadRetryPolicy::default());
...
mod tests {
    ...
    fn dynamic_resolver_refreshes_proxy_token() {
        ...
        let refreshed_auth = AtelierAuth {
    ...
    async fn resolve_async_drives_refresh_chain_when_token_expired() {
        ...
        impl crate::auth::refresh::TokenRefresher for FreshRefresher {
            async fn refresh(
                ...
                crate::auth::refresh::RefreshOutcome::Success(Box::new(crate::auth::AtelierAuth {
    ...
    async fn proactive_refresh_makes_trace_resolve_a_cache_hit() {
        ...
        auth_manager.hot_swap(AtelierAuth {
            key: "expired-oidc".into(),
    ...
    fn dynamic_resolver_preserves_token_when_auth_unavailable() {
        use crate::session::repo_changes::UploadMethod;
        let dir = tempfile::tempdir().unwrap();
        let atelier_com_config = crate::auth::AtelierComConfig::default();
        let auth_manager = Arc::new(crate::auth::AuthManager::new(
            dir.path(),
            atelier_com_config,
        ));
    ...
    async fn dynamic_resolver_auth_recovery_wakes_on_already_rotated_token() {
        ...
        auth_manager.hot_swap(crate::auth::AtelierAuth {
