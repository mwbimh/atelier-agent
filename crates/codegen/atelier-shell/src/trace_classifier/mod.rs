...
/// two-function shape in `atelier-tools` (F29).
...
/// `atelier-tools` — existing items keep their prior content when
...
/// binary. Wraps a `atelier_sampler::SamplingClient` and pulls the
/// text content out of the response.
pub struct SamplerClassifierClient {
    inner: atelier_sampler::SamplingClient,
}

impl SamplerClassifierClient {
    pub fn new(inner: atelier_sampler::SamplingClient) -> Self {
        Self { inner }
    }
}
...
pub fn build_classifier_request(
    ...
    ConversationRequest {
        ...
        x_atelier_conv_id: session_id_str.clone(),
        x_atelier_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
        x_atelier_session_id: session_id_str,
...
/// Resolve the API key from `--api-key` or `$ATELIER_API_KEY`.
/// Empty or whitespace-only values fall through to the other explicit source.
pub async fn resolve_api_key(explicit: Option<&str>) -> Result<String> {
    ...
    let env_key = std::env::var("ATELIER_API_KEY").ok();
    if let Some(k) = env_key.as_deref().map(str::trim).filter(|s| !s.is_empty()) {
        return Ok(k.to_owned());
    }
    Err(anyhow!("no API key: pass --api-key or set ATELIER_API_KEY"))
}
...
async fn build_sampler_client(
    ...
    model: String,
    api_key: Option<&str>,
) -> Result<atelier_sampler::SamplingClient> {
    let resolved = resolve_api_key(api_key).await?;
    let config = atelier_sampler::SamplerConfig {
        api_key: Some(resolved),
        base_url,
        model,
        max_completion_tokens: Some(LAZINESS_MAX_OUTPUT_TOKENS),
        ..atelier_sampler::SamplerConfig::default()
    };
    atelier_sampler::SamplingClient::new(config).map_err(|e| anyhow!("build SamplingClient: {e}"))
}
...
mod tests {
    use super::*;
    use atelier_sampling_types::{AssistantItem, ToolCall, ToolResultItem};
    ...
    async fn classifier_request_matches_production_shape() {
        ...
        assert_eq!(req.x_atelier_conv_id.as_deref(), Some("sess-x"));
        assert_eq!(req.x_atelier_session_id.as_deref(), Some("sess-x"));
        let req_id = req.x_atelier_req_id.as_deref().expect("req id");
        ...
        assert_eq!(
            req.x_atelier_agent_id.as_deref(),
            Some(atelier_telemetry::id::agent_id().as_str())
        );
    }
    ...
    async fn resolve_api_key_precedence() {
        let saved = std::env::var("ATELIER_API_KEY").ok();
        unsafe { std::env::remove_var("ATELIER_API_KEY") };

        assert!(resolve_api_key(None).await.is_err());
        assert!(resolve_api_key(Some("")).await.is_err());
        assert!(resolve_api_key(Some("   ")).await.is_err());
        assert_eq!(
            resolve_api_key(Some("from-flag")).await.expect("flag"),
            "from-flag"
        );

        unsafe { std::env::set_var("ATELIER_API_KEY", "from-env") };
        assert_eq!(resolve_api_key(None).await.expect("env"), "from-env");
        assert_eq!(
            resolve_api_key(Some("from-flag")).await.expect("flag"),
            "from-flag"
        );
        assert_eq!(
            resolve_api_key(Some(""))
                .await
                .expect("empty flag falls through"),
            "from-env"
        );

        match saved {
            Some(value) => unsafe { std::env::set_var("ATELIER_API_KEY", value) },
            None => unsafe { std::env::remove_var("ATELIER_API_KEY") },
        }
    }

    #[tokio::test]
    #[serial_test::serial]
    async fn resolve_api_key_ignores_legacy_environment() {
        let saved_atelier = std::env::var("ATELIER_API_KEY").ok();
        let saved_xai = std::env::var("XAI_API_KEY").ok();
        unsafe {
            std::env::remove_var("ATELIER_API_KEY");
            std::env::set_var("XAI_API_KEY", "legacy-key");
        }

        let error = resolve_api_key(None)
            .await
            .expect_err("legacy XAI_API_KEY must not enable trace classification");
        assert!(error.to_string().contains("ATELIER_API_KEY"));
    ...
    #[tokio::test]
    #[serial_test::serial]
    async fn resolve_api_key_error_names_only_explicit_sources() {
        let saved = std::env::var("ATELIER_API_KEY").ok();
        unsafe { std::env::remove_var("ATELIER_API_KEY") };
        let error = resolve_api_key(None).await.expect_err("missing key");
        assert!(error.to_string().contains("--api-key"));
        assert!(error.to_string().contains("ATELIER_API_KEY"));
        assert!(!error.to_string().contains("auth.json"));
        assert!(!error.to_string().contains("atelier login"));
    ...
    async fn run_rejects_missing_trace_path() {
        let prev = std::env::var("ATELIER_API_KEY").ok();
        unsafe { std::env::remove_var("ATELIER_API_KEY") };
        let args = RunArgs {
            ...
            model_id: "m".into(),
            api_base_url: "https://x".into(),
            // The trace-path validation runs before API-key resolution.
            api_key: None,
            min_confidence: None,
            include_reasoning: None,
        };
        ...
        match prev {
            Some(v) => unsafe { std::env::set_var("ATELIER_API_KEY", v) },
            None => unsafe { std::env::remove_var("ATELIER_API_KEY") },
        }
    }
