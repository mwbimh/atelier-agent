...
/// after we stop waiting. Mirrors the bound the `atelier-tools` local terminal
...
fn notification_interval() -> Duration {
    std::env::var("ATELIER_TERMINAL_NOTIFICATION_INTERVAL_MS")
        .ok()
...
#[async_trait::async_trait]
impl SessionNotificationSender for atelier_acp_runtime::AcpAgentGatewaySender {
    async fn session_notification(
        &self,
        notification: acp::SessionNotification,
    ) -> Result<(), acp::Error> {
...
fn spawn_shell_command(
    ...
    #[cfg(not(unix))]
    {
        ...
        let args: Vec<std::ffi::OsString> = inv
            ...
            .collect();
        let (program, args) = crate::terminal::windows_sandbox_invocation(
...
fn spawn_program_with_args(
    ...
    #[cfg(not(unix))]
    {
        let original_args: Vec<std::ffi::OsString> =
            args.iter().cloned().map(std::ffi::OsString::from).collect();
        let (program, args) = crate::terminal::windows_sandbox_invocation(
        ...
        let program = program.to_string_lossy().into_owned();
        return spawn_with_argv(&program, cwd, env, |cmd| {
            cmd.args(args);
        });
    }
