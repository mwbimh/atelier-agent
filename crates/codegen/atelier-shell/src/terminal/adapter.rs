//! AcpTerminalAdapter: implements `atelier-tools::TerminalBackend` using ACP gateway calls.
//!
//! This adapter enables bash tool execution over ACP (remote execution).
//! It translates atelier-tools' `TerminalBackend` trait into ACP protocol calls:
...
fn wrap_command(command: &str) -> Result<String, ComputerError> {
    // On Windows the ACP client (atelier-desktop) spawns with `shell: true`
...
/// Wraps atelier-shell's ACP gateway to satisfy atelier-tools' TerminalBackend.
