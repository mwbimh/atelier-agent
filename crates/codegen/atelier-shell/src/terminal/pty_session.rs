...
const NOTIFICATION_METHOD: &str = "atelier/terminal/pty/notification";
...
pub async fn create_pty(
    ...
    let resolved_cwd = cwd
        ...
        .ok_or_else(|| TerminalExtError::Internal("failed to resolve PTY cwd".into()))?;

    #[cfg(windows)]
    let (command_program, command_args) = {
        let args: Vec<std::ffi::OsString> = shell_args
            .iter()
            .cloned()
        ...
        .map_err(|e| TerminalExtError::Internal(e.to_string()))?;
        (program.to_string_lossy().into_owned(), args)
    };

    #[cfg(not(windows))]
    let command_program = shell_path.clone();
    #[cfg(not(windows))]
    let command_args: Vec<std::ffi::OsString> = shell_args
        .iter()
        .cloned()
        .map(std::ffi::OsString::from)
        .collect();

    let mut cmd = CommandBuilder::new(&command_program);
    ...
    let resolved_cwd = Some(resolved_cwd.to_string_lossy().to_string());
