...
/// is an error; direct spawning is permitted only when the caller explicitly
/// selected the `unsafe` sandbox backend.
#[cfg(windows)]
pub(crate) fn windows_sandbox_invocation(
    ...
    .map_err(|error| std::io::Error::other(error.to_string()))?;
    Ok((runner, runner_args))
}

/// Resolved absolute path to bash. On Unix uses the `atelier_config` shell
/// resolution cascade (`$ATELIER_SHELL` > `$SHELL` > `which` > common dirs >
...
pub fn default_shell_path() -> &'static str {
    #[cfg(unix)]
    {
        atelier_config::shell::unix_shell_path(atelier_config::shell::UnixShellKind::Bash)
    }
    #[cfg(not(unix))]
    {
        "/bin/bash"
...
/// Delegates to the canonical implementation in `atelier-tools`.
pub use atelier_tools::util::pager_env;
...
/// tool output. Used when the client sets `atelier/bashOutputNoColor: true` (e.g.
/// atelier-pager which renders its own UI and doesn't need raw ANSI codes).
///
