...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new_local(&atelier_home));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub fn apply_config(&self, new_config: config::Config) {
        ...
        *self.inner.models.write() = new_catalog;
        ...
        // Push the new catalog to connected clients (`atelier/models/update`).
    ...
    /// Reload the local Provider registry into the live catalog.
    ///
    /// This path is independent of the remote `/models` fetcher. Provider
    /// CRUD can therefore take effect for the next turn without re-enabling
    ...
    ///
    /// `model_id` may be a routing slug (`config.model`, e.g. `atelier-4.5`)
    /// OR a catalog key; the catalog map is keyed by the config key, which can
    /// differ from the slug for custom/enterprise ids (e.g. key `enterprise-atelier-build`
    /// → slug `atelier-4.5`). Resolve to the catalog key first so a slug
    ...
    /// Hot-reload the catalog from `~/.atelier/models_cache.json` after an
    ...
    /// When another atelier process sharing `~/.atelier` (a `--no-leader` run, a
    /// newer client, atelier-desktop) fetches a fresher `/v1/models` catalog and
    ...
    /// `ModelsCacheManager` path is fixed to `atelier_home()`, a process-wide
    /// `OnceLock`).
...
impl ModelsCacheManager {
    fn new() -> Self {
        Self {
            path: crate::util::atelier_home::atelier_home().join(MODELS_CACHE_FILE),
            ttl: CACHE_TTL,
        }
    }
    ...
    fn load_fresh(
        ...
        if cache.atelier_version.as_deref() != Some(atelier_version::VERSION) {
    ...
    fn persist(
        ...
        let cache = ModelsCache {
            fetched_at: Utc::now(),
            atelier_version: Some(atelier_version::VERSION.to_string()),
...
/// routing slug (e.g. "Auto" and "Atelier" both route to `atelier-build`)
...
fn resolve_prefetch_env_with_auth(auth: Option<AtelierAuth>) -> Option<PrefetchEnv> {
...
fn resolve_prefetch_env(atelier_com_config: Option<AtelierComConfig>) -> Option<PrefetchEnv> {
    let atelier_home = crate::util::atelier_home::atelier_home();
    let auth_manager = AuthManager::new(&atelier_home, atelier_com_config.unwrap_or_default());
...
pub fn start_early_prefetch_with_auth(auth: Option<AtelierAuth>) -> Option<EarlyPrefetchHandle> {
    let _ = auth;
    // Remote model/settings prefetch is removed from the private runtime.
    None
}
...
pub fn start_early_prefetch(
    atelier_com_config: Option<AtelierComConfig>,
...
///
/// Sessions persist the routing slug (`[model.X].model`, e.g. `atelier-4.5`);
/// the catalog and `/model` picker use config keys (e.g. `enterprise-atelier-build`).
...
pub fn resolve_model_catalog(
    ...
    // must not stamp `none` onto atelier-4.5, which only offers low/medium/high).
...
mod tests {
    ...
    fn test_manager() -> ModelsManager {
        ...
        let tmp = std::env::temp_dir().join("atelier-test-models-manager");
    ...
    #[test]
    #[serial]
    fn reload_local_provider_catalog_updates_live_models() {
        ...
        let model = manager
            .models()
            .shift_remove("local/test-model")
            .expect("reloaded model should be available");
        assert_eq!(model.info.base_url, "http://127.0.0.1:4317/v1");
        assert_eq!(
    ...
    async fn model_switch_generation_snapshot_reflects_current_state() {
        ...
        mgr.set_current_model_id(acp::ModelId::new("atelier-3"));
    ...
    fn current_reasoning_effort_seeded_from_config() {
        let tmp = std::env::temp_dir().join("atelier-test-models-manager-seed");
        let auth_manager = Arc::new(AuthManager::new(&tmp, AtelierComConfig::default()));
    ...
    fn reasoning_effort_override_skips_models_that_do_not_offer_level() {
        use atelier_sampling_types::ReasoningEffortOption;
        use indexmap::IndexMap;
        ...
        let mut no_none = ModelEntry {
            info: config::ModelInfo::fallback("atelier-4.5"),
            api_key: None,
            env_key: None,
            api_base_url: None,
            request_payload: serde_json::Map::new(),
        ...
        prefetched.insert("atelier-4.5".to_string(), no_none);
        ...
        let catalog = resolve_model_catalog(&cfg, Some(prefetched));
        assert_eq!(
            catalog["atelier-4.5"].info.reasoning_effort,
            Some(ReasoningEffort::High),
    ...
    fn config_menu_only_model_derives_support_and_default() {
        ...
        let tmp = std::env::temp_dir().join("atelier-test-models-manager-menu-only");
        let auth_manager = Arc::new(AuthManager::new(&tmp, AtelierComConfig::default()));
        let mgr = ModelsManager::new(
            None,
            catalog,
    ...
    fn subsequent_refresh_reselects_when_model_removed() {
        ...
        // Second refresh with atelier-4 removed.
        let prefetched = make_prefetched(&["atelier-3", "atelier-4.5"]);
        mgr.apply_refresh_result(&cfg, Some(prefetched), None);
    ...
    fn reload_from_disk_cache_applies_external_catalog() {
        ...
        cache.persist(
            &make_prefetched(&["atelier-4.5", "atelier-4.3"]),
            Some("etag-ext"),
            auth_method,
            &mgr.cache_origin(),
        );
        ...
        assert!(mgr.has_fetched_real_catalog());
        assert!(mgr.models().contains_key("atelier-4.5"));
        assert!(mgr.models().contains_key("atelier-4.3"));
    ...
    fn reload_from_disk_cache_ignores_stale_cache() {
        ...
        let stale = ModelsCache {
            ...
            models: make_prefetched(&["atelier-stale"]),
        };
        cache.atomic_write(&stale);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("atelier-stale"));
    ...
    fn reload_from_disk_cache_ignores_auth_method_mismatch() {
        ...
        cache.persist(
            &make_prefetched(&["atelier-other-auth"]),
            Some("etag-x"),
            other,
            &mgr.cache_origin(),
        );
        ...
        assert!(!mgr.models().contains_key("atelier-other-auth"));
    }
    ...
    fn reload_from_disk_cache_ignores_origin_mismatch() {
        ...
        let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
        cache.persist(
            &make_prefetched(&["atelier-other-origin"]),
            Some("etag-y"),
            auth_method,
        ...
        assert!(!mgr.models().contains_key("atelier-other-origin"));
        assert!(mgr.inner.etag.read().is_none());
    }
    ...
    fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
        ...
        let legacy = ModelsCache {
            ...
            models: make_prefetched(&["atelier-legacy"]),
        };
        cache.atomic_write(&legacy);

        mgr.reload_from_cache_manager(&cache);

        assert!(!mgr.models().contains_key("atelier-legacy"));
    }
    ...
    fn build_prefetched_map_distinct_ids_same_slug() {
        let entries = vec![
            make_entry_config_with_id(Some("auto"), "atelier-build", Some("Auto")),
            make_entry_config_with_id(Some("atelier-build"), "atelier-build", Some("Atelier")),
            make_entry_config_with_id(
                Some("atelier-composer-2.5-fast"),
        ...
        assert!(map.contains_key("atelier-build"));
        assert!(map.contains_key("atelier-composer-2.5-fast"));
        assert_eq!(
            map["auto"].info.model, "atelier-build",
            "auto entry should still route to atelier-build"
        );
        assert_eq!(map["atelier-build"].info.model, "atelier-build");
    }
    ...
    fn build_prefetched_map_duplicate_id_overwrites() {
        let entries = vec![
            make_entry_config_with_id(Some("atelier-build"), "atelier-build", Some("First")),
            make_entry_config_with_id(Some("atelier-build"), "atelier-build", Some("Second")),
        ];
        let map = build_prefetched_map(entries, None);

        assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
        assert_eq!(map["atelier-build"].info.name.as_deref(), Some("Second"));
    }
    ...
    fn resolve_default_model_prefers_id_over_model_slug() {
        ...
        catalog.insert(
            "auto-atelier-build".to_string(),
            make_model_entry("atelier-build"),
        );
        catalog.insert(
    ...
    fn resolve_catalog_key_maps_routing_slug_to_config_key() {
        ...
        models.insert(
            "enterprise-atelier-build".to_string(),
            make_model_entry("atelier-4.5"),
        );
        models.insert("atelier-4.3".to_string(), make_model_entry("atelier-4.3"));

        let persisted = acp::ModelId::new("atelier-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "enterprise-atelier-build");
    }

    #[test]
    fn resolve_catalog_key_prefers_exact_key_match() {
        let mut models = IndexMap::new();
        models.insert("atelier-4.5".to_string(), make_model_entry("atelier-4.5"));

        let persisted = acp::ModelId::new("atelier-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("exact key must resolve");
        assert_eq!(key.0.as_ref(), "atelier-4.5");
    }
    ...
    fn resolve_catalog_key_last_slug_match_wins() {
        let mut models = IndexMap::new();
        models.insert(
            "default-atelier-build".to_string(),
            make_model_entry("atelier-4.5"),
        );
        models.insert(
            "user-atelier-build".to_string(),
            make_model_entry("atelier-4.5"),
        );

        let persisted = acp::ModelId::new("atelier-4.5");
        let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
        assert_eq!(key.0.as_ref(), "user-atelier-build");
    }
    ...
    fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
        ...
        let available = test_available_keys(&["enterprise-atelier-build", "atelier-4.3"]);

        let persisted = acp::ModelId::new("atelier-build");
        assert_eq!(
    ...
    fn selectable_prefers_exact_key_over_later_slug_match() {
        let mut models = IndexMap::new();
        models.insert("atelier-build".to_string(), make_model_entry("atelier-4.5"));
        models.insert("other".to_string(), make_model_entry("atelier-build"));

        let available = test_available_keys(&["atelier-build", "other"]);

        let persisted = acp::ModelId::new("atelier-build");
        let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
            .expect("exact selectable key must win");
        assert_eq!(key.0.as_ref(), "atelier-build");
    }
