...
//! connect to a atelier agent running on a different machine.
//!
    ...
    // respawn a fresh one.
    {
        ...
        if agent_tx_guard.is_none() {
            ...
            let _agent_thread = thread::Builder::new()
                ...
                .spawn(move || {
                    // The local Provider registry is the sole model source;
                    // never prefetch the inherited remote catalog here.
                    let prefetched_models = None;
