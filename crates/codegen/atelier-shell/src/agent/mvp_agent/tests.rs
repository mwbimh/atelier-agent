...
fn jwt_tier_claim_maps_free_and_paid() {
    ...
    assert_eq!(
        ...
        Some("superatelier_heavy")
    );
    assert_eq!(
        jwt_tier_claim(&jwt_with_tier(6)).as_deref(),
        Some("superatelier_lite")
    );
    assert_eq!(jwt_tier_claim(&jwt_with_tier(99)).as_deref(), Some("99"));
}
fn auth_with_mode(mode: crate::auth::AuthMode, key: &str) -> crate::auth::AtelierAuth {
    crate::auth::AtelierAuth {
        key: key.into(),
        auth_mode: mode,
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ("superatelier", "AtelierPro"),
        ("x_basic", "XBasic"),
        ("x_premium", "XPremium"),
        ("x_premium_plus", "XPremiumPlus"),
        ("superatelier_heavy", "SuperAtelierPro"),
        ("superatelier_lite", "SuperAtelierLite"),
    ];
...
fn harness_pair(id: &str) -> Vec<atelier_sampling_types::conversation::ConversationItem> {
    use atelier_sampling_types::ToolCall;
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_atelier_build() {
...
/// though the default chain would return atelier-build.
#[test]
#[serial_test::serial]
...
fn inherited_harness_template_skips_nonstrict_model() {
    use atelier_agent::prompt::user_message::UserMessageTemplate;
    let tmp = tempfile::tempdir().unwrap();
    assert!(
...
/// flow: a zero-turn switch from `atelier-build` (a client profile name) to
/// `atelier-build-plan` (the default model agent_type) must be
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("atelier-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "atelier-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_atelier_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "atelier-build-plan"),
        "atelier-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_atelier_build() {
    assert_eq!(
        resolve_required_agent_type(None, "atelier-build"),
        "atelier-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "atelier-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "atelier-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "atelier-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use atelier_tools::computer::local::{LocalFs, LocalTerminalBackend};
    ...
    let builder = atelier_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: atelier_tools::implementations::web_search::WebSearchConfig::default(),
...
async fn lookup_session_model_fallback_no_session() {
    let default_model = acp::ModelId::new("atelier-3");
...
async fn set_session_model_does_not_cross_contaminate() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (sid_a.clone(), make_test_handle("atelier-3", false, None)),
        (sid_b.clone(), make_test_handle("atelier-3", false, None)),
    ]
    .into();
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use atelier_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("atelier-3", false, Some("atelier-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("atelier-3", false, Some("atelier-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("atelier-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("atelier-3", true, Some("atelier-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("atelier-3", true, Some("atelier-code-extension")),
        ),
    ]
    .into();
    let updated = apply_yolo_mode_to_matching_sessions(&mut sessions, Some("atelier-tui"), false);
...
fn parse_code_nav_capability_present_and_true() {
    ...
    meta.insert(
        "atelier/codeNavigation".to_string(),
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("atelier-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("atelier-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::AtelierWeb) {
...
async fn ext_method_routes_auth_cleared_without_touching_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::AtelierAuth {
                key: "eligible".into(),
...
async fn resident_activity_reports_needs_input_when_pending() {
    ...
    let handle = make_test_handle("atelier-3", false, None);
...
/// Drain the agent gateway, returning the first `atelier/sessions/changed`
...
/// an `atelier/sessions/changed` upsert carrying the *overridden* activity, so
...
async fn push_roster_activity_delta_broadcasts_overridden_activity() {
    ...
    agent
        .sessions
        .borrow_mut()
        .insert(sid.clone(), make_test_handle("atelier-3", false, None));
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("atelier-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("atelier-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("atelier-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("atelier-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("atelier-web"));
...
mod eligibility_gates {
    ...
    fn check_gates(
        ...
        if !matches!(client_type, ClientType::AtelierWeb) {
            return Err(CodeNavEligibility::ClientNotWeb);
        }
        if !code_nav_enabled {
            return Err(CodeNavEligibility::CapabilityNotAdvertised);
    ...
    fn tui_client_rejected() {
        assert_eq!(
            check_gates(ClientType::AtelierTUI, true, true),
            Err(CodeNavEligibility::ClientNotWeb)
        );
    }
    #[test]
    fn web_client_no_capability_rejected() {
        assert_eq!(
            check_gates(ClientType::AtelierWeb, false, true),
            Err(CodeNavEligibility::CapabilityNotAdvertised)
        );
    }
    #[test]
...
fn bg_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":1,"method":"_atelier/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_backgrounded","task_id":"{task_id}","command":"sleep 99","cwd":"/tmp"}}}}}}"#
    )
}
fn completed_line(task_id: &str) -> String {
    format!(
        r#"{{"timestamp":2,"method":"_atelier/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"task_completed","task_snapshot":{{"task_id":"{task_id}","completed":true}}}}}}}}"#
    )
}
...
fn orphaned_tasks_ignores_unrelated_updates() {
    let tmp = tempfile::tempdir().unwrap();
    let bg = bg_line("t1");
    let unrelated = r#"{"timestamp":1,"method":"_atelier/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"auto_compact_started","percentage":80}}}"#;
...
fn orphaned_tasks_filters_rewind_dead_branches() {
    ...
    let rewind = r#"{"timestamp":3,"method":"_atelier/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"rewind_marker","target_prompt_index":0,"created_at":"2025-01-01T00:00:00Z"}}}"#;
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::ATELIER_COM_METHOD_ID,
...
fn parse_session_kind_matrix() {
    ...
    let cases: &[(&str, serde_json::Value, SessionKind)] = &[
        (
            "chat",
            json!({ "atelier/session" : { "kind" : "chat" } }),
            SessionKind::Chat,
        ),
        (
            "build",
            json!({ "atelier/session" : { "kind" : "build" } }),
            SessionKind::Build,
        ),
        (
            "chat_malformed_sibling",
            json!({ "atelier/session" : { "kind" : "chat", "facets" : "not-a-map" } }),
            SessionKind::Chat,
        ),
        (
            "unknown_kind",
            json!({ "atelier/session" : { "kind" : "frob" } }),
            SessionKind::Build,
        ),
...
fn chat_new_session_model_state_matrix() {
    ...
    let cases: &[(&str, acp::SessionModelState, Option<&str>, &str)] = &[
        (
            ...
            state_with("auto", &["auto", "atelier-4"]),
            Some("atelier-4"),
            "atelier-4",
        ),
        (
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = atelier_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::ATELIER_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
        let params = serde_json::json!({ "sessionId" : "sess-local" });
        let err = agent
            .ext_method(acp::ExtRequest::new(
                "atelier/rewind/points",
                std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
            ))
            .await
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("atelier-tui"));
...
/// Drive `atelier/internal/evict_sessions` through the real `ext_notification`
...
/// `atelier/internal/evict_sessions` notification — the realistic shape of a
...
async fn drive_disconnect_many(agent: &MvpAgent, sids: &[&acp::SessionId]) {
    ...
    agent
        .ext_notification(acp::ExtNotification::new(
            "atelier/internal/evict_sessions",
            raw.into(),
        ))
        .await
        .expect("evict_sessions notification must be handled");
}
/// Drive `atelier/session/close` through the real `ext_method` dispatch
...
/// Mixed batch in a *single* `atelier/internal/evict_sessions` notification —
...
fn interactive_trust_prompt_grant_reloads_project_mcp() {
    use atelier_test_support::EnvGuard;
    use atelier_workspace::trust::{TrustStore, workspace_key};
...
fn ann(id: &str) -> atelier_announcements::RemoteAnnouncement {
    atelier_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = atelier_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = atelier_announcements::RemoteAnnouncement {
...
mod direct_hub_cloud_removed {
    ...
    fn cloud_server_id_meta_is_hard_error() {
        let meta = serde_json::json!({ "atelier/cloud_server_id" : "srv-123" });
    ...
    fn cloud_server_id_null_still_present_is_hard_error() {
        let meta = serde_json::json!({ "atelier/cloud_server_id" : null });
        let err = reject_direct_hub_cloud_meta(meta.as_object()).expect_err("must reject");
        assert_direct_hub_error(err);
    }
    #[test]
    fn cloud_server_id_with_gateway_meta_still_hard_error() {
        let meta = serde_json::json!(
            { "atelier/cloud_server_id" : "srv-legacy", "envId" : "env-1",
            "atelier/cloud_existing_workspace" : { "server_id" : "ws-1", "cwd" :
            "/workspace" } }
        );
