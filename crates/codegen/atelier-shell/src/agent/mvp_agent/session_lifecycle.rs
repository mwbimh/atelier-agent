...
impl MvpAgent {
    ...
    /// `atelier/sessions/changed` notification with the session in `removed` so
    ...
    /// `atelier/sessions/changed` notification with the current entry in
    ...
    /// Emit an `atelier/sessions/changed` upsert for a resident session with an
    ...
    /// Fan an `atelier/sessions/changed` delta out to every attached client.
    ///
    ...
    /// the `atelier/sessions/changed` special-case in `leader/server.rs`).
