...
use atelier_telemetry::id::{agent_id, agent_instance_id};
use atelier_telemetry::session_ctx::log_event;
use atelier_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use atelier_workspace::permission::{ClientType, PermissionEvent};
...
pub(crate) fn reject_direct_hub_cloud_meta(
    ...
    if session_meta.and_then(|m| m.get("atelier/cloud_server_id")).is_some() {
...
/// If `persist_data` is provided, it will be included in the meta under `atelier/persist`.
...
pub(crate) fn resolve_subscription_tier_for_telemetry(
    ...
    auth: Option<&crate::auth::AtelierAuth>,
) -> Option<String> {
...
/// `/user` already reports a newly qualifying tier (e.g. `SuperAtelierPro`). In
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        "AtelierPro" => jwt_claim == "superatelier",
        "XBasic" => jwt_claim == "x_basic",
        "XPremium" => jwt_claim == "x_premium",
        "XPremiumPlus" => jwt_claim == "x_premium_plus",
        "SuperAtelierPro" => jwt_claim == "superatelier_heavy",
        "SuperAtelierLite" => jwt_claim == "superatelier_lite",
        _ => false,
    }
}
...
pub(crate) const SESSION_PLUGIN_DIRS_CAPABILITY_KEY: &str = "atelier/pluginDirs";
...
/// Used to stamp `atelier/leaderClientId` onto replay notifications so the leader can
...
fn mark_as_replay(
    ...
    if let Some(persist) = persist_data {
        obj.insert("atelier/persist".to_string(), persist.clone());
    }
}
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a atelier_sampling_types::TokenUsage>,
...
/// Typed payload for the `atelier/settings/update` notification sent to pager
...
fn announcements_push_payload(
    stored: Option<&[atelier_announcements::RemoteAnnouncement]>,
    last_emitted: &[atelier_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<atelier_announcements::RemoteAnnouncement>> {
    let current = atelier_announcements::filter_expired_at(
...
/// Override with `ATELIER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`. Clamped to
/// >= 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("ATELIER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
/// gates fails.  Used in `atelier/code/status` responses and to generate
...
pub enum CodeNavEligibility {
    ...
    ClientNotWeb,
    /// Client did not advertise `atelier/codeNavigation.enabled`.
...
pub struct MvpAgent {
    ...
    /// Whether the current client advertised `atelier/codeNavigation.enabled`.
    ...
    /// Whether the current client advertised `atelier/folderTrust.interactive` (it
    ...
    /// list. Set by `enforce_atelier_code_access`; defaults to `true` (API-key and
    ...
    /// **non-recursive** watch on `<cwd>/` and `<cwd>/.atelier/`).
    ///
    ...
    subagent_event_tx: tokio::sync::mpsc::UnboundedSender<
        atelier_tools::implementations::atelier_build::task::types::SubagentEvent,
    >,
    ...
    monitor_event_buffer: atelier_tools::implementations::atelier_build::task::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: atelier_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// cwd→git root plus user/marketplace dirs and stalled atelier-desktop's first
...
pub(crate) fn inherited_harness_template(
    current: &atelier_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<atelier_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !atelier_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = atelier_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use atelier_agent::config::is_strict_harness_agent_type;
    match (
...
///
/// Headers injected:
///  - `x-atelier-client-version` -- required by the proxy's version-gate check.
...
fn inject_proxy_headers(
    ...
    headers
        .entry("x-atelier-client-version".to_string())
        .or_insert_with(|| {
            client_version
                .map(String::from)
                .unwrap_or_else(|| atelier_version::VERSION.to_string())
        });
...
impl MvpAgent {
    ...
    fn forward_raw_replay_line(
        ...
        let is_xai = method == "_atelier/session/update";
        if is_xai {
                ...
                if let Some(obj) = params.as_object_mut() {
                    ...
                    if let Some(m) = meta.as_object_mut() {
                        ...
                        if let Some(pd) = persist_data {
                            m.insert("atelier/persist".to_string(), pd.clone());
                        }
                        if let Some(tid) = target_client_id {
                            m.insert("atelier/leaderClientId".to_string(), tid.clone());
                        }
                    }
                }
            ...
            if let Some(tid) = target_client_id {
                stamp_meta_value(&mut notification.meta, "atelier/leaderClientId", tid);
            }
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        for task in &orphaned {
            let snapshot = atelier_tools::types::TaskSnapshot {
...
/// absent — matching the `atelier_build_access_gate` flag's server-side default.
///
/// Used by both `enforce_atelier_code_access` (initial login gate) and
