...
//! (`atelier/folder_trust/request`) that asks a GUI client (atelier-desktop) to decide
...
//! `atelier/folderTrust.interactive` AND the folder-trust feature flag is on AND the
//! verdict is [`atelier_workspace::folder_trust::TrustOutcome::Prompt`]. No
...
//! `.atelier/lsp.json` servers start on the NEXT session open (the durable grant
...
/// ACP `atelier/folder_trust/request` payload (agent → GUI client). Serialized as
...
/// ACP `atelier/folder_trust/request` response (GUI client → agent).
...
impl MvpAgent {
    /// Parse the `atelier/folderTrust.interactive` capability from an initialize
    ...
    pub(crate) fn parse_interactive_trust_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("atelier/folderTrust"))
    ...
    /// `atelier/folderTrust.interactive` AND [`folder_trust::prompt_warranted`]
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = atelier_workspace::trust::workspace_key(cwd);
        ...
        let targets: Vec<ReloadTarget> = self
            ...
            .values()
            .filter(|h| {
                atelier_workspace::trust::workspace_key(std::path::Path::new(&h.info.cwd)) == key
            })
            .map(|h| ReloadTarget {
        ...
        tokio::task::spawn_local(async move {
            ...
            let ext_request = acp::ExtRequest::new("atelier/folder_trust/request", raw_params.into());
...
struct ReloadAfterGrant<'a> {
    ...
    plugin_handle: &'a atelier_agent::plugins::SharedPluginRegistryHandle,
    ...
    proxy_url: &'a str,
    compat: &'a atelier_tools::types::CompatConfig,
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // The session's OWN project hooks (`.atelier/hooks`, `.cursor/hooks.json`),
