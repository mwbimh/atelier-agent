...
impl MvpAgent {
    /// Parse the `atelier/codeNavigation.enabled` capability from an initialize
    ...
    pub(crate) fn parse_code_nav_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("atelier/codeNavigation"))
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        if !code_nav_enabled {
            tracing::info!(
                ...
                "code-nav eligibility check: skipping (atelier/codeNavigation.enabled not advertised)"
            );
        ...
        let git_root = atelier_workspace::session::git::find_git_root_from_path(cwd).ok();
        match &setting {
    ...
    pub fn code_nav_eligibility_for_request(
        ...
        let session_id = match session_id {
            ...
            // global state.  Callers must provide sessionId for atelier/code/* requests.
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = atelier_workspace::session::git::find_git_root_from_path(cwd)
