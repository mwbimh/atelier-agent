...
const CLIENT_VERSION_HEADER: &str = "x-atelier-client-version";

// ============================================================================
// Turn delta wire types (local to atelier-shell until cli-chat-proxy catches up)
// ============================================================================
...
pub struct FeedbackClient {
    ...
    base_url: String,
    credentials: crate::util::atelier_auth_credentials::AtelierAuthCredentials,
...
impl FeedbackClient {
    pub fn new(base_url: impl Into<String>, user_token: Option<String>) -> Self {
        ...
        let credentials =
            crate::util::atelier_auth_credentials::AtelierAuthCredentials::new(user_token);
    ...
    fn build_middleware_client(
        ...
        reqwest_middleware::ClientBuilder::new(http.clone())
            .with(atelier_auth::AuthRetryMiddleware::new(provider, 0))
            .build()
    }

    fn make_auth_provider(
        credentials: &crate::util::atelier_auth_credentials::AtelierAuthCredentials,
    ) -> Arc<dyn atelier_auth::AuthCredentialProvider> {
        if let Some(am) = credentials.auth_manager() {
            ...
            Arc::new(atelier_auth::StaticAuthCredentialProvider::new(
    ...
    fn add_common_headers(&self, builder: RequestBuilder) -> RequestBuilder {
        let builder = builder
            .header(CLIENT_VERSION_HEADER, atelier_version::VERSION)
            .header(
        ...
        if self.credentials.deployment_key.is_none() {
            builder.header("X-XAI-Token-Auth", "atelier-cli")
        } else {
            builder
        }
    }
...
mod tests {
    ...
    fn test_signals_to_update() {
        let signals = crate::session::signals::SessionSignals {
            ...
            models_used: vec!["atelier-3".to_string()],
            primary_model_id: Some("atelier-3".to_string()),
        ...
        assert_eq!(update.primary_model_id, Some("atelier-3".to_string()));
