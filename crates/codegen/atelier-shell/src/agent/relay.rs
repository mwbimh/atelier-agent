...
//! connection to the atelier.invalid relay server with automatic reconnection.
...
/// Config for the atelier.invalid WebSocket relay. Fields are private so the only
...
impl RelayConfig {
    /// Session gate: builds only for a atelier.invalid first-party session
    ...
    pub(crate) fn for_session(
        ...
        Some(Self {
            ws_url: ctx.atelier_ws_url.clone(),
            ws_origin: ctx.atelier_ws_origin.clone(),
...
///
/// The relay maintains a persistent WebSocket connection to atelier.invalid with
...
async fn attempt_auth_recovery(
    ...
    let Some(ref am) = config.auth_manager else {
        teprintln!("Authentication required. Run `atelier login` to re-authenticate.");
        cancel.cancel();
        return false;
    };
    ...
    loop {
        ...
        match connect_to_relay(&config, proxy_url.as_deref(), &cancel).await {
            Ok(ws) => {
                ...
                tprintln!("Disconnected from Atelier WebSocket server");
...
fn build_relay_request(config: &RelayConfig) -> anyhow::Result<axum::http::Request<()>> {
    ...
    req.headers_mut().insert(
        ...
        axum::http::header::HeaderValue::from_static(atelier_version::VERSION),
    );
    req.headers_mut().insert(
    ...
    let read_from_ws = async move {
        loop {
            tokio::select! {
                ...
                atelier_telemetry::unified_log::warn("relay: read liveness timeout, reconnecting",
...
mod tests {
    ...
    /// Helper to create a test AtelierAuth with the given key.
    fn test_auth(key: &str) -> AtelierAuth {
        AtelierAuth {
            key: key.to_string(),
    ...
    fn for_session_builds_only_for_xai_issuer() {
        ...
        let builds = |a: &AtelierAuth| RelayConfig::for_session(a, &cfg, None, None).is_some();
        let xai = AtelierAuth {
    ...
    /// Helper: write a AtelierAuth to disk under the given scope.
    fn write_test_auth_to_disk(dir: &std::path::Path, scope: &str, auth: &AtelierAuth) {
    ...
    async fn auth_recovery_refreshes_and_heals_missing_auth_json() {
        ...
        impl TokenRefresher for CountingRefresher {
            async fn refresh(
                ...
                RefreshOutcome::Success(Box::new(AtelierAuth {
