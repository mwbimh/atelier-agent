//! atelier.invalid chat-product model catalog: caches `/rest/modes` and maps modes to
...
/// ~54 min, matching atelier-web's refetch cadence.
...
pub const ATELIER_CHAT_MODE_ENV: &str = "ATELIER_CHAT_MODE";
...
pub fn process_chat_mode_enabled() -> bool {
    ...
    match std::env::var(ATELIER_CHAT_MODE_ENV) {
        Ok(v) => {
            let v = v.trim();
...
impl ChatModesManager {
    ...
    /// The active atelier.invalid identity, or `None` when unauthenticated. Modes are
...
/// Maps atelier.invalid modes → `SessionModelState`: keeps only `available` modes,
...
mod tests {
    ...
    fn name_falls_back_to_id_when_title_blank() {
        let mut m = available("atelier-4.5", "");
        m.title = "   ".to_owned();
        let resp = ListModesResponse {
            modes: vec![m],
            default_mode_id: String::new(),
        ...
        let state = modes_to_model_state(&resp);
        assert_eq!(state.available_models[0].name, "atelier-4.5");
    }
}
