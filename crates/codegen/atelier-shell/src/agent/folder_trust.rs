...
//! inside a cloned repository (`.mcp.json`, project `.atelier/config.toml`,
//! `~/.claude.json` `projects.<cwd>`, project `.atelier/lsp.json`). Those configs
...
//! interactive prompt, and the durable [`atelier_workspace::trust::TrustStore`]
//! reads/writes — lives in `atelier-workspace` (client-side); this module keeps
...
//! on [`atelier_workspace::folder_trust::decide`]; the consume-side nuance is
...
use atelier_workspace::trust::{TrustStore, is_unsafe_trust_root, workspace_key};
use parking_lot::Mutex;

// Decision-side (scan/decide/prompt/store) relocated to `atelier-workspace`
// (client crate). `grant_folder_trust` is the ONLY moved item referenced from
// OUTSIDE this module (shell call sites + the pager's
// `atelier_shell::agent::folder_trust::grant_folder_trust`), so only it is
...
pub use atelier_workspace::folder_trust::grant_folder_trust;
...
// (`atelier_agent::plugins::TrustStore` at `~/.atelier/trusted-plugins`, plus the
...
/// [`atelier_workspace::folder_trust::revoke_folder_trust_store`].
///
...
pub fn revoke_folder_trust(cwd: &Path) -> bool {
    ...
    let was_trusted = atelier_workspace::folder_trust::revoke_folder_trust_store(cwd);
...
/// `atelier/folderTrust.interactive`); the TTY-based [`decide_inputs`] default is
...
/// ([`atelier_workspace::folder_trust::repo_config_kinds`] /
...
/// folder is gated (so an `.atelier/lsp.json`-only repo still has a non-empty reason
...
pub(crate) fn detected_config_kinds(cwd: &Path) -> Vec<String> {
    atelier_workspace::folder_trust::repo_config_kinds(cwd)
        .into_iter()
        .map(str::to_string)
        .collect()
}
...
///   `store_trusted`: a `atelier --trust` grant issued AFTER this workspace was
...
/// (session merge, the session-less agent pool, `atelier mcp doctor`). It MUST
...
///
/// Sources: project `.atelier/config.toml [mcp_servers]` (NOT the user-tier global
...
/// `~/.atelier/config.toml` is dropped when untrusted. This is intended — untrusted
...
pub fn project_scoped_mcp_names(cwd: &Path) -> HashSet<String> {
    ...
    // `.atelier/config.toml [mcp_servers]` entries tagged project (the loader's key
...
pub fn filter_untrusted_project_lsp(
    ...
    sourced: std::collections::BTreeMap<
        String,
        (
            atelier_tools::implementations::lsp::config::LspServerConfig,
            atelier_tools::types::config_source::ConfigSource,
        ),
    >,
) -> std::collections::BTreeMap<String, atelier_tools::implementations::lsp::config::LspServerConfig>
{
    atelier_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
        sourced,
...
mod tests {
    ...
    use atelier_workspace::folder_trust::repo_configs_present;
    ...
    fn simulate_release_build() -> EnvGuard {
        EnvGuard::set(atelier_version::TEST_VERSION_ENV, "0.0.0-sim")
    }
    ...
    fn revoke_folder_trust_downgrades_cache() {
        ...
        // true; ATELIER_HOME-isolated so the seed can't touch the real user file and
        // `#[serial]` because ATELIER_HOME is process-global.
    ...
    fn revoke_never_trusted_folder_writes_no_deny_and_preserves_cascade() {
        ...
        // mid-session untrust. ATELIER_HOME-isolated so the grant writes to a temp
        // store; `#[serial]` because ATELIER_HOME is global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
    ...
    fn revoke_ancestor_cascade_trusted_child_writes_explicit_untrust() {
        ...
        // the cache. ATELIER_HOME-isolated so the grant writes to a temp store;
        // `#[serial]` because ATELIER_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
    ...
    fn envrc_gate_drops_untrusted_then_loads_when_store_trusted() {
        ...
        // actually reads `.envrc`. ATELIER_HOME-isolated so the trust store is empty;
        // ATELIER_FOLDER_TRUST unset so the default-on feature flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_project_env_when_untrusted() {
        ...
        // folder merges it. ATELIER_HOME-isolated so the trust store is empty;
        // ATELIER_FOLDER_TRUST unset so the default-on feature flag applies.
        use atelier_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn claude_env_gate_drops_subdir_project_env_when_untrusted() {
        ...
        // this). ATELIER_HOME-isolated so the trust store is empty.
        use atelier_workspace::permission::claude_settings::load_claude_env_with_project;
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_denies_untrusted_repo_with_configs() {
        ...
        // code-exec config (here `.atelier/hooks`), and no store grant must be
        // DENIED — even though no verdict was recorded first (the gate re-resolves
        // fail-closed rather than defaulting open). ATELIER_HOME-isolated (empty
        // store); ATELIER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".atelier").join("hooks")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn project_scope_allowed_allows_repo_without_configs() {
        ...
        // would wrongly deny it. Release-stamped + ATELIER_HOME-isolated so the
    ...
    fn project_scope_allowed_allows_store_trusted_repo() {
        ...
        // configs present. ATELIER_HOME-isolated so the seeded store is the temp one;
        // ATELIER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
    ...
    fn project_scope_allowed_allows_inert_local_build() {
        ...
        // ATELIER_TEST_VERSION unset so `is_local_build()` is genuinely true.
        let _unset_ver = EnvGuard::unset(atelier_version::TEST_VERSION_ENV);
        if option_env!("ATELIER_VERSION").is_some() {
    ...
    fn project_scope_allowed_denies_untrusted_plugin_only_repo() {
        // A plugin-only untrusted repo (just `.atelier/plugins/<x>/`, no
        ...
        // discover_plugins/build_for_cwd/reload. ATELIER_HOME-isolated (empty store);
        // ATELIER_FOLDER_TRUST unset so the default-on flag applies.
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".atelier").join("plugins").join("evil")).unwrap();
        assert!(
            !project_scope_allowed(tmp.path()),
    ...
    fn kill_switch_allows_untrusted_repo_after_authoritative_resolve() {
        ...
        let cold = repo_tmp();
        std::fs::create_dir_all(cold.path().join(".atelier").join("hooks")).unwrap();
        assert!(
    ...
    fn build_for_cwd_with_trust_verdict_gates_active_project_plugin() {
        ...
        // un-gating here.
        //
        // ATELIER_HOME-isolated so both the folder-trust store and the plugin trust
        // store start empty (deterministic untrusted); ATELIER_FOLDER_TRUST unset so
        // the default-on flag applies; `#[serial]` because both are process-global.
        use atelier_agent::plugins::discovery::DiscoveryConfig;
        use atelier_agent::plugins::{PluginRegistry, SharedPluginRegistryHandle};
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
    ...
    fn discover_hooks_gates_then_loads_project_hook_via_trust_verdict() {
        ...
        // ATELIER_HOME-isolated so the grant writes to a temp store; ATELIER_FOLDER_TRUST unset
        // so the default-on flag applies.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let tmp = repo_tmp();
        let hooks_dir = tmp.path().join(".atelier").join("hooks");
        ...
        let has_project_hook = |reg: &atelier_hooks::discovery::HookRegistry| {
            reg.all_hooks()
                .iter()
        ...
        let git_root = atelier_workspace::session::git::find_git_root_from_path(tmp.path()).ok();
    ...
    fn filter_untrusted_project_lsp_drops_only_project() {
        use atelier_tools::implementations::lsp::config::LspServerConfig;
            ...
            m.insert(
                ...
                (
                    ...
                    ConfigSource::Project {
                        path: PathBuf::from("/repo/.atelier/lsp.json"),
                    },
                ),
            );
            m.insert(
                ...
                (
                    LspServerConfig::default(),
                    ConfigSource::User {
                        path: PathBuf::from("/home/.atelier/lsp.json"),
                    },
                ),
            );
            m
    ...
    fn load_servers_sourced_tags_project_lsp_json() {
        use atelier_tools::implementations::lsp::config::load_servers_with_plugins_sourced;
        use atelier_tools::types::config_source::ConfigSource;

        // A `<cwd>/.atelier/lsp.json` server must be tagged `Project` so the gate
        // can distinguish it from user/plugin servers. Asserts on the specific
        // key, so any real `~/.atelier/lsp.json` on the test host is irrelevant.
        let tmp = repo_tmp();
        let atelier = tmp.path().join(".atelier");
        std::fs::create_dir_all(&atelier).unwrap();
        std::fs::write(
            atelier.join("lsp.json"),
    ...
    fn untrusted_workspace_drops_loaded_project_lsp() {
        ...
        // from `<cwd>/.atelier/lsp.json` is dropped once the workspace is untrusted.
        let tmp = repo_tmp();
        let atelier = tmp.path().join(".atelier");
        std::fs::create_dir_all(&atelier).unwrap();
        std::fs::write(
    ...
    /// (`.mcp.json`) and `projtoml` (`.atelier/config.toml [mcp_servers]`).
    ...
    /// `.atelier/config.toml`, `.mcp.json`, and `.cursor/mcp.json` must appear in
    ...
    fn cached_untrusted_is_upgraded_by_a_later_grant() {
        ...
        record(&key, false);
        assert!(!project_scope_allowed(tmp.path()));
        // Simulate a `atelier --trust` grant landing in the store after the
    ...
    fn explicit_child_untrust_survives_reload_despite_ancestor_trust() {
        ...
        let tmp = tempfile::tempdir().unwrap();
        let store_path = tmp.path().join(atelier_workspace::trust::TRUST_FILE_NAME);
    ...
    fn no_configs_trust_is_provisional_and_regated_when_configs_appear() {
        ...
        unsafe { std::env::set_var("ATELIER_FOLDER_TRUST", "1") };
        // Simulate a release-stamped build: an unstamped local/dev build (as in CI,
        // no ATELIER_VERSION) auto-trusts, so the gate would never engage without this.
        unsafe { std::env::set_var(atelier_version::TEST_VERSION_ENV, "0.0.0-sim") };
        let tmp = repo_tmp();
        ...
        unsafe { std::env::remove_var(atelier_version::TEST_VERSION_ENV) };
        unsafe { std::env::remove_var("ATELIER_FOLDER_TRUST") };
    }
    ...
    fn resolve_launch_dir_trust_matches_resolve_and_record() {
        ...
        // ATELIER_HOME so the store is empty/seeded in temp. `#[serial]` because both
        // vars are process-global.
        let _feature = EnvGuard::set("ATELIER_FOLDER_TRUST", "1");
        let _sim = simulate_release_build();
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        ...
        let untrusted = repo_tmp();
        std::fs::create_dir_all(untrusted.path().join(".atelier").join("hooks")).unwrap();
        ...
        let trusted = repo_tmp();
        std::fs::create_dir_all(trusted.path().join(".atelier").join("hooks")).unwrap();
        let mut store = TrustStore::load();
    ...
    fn local_build_is_inert_launch_trust_auto_trusts() {
        ...
        // `is_local_build_honors_test_version_override`), with ATELIER_TEST_VERSION
        // unset so `is_local_build()` is genuinely true. ATELIER_HOME-isolated so the
        // real store is never touched.
        let _sim = EnvGuard::unset(atelier_version::TEST_VERSION_ENV);
        if option_env!("ATELIER_VERSION").is_some() {
            return; // a release-stamped test binary is not a local build
        }
        let home = tempfile::tempdir().unwrap();
    ...
    fn prompt_warranted_true_for_untrusted_repo_with_configs() {
        ...
        // => the GUI prompt is warranted. ATELIER_HOME-isolated so the store starts
        // empty; `#[serial]` because ATELIER_HOME is process-global.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let tmp = repo_tmp();
        ...
        // ATELIER_FOLDER_TRUST unset: env outranks the remote flag, so an ambient
    ...
    fn prompt_warranted_false_when_feature_disabled() {
        ...
        // local-build path is not what's under test; ATELIER_HOME-isolated and
        // ATELIER_FOLDER_TRUST unset so the kill-switch is the only signal.
        let home = tempfile::tempdir().unwrap();
        let _env = EnvGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
        let _sim = simulate_release_build();
    ...
    fn detected_config_kinds_summarizes_present_markers() {
        ...
        std::fs::write(tmp.path().join(".mcp.json"), "{}").unwrap();
        std::fs::create_dir_all(tmp.path().join(".atelier").join("hooks")).unwrap();
        std::fs::write(tmp.path().join(".atelier").join("lsp.json"), "{}").unwrap();
    ...
    fn detected_config_kinds_reports_lsp_only_repo() {
        ...
        // `.atelier/lsp.json` must still produce a non-empty reason list.
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".atelier")).unwrap();
        std::fs::write(tmp.path().join(".atelier").join("lsp.json"), "{}").unwrap();
        let kinds = detected_config_kinds(tmp.path());
