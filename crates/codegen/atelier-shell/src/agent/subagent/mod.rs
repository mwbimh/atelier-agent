...
use crate::local_artifacts::artifacts::{
    ...
    local_sandbox_telemetry, write_config, write_metadata, write_session_state, write_turn_result,
};
...
pub(crate) struct SubagentTracker {
    ...
    pub color: Option<atelier_agent::config::AgentColor>,
}
...
pub(crate) struct AutoCompactThresholdTiers {
    ...
    /// `cfg.local_runtime_settings.auto_compact_threshold_percent` (GB global).
...
pub(crate) struct SubagentSpawnContext {
    ...
    pub lsp: Option<std::sync::Arc<dyn atelier_tools::implementations::lsp::LspBackend>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    ...
    pub auth: Option<crate::auth::AtelierAuth>,
    ...
    pub parent_terminal_backend: Option<Arc<dyn atelier_tools::computer::types::TerminalBackend>>,
    ...
    pub parent_notification_handle:
        Option<atelier_tools::notification::types::ToolNotificationHandle>,
    ...
    pub web_search_sampling_config: Option<atelier_sampler::SamplerConfig>,
    /// Resolved config for web fetch.
    pub web_fetch_config: atelier_tools::implementations::atelier_build::web_fetch::WebFetchConfig,
    /// Image generation config (parent-inherited).
    pub image_gen_config: atelier_tools::implementations::atelier_build::image_gen::ImageGenConfig,
    /// Resolved config for video generation.
    pub video_gen_config: atelier_tools::implementations::atelier_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<atelier_chat_state::ChatStateHandle>,
    ...
    pub plugin_registry: Option<std::sync::Arc<atelier_agent::plugins::PluginRegistry>>,
    ...
    pub file_tool_overrides: Option<Vec<atelier_tools::registry::types::ToolConfig>>,
    /// Parent session's agent config snapshot.
    pub agent_config: Option<crate::agent::config::Config>,
    pub hook_registry: Option<std::sync::Arc<atelier_hooks::discovery::HookRegistry>>,
    #[expect(
        dead_code,
        reason = "unused in production; remove expect when wired or delete the item"
    )]
    pub hook_workspace_root: String,
    pub permission_handle: Option<atelier_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<atelier_tools::types::SharedApiKeyProvider>,
    pub image_description_model: String,
    /// Dual-mode workspace operations handle.
    pub workspace_ops: atelier_workspace::WorkspaceOps,
    ...
    /// Parent session's agent name (e.g. "atelier-build").
    ...
    pub parent_tool_snapshot: Option<Vec<atelier_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<atelier_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: atelier_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: atelier_tools::types::compat::CompatConfig,
...
impl SubagentSpawnContext {
    ...
    fn apply_session_cli_overrides(&self, def: &mut atelier_agent::config::AgentDefinition) {
    ...
    pub fn resolve_compaction_verbatim_input(&self) -> bool {
        crate::agent::config::BoolFlag::env("ATELIER_COMPACTION_VERBATIM_INPUT")
            .config(
                self.agent_config
                    .as_ref()
    ...
    pub fn resolve_subagent_worktree_snapshot_enabled(&self) -> bool {
        crate::agent::config::BoolFlag::env("ATELIER_SUBAGENT_WORKTREE_SNAPSHOT")
            .config(
                self.agent_config
                    .as_ref()
...
use atelier_subagent_resolution::ResumeSourceData;
...
use atelier_subagent_resolution::resolve_effective_overrides;
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &atelier_agent::config::ModelOverride,
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &atelier_sampler::SamplerConfig,
...
fn resolve_model_override_to_config(
    model_id: &str,
    ctx: &SubagentSpawnContext,
) -> Option<(atelier_sampler::SamplerConfig, acp::ModelId)> {
...
pub(crate) fn resume_inherited_prefix_len(
    conversation: &[atelier_sampling_types::conversation::ConversationItem],
) -> usize {
...
struct InitialContext {
    ...
    conversation: Vec<atelier_sampling_types::conversation::ConversationItem>,
...
fn forked_initial_context(
    mut items: Vec<atelier_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        atelier_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[atelier_sampling_types::conversation::ConversationItem],
) -> bool {
    use atelier_sampling_types::conversation::ConversationItem;
    matches!(
...
fn verbatim_or_normalize_fork(
    items: Vec<atelier_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = atelier_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        atelier_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
        ...
        return match storage.copy_session_data_sync(
            ...
            Ok(result) => {
                ...
                let estimated_tokens =
                    atelier_chat_state::estimate_conversation_tokens(&conversation);
...
fn filter_pool_by_inheritance(
    ...
    inheritance: &atelier_agent::config::McpInheritance,
) -> Option<crate::session::mcp_servers::SharedMcpPool> {
    use atelier_agent::config::McpInheritance;
    match inheritance {
...
/// agent definition.
///
/// The harness flavor (alternate vs atelier-build) normally follows the PARENT
/// agent: `AtelierBuildOrchestrator` parents give children
...
/// (so a atelier-build session can run an alternate-harness verifier and vice-versa);
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut atelier_agent::config::AgentDefinition,
) {
...
fn summarize_tool_config(
    config: &atelier_tools::registry::types::ToolServerConfig,
) -> SubagentTypeSummary {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    atelier_workspace::session::git::find_main_repo_root_from_path(&source_cwd)
...
/// auto-backgrounded (non-destructively). Env override: `ATELIER_SUBAGENT_AWAIT_BUDGET_MS`.
const SUBAGENT_AWAIT_BUDGET: std::time::Duration = std::time::Duration::from_secs(600);
fn subagent_await_budget() -> std::time::Duration {
    std::env::var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS")
        .ok()
...
fn inject_subagent_completed_prompt(
    ...
    request: &SubagentRequest,
    auto_wake_delivered: &Option<atelier_tools::reminders::task_completion::AutoWakeDeliveredIds>,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
    task_output_tool_name: &str,
    synthetic_trace_tx: &Option<
        mpsc::UnboundedSender<crate::local_artifacts::turn::SyntheticTurnTraceRequest>,
    >,
) {
    ...
    let message = atelier_tools::reminders::task_completion::format_subagent_completion(
        &summary,
        Some(task_output_tool_name),
    );
    let wrapped = atelier_tools::reminders::wrap_reminder(&message);
    ...
    if let Some(trace_tx) = synthetic_trace_tx {
        let _ = trace_tx.send(crate::local_artifacts::turn::SyntheticTurnTraceRequest {
...
fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::ExtensionSessionNotification {
...
/// `atelier/subagent/list_running` on reconnect.
    ...
    tokio::task::spawn_local(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::ExtensionSessionNotification { notification });
            }
            if let Some(params) = params {
                let ext_notification =
                    acp::ExtNotification::new("atelier/session_notification", params.into());
...
fn write_subagent_meta(dir: &Path, meta: &SubagentMeta) -> bool {
    if let Err(error) = std::fs::create_dir_all(dir) {
        tracing::warn!(%error, "failed to create subagent meta dir");
        return false;
    }
    ...
    let json = match serde_json::to_string_pretty(meta) {
        Ok(json) => json,
        Err(error) => {
            tracing::warn!(%error, "failed to serialize subagent meta");
            return false;
        }
    };
    if let Err(error) = std::fs::write(&meta_path, json) {
        tracing::warn!(%error, path = %meta_path.display(), "failed to write subagent meta");
        return false;
    }
    true
}
