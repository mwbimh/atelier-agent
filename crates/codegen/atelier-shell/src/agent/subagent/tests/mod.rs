...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
        resume_worktree_action(true, Some("refs/atelier/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
    assert_eq!(
        resume_worktree_action(false, Some("refs/atelier/subagents/x")),
        ResumeWorktreeAction::Rehydrate
    );
...
fn subagent_inherits_parent_lsp_via_context() {
    let parent: std::sync::Arc<dyn atelier_tools::implementations::lsp::LspBackend> = Arc::new(
        DummyLspDispatch,
    );
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let auto_wake = atelier_tools::reminders::task_completion::AutoWakeDeliveredIds::default();
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
        ...
        ConversationItem::Reasoning(atelier_sampling_types::synthesized_reasoning_item("deliberating",)),
...
fn resume_vs_fork_helper_shapes_differ() {
    ...
    assert!(
        ...
        atelier_sampling_types::conversation::ContentPart::Text { text } if text
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use atelier_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
        ConversationItem::system("parent system"),
...
async fn bootstrap_fork_live_parent_chat_state_is_forked_with_marker() {
    ...
    let req = bootstrap_test_request(true);
    let mut ctx = ctx_with_toggle(HashMap::new());
    let chat = spawn_test_parent_chat_state("atelier-4.5");
...
async fn handle_subagent_request_rejects_file_as_cwd() {
    ...
    let tmp_file = tmp_dir.path().join("atelier-test-cwd-file");
...
fn subagent_await_budget_default_and_override() {
    unsafe { std::env::remove_var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS") };
    assert_eq!(SUBAGENT_AWAIT_BUDGET, std::time::Duration::from_secs(600));
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS", "1500") };
    assert_eq!(subagent_await_budget(), std::time::Duration::from_millis(1500));
    unsafe { std::env::set_var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS", "0") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::set_var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS", "not-a-number") };
    assert_eq!(subagent_await_budget(), SUBAGENT_AWAIT_BUDGET);
    unsafe { std::env::remove_var("ATELIER_SUBAGENT_AWAIT_BUDGET_MS") };
}
#[test]
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("AtelierBuild:read_file");
...
/// Regression: on the DEFAULT atelier-build host —
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::AtelierBuildPlan.as_ref().to_string(),
    );
...
fn fresh_tool_model_accepts_visible_key_and_internal_id() {
    ...
    models.insert("atelier-3".to_string(), test_model_entry("atelier-3-2025-02-15"));
    assert!(
        super::handle_request::task_model_override_error(Some("atelier-3"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
        "key lookup should succeed"
    );
    assert!(
        super::handle_request::task_model_override_error(Some("atelier-3-2025-02-15"),
        ModelOverrideProvenance::Tool, false, & models, false,).is_none(),
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(! error.contains("atelier models"));
    }
    assert!(
...
fn test_sampling_config(model_slug: &str) -> atelier_sampling_types::SamplingConfig {
