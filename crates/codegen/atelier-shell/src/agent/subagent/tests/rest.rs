...
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = atelier_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = atelier_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = atelier_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = atelier_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        atelier_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
        ...
        .filter(|i| {
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                            p, atelier_sampling_types::conversation::ContentPart::Text {
    ...
    coordinator
        ...
        .insert(
            ...
            CompletedSubagent {
                ...
                snapshot_ref: None,
                effective_model_id: "atelier-3".into(),
                block_waited: false,
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        snapshot_ref: Some("refs/atelier/subagent-snapshots/sa-snap".into()),
        effective_model_id: None,
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/atelier/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
        parsed.snapshot_ref.as_deref(), Some("refs/atelier/subagent-snapshots/sa-snap")
    );
}
#[test]
...
fn update_subagent_meta_snapshot_ref_persists_to_disk() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/atelier/subagents/sa-write",
    ...
    assert_eq!(reread.snapshot_ref.as_deref(), Some("refs/atelier/subagents/sa-write"));
    assert_eq!(reread.status, "completed");
    assert_eq!(reread.worktree_path.as_deref(), Some("/tmp/atelier-wt/subagent-x"));
}
...
fn update_subagent_meta_snapshot_ref_reports_failure_when_meta_missing() {
    let dir = tempfile::TempDir::new().unwrap();
    assert!(
        ! update_subagent_meta_snapshot_ref(dir.path(), "refs/atelier/subagents/sa-missing",
        "completed")
    );
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(
        update_subagent_meta_snapshot_ref(dir.path(), "refs/atelier/subagents/x",
        "completed")
    );
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(Some("refs/atelier/subagents/x"), reread.snapshot_ref.as_deref());
...
async fn set_completed_snapshot_ref_updates_in_memory_entry() {
    ...
    coordinator
        .set_completed_snapshot_ref("sa-mem", "refs/atelier/subagents/sa-mem".into());
    let after = coordinator
        .resumable_source_for("sa-mem", "session-A", Path::new("/tmp"))
        .unwrap();
    assert_eq!(after.snapshot_ref.as_deref(), Some("refs/atelier/subagents/sa-mem"));
}
/// Unknown id is a no-op (entry already TTL-evicted; meta.json still holds it).
#[test]
fn set_completed_snapshot_ref_unknown_id_is_noop() {
    let mut coordinator = SubagentCoordinator::new();
    coordinator.set_completed_snapshot_ref("ghost", "refs/atelier/subagents/ghost".into());
    assert!(
...
async fn completion_snapshot_sequence_persists_ref_then_removes_worktree() {
    ...
    let ref_name = "refs/atelier/subagents/glue-1";
...
async fn gate_on_completion_clears_model_facing_worktree_path_but_resume_retains_it() {
    let wt = PathBuf::from("/tmp/atelier-wt/subagent-disp-1");
    let mut coordinator = SubagentCoordinator::new();
    ...
    coordinator
        .set_completed_snapshot_ref("disp-1", "refs/atelier/subagents/disp-1".into());
    ...
    assert_eq!(Some("refs/atelier/subagents/disp-1"), src.snapshot_ref.as_deref());
}
...
async fn gate_on_completion_retains_worktree_path_when_not_removed() {
    let wt = PathBuf::from("/tmp/atelier-wt/subagent-keep-1");
    let mut coordinator = SubagentCoordinator::new();
...
async fn disposal_completes_before_subagent_is_observable() {
    ...
    let ref_name = "refs/atelier/subagents/order-1";
    let snapshot_ref = crate::session::worktree::snapshot_subagent_worktree(
            &wt,
            &repo,
            ref_name,
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.atelier/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
        worktree.as_deref(),
        Some(Path::new("/home/user/.atelier/worktrees/myrepo/subagent-sub-wt",)),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/atelier-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.atelier/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_accounts_for_images() {
    use atelier_sampling_types::conversation::{ContentPart, ConversationItem, UserItem};
    let text_only = vec![
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    let dir = std::env::temp_dir()
        .join("atelier-test-durable-resume")
    ...
    let meta = SubagentMeta {
        ...
        snapshot_ref: None,
        effective_model_id: Some("atelier-3".into()),
    };
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/atelier-wt/sa-dur"));
...
fn durable_fallback_rejects_running_status() {
    let dir = std::env::temp_dir()
        .join("atelier-test-durable-status")
        .join(uuid::Uuid::now_v7().to_string());
...
fn durable_meta_roundtrips_effective_model_id() {
    let dir = std::env::temp_dir()
        .join("atelier-test-model-roundtrip")
        .join(uuid::Uuid::now_v7().to_string());
    let _ = std::fs::create_dir_all(&dir);
    let meta = SubagentMeta {
    ...
    let loaded: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(
        loaded.effective_model_id.as_deref(), Some("atelier-3"),
...
fn resume_model_pinning_overrides_default_resolution() {
    let source_model = Some("atelier-3".to_string());
    let resolved_model = "atelier-light";
...
async fn read_parent_sampling_config_keeps_auto_catalog_id_with_routing_slug() {
    ...
    models.insert("auto".to_string(), test_model_entry("atelier-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "atelier-4.5", "composer-2-fast", models);
...
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    let mut models = indexmap::IndexMap::new();
    models.insert("atelier-4.5".to_string(), test_model_entry("atelier-4.5"));
    let ctx = ctx_with_parent_chat_state("auto", "atelier-4.5", "auto", models);
    let (config, model_id) = read_parent_sampling_config(&ctx).await;
    assert_eq!(config.model, "atelier-4.5");
    assert_eq!(model_id.0.as_ref(), "auto");
}
...
async fn read_parent_sampling_config_resolves_backend_search_from_catalog() {
    let mut entry = test_model_entry("atelier-4.5");
    entry.info.supports_backend_search = true;
    let mut models = indexmap::IndexMap::new();
    models.insert("auto".to_string(), entry);
    let mut ctx = ctx_with_parent_chat_state("auto", "atelier-4.5", "auto", models);
...
async fn resolve_subagent_inherits_parent_model_without_pins() {
    use atelier_agent::config::ModelOverride;
    for parent_model in ["atelier-4.5", "composer-2-fast", "my-custom-byok-model"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
...
/// (`atelier-4.5`) and a custom parent (`composer-2-fast`)
...
async fn resolve_subagent_config_override_pin_applies_for_any_parent() {
    use atelier_agent::config::ModelOverride;
    for parent_model in ["atelier-4.5", "composer-2-fast"] {
        let mut ctx = ctx_with_toggle(HashMap::new());
        ctx.sampling_config.model = parent_model.to_string();
        ctx.model_id = acp::ModelId::new(parent_model);
        ctx.available_models
...
async fn resolve_subagent_agent_definition_pin_applies_for_light_parent() {
    use atelier_agent::config::ModelOverride;
    let mut ctx = ctx_with_toggle(HashMap::new());
    ctx.sampling_config.model = "atelier-4.5".to_string();
    ctx.model_id = acp::ModelId::new("atelier-4.5");
    ctx.available_models
        .insert("pinned-model".to_string(), test_model_entry("pinned-model"));
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(& result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &atelier_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
