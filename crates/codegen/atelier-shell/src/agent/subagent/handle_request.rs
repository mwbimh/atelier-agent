...
use crate::local_artifacts::turn::{ArtifactWriteWait, PromptTraceContext, complete_prompt_trace};
...
pub(crate) async fn handle_subagent_request(
    ...
    }
    {
        ...
        if effective_runtime.isolation == SubagentIsolationMode::None
            && definition.isolation
                == Some(atelier_agent::config::IsolationMode::Worktree)
        {
    ...
    let worktree_path = if let Some(ref source) = resume_source {
        if effective_runtime.isolation != atelier_tool_types::SubagentIsolationMode::None
    ...
    } else if effective_runtime.isolation != atelier_tool_types::SubagentIsolationMode::None
    {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(
            ...
            Err(e) => {
                ...
                std::env::temp_dir().join("atelier-subagent-worktrees").join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: atelier_fast_worktree::CreationMode = ctx.worktree_type.into();
        let btrfs_delegate = crate::session::worktree::btrfs_delegate_from_env();
        match tokio::task::spawn_blocking(move || {
                let mut builder = atelier_fast_worktree::WorktreeBuilder::new(
                        &source_clone,
                        &dest,
                    )
                    .working_tree_mode(
                        atelier_fast_worktree::WorkingTreeMode::PreserveWorkingTree,
                    )
    ...
    {
        use atelier_tools::implementations::atelier_build::task::MAX_SUBAGENT_DEPTH;
    ...
    let child_cwd_abs = atelier_paths::AbsPathBuf::new(child_cwd)
        .unwrap_or_else(|_| {
            atelier_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let initial_child_tokens = atelier_chat_state::estimate_conversation_tokens(
        &forked_conversation,
    );
    ...
    let attribution_callback: Option<atelier_sampler::SharedAttributionCallback> = effective_sampling_config
    ...
    if let Some(scope) = agent_memory_scope {
        ...
        let memory_tools: Vec<atelier_tools::registry::types::ToolConfig> = vec![
            (& atelier_build::ReadFileTool).into(), (& atelier_build::SearchReplaceTool)
        ...
        if memory_md.is_file() && let Ok(content) = std::fs::read_to_string(&memory_md) {
            ...
            let truncated: String = content
                ...
                .collect::<Vec<_>>()
                .join("\n");
            let truncated = atelier_tools::util::truncate::truncate_str(
                    &truncated,
                    MAX_BYTES,
                )
                .to_string();
    ...
    let agent_mcp_servers: Vec<_> = definition
                ...
                .filter_map(|entry| match entry {
                    atelier_agent::config::McpServerRef::Named(name) => {
                    ...
                    atelier_agent::config::McpServerRef::Inline { name, config } => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            atelier_agent::prompt::skills::list_skills_with_plugins(
                    Some(&parent_cwd_str),
    ...
    atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::SubagentLaunched {
        subagent_id: request.id.clone(),
    ...
    let spawn_result = session::spawn_session_on_thread(
            ...
            atelier_agent::DEFAULT_SYSTEM_PROMPT_LABEL.to_string(),
            atelier_chat_state::CompactionMode::Summary,
            ...
            ctx.yolo_mode
                || matches!(
                    ...
                    atelier_agent::config::PermissionMode::BypassPermissions
                ),
            false,
            None,
    ...
    let outcome = if result.success {
        atelier_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        atelier_telemetry::events::Outcome::Cancelled
    } else {
        atelier_telemetry::events::Outcome::Error
    };
    atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        outcome,
    ...
    if let Some(ref wt_path) = worktree_path {
        if snapshot_dispose_enabled {
            let ref_name = format!("refs/atelier/subagents/{}", request.id);
            let source_repo = resolve_subagent_source_repo(&ctx);
