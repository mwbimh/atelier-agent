...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct AtelierSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        let models = [
            model("atelier-build", "Atelier"),
            model("atelier-4.5", "Atelier 4.5"),
        ];
        let current = acp::ModelId::from("atelier-build");
        ...
        assert_eq!(selected_models[0].id, "atelier-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("atelier-build", "Atelier")];
        let current = acp::ModelId::from("atelier-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("atelier-build", "")];
        let current = acp::ModelId::from("atelier-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "atelier-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "atelier-build".to_string(),
            category: "model".to_string(),
        ...
        assert_eq!(v["id"], "atelier-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Atelier");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn atelier_session_detail_serializes_camel_case() {
        let detail = AtelierSessionDetail::build(
            "sess-1".to_string(),
        ...
        assert_eq!(v["currentModelId"], "atelier-build");
