...
/// Router for atelier/session/* and atelier/session_summaries/* methods.
pub async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "atelier/session/info" => handle_session_info(agent, args).await,
        "atelier/session/close" => handle_session_close(agent, args).await,
        "atelier/session/list" => handle_session_list(agent, args).await,
        "atelier/sessions/list" => handle_roster_list(agent, args).await,
        m if m.starts_with("atelier/session_summaries/") => {
...
/// `atelier/sessions/list` — the FleetView roster. Returns every
...
/// `atelier/sessions/changed` broadcast.
...
async fn handle_session_close(
    ...
    if existed {
        ...
        tracing::info!(session_id = %req.session_id, "session closed via atelier/session/close");
    } else {
...
async fn handle_session_summaries(
    ...
    match args.method.as_ref() {
        "atelier/session_summaries/session_list" => {
        ...
        "atelier/session_summaries/workspace_list" => {
        ...
        "atelier/session_summaries/workspace_list_recent" => {
