...
//! lift the gate.
//!
//! The pager drives the polling via `atelier/auth/check_subscription`: the 5s
...
/// Subscription tiers that qualify for Atelier access.
...
async fn fetch_user_info(
    ...
    let request = http_client
        ...
        .header(
            "X-XAI-Token-Auth",
            auth_manager.atelier_com_config().token_header.as_str(),
        )
...
/// the paywall is shown (`atelier/auth/check_subscription`).
///
...
mod tests {
    ...
    fn partial_tier_name_is_not_qualifying() {
        ...
        assert!(!QUALIFYING_TIERS.contains(&"Atelier"));
