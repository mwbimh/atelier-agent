//! Data APIs for `atelier models`. Clients own display.
...
mod tests {
    ...
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 5]) {
        ...
        let guards = [
            ...
            EnvGuard::unset("ATELIER_WS_ORIGIN"),
            EnvGuard::unset("ATELIER_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
    ...
    fn resolve_oauth_session() {
        ...
        let _auth = EnvGuard::set("ATELIER_AUTH", &json);

        assert_eq!(
            AuthStatus::resolve(&Config::default()),
            AuthStatus::LoggedIn("atelier.invalid".to_owned())
        );
    }
