...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.atelier/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** atelier process
    /// sharing the same `~/.atelier` (the writer may also be this process;
...
/// Watches `~/.atelier/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.atelier/config.toml`, `.mcp.json`,
...
/// processes (e.g. `atelier login` in another terminal).
///
/// Adds two **non-recursive** watches per `cwd` argument:
/// `<cwd>/` (catches `.mcp.json` and `.claude.json` at the project root) and
/// `<cwd>/.atelier/` (catches `<cwd>/.atelier/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.atelier/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        let atelier_home_buf = atelier_home.to_path_buf();
        ...
        let mut debouncer = new_filtered_debouncer(debounce, move |res: DebounceEventResult| {
            ...
            for event in events {
                ...
                let change = match name {
                    Some("auth.json") if parent == Some(atelier_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::AuthChanged)
                    }
                    Some("config.toml") if parent == Some(atelier_home_buf.as_path()) => {
                        Some(ConfigChangeEvent::GlobalConfigChanged)
                    }
                    Some("models_cache.json") if parent == Some(atelier_home_buf.as_path()) => {
        ...
        debouncer
            .watcher()
            .watch(atelier_home, RecursiveMode::NonRecursive)
            .map_err(|e| {
                tracing::warn!(
                    path = %atelier_home.display(),
                    error = %e,
                    "failed to watch atelier home directory"
                )
            })
            .ok()?;
        ...
        // includes `<cwd>/.atelier/config.toml` so the loop above
        // watches `<cwd>/.atelier/`), the call below installs a
        ...
        tracing::info!(
            atelier_home = %atelier_home.display(),
    ...
    /// Register `<cwd>/` and `<cwd>/.atelier/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.atelier/config.toml` trigger a
    ...
    /// `<cwd>/.atelier/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.atelier/` does not yet
/// exist at session-open time, the `.atelier/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.atelier/` followed by a
/// write to `<cwd>/.atelier/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let atelier_dir = cwd.join(".atelier");
    if let Err(e) = debouncer
        .watcher()
        .watch(&atelier_dir, RecursiveMode::NonRecursive)
    {
        log_watch_error(
            &e,
            "failed to watch project .atelier directory (non-recursive)",
        );
    }
}
...
fn unwatch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let atelier_dir = cwd.join(".atelier");
    if let Err(e) = debouncer.watcher().unwatch(&atelier_dir) {
        tracing::debug!(error = %e, "failed to unwatch project .atelier directory");
    }
}

/// Log a `notify` watch failure, distinguishing the benign
/// "directory doesn't exist yet" case (logged at `debug!` — it's
/// expected for a freshly-opened session whose `<cwd>/.atelier/` hasn't
...
/// Watches skill directories (`~/.atelier/skills/`, `<repo>/.atelier/skills/`, etc.)
...
/// recursively: `atelier_home` (`~/.atelier`, or `$ATELIER_HOME`) or a known vendor dir
...
fn is_global_config_dir(dir: &Path, atelier_home: &Path) -> bool {
    #[allow(deprecated)]
    let home = std::env::home_dir();
    is_global_config_dir_impl(dir, atelier_home, home.as_deref())
}
...
const HOME_VENDOR_DIRS: &[&str] = &[".atelier", ".agents", ".claude", ".cursor"];

/// Testable core of [`is_global_config_dir`] with `$HOME` injected.
fn is_global_config_dir_impl(dir: &Path, atelier_home: &Path, home: Option<&Path>) -> bool {
    let canon = |p: &Path| dunce::canonicalize(p).unwrap_or_else(|_| p.to_path_buf());
    if canon(dir) == canon(atelier_home) {
        return true;
    }
...
/// `~/.atelier` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](atelier_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = atelier_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &atelier_home,
            config_paths,
        ...
        for dir in &dirs_to_watch {
            if is_global_config_dir(dir, &atelier_home) {
...
mod tests {
    ...
    /// `is_global_config_dir` must scope down only atelier_home and the known
    ...
    /// recursively watched.
    #[test]
    fn is_global_config_dir_matches_only_atelier_home_and_vendor_dirs() {
        let home = TempDir::new().unwrap();
        let home = home.path();
        let atelier_home = home.join(".atelier");

        let g = |dir: &Path| is_global_config_dir_impl(dir, &atelier_home, Some(home));

        // atelier_home and vendor dirs directly under $HOME: scoped (global).
        assert!(g(&atelier_home));
        ...
        assert!(!g(&home.join("repo").join(".atelier")));
    }

    /// Regression for the ~/.atelier inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.atelier/worktrees/`) must not
    ...
    /// A write to `<atelier_home>/models_cache.json` must surface as
    /// `ConfigChangeEvent::ModelsCacheChanged` so a long-running leader can
    /// hot-load a catalog fetched by another atelier process.
    #[test]
    #[cfg_attr(
        target_os = "macos",
        ignore = "flaky on macOS: FSEvents does not reliably deliver events in test harness"
    ...
    /// A write to `<cwd>/.atelier/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_atelier = cwd.path().join(".atelier");
        fs::create_dir_all(&project_atelier).unwrap();
        // Seed the file before the watcher starts so we observe the
        // modification rather than the creation event.
        fs::write(project_atelier.join("config.toml"), "").unwrap();
        ...
        fs::write(
            project_atelier.join("config.toml"),
            "[mcp_servers.x]\ncommand = \"/bin/true\"",
        )
        .unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.atelier/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.atelier/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_atelier = new_cwd.path().join(".atelier");
        fs::create_dir_all(&project_atelier).unwrap();
        fs::write(project_atelier.join("config.toml"), "").unwrap();
