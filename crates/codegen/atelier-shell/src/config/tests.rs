...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "ATELIER_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$ATELIER_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${ATELIER_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$ATELIER_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$ATELIER_TEST_CONFIG_MISSING/bin/server");
}
...
fn storage_mode_is_always_local_in_the_private_build() {
    ...
    with_env_var("ATELIER_STORAGE_MODE", "writeback", || {
        assert_eq!(
            StorageMode::resolve(Some("writeback"), Some(&remote)),
            StorageMode::Local,
            "private builds must not enable vendor session writeback"
        );
    });
}
...
/// Mutex to serialize tests that touch the ATELIER_MEMORY env var.
...
/// Run `f` with ATELIER_MEMORY explicitly unset.
fn without_atelier_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("ATELIER_MEMORY", None, f)
}
/// Run `f` with ATELIER_MEMORY set to a specific value.
fn with_atelier_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("ATELIER_MEMORY", Some(value), f)
}
#[test]
...
fn memory_config_env_var_zero_does_not_enable() {
    with_atelier_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "ATELIER_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_atelier_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(! mem.enabled, "ATELIER_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_atelier_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "ATELIER_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_atelier_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "ATELIER_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_atelier_memory(
        ...
        || {
            ...
            let mem = MemoryConfig::resolve(true, false, &config, None);
            assert!(
                mem.enabled, "CLI --experimental-memory should override ATELIER_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_all() {
    with_atelier_memory(
        ...
        || {
            ...
            assert!(
                ! mem.enabled,
                "--no-memory should override --experimental-memory, ATELIER_MEMORY=1, and TOML enabled=true"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_atelier_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(! mem.enabled, "--no-memory should override ATELIER_MEMORY=1");
        },
    );
}
#[test]
...
"#;
        ...
        assert_eq!(mem.flush.flush_model.as_deref(), Some("atelier-4"));
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$ATELIER_TEST_USER $ATELIER_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the ATELIER_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with ATELIER_SUBAGENTS explicitly unset.
fn without_atelier_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("ATELIER_SUBAGENTS", None, f)
}
/// Run `f` with ATELIER_SUBAGENTS set to a specific value.
fn with_atelier_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("ATELIER_SUBAGENTS", Some(value), f)
}
#[test]
    ...
    with_atelier_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(! sa.enabled, "ATELIER_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_atelier_subagents(
        ...
        || {
            ...
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(
                ! sa.enabled, "ATELIER_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_atelier_subagents(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let sa = SubagentsConfig::resolve(true, &config, None);
            assert!(sa.enabled, "--subagents CLI flag should override ATELIER_SUBAGENTS=0");
        },
    );
}
#[test]
fn subagents_config_models_parsed() {
    without_atelier_subagents(|| {
        ...
        assert_eq!(sa.models.get("explore").unwrap(), "atelier-3-fast");
        assert_eq!(sa.models.get("plan").unwrap(), "atelier-4.5");
    });
}
#[test]
    ...
    with_atelier_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config, None);
            assert!(sa.enabled, "ATELIER_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "atelier-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("ATELIER_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "ATELIER_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "ATELIER_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "ATELIER_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("ATELIER_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_atelier_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_atelier_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "ATELIER_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("ATELIER_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_atelier_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_atelier_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_atelier_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn tools_config_env_var_disables() {
    with_atelier_respect_gitignore(
        "0",
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let tc = ToolsConfig::resolve(&config);
    ...
    with_atelier_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "ATELIER_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".atelier/prompts/impl.md"
        "#;
    ...
    assert_eq!(researcher.model.as_deref(), Some("atelier-3"));
    ...
    assert_eq!(implementer.prompt_file.as_deref(), Some(".atelier/prompts/impl.md"));
}
#[test]
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".atelier/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".atelier").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(concise.instructions_file.as_deref(), Some(".atelier/personas/concise.md"));
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".atelier").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            workspace.join(".atelier/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".atelier/personas/reviewer.toml"),
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.atelier/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
fn roles_coexist_with_models_and_toggle() {
    ...
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.enabled);
    assert_eq!(cfg.models.get("explore").map(| s | s.as_str()), Some("atelier-fast"));
...
fn validate_hooks_path_rejects_outside_atelier_home() {
    ...
    assert!(
        msg.contains("must be under ~/.atelier/"),
        "should mention ~/.atelier/ restriction, got: {msg}"
    );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let atelier_home = crate::util::atelier_home::atelier_home();
    let traversal = format!("{}/../evil", atelier_home.display());
...
#[test]
fn validate_hooks_path_accepts_atelier_hooks_subdir() {
    let atelier_home = crate::util::atelier_home::atelier_home();
    let valid_path = atelier_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.atelier/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use atelier_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (-> features.disable_yolo) must NOT clamp the user's own atelier yolo.
#[test]
...
fn simulate_release_build() -> atelier_test_support::EnvGuard {
    atelier_test_support::EnvGuard::set(atelier_version::TEST_VERSION_ENV, "0.0.0-sim")
}
...
/// verdicts untouched). ATELIER_HOME-isolated + `#[serial]` for folder-trust
/// store hygiene (empty store ⇒ deterministic untrusted;
/// `EnvGuard` restores ATELIER_HOME even on panic). No user-global
/// `$ATELIER_HOME/config.toml` is seeded: `atelier_home()` is `OnceLock`-cached,
...
fn resolve_effective_plugins_config_gates_project_paths_on_folder_trust() {
    ...
    let atelier = repo.path().join(".atelier");
...
/// in `atelier-agent`. An ABSOLUTE plugin path is used so the merged
...
/// ATELIER_HOME-isolated + `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
fn discover_plugins_excludes_untrusted_configpath_plugin_end_to_end() {
    use atelier_agent::plugins::{TrustStore, discover_plugins};
    use atelier_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let _env = EnvGuard::set("ATELIER_HOME", home.path());
    let _flag = EnvGuard::unset("ATELIER_FOLDER_TRUST");
...
/// arm (store-only reconcile) could never lift. ATELIER_HOME-isolated (empty
/// store); ATELIER_FOLDER_TRUST unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
