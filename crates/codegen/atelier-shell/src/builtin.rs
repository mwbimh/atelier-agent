//! Built-in files extracted to `~/.atelier/` on startup.
...
///
/// These directories under `~/.atelier/skills/` will be deleted on startup
...
/// `<atelier_home>/skills/<name>/SKILL.md`. Exact-path (not prefix) so a
/// user-authored skill that reuses a bundled name — even elsewhere under
/// `<atelier_home>/skills/` — is never labeled bundled. Lives beside the
...
pub(crate) fn is_extracted_bundled_skill(
    ...
    BUNDLED_SKILLS.iter().any(|&(n, _)| n == name)
        && path == atelier_home.join("skills").join(name).join("SKILL.md")
}

/// Resolve the content for a skill, applying any name-specific transforms.
fn resolve_skill_content(name: &str, raw: &str, atelier_home: &std::path::Path) -> String {
    match name {
        // Help skill needs path substitution so absolute paths work.
        "help" => {
            let atelier_home_str = format!("{}/", atelier_home.to_string_lossy());
            raw.replace("~/.atelier/", &atelier_home_str)
        }
        _ => raw.to_string(),
    }
}

/// Extract bundled files to `~/.atelier/` on startup.
///
...
pub fn extract_bundled_files(atelier_home: &std::path::Path) {
    ...
    remove_legacy_bundled_skills(atelier_home);

    let version = atelier_version::VERSION;
    let marker = atelier_home.join(".metadata_version");
        ...
        && existing.trim() == version
    {
        ...
        extract_missing_skills(atelier_home);
        return;
    }
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(atelier_home.join(stale));
    }

    for &(filename, content) in BUNDLED_FILES {
        if let Err(e) = std::fs::write(atelier_home.join(filename), content) {
    ...
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_dir = atelier_home.join("skills").join(name);
        let _ = std::fs::create_dir_all(&skill_dir);
        let content = resolve_skill_content(name, raw, atelier_home);
...
fn extract_missing_skills(atelier_home: &std::path::Path) {
    for &(name, raw) in BUNDLED_SKILLS {
        let skill_md = atelier_home.join("skills").join(name).join("SKILL.md");
        if skill_md.exists() {
            continue;
        }
...
fn remove_legacy_bundled_skills(atelier_home: &std::path::Path) {
    remove_legacy_skills(atelier_home, LEGACY_BUNDLED_SKILL_NAMES, BUNDLED_SKILLS);
}
...
fn remove_legacy_skills(
    ...
    for name in legacy_names {
        ...
        let dir = atelier_home.join("skills").join(name);
        if dir.exists() {
...
mod tests {
    ...
    async fn help_skill_discovered_by_skill_pipeline() {
        ...
        std::fs::create_dir_all(workspace.join(".atelier").join("skills").join("help")).unwrap();
        std::fs::copy(
            home.join("skills/help/SKILL.md"),
            workspace.join(".atelier/skills/help/SKILL.md"),
        )
        .unwrap();
