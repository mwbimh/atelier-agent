...
mod tests {
    ...
    fn test_nonblocking_acquire_writes_holder_info() {
        ...
        // Windows LockFileEx prevents a second handle from reading the locked
        // range. Holder info is persistent, so inspect it after releasing.
        drop(lock);
    ...
    #[cfg(unix)]
    const LOCK_HOLDER_READY: &str = "__ATELIER_LOCK_HOLDER_READY__";

    /// Subprocess entry point for the cross-process lock tests. Only does
    /// anything when re-executed with `ATELIER_TEST_LOCK_HOLDER` set; a normal
    ...
    fn subprocess_lock_holder() {
        let Ok(spec) = std::env::var("ATELIER_TEST_LOCK_HOLDER") else {
    ...
    fn spawn_lock_holder_subprocess(
        ...
        let mut child = std::process::Command::new(exe)
            .env("ATELIER_TEST_LOCK_HOLDER", spec)
            .args([
                "--ignored",
                "--exact",
                "--nocapture",
