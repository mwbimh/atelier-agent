...
use crate::auth::model::{AtelierAuth, UserInfo, lookup_auth};
...
pub(super) fn spawn(manager: Arc<AuthManager>, auth: AtelierAuth) {
...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let token_header = &manager.atelier_com_config.token_header;
...
pub(super) async fn enrich_inline(manager: &AuthManager, auth: &mut AtelierAuth) {
...
async fn run_user_info_enrichment(manager: &AuthManager, auth: AtelierAuth) {
    let started = std::time::Instant::now();
...
pub(super) fn apply_user_info_enrichment(disk: &mut AtelierAuth, user_info: UserInfo) {
