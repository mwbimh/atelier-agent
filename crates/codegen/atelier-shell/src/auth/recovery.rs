...
use crate::auth::model::AtelierAuth;
use crate::auth::token_type::TokenType;
use atelier_telemetry::events::{AuthTokenKind, ManualAuth, ManualAuthReason, ManualAuthSurface};
...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&AtelierAuth>) -> Self {
        Self {
...
impl ManualAuthTracker {
    ...
    pub(crate) fn record(
        ...
        atelier_telemetry::session_ctx::log_event(event);
    }
...
impl UnauthorizedRecovery {
    ...
    pub async fn next(&mut self) -> Result<AtelierAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<AtelierAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<AtelierAuth, AuthError> {
        loop {
            match self.step {
    ...
    async fn try_reload_from_disk(&self) -> Option<AtelierAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            atelier_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<AtelierAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<AtelierAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &AtelierAuth) -> bool {
...
mod tests {
    ...
    use super::*;
    use crate::auth::config::AtelierComConfig;
    use crate::auth::error::{RefreshTokenError, RefreshTokenFailedReason};
    use crate::auth::model::{AtelierAuth, AuthMode};
    ...
    fn rejected_cred() -> Option<AtelierAuth> {
        Some(AtelierAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), AtelierComConfig::default()));
        (dir, m)
    }
    ...
    async fn recover_with_ok_refresher(
        m: &Arc<AuthManager>,
    ) -> (Result<AtelierAuth, AuthError>, u32) {
        let calls = Arc::new(AtomicU32::new(0));
        m.set_refresher(Arc::new(OkRefresher {
            calls: calls.clone(),
        }));
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.atelier_com_config().auth_scope();
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.atelier_com_config().auth_scope();
        let expired_different = AtelierAuth {
