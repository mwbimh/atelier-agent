...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> AtelierAuth {
    AtelierAuth {
...
#[test]
#[serial]
fn local_manager_ignores_legacy_auth_environment() {
    ...
    let _env = atelier_test_support::EnvGuard::set("ATELIER_AUTH", &auth_json);
    let dir = tempfile::tempdir().expect("temp home");

    let manager = AuthManager::new_local(dir.path());

    assert!(manager.is_local_only());
...
fn legacy_scope_fallback_reads_old_auth_json() {
    ...
    // a machine that was authenticated with an older atelier version).
...
fn new_scope_takes_precedence_over_legacy() {
    ...
    let legacy_auth = AtelierAuth {
        key: "legacy-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    let new_auth = AtelierAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
/// legacy `https://accounts.atelier/sign-in` entry from auth.json.
#[test]
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(AtelierAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let mem_auth = AtelierAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);

    let disk_auth = AtelierAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
async fn auth_returns_expired_api_key_consistently_with_current() {
    ...
    // funnel the user back through `atelier login`.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
async fn permanent_failure_reads_absent_after_clear_so_auth_reports_not_logged_in() {
    ...
    // User runs `atelier logout` which calls clear().
    mgr.clear().unwrap();

    // The diagnostic the user now sees on the next request should be
    // "Not logged in. Run `atelier login`.", not the stale invalid_grant.
    let err = mgr.auth().await.unwrap_err();
    assert!(
...
/// `has_atelier_code_access`. The `&mut AtelierAuth` signature already
...
/// contributor adding a token-like field to both `AtelierAuth` and
/// `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = AtelierAuth {
        key: "ROT_KEY".into(),
        ...
        has_atelier_code_access: Some(true),
    ...
    assert_eq!(
        disk.has_atelier_code_access,
        snapshot.has_atelier_code_access
    );
...
async fn current_api_key_async_drives_refresh_chain() {
    use atelier_tools::types::ApiKeyProvider;
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    ...
    let valid_but_rejected = AtelierAuth {
...
async fn enrich_auth_inline_populates_zdr_flags() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let mgr = AuthManager::new(dir.path(), AtelierComConfig::default()).with_proxy_base_url(&base);

    let mut auth = AtelierAuth {
        key: "tok".into(),
        ..AtelierAuth::test_default()
    };
...
fn oidc_session_for_team(principal_id: &str) -> AtelierAuth {
    AtelierAuth {
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || AtelierAuth {
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), AtelierComConfig::default()));
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use atelier_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use atelier_telemetry::events::{AuthTokenKind, ManualAuthSurface};
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, AtelierComConfig::default()));
