...
use super::model::{API_KEY_SCOPE, AtelierAuth, AuthMode, AuthStore, lookup_auth};
...
/// Falls back to the legacy `https://accounts.atelier/sign-in` scope key
/// when the requested scope is not found (devbox auth.json migration).
pub fn read_token_by_scope(atelier_home: &Path, scope: &str) -> anyhow::Result<String> {
    let path = atelier_home.join("auth.json");
    let store = read_auth_json(&path)
        .map_err(|_| anyhow::anyhow!("Not logged in. Run `atelier login`."))?;
    lookup_auth(&store, scope).map(|a| a.key).ok_or_else(|| {
        anyhow::anyhow!("Your auth token is invalid. Run `atelier login` to re-authenticate.")
    })
}

/// Read the API key from the `xai::api_key` scope in auth.json.
pub fn read_api_key(atelier_home: &Path) -> Option<String> {
    let path = atelier_home.join("auth.json");
...
pub fn store_api_key(atelier_home: &Path, api_key: &str) -> std::io::Result<()> {
    let path = atelier_home.join("auth.json");
...
pub fn clear_api_key(atelier_home: &Path) -> std::io::Result<()> {
    let path = atelier_home.join("auth.json");
