...
use crate::auth::config::{AtelierComConfig, PreferredAuthMethod};
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use atelier_telemetry::events::ManualAuthSurface;
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<AtelierAuth>>>,
    path: PathBuf,
    scope: String,
    atelier_com_config: AtelierComConfig,
    proxy_base_url: String,
    /// Production Atelier managers are local-only and must not inspect the
    /// removed vendor credential store or start refresh/network flows.
    local_only: bool,
    ...
    power_listener: parking_lot::Mutex<Option<atelier_system_power::SystemPowerListener>>,
...
enum LockOutcome {
    Held(AuthFileLock),
    Adopted(Box<AtelierAuth>),
}

// ── Construction + builders ──────────────────────────────────────────

impl AuthManager {
    /// Construct an empty credential manager for the local Provider runtime.
    ///
    /// Unlike [`Self::new`], this constructor deliberately does not read
    /// `ATELIER_AUTH`, `ATELIER_AUTH_PATH`, or `$ATELIER_HOME/auth.json`.
    pub fn new_local(atelier_home: &Path) -> Self {
        let mut config = AtelierComConfig::default();
        config.preferred_method = Some(PreferredAuthMethod::ApiKey);
        let proxy_base_url =
            crate::agent::config::EndpointsConfig::from_effective_config().proxy_url();
        let mut manager = Self::assemble(
            None,
            atelier_home.join("auth.json"),
            "atelier/local".to_owned(),
            config,
            proxy_base_url,
    ...
    pub fn new(atelier_home: &Path, atelier_com_config: AtelierComConfig) -> Self {
        let scope = atelier_com_config.auth_scope();
        let proxy_base_url =
            crate::agent::config::EndpointsConfig::from_effective_config().proxy_url();
        ...
        atelier_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "atelier_home": atelier_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "ATELIER_HOME": std::env::var("ATELIER_HOME").unwrap_or_else(|_| "(unset)".into()),
                "ATELIER_AUTH_PATH": std::env::var("ATELIER_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "ATELIER_AUTH": std::env::var("ATELIER_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // ATELIER_AUTH: inline JSON credentials (highest priority, read-only).
        if let Ok(inline_json) = std::env::var("ATELIER_AUTH") {
            if let Ok(auth) = serde_json::from_str::<AtelierAuth>(&inline_json) {
            ...
            tracing::warn!("ATELIER_AUTH set but failed to parse as JSON, falling back to file");
        }

        // ATELIER_AUTH_PATH: custom file path (overrides default $ATELIER_HOME/auth.json).
        let path = std::env::var("ATELIER_AUTH_PATH")
            .map(PathBuf::from)
            .unwrap_or_else(|_| atelier_home.join("auth.json"));
    ...
    /// else allowed.
    pub(crate) fn cached_token_policy_error(&self, auth: &AtelierAuth) -> Option<AuthError> {
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.atelier_com_config)?;
    ...
    fn vet_cached(&self, auth: AtelierAuth) -> Option<AtelierAuth> {
    ...
    pub(crate) fn current(&self) -> Option<AtelierAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    #[inline]
    pub(crate) fn with_inner_write<R>(&self, f: impl FnOnce(&mut Option<AtelierAuth>) -> R) -> R {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&AtelierAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<AtelierAuth> {
    ...
    pub(crate) fn expired_auth(&self) -> Option<AtelierAuth> {
        let auth = self
            .inner
            .read()
            .as_ref()
    ...
    fn is_token_expired(&self, auth: &AtelierAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &AtelierAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &AtelierAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.atelier_com_config.auth_token_ttl
        {
    ...
    ///
    /// Returns the input `AtelierAuth` BEFORE enrichment lands; callers
    /// needing the post-enrichment view re-read `current()`.
    pub(crate) async fn update(
        self: &Arc<Self>,
        auth: AtelierAuth,
    ) -> std::io::Result<AtelierAuth> {
        ...
        match &write_result {
            Ok(()) => atelier_telemetry::unified_log::info(
            ...
            Err(e) => atelier_telemetry::unified_log::error(
    ...
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: AtelierAuth) {
        enrichment::spawn(Arc::clone(self), auth);
    }

    /// Blocking `/user` enrichment for login flows that exit before the background task lands.
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut AtelierAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    pub(crate) fn atelier_com_config(&self) -> &AtelierComConfig {
        &self.atelier_com_config
    }
    ...
    pub(crate) fn run_external_refresh_command(&self, command: &str) -> Option<AtelierAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: AtelierAuth) {
    ...
    fn inner_auth_or_external_default(&self) -> AtelierAuth {
        self.inner.read().clone().unwrap_or_else(|| AtelierAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: AtelierAuth) -> Option<AtelierAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<AtelierAuth> {
    ...
    fn read_disk_auth_silent(&self) -> Option<AtelierAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<AtelierAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in ATELIER_AUTH mode).
            DiskAuthState::Ok => {
                atelier_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<AtelierAuth, AuthError> {
        if self.local_only {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<AtelierAuth, AuthError> {
        // Snapshot inner ONCE for dispatch atomicity (closes a TOCTOU
        // where a concurrent `clear()` raced `token_type()` + `inner.read()`).
        let snapshot: Option<AtelierAuth> = self.with_inner_read(|inner| inner.cloned());
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.atelier_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // assumption that a sibling process (`atelier login` from
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.atelier_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery().await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(self: &Arc<Self>) -> Result<AtelierAuth, AuthError> {
        if self.atelier_com_config.blocks_automatic_oidc() {
            tracing::debug!(
        ...
        atelier_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    fn check_refresh_deferral(&self, reason: RefreshReason) -> Result<(), AuthError> {
        ...
        // Dark wake (see `atelier_system_power::PowerState` for the canonical
    ...
    pub(crate) fn is_different_token(&self, candidate: &AtelierAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<atelier_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // `[atelier_com] auth_token_ttl` to drive a real schedule.
...
impl atelier_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
/// Build a refreshing [`ApiKeyProvider`](atelier_tools::types::ApiKeyProvider)
...
/// [`current_api_key_async`](atelier_tools::types::ApiKeyProvider::current_api_key_async),
/// so it works for both OAuth/session (refreshes) and API-key auth.
pub fn shared_api_key_provider(
    auth_manager: Arc<AuthManager>,
) -> atelier_tools::types::SharedApiKeyProvider {
