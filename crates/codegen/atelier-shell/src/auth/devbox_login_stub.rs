...
//! points that can still be reached directly (`atelier login --devbox`) return a
//! descriptive error.

use super::manager::AuthManager;
use super::model::AtelierAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<AtelierAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors
/// defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<AtelierAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `atelier login --devbox` entry point: always errors in this build.
