...
use crate::auth::{AtelierAuth, AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode};
...
/// `x-atelier-client-surface` header so device-flow metrics can separate logins a
...
pub async fn request_device_code(
    ...
    let resp = with_alpha_test_key(
        client
            ...
            .header("x-atelier-client-surface", surface.as_str())
            .form(&[
                ("client_id", client_id),
                ("scope", scope_str.as_str()),
                ("referrer", "atelier-build"),
            ]),
        &url,
    )
    .send()
...
///
/// On success, persists credentials to `~/.atelier/auth.json` and returns
/// the authenticated `AtelierAuth`.
///
...
pub async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `atelier login --device-auth` again.");
        }
...
pub async fn run_device_code_login_channels(
    ...
    // `atelier/auth/get_url` isn't blocked on a slow/hanging browser launch
...
pub(crate) mod tests {
    ...
    fn auth_manager_with_atelier_home(
        ...
        Arc::new(
            AuthManager::new(atelier_home, AtelierComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let atelier_home = temp_dir.path().join(".atelier");
        std::fs::create_dir_all(&atelier_home).unwrap();
        let auth_manager = auth_manager_with_atelier_home(&atelier_home, "http://127.0.0.1:9");
        let tokens = super::TokenOk {
            access_token: "access-token".to_string(),
            refresh_token: Some("refresh-token".to_string()),
            expires_in: Some(900),
            scope: Some("openid email offline_access atelier-cli:access".to_string()),
            id_token: Some(
    ...
    fn build_auth_seeds_team_metadata_from_access_token() {
        ...
        let claims = serde_json::json!({
            ...
            "exp": 9999999999u64,
            "iat": 1000000000u64,
            "scope": "offline_access atelier-cli:access team:read",
            "principal_type": "Team",
        ...
        let tokens = super::TokenOk {
            ...
            expires_in: Some(900),
            scope: Some("offline_access atelier-cli:access team:read".to_owned()),
            id_token: None,
        };
    ...
    fn assert_build_auth_rejected(
        ...
        let auth_manager = Arc::new(
            AuthManager::new(&atelier_home, cfg).with_proxy_base_url("http://127.0.0.1:9"),
        );
        ...
        assert!(
            !atelier_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::AtelierAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_atelier_home(temp_dir.path(), "http://127.0.0.1:9");
