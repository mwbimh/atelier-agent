...
use super::super::config::{AtelierComConfig, OidcAuthConfig};
use super::super::{AtelierAuth, AuthManager};
use super::protocol::{
    OidcError, build_atelier_auth, build_authorize_url, discover, enforce_login_principal,
...
fn callback_response(result: &CallbackResult) -> (StatusCode, Html<String>) {
    let (title, message) = match result {
        Ok(_) => (
            "Signed in",
            "You can close this window and return to Atelier.",
        ),
...
pub async fn run_login_flow_with_config(
    ...
    let oauth2 = auth_manager.atelier_com_config().oauth2.as_ref();
    ...
    let principal_policy = login_principal_policy(auth_manager.atelier_com_config());
    ...
    let mut auth = build_atelier_auth(tokens, user_info, &oidc.issuer, &oidc.client_id);
...
mod tests {
    ...
    async fn full_login_flow_via_race() {
        ...
        let auth_manager = Arc::new(
            AuthManager::new(temp_dir.path(), AtelierComConfig::default())
        ...
        .await
        .unwrap();
        let auth = build_atelier_auth(tokens, user_info, &oidc_cfg.issuer, &oidc_cfg.client_id);
