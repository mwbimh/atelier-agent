...
use super::super::config::{
    AtelierComConfig, ForceLoginTeam, OAuth2ProviderConfig, OidcAuthConfig,
};
use super::super::{AtelierAuth, AuthMode};
use base64::Engine;
...
pub fn is_configured(config: &AtelierComConfig) -> bool {
    config.oidc.is_some()
}
...
pub(crate) fn login_principal_policy(cfg: &AtelierComConfig) -> Option<ForceLoginTeam> {
...
pub(super) fn build_atelier_auth(
...
mod tests {
    ...
    fn authorize_url_includes_required_oidc_params() {
        let config = OidcAuthConfig {
            ...
            audience: Some("api://atelier".into()),
        };
    ...
    fn authorize_url_includes_team_principal_params() {
        let config = OidcAuthConfig {
            ...
            client_id: TEST_CLIENT_ID.into(),
            scopes: vec!["offline_access".into(), "atelier-cli:access".into()],
            audience: None,
        };
        ...
        let discovery = Discovery {
            authorization_endpoint: "https://auth.atelier/authorize".into(),
            token_endpoint: "https://auth.atelier/token".into(),
            jwks_uri: None,
            id_token_signing_alg_values_supported: None,
        };
        let pkce = Pkce {
        ...
        assert!(url.contains("referrer=atelier-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
            "expected exactly one referrer param, got: {url}"
    ...
    fn authorize_url_uses_oauth2_referrer_override_once() {
        ...
        let oauth2 = OAuth2ProviderConfig {
            ...
            principal_type: None,
            principal_id: None,
            referrer: Some("atelier-desktop".into()),
        };
        let discovery = Discovery {
            authorization_endpoint: "https://auth.atelier/authorize".into(),
            token_endpoint: "https://auth.atelier/token".into(),
        ...
        assert!(url.contains("referrer=atelier-desktop"));
        assert!(!url.contains("referrer=atelier-build"));
        assert_eq!(
            url.matches("referrer=").count(),
            1,
    ...
    fn peek_access_token_principal_matrix() {
        ...
        let team_jwt = make_jwt(serde_json::json!(
            ...
            "offline_access atelier-cli:access api:access", "principal_type" : "Team",
