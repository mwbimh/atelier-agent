...
///
/// - `"atelier-shell"`   — classic Atelier CLI / TUI (atelier-shell)
/// - `"atelier"`   — new Atelier Pager / TUI (atelier-pager)
/// - `"atelier-desktop"` — Atelier Desktop app
/// - `"atelier-extension"` — VS Code / browser extension
///
/// The factory forwards both:
/// - `x-atelier-client-version`   (e.g. "0.1.210-alpha.5 (279ffacddb)")
...
pub fn build_storage_client_for_proxy(
    ...
    if let Some(am) = auth_manager {
        ...
        .with_client_identity(atelier_version::VERSION, client_identifier)
        .with_client_mode(crate::http::process_client_mode())
        .with_attribution(bridge)
    } else {
        let mut creds = AtelierAuthCredentials::new(user_token);
...
impl HttpAuth for OtelAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        ...
        let mut creds = AtelierAuthCredentials::new(None);
...
pub fn sync_external_otel_identity() {
    if let Some(provider) = OTEL_PROVIDER.get() {
        ...
        atelier_telemetry::external::set_identity(
            atelier_telemetry::external::IdentityAttrs::from_snapshot(&snapshot),
        );
    }
}
...
/// `atelier-pager` and `atelier-tui` need at tracing init time.
///
...
pub fn build_default_otel_layer_config() -> atelier_telemetry::otel_layer::OtelLayerConfig {
    let endpoints = crate::agent::config::EndpointsConfig::default();
    let atelier_com_config = crate::auth::AtelierComConfig::default();
    let exporter = atelier_telemetry::otel_layer::OtelExporterConfig {
    ...
    let token_header_value = atelier_com_config.token_header.clone();
    let atelier_home = crate::util::atelier_home::atelier_home();
    let bootstrap = Arc::new(AuthManager::new(&atelier_home, atelier_com_config));
    let provider = Arc::new(OtelAuthCredentialProvider::new(bootstrap));
    let _ = OTEL_PROVIDER.set(provider.clone());
    atelier_telemetry::otel_layer::OtelLayerConfig {
