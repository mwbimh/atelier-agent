...
/// Parse process output (stdout) into a `AtelierAuth`. Accepts bare token or JSON.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<AtelierAuth> {
...
pub(crate) fn refresh_with_command(command: &str, prev_auth: &AtelierAuth) -> Option<AtelierAuth> {
