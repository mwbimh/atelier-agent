...
/// Default scopes for the xAI OAuth2 provider. Includes `atelier-cli:access`
...
pub enum PreferredAuthMethod {
    ...
    ApiKey,
    /// OIDC / OAuth2 session (`cached_token`, interactive `atelier.invalid` / `oidc`,
    /// including devbox-minted OIDC).
    Oidc,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct AtelierComConfig {
    pub atelier_ws_origin: String,
    pub atelier_ws_url: String,
    ...
    /// Login button label (env: `ATELIER_AUTH_PROVIDER_LABEL`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// refresh works. Env: `ATELIER_AUTH_TOKEN_TTL`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// can't bypass the deployment's IdP login. Env: `ATELIER_DISABLE_API_KEY_AUTH`.
...
/// Customer OIDC Identity Provider configuration (`[atelier_com_config.oidc]`).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// OAuth2 provider configuration (`ATELIER_OAUTH2_ISSUER` / `ATELIER_OAUTH2_CLIENT_ID`).
///
...
pub fn use_local_auth() -> bool {
    std::env::var("ATELIER_LOCAL_AUTH")
...
impl AtelierComConfig {
    ...
    /// `ATELIER_DISABLE_API_KEY_AUTH` env lockdown is sticky: because the env value
    ...
    /// pin is fail-closed. Explicit `atelier login --devbox` / `--api-key` bypass
...
impl OAuth2ProviderConfig {
    ...
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("ATELIER_OAUTH2_ISSUER").ok()?;
        let client_id = std::env::var("ATELIER_OAUTH2_CLIENT_ID").ok()?;
        let principal_type = std::env::var("ATELIER_OAUTH2_PRINCIPAL_TYPE").ok();
        let principal_id = std::env::var("ATELIER_OAUTH2_PRINCIPAL_ID").ok();
        ...
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("ATELIER_OAUTH2_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or(default_scopes),
            principal_type,
            principal_id,
            referrer: Some(
                std::env::var("ATELIER_OAUTH2_REFERRER")
...
impl Default for AtelierComConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            atelier_ws_origin: String::new(),
            atelier_ws_url: String::new(),
            token_header: "atelier-cli".to_owned(),
            oidc,
            oauth2,
            auth_provider_command: std::env::var("ATELIER_AUTH_PROVIDER_COMMAND").ok(),
            auth_provider_label: std::env::var("ATELIER_AUTH_PROVIDER_LABEL").ok(),
            auth_token_ttl: std::env::var("ATELIER_AUTH_TOKEN_TTL")
                .ok()
                .and_then(|v| v.parse().ok()),
            disable_api_key_auth: std::env::var("ATELIER_DISABLE_API_KEY_AUTH")
                .ok()
...
/// Parse a boolean env-var value for atelier's on/off flags. A bare presence
/// enables the flag, but the common falsy spellings (`0`, `false`, `off`,
/// `no`, empty) count as disabled — so e.g. `ATELIER_DISABLE_API_KEY_AUTH=false`
...
/// True when the admin has set `ATELIER_DISABLE_API_KEY_AUTH` to a truthy value in
...
fn env_lockdown_forced() -> bool {
    std::env::var("ATELIER_DISABLE_API_KEY_AUTH")
        .ok()
        .is_some_and(|v| env_flag_enabled(&v))
}
impl OidcAuthConfig {
    pub fn from_env() -> Option<Self> {
        let issuer = std::env::var("ATELIER_OIDC_ISSUER").ok()?;
        let client_id = std::env::var("ATELIER_OIDC_CLIENT_ID").ok()?;
        Some(Self {
            issuer,
            client_id,
            scopes: std::env::var("ATELIER_OIDC_SCOPES")
                .map(|s| s.split(',').map(|s| s.trim().to_owned()).collect())
                .unwrap_or_else(|_| default_oidc_scopes()),
            audience: std::env::var("ATELIER_OIDC_AUDIENCE").ok(),
        })
    }
}
#[cfg(test)]
mod tests {
    ...
    fn preferred_method_deserializes_from_toml() {
        let cfg: AtelierComConfig = toml::from_str(
            r#"
        ...
        let cfg: AtelierComConfig = toml::from_str("").expect("parse empty");
