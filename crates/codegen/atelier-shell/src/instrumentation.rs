//! Shim — see `atelier_telemetry::instrumentation` for the implementation.
//!
...
///
/// Macro stays in shell so `$crate` continues to resolve to `atelier_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `atelier_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `atelier_telemetry::instrumentation`.
#[macro_export]
