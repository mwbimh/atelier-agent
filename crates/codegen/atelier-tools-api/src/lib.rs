//! Shared API definitions for Atelier tools: protobuf types, config validation,
...
///
/// Tool ids are colon-separated `Namespace:tool` (e.g. `AtelierBuild:grep`); the
...
mod default_client_name_tests {
    ...
    fn pins_first_colon_derivation() {
        assert_eq!(default_client_name("AtelierBuild:grep"), "grep");
