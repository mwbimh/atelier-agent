...
//! Backend environment presets for the Atelier crate family.
//!
//! The private build deliberately has no compiled remote endpoints. Explicit
//! Provider configuration is the only source of inference destinations; the
//! `ATELIER_PRODUCTION_*` variables remain available for local test harnesses.
/// The endpoint set for one backend environment.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AtelierBuildEndpoints {
...
const PRODUCTION_ENDPOINTS: AtelierBuildEndpoints = AtelierBuildEndpoints {
...
pub enum AtelierBuildEnvironment {
    #[default]
    Production,
}
impl AtelierBuildEnvironment {
    pub fn from_flags(_dev: bool, _staging: bool) -> Self {
        AtelierBuildEnvironment::Production
    }
    /// Indicator string for display; `None` for Production.
    pub fn indicator(&self) -> Option<&'static str> {
        match self {
            AtelierBuildEnvironment::Production => None,
        }
    }
    pub fn is_production(&self) -> bool {
        matches!(self, AtelierBuildEnvironment::Production)
    }
    fn env_prefix(&self) -> &'static str {
        match self {
            AtelierBuildEnvironment::Production => "ATELIER_PRODUCTION",
        }
    }
    /// Compiled endpoint set for this environment (empty in the private build).
    pub fn endpoints(&self) -> AtelierBuildEndpoints {
        match self {
            AtelierBuildEnvironment::Production => PRODUCTION_ENDPOINTS,
        }
    }
    /// Env-var override when set, else the empty compiled endpoint.
    ...
    /// The relay WebSocket URL (Web Frontend at `atelier.invalid/code` driving a
    ...
    /// `ATELIER_GATEWAY_URL` opt-in takes precedence.
    pub fn gateway_ws_url(&self) -> String {
        self.resolve("_GATEWAY_WS_URL", self.endpoints().gateway_ws_url)
    }
}
impl std::fmt::Display for AtelierBuildEnvironment {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            AtelierBuildEnvironment::Production => write!(f, "production"),
        }
    }
}
...
mod tests {
    ...
    fn test_env_prefix() {
        assert_eq!(
            AtelierBuildEnvironment::Production.env_prefix(),
            "ATELIER_PRODUCTION"
        );
    }
    #[test]
    ...
    /// The private build has no compiled vendor endpoints. Explicit Provider
    /// configuration owns every inference/network destination.
    #[test]
    fn production_defaults_are_local_only() {
        let env = AtelierBuildEnvironment::Production;
        assert!(env.cli_chat_proxy_base_url().is_empty());
        assert!(env.asset_server_url().is_empty());
        assert!(env.relay_ws_url().is_empty());
        assert!(env.gateway_ws_url().is_empty());
        assert!(env.ws_origin().is_empty());
    }
    #[test]
    fn test_from_flags() {
        assert_eq!(
            AtelierBuildEnvironment::from_flags(false, false),
            AtelierBuildEnvironment::Production
        );
    }
}
