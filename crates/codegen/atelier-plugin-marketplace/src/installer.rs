...
use atelier_agent::plugins::manifest::{ManifestLoadResult, load_manifest, name_from_dirname};
...
fn ensure_manifest_for_root_skills(
    ...
    use atelier_agent::plugins::manifest::load_manifest;

    // Skip if a manifest already exists.
    if let Ok(atelier_agent::plugins::manifest::ManifestLoadResult::Found(_)) =
...
mod tests {
    ...
    fn with_test_registry<T>(f: impl FnOnce(&mut InstallRegistry) -> T) -> T {
        ...
        // process-global `atelier_home()` `OnceLock` (first-write-wins). A parallel
        // test in this binary can cache the real `~/.atelier` before this runs,
