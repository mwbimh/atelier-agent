...
//! `.atelier-plugin/plugin-index.json` (preferred), then
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_atelier_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".atelier-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_atelier_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".atelier-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".atelier-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
    ...
    fn load_catalog_ignores_unknown_fields() {
        ...
        write_catalog(
            ...
            ".atelier-plugin",
            r#"{
                "$schema": "https://atelier/atelier/plugin-index.schema.json",
                "version": 1,
