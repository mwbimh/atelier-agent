...
//!
//! Catalog file lookup, in order: `.atelier-plugin/marketplace.json` (preferred),
//! `.atelier-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.atelier-plugin/marketplace.json` (preferred Atelier convention)
/// 2. `.atelier-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_atelier_dir() {
        let dir = tempfile::tempdir().unwrap();
        let atelier_dir = dir.path().join(".atelier-plugin");
        std::fs::create_dir_all(&atelier_dir).unwrap();
        std::fs::write(
            atelier_dir.join("marketplace.json"),
            r#"{"name": "atelier", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "atelier");
    }

    #[test]
    fn load_index_atelier_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".atelier-plugin", "atelier"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "atelier");
    }
