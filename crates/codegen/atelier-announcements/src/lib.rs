//! Shared announcement types, persistence, and formatting for Atelier CLI apps.
//!
//! This crate provides the common logic used by `atelier-shell` and
//! `atelier-pager` for handling announcements (banner notifications).
...
/// Payload for `atelier/announcements/update` ACP notification.
...
/// Read hidden announcement ids from `~/.atelier/announcements.json`.
...
/// Write hidden announcement ids to `~/.atelier/announcements.json`.
...
fn announcements_state_path() -> PathBuf {
    atelier_tools::util::atelier_home::atelier_home().join("announcements.json")
}
...
/// Resolve startup announcements from the local explicit override only.
/// Vendor/remote announcements are not part of the private runtime.
pub fn resolve_startup(
    remote_announcements: Option<Vec<RemoteAnnouncement>>,
) -> Option<Vec<RemoteAnnouncement>> {
    let _ = remote_announcements;
    if let Ok(raw) = std::env::var("ATELIER_ANNOUNCEMENTS_OVERRIDE") {
        match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
            Ok(list) => return Some(list),
            Err(e) => {
                tracing::warn!(error = %e, "invalid ATELIER_ANNOUNCEMENTS_OVERRIDE JSON; ignoring override");
            }
        }
    }
    None
...
mod tests {
    ...
    fn resolve_startup_env_override() {
        ...
        unsafe {
            std::env::set_var("ATELIER_ANNOUNCEMENTS_OVERRIDE", r#"[{"id":"test"}]"#);
        }
        ...
        // SAFETY: test-only
        unsafe {
            std::env::remove_var("ATELIER_ANNOUNCEMENTS_OVERRIDE");
        }
    }
    ...
    fn cta_parses_nested_partial_and_absent() {
        let full: RemoteAnnouncement = serde_json::from_str(
            r#"{"id":"p","severity":"promo","cta":{"label":"Get SuperAtelier","url":"https://atelier/atelier","caption":"or use Ctrl+O"}}"#,
        )
        .unwrap();
        let cta = full.cta.as_ref().expect("cta present");
        assert_eq!(cta.label.as_deref(), Some("Get SuperAtelier"));
        assert_eq!(cta.url.as_deref(), Some("https://atelier/atelier"));
