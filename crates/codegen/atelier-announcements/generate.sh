...
# are the single source of truth for the `atelier/announcements/update` payload.
# Pipeline:
...
DESKTOP_DIR="$REPO_ROOT/frontend/apps/atelier-desktop"
...
for f in "${bindings[@]}"; do
  {
    ...
    echo "// Regenerate via generate.sh in the atelier-announcements package."
    cat "$f"
  } >"$DST/$(basename "$f")"
done
