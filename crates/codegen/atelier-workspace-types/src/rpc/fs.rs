...
//! the shell's `atelier/fs/*` ACP methods.
...
// workspace server (`atelier-workspace`) and the atelier.invalid backend
...
// `workspace.fs_*` ops above already serve the shell's `atelier/fs/*`
...
/// Filesystem node kind. Wire values match the shell's `atelier/fs/list`
/// node `type` strings.
...
/// `atelier/fs/list` plus `offset` pagination).
