...
//!
//! `atelier_hooks` pulls in `git2`/`reqwest`/`atelier-tools`, too heavy for
...
/// Wire mirror of `atelier_hooks::discovery::HookRegistry`.
///
...
/// Wire mirror of `atelier_hooks::config::HookSpec`.
///
...
/// `atelier-hooks`, so a server-side test (`atelier-workspace`'s
...
/// Wire mirror of `atelier_hooks::event::HookEventName`.
///
        ...
        let json = serde_json::json!({
            "hooks": {
                "pre_tool_use": [{
                    ...
                    "url_raw": null,
                    "timeout_ms": 5000,
                    "source_dir": "/home/u/.atelier/hooks",
                    "extra_env": { "FOO": "bar" }
                }]
            }
        });
