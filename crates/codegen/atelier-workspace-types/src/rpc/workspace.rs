...
/// over `atelier_tools`'s `TaskSnapshot`). `tool_name`, when set, is the
...
/// needs (a slim DTO over `atelier_tools`'s `TaskSnapshot`).
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// One TODO list item (slim DTO over `atelier_tools`'s `TodoState`). `status`
...
/// in `atelier-workspace/src/hub_server.rs`.
