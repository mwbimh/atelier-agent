...
//! `atelier-tools/src/implementations/skills/types.rs` (the type the
...
pub enum SkillScope {
    /// cwd/.atelier/skills
    Local,
    /// repo_root/.atelier/skills
    Repo,
    /// ~/.atelier/skills
    User,
    /// ~/.atelier/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.atelier/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the atelier-tools SkillInfo
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.atelier/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.atelier/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.atelier/plugins/infra-plugin",
            "plugin_data": "/root/.atelier/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "atelier-4",
            "effort": "high",
            "user_invocable": true,
