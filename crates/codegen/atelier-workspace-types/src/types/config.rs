...
//! agent-session config types in `atelier-config` and friends.
...
/// `atelier-tools` once the wire surface is firm.
...
///
/// TODO(workspace): align with `atelier_config::ProjectConfig`.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
