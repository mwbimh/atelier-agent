...
//!
//! - **`Tool`**: a natural definition would be `Tool(#[from] atelier_tools::ToolError)`.
//!   That coupling would force this crate to depend on
//!   `atelier-tools`, which would defeat the lightweight
...
/// notably `std::io::Error` and `atelier_tools::ToolError`) happens
