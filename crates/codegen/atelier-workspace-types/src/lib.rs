//! Wire types for the `atelier-workspace` API.
//!
...
//! `atelier-workspace-grpc` crate. The Rust types defined here are
//! the source of truth.
...
///
/// Lives in `atelier-workspace-types` (instead of `atelier-workspace` or
/// `atelier-mcp`) so both the permission-validation layer and the MCP
/// transport layer can depend on it without dragging the full workspace or
/// rmcp into each other. Re-exported by `atelier_workspace::permission`
