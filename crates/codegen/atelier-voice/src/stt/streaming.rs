...
/// Streaming STT over `wss://api.atelier/v1/stt`.
...
impl StreamingSttSession {
    ...
    pub async fn connect(config: &VoiceConfig, bearer: &str) -> Result<Self, VoiceError> {
        ...
        return Err(VoiceError::Stt(
            "vendor voice service is disabled in the private build".into(),
        ));
...
mod tests {
    ...
    fn optional_header_inserted_when_present_skipped_when_empty() {
        let mut req = "wss://api.atelier/v1/stt".into_client_request().unwrap();
        insert_optional_header(&mut req, "x-atelier-client-identifier", "atelier-shell");
        insert_optional_header(&mut req, "User-Agent", "");
        assert_eq!(
            req.headers().get("x-atelier-client-identifier").unwrap(),
            "atelier-shell"
        );
        assert!(
