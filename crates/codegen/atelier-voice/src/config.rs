...
/// owned by the pager (`ATELIER_VOICE_MODE` / `[features] voice_mode` / remote);
...
pub struct VoiceConfig {
    ...
    /// `x-atelier-client-identifier` / `User-Agent` headers the sampler and imagine
    ...
    ///
    /// `x-atelier-client-identifier` value (e.g. `"atelier-shell"`).
    #[serde(skip)]
    pub client_identifier: String,
    /// `User-Agent` value (e.g. `"atelier-shell/1.2.3 (macos; aarch64)"`).
    #[serde(skip)]
...
mod tests {
    ...
    /// voice gate — default on, remote kill switch / `ATELIER_VOICE_MODE`).
    #[test]
