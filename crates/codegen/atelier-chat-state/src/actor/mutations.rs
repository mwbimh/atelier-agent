...
impl ChatStateActor {
        ...
        self.state
            ...
            .map(|item| match item {
                ...
                ConversationItem::Reasoning(r) => {
                    atelier_sampling_types::reasoning_item_text(r).len()
                        + r.encrypted_content.as_deref().map(str::len).unwrap_or(0)
                }
                ConversationItem::Compaction(c) => c.encrypted_content.len(),
            })
            .sum()
    }
    ...
    pub(super) fn record_last_turn_usage(&mut self, usage: atelier_sampling_types::TokenUsage) {
    ...
    pub(super) fn record_model_call_usage(
        ...
        usage: &atelier_sampling_types::TokenUsage,
