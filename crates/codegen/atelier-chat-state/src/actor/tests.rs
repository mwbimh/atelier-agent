...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_atelier_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_atelier_req_id, Some("req-1".to_string()));
}
...
async fn build_request_with_tool_definitions() {
    use atelier_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("atelier-3".to_string()));
...
/// Simulates the exact sequence that `atelier-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools = ConversationItem::Assistant(atelier_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use atelier_sampling_types::{ContentPart, UserItem};
...
async fn get_last_model_metadata_returns_both_fields() {
    let h = TestHarness::with_conversation(vec![
        ...
        ConversationItem::Assistant(atelier_sampling_types::AssistantItem {
            content: "hello".into(),
            tool_calls: vec![],
            model_id: Some("atelier-4.5".into()),
            model_fingerprint: Some("fp_abc123".into()),
            reasoning_effort: None,
        }),
    ]);
    let meta = h.handle.get_last_model_metadata().await;
    assert_eq!(meta.resolved_model_id.as_deref(), Some("atelier-4.5"));
...
async fn sampling_config_survives_compaction_replacement() {
    ...
    assert_eq!(pre.model, "atelier-build");
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("atelier-4.5"));
...
/// If that model slug is e.g. "atelier-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            atelier_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    assert_eq!(post.model, "atelier-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(
    req: &atelier_sampling_types::ConversationRequest,
) -> serde_json::Value {
...
fn assert_prefix_stable_pair(
    base: &atelier_sampling_types::ConversationRequest,
    extended: &atelier_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_model_switch() {
    ...
    let new_config = SamplingConfig {
        model: "atelier-3-mini".to_string(),
        ..test_config()
    };
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(atelier_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(atelier_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use atelier_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
...
// ============================================================================
// Out-of-band history repair (atelier/session/repair)
// ============================================================================
