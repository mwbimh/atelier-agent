...
//! no I/O, no actor state. They live in `atelier-chat-state` so that both
//! this crate and `atelier-shell` can share them without duplication.
use atelier_sampling_types::{ContentPart, ConversationItem, ToolResultItem};
...
///
/// Stored here (rather than only in `atelier-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in atelier-tools. The mapping between them
/// happens in `run_compact_inner()` (atelier-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (atelier-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in atelier-tools.
...
/// handled by the consumer (e.g. `atelier-shell`), which has access to
...
impl CompactionStateContext {
    ...
    /// model to re-read the same files. atelier-build retains
...
/// `atelier-chat-state` and `atelier-shell` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (atelier-build
    /// ordering).
    ...
    /// carrier and the atelier-build summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the atelier-build
...
/// `run_compact_inner` in `atelier-shell` assembles inline, but is
...
pub struct RemoteCompactedHistoryInput {
    pub system_message: ConversationItem,
    pub user_message_prefix: String,
    pub agents_md_reminder: Option<String>,
    pub remote_items: Vec<ConversationItem>,
    pub system_reminder: Option<String>,
}

/// Reinstall the local runtime-owned prefix around an opaque Responses
/// compaction result. Provider-owned compaction items are preserved verbatim;
/// stale System/runtime reminder copies returned by the endpoint are removed.
pub fn build_remote_compacted_history(input: RemoteCompactedHistoryInput) -> Vec<ConversationItem> {
    let mut compacted = vec![
        input.system_message,
        ConversationItem::user_meta(input.user_message_prefix.clone()),
    ];
    if let Some(reminder) = input.agents_md_reminder.as_ref() {
        compacted.push(ConversationItem::project_instructions(reminder.clone()));
    }
    for item in input.remote_items {
        let text = item.text_content();
        let is_project_instructions = matches!(
            &item,
            ConversationItem::User(user)
                if user.synthetic_reason
                    == Some(atelier_sampling_types::SyntheticReason::ProjectInstructions)
        );
        let duplicates_runtime_prefix = text == input.user_message_prefix
            || input
                .agents_md_reminder
                .as_deref()
        ...
        if matches!(item, ConversationItem::System(_))
            || is_project_instructions
            || duplicates_runtime_prefix
        {
            continue;
        }
        compacted.push(item);
    }
    if let Some(reminder) = input.system_reminder {
        compacted.push(ConversationItem::system_reminder(reminder));
    }
    compacted
}
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = atelier_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = atelier_sampling_types::repair_dangling_tool_calls(
        items,
        atelier_sampling_types::DanglingToolCallReason::HarnessHalted {
...
mod tests {
    use super::*;
    use atelier_sampling_types::SyntheticReason;

    #[test]
    fn remote_history_reinstalls_runtime_prefix_and_preserves_opaque_item() {
        let history = build_remote_compacted_history(RemoteCompactedHistoryInput {
            system_message: ConversationItem::system("canonical system"),
            user_message_prefix: "<user_info>fresh</user_info>".to_owned(),
            agents_md_reminder: Some("fresh project instructions".to_owned()),
            remote_items: vec![
                ConversationItem::system("stale system"),
                ConversationItem::user("retained user"),
                ConversationItem::Compaction(
                    atelier_sampling_types::rs::CompactionSummaryItemParam {
                        id: Some("cmp_1".to_owned()),
                        encrypted_content: "opaque-secret".to_owned(),
                    },
                ),
            ],
            system_reminder: Some("<system-reminder>fresh state</system-reminder>".to_owned()),
        });
        assert_eq!(history[0].text_content(), "canonical system");
        assert_eq!(history[1].text_content(), "<user_info>fresh</user_info>");
        assert_eq!(history[2].text_content(), "fresh project instructions");
        assert!(matches!(history[4], ConversationItem::Compaction(_)));
        assert!(history[4].text_content().is_empty());
        assert_eq!(
            history.last().unwrap().text_content(),
            "<system-reminder>fresh state</system-reminder>"
        );
    }
...
actual user question";
    ...
    fn call(id: &str) -> atelier_sampling_types::ToolCall {
        atelier_sampling_types::ToolCall {
            id: id.into(),
            name: "read_file".to_string(),
            arguments: "{}".into(),
        }
    ...
    async fn build_compacted_history_transcript_hint() {
        ...
        let summary = build_compacted_history(input(Some(
            "/home/user/.atelier/sessions/abc/updates.jsonl".to_string(),
        )))
        .last()
        .unwrap()
        .text_content();
        assert!(summary.contains("/home/user/.atelier/sessions/abc/updates.jsonl"));
    ...
    ///
    /// Simulates the exact conversation shape produced by atelier-shell:
    ///
    ...
    async fn build_compacted_history_multi_turn_with_parallel_tool_calls() {
        use atelier_sampling_types::{AssistantItem, ToolCall};
        let conversation = vec![
            ...
            ConversationItem::Assistant(AssistantItem {
                ...
                model_id: Some("atelier-3".to_string()),
...
The user asked to read main.rs and lib.rs. main.rs prints hello world, lib.rs has an add function. The user then asked to fix a typo in main.rs and run tests. The typo was fixed and tests passed.";
    ...
    fn conversation_item_preserves_reasoning_siblings() {
        use atelier_sampling_types::{AssistantItem, rs};
        let result = strip_tool_messages_for_conversation_item(vec![
            ConversationItem::system("system"),
    ...
    fn prepare_for_summarization_drops_reasoning_sibling_on_mutated_assistant() {
        use atelier_sampling_types::{AssistantItem, ToolCall, rs};
        let mk_reasoning = || {
            ConversationItem::Reasoning(rs::ReasoningItem {
                id: "r_123".to_string(),
                summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
    ...
    fn verbatim_reasoning_kept_unless_messages_backend() {
        use atelier_sampling_types::{ToolCall, rs};
        let mk = || {
            vec![
                ConversationItem::system("sys"),
                ConversationItem::Reasoning(rs::ReasoningItem {
        ...
        assert!(
            ...
            "reasoning must be kept when strip_reasoning = false (Atelier backends)"
        );
    ...
    fn fit_counts_user_images_against_budget() {
        use atelier_sampling_types::ContentPart;
