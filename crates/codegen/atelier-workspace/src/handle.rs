...
static DRAIN_COMPLETED_TOTAL: std::sync::LazyLock<IntCounterVec> = std::sync::LazyLock::new(|| {
    register_int_counter_vec!(
        "atelier_workspace_drain_completed_total",
...
static DRAIN_DURATION: std::sync::LazyLock<Histogram> = std::sync::LazyLock::new(|| {
    register_histogram!(
        "atelier_workspace_drain_duration_seconds",
...
static DRAIN_LOST_ITEMS_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "atelier_workspace_drain_lost_items_total",
...
static PRODUCER_SPAWNED_AFTER_DRAIN_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "atelier_workspace_producer_spawned_after_drain_total",
...
static WORKSPACE_BIND_ZERO_TOOLS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_bind_zero_tools_total",
            "session.bind resolutions advertising zero model-facing tools, by reason",
            &["reason"]
        )
        .unwrap()
...
static WORKSPACE_BIND_FAILED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_bind_failed_total",
...
static WORKSPACE_BIND_UNSERVED_TOOLS_TOTAL: std::sync::LazyLock<IntCounter> =
    std::sync::LazyLock::new(|| {
        register_int_counter!(
            "atelier_workspace_bind_unserved_tools_total",
...
static WORKSPACE_BIND_ADVERTISED_TOOLS: std::sync::LazyLock<Histogram> =
    std::sync::LazyLock::new(|| {
        register_histogram!(
            "atelier_workspace_bind_advertised_tools",
...
pub(crate) static WORKSPACE_TERMINAL_BACKEND_ORPHANED_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_terminal_backend_orphaned_total",
...
static ENV_CAPTURE_PANIC_TOTAL: std::sync::LazyLock<IntCounter> = std::sync::LazyLock::new(|| {
    register_int_counter!(
        "atelier_workspace_env_capture_panic_total",
...
pub(crate) static REWIND_CHECKPOINT_CAPTURE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rewind_checkpoint_capture_total",
...
pub(crate) static REWIND_CHECKPOINT_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rewind_checkpoint_finalize_total",
...
pub(crate) static REWIND_RESTORE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rewind_restore_total",
...
pub(crate) static REWIND_CHECKPOINT_DURATION: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "atelier_workspace_rewind_checkpoint_duration_seconds",
...
pub(crate) static REWIND_NON_COMPLETED_FINALIZE_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rewind_non_completed_finalize_total",
...
impl WorkspaceHandle {
    ...
    fn build(
        ...
        let (hook_registry, hook_load_errors) = {
            use atelier_hooks::discovery::{HookSource, load_hooks_from_sources};
        ...
        let lsp: Option<Arc<dyn atelier_tools::implementations::lsp::LspBackend>> = {
            let sourced =
                atelier_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
                    &config.root_cwd,
                    &[],
                    &[],
                    &[],
            ...
            let servers =
                atelier_tools::implementations::lsp::config::filter_project_lsp_when_untrusted(
                    sourced,
    ...
    /// Configure the filesystem used by workspace-session bookkeeping before
    /// the first session is created. Local agent sessions use this to route
    /// rewind and workspace extension operations through the same Worker as
    /// built-in file tools.
    pub fn set_session_filesystem(
        &self,
    ...
    pub fn create_session_with_tracker_and_viewer_ctx(
        ...
        let system_notify_channel = system_notifications
            .then(atelier_tools::notification::types::ToolNotificationHandle::channel);
    ...
    pub(crate) async fn update_tool_config(
        ...
        session_id: &str,
        new_config: atelier_tools::registry::types::ToolServerConfig,
    ...
    async fn resolve_and_swap_session_toolset_locked(
        ...
        let _ = self
            .shared
            .events
            .send(atelier_workspace_types::WorkspaceEvent::ToolsChanged {
    ...
    pub(crate) async fn rebind_existing_hub_session(
        &self,
        session_id: &str,
        explicit_cfg: Option<atelier_tools::registry::types::ToolServerConfig>,
    ...
    ///
    /// No-op when the `ATELIER_WORKSPACE_TOOL_DEFS_ENABLED` flag is off, no upload
    ...
    pub fn hook_registry(&self) -> atelier_hooks::discovery::HookRegistry {
    ...
    pub fn hook_load_errors(&self) -> &[atelier_hooks::error::HookError] {
    ...
    async fn resolve_path_within_root(
        ...
        #[cfg(windows)]
        let textual_within_root = atelier_windows_sandbox::path_is_within(root, &normalized)
            || atelier_windows_sandbox::path_is_within(canonical_root, &normalized);
        #[cfg(not(windows))]
        let textual_within_root =
            normalized.starts_with(root) || normalized.starts_with(canonical_root);
        if !textual_within_root {
            return Err(WorkspaceError::HubError(format!(
                "path escapes workspace root: {req_path}"
            )));
        }
        #[cfg(windows)]
        atelier_windows_sandbox::ensure_no_reparse_points(&normalized).map_err(|error| {
            WorkspaceError::HubError(format!("unsafe Windows reparse path: {error}"))
        })?;
        ...
        loop {
            ...
            match tokio::fs::canonicalize(&check_path).await {
                Ok(canonical) => {
                    ...
                    #[cfg(windows)]
                    let canonical_within_root =
                        atelier_windows_sandbox::path_is_within(canonical_root, &canonical);
                    #[cfg(not(windows))]
                    let canonical_within_root = canonical.starts_with(canonical_root);
                    if !canonical_within_root {
                        return Err(WorkspaceError::HubError(format!(
    ...
    /// polling / stop. Drives the `atelier/search/fuzzy/status` notification loop.
    ...
    /// Drive the `atelier/search/fuzzy/status` stream for an active search: poll
    ...
    pub async fn run_fuzzy_notifications(
        ...
        for _ in 0..max_polls {
            ...
            self.emit_client_ext("atelier/search/fuzzy/status".to_string(), params);
            if data.done {
                break;
            }
        }
    ...
    /// batch as `atelier/search/content/status` via the client sink, and returning
    ...
    pub async fn run_content_search(
        ...
        crate::file_system::content_search_streaming(&cwd, &params, cancel, move |batch| {
            ...
            handle.emit_client_ext("atelier/search/content/status".to_string(), params);
        })
        .await
    ...
    fn spawn_codebase_index_event_forwarder(&self) -> tokio::task::JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(atelier_workspace_types::WorkspaceEvent::FsChanged { ref path, kind }) => {
                    ...
                    Ok(atelier_workspace_types::WorkspaceEvent::GitHeadChanged { .. }) => {
    ...
    fn spawn_tool_definitions_event_forwarder(&self) -> Option<tokio::task::JoinHandle<()>> {
        ...
        Some(tokio::spawn(async move {
            ...
            loop {
                match rx.recv().await {
                    Ok(atelier_workspace_types::WorkspaceEvent::ToolsChanged { session_id }) => {
    ...
    pub async fn start_session_mcp_servers(
        ...
        let mcp_results: Vec<
            Result<atelier_mcp::servers::McpClient, atelier_mcp::servers::McpError>,
        > = tokio::task::spawn_blocking(move || {
            use atelier_mcp::oauth_config::McpOAuthConfigMap;
            use atelier_mcp::servers::{McpClientTimeoutOverrides, McpMetaConfigMap};
            ...
            rt_handle.block_on(atelier_mcp::servers::start_mcp_servers(
                configs,
                ...
                &oauth_config_map,
                &event_writer,
                atelier_mcp::servers::OauthInteractivity::Interactive,
            ))
        })
        .await
    ...
    pub fn on_mcp_snapshot_changed(
        &self,
        new_snapshot: Vec<atelier_tools::registry::types::ToolConfig>,
    ) -> usize {
    ...
    pub fn on_hub_tools_changed(
        &self,
        new_hub_tools: Vec<atelier_tools::registry::types::ToolConfig>,
    ) -> usize {
        self.shared
            .hub_tools_snapshot
            ...
            move |sid: atelier_tool_protocol::SessionId, params: Option<serde_json::Value>| {
                ...
                async move {
                    ...
                    let bind_config = params
                        ...
                        .unwrap_or_default();
                    let empty_toolset = || atelier_tools::registry::types::ToolServerConfig {
                        tools: vec![],
    ...
    pub async fn connect_hub(&self) -> WorkspaceResult<()> {
        ...
        let (activity_notify_handle, activity_notify_rx) =
            atelier_tools::notification::types::ToolNotificationHandle::channel();
...
pub(crate) fn apply_background_task_notification(
    ...
    notification: &atelier_tools::notification::types::ToolNotification,
) {
    use atelier_tools::notification::types::ToolNotification;
    match notification {
...
pub(crate) async fn run_activity_feed(
    ...
    mut rx: tokio::sync::mpsc::UnboundedReceiver<
        atelier_tools::notification::types::ToolNotification,
    >,
) {
...
impl DrainReason {
    /// Stable `reason` label for `atelier_workspace_drain_started_total`.
...
impl DrainOutcome {
    /// Stable `outcome` label for `atelier_workspace_drain_completed_total`.
    pub fn as_str(self) -> &'static str {
        match self {
...
/// The SIGTERM drain budget from `ATELIER_WORKSPACE_TERMINATION_GRACE_MS`
...
pub fn termination_grace_from_env() -> std::time::Duration {
    grace_budget_from_raw(std::env::var("ATELIER_WORKSPACE_TERMINATION_GRACE_MS").ok())
}
...
/// Path of the preStop drain marker (`ATELIER_WORKSPACE_DRAINING_FILE` or
/// [`DEFAULT_DRAINING_FILE`]).
fn draining_file_path() -> std::path::PathBuf {
    std::env::var("ATELIER_WORKSPACE_DRAINING_FILE")
        .map(std::path::PathBuf::from)
...
///
/// `confine_fs_to_workspace_root` confines `atelier/fs/*` resolution to the root.
/// The standalone workspace server defaults it on (it always backs a remote
/// sandbox; override via `ATELIER_WORKSPACE_CONFINE_FS_TO_ROOT`); the CLI leader
/// passes `false`.
///
...
pub async fn connect_local_workspace(
    ...
    // The private runtime never creates a remote artifact queue. Keep this
    // hard-disabled even when legacy environment/config values are present.
    let data_collection_disabled = true;
    // The workspace worker is the only filesystem boundary for production
    // sessions.  Do not pass legacy vendor auth or a default vendor endpoint
    // into the tool factory; model inference belongs to the Provider runtime.
    let mut factory =
        WorkspaceSessionContextFactory::new().with_worker_client(cwd.clone(), workspace_worker);
    if crate::session::tool_config::tool_state_enabled() {
    ...
    let tool_config = atelier_agent::workspace_atelier_build_toolset();
    ...
    if let Ok(dir) = std::env::var("ATELIER_WORKSPACE_SERVER_SKILLS_DIR")
        && !dir.is_empty()
    {
        ws_config.skills_config.server_skill_dirs = vec![dir];
    }
    if let Ok(dir) = std::env::var("ATELIER_WORKSPACE_BUNDLED_SKILLS_DIR")
        && !dir.is_empty()
    {
        let allowlist = std::env::var("ATELIER_WORKSPACE_BUNDLED_SKILLS_ALLOWLIST").ok();
        ws_config
            .skills_config
            .ignore
...
/// Resolve `$ATELIER_WORKSPACE_HOME` — the workspace-owned on-disk state root.
///
/// Precedence:
/// 1. `$ATELIER_WORKSPACE_HOME` (operator override).
/// 2. `<atelier_home>/workspace`, where `<atelier_home>` honours `$ATELIER_HOME` and
///    otherwise falls back to `~/.atelier` (see [`atelier_config::atelier_home`]).
pub fn resolve_workspace_home() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("ATELIER_WORKSPACE_HOME") {
        if !p.trim().is_empty() {
            return std::path::PathBuf::from(p);
        }
    }
    atelier_config::atelier_home().join("workspace")
}

/// Verify the process boundary exists before constructing a production
/// workspace.  A missing worker must stop startup rather than silently
/// falling back to `LocalFs` in the host process.
fn require_workspace_worker_binary(path: &std::path::Path) -> WorkspaceResult<()> {
    if path.is_file() {
        return Ok(());
    }
...
fn tool_defs_enabled() -> bool {
    std::env::var("ATELIER_WORKSPACE_TOOL_DEFS_ENABLED").as_deref() == Ok("true")
}
...
fn ephemeral_workspace_home() -> std::path::PathBuf {
    std::env::temp_dir().join(format!(
        "atelier-workspace-ephemeral-{}",
        std::process::id()
    ))
}
/// Resolve `workspace_rewind_all_outcomes` from `ATELIER_WORKSPACE_REWIND_ALL_OUTCOMES` (default off).
...
impl WorkspaceHandle {
    ...
    /// Minimal handle for a worker process.
    ///
    /// Unlike the historical local harness constructor, a worker is always
    /// bound to one workspace root.  All file and workspace RPCs therefore
    ...
    fn new_minimal_with_confinement(
        ...
        let config = WorkspaceConfig {
            root_cwd: cwd,
            default_tool_config: atelier_tools::registry::types::ToolServerConfig {
                tools: vec![],
                behavior_preset: None,
            },
...
pub(crate) mod tests {
    ...
    use atelier_tools::registry::types::ToolServerConfig;
    use atelier_tools::types::tool::ToolKind;
    use atelier_workspace_types::WorkspaceEvent;
    use std::sync::Arc;
    ...
    impl atelier_tools::types::tool_metadata::ToolMetadata for BashCcoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> atelier_tools::types::tool::ToolNamespace {
            atelier_tools::types::tool::ToolNamespace::MCP
        }
        ...
        ) -> Result<atelier_tools::types::output::ToolOutput, atelier_tool_runtime::ToolError>
        {
            ...
            Ok(atelier_tools::types::output::ToolOutput::Bash(
                atelier_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        atelier_tools::types::output::BashOutput::make_output_for_prompt(
    ...
    async fn build_session_routed_handlers_skips_invalid_client_name_without_panic() {
        let handle = make_handle();
        let mut renamed = tc("AtelierBuild:read_file", Some(ToolKind::Read));
        ...
        let session = handle
            .create_session_with_config(
                ...
                Some(ToolServerConfig {
                    tools: vec![renamed, tc("AtelierBuild:grep", Some(ToolKind::Read))],
                    behavior_preset: None,
                }),
                CapabilityMode::All,
                None,
    ...
    /// ABSENT from that (atelier-build) catalog is still advertised. The old
    /// `catalog ∩ session-names` filter silently dropped exactly such tools
    /// (atelier-build renames → 6/11).
    #[tokio::test]
    ...
    async fn create_fingerprint_write_does_not_clobber_concurrent_rebind() {
        ...
        let fp_a = serde_json::to_value(&ToolServerConfig {
            tools: vec![tc("AtelierBuild:list_dir", Some(ToolKind::ListDir))],
            behavior_preset: None,
        })
        .ok();
    ...
    async fn toolset_terminal(
        toolset: &Arc<atelier_tools::registry::types::FinalizedToolset>,
    ) -> Arc<dyn atelier_tools::computer::types::TerminalBackend> {
        let res = toolset.resources.lock().await;
        res.get::<atelier_tools::types::resources::Terminal>()
            .map(|t| t.0.clone())
    ...
    pub(crate) fn background_capable_cfg() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                tc("AtelierBuild:read_file", Some(ToolKind::Read)),
                tc("AtelierBuild:run_terminal_cmd", Some(ToolKind::Execute)),
                tc(
                    "AtelierBuild:get_task_output",
                    Some(ToolKind::BackgroundTaskAction),
                ),
                tc("AtelierBuild:kill_task", Some(ToolKind::KillTaskAction)),
            ],
            behavior_preset: None,
        }
    }
    ...
    ///
    /// [`TerminalRunRequest`]: atelier_tools::computer::types::TerminalRunRequest
    pub(crate) fn terminal_run_request(
        command: &str,
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> atelier_tools::computer::types::TerminalRunRequest {
        atelier_tools::computer::types::TerminalRunRequest {
            ...
            notification_handle: atelier_tools::notification::ToolNotificationHandle::noop(),
            tool_call_id: tool_call_id.to_string(),
            display_command: None,
            auto_background_on_timeout: false,
            foreground_block_budget: None,
            kind: atelier_tools::computer::types::TaskKind::Bash,
    ...
    pub(crate) async fn start_background_sleep(
        ...
        out_dir: &std::path::Path,
        tool_call_id: &str,
    ) -> atelier_tools::computer::types::BackgroundHandle {
        session
            .terminal_backend()
    ...
    impl crate::config::SessionContextFactory for PersistentShellFactory {
        fn build_session_context(
            ...
            backend: Arc<dyn atelier_tools::computer::types::TerminalBackend>,
        ) -> atelier_tools::registry::types::SessionContext {
            self.inner
        ...
        fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
            crate::config::SessionTerminalBackend::local(
                atelier_tools::computer::local::LocalTerminalBackend::with_persistent_shell(),
            )
        }
        fn registry_builder(&self) -> atelier_tools::registry::types::ToolRegistryBuilder {
    ...
    async fn client_ext_sink_receives_emitted_notification() {
        ...
        handle.emit_client_ext("atelier/noop".to_string(), serde_json::json!({}));
        ...
        handle.emit_client_ext(
            "atelier/search/fuzzy/status".to_string(),
            serde_json::json!({ "a" : 1 }),
        );
        let got = captured.lock();
        assert_eq!(got.len(), 1);
        assert_eq!(got[0].0, "atelier/search/fuzzy/status");
    ...
    /// `atelier/search/fuzzy/status` is delivered through the sink with the match.
    #[tokio::test]
    async fn fuzzy_change_streams_status_through_sink() {
        ...
        let sink_captured = captured.clone();
        handle.set_client_ext_sink(Arc::new(move |method, params| {
            if method == "atelier/search/fuzzy/status" {
    ...
    async fn events_jsonl_captures_turn_tool_toggle_and_mcp_variants() {
        ...
        assert_eq!(ts["model_id"], "atelier-4");
    ...
    async fn fork_session_inherits_parent_tool_config_when_none() {
        ...
        let new_parent_baseline = ToolServerConfig {
            tools: vec![tc("AtelierBuild:read_file", Some(ToolKind::Read))],
            behavior_preset: None,
        };
    ...
    async fn fork_session_uses_explicit_tool_config_when_provided() {
        ...
        let custom = ToolServerConfig {
            tools: vec![
                tc("AtelierBuild:read_file", Some(ToolKind::Read)),
                tc("AtelierBuild:list_dir", Some(ToolKind::ListDir)),
            ],
            behavior_preset: None,
        };
        let child = handle
    ...
    async fn fork_session_uses_main_session_when_parent_session_id_is_none() {
        ...
        let baseline_ids: Vec<String> = child
            ...
            .map(|t| t.id.clone())
            .collect();
        assert_eq!(baseline_ids, vec!["AtelierBuild:read_file".to_string()]);
    }
    #[tokio::test]
    ...
    async fn fork_session_cwd_override_used_when_set() {
        let handle = make_handle();
        let alt = std::env::temp_dir().join("atelier-workspace-test-cwd-override");
    ...
    async fn on_mcp_snapshot_changed_emits_per_session_events_and_rebuilds() {
        ...
        handle
            ...
            .expect("subB ok");
        let mut rx = handle.shared.events.subscribe();
        let mcp_tool = tc("AtelierBuild:read_file", Some(ToolKind::Read));
    ...
    async fn hook_registry_snapshot_is_disconnected() {
        ...
        {
            let spec = atelier_hooks::config::HookSpec {
                name: "injected".into(),
                event: atelier_hooks::event::HookEventName::SessionStart,
    ...
    async fn strict_bind_without_explicit_toolset_fails_closed_end_to_end() {
        ...
        assert!(
            reason.contains(atelier_version::VERSION),
    ...
    fn owner_full_bind_metadata() -> serde_json::Value {
        serde_json::json!(
            ...
            "AtelierBuild:read_file" }, { "id" : "AtelierBuild:search_replace" }, { "id" :
            "AtelierBuild:grep" }, { "id" : "AtelierBuild:list_dir" },], }, }
        )
    }
    ...
    /// Unique skill names: discovery also reads the dev machine's `~/.atelier`.
    #[tokio::test]
    ...
    #[test]
    fn missing_workspace_worker_binary_is_rejected() {
        let temp = tempfile::tempdir().unwrap();
        ...
        let error = require_workspace_worker_binary(&missing).unwrap_err();

        assert!(error.to_string().contains("workspace worker binary"));
        assert!(error.to_string().contains(&missing.display().to_string()));
    }

    #[test]
    fn session_filesystem_is_injected_before_session_creation() {
        let handle = WorkspaceHandle::for_test();
        let root = handle.root_cwd().unwrap();
        let filesystem = Arc::new(crate::file_system::MockFs::new(root.clone()));
        handle
            .set_session_filesystem(filesystem)
            .expect("filesystem must be configurable before the first session");

        let session = handle
            .create_session("worker-backed")
            .expect("session creation");
        assert_eq!(session.async_fs().root(), root.as_path());
    }

    #[test]
    fn session_filesystem_cannot_change_after_session_creation() {
        let handle = WorkspaceHandle::for_test();
        handle.create_session("already-created").expect("session");
        let root = handle.root_cwd().unwrap();
        let filesystem = Arc::new(crate::file_system::MockFs::new(root));
        let error = handle
            .set_session_filesystem(filesystem)
