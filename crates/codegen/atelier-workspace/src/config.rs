...
use atelier_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: atelier_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn atelier_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub tools: Option<Vec<atelier_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(atelier_tools::registry::proto_convert::ToolConfigEntryError),
}
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = atelier_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match atelier_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    .manifest_version, running_version = atelier_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "atelier-computer", "bogus"] {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        let v = serde_json::json!(
            { "preset" : "explore", "tools" : [{ "id" : "AtelierBuild:grep", "params_json" :
        ...
        assert_eq!(grep.id, "AtelierBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "AtelierBuild:read_file");
    }
    #[test]
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.atelier/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.atelier/hooks/`).
    ...
    /// `<cwd>/.atelier/lsp.json`: `false` drops them at load, `true` keeps them. The
    ...
    /// Confine `atelier/fs/*` / `workspace.fs_*` resolution to the workspace root
...
pub struct WorkspaceServerMetadata {
    ...
    /// Logical sandbox-service session UUID, from the `ATELIER_SESSION_ID` env
...
impl WorkspaceServerMetadata {
    ...
    ///
    /// `env_session_id` is the raw `ATELIER_SESSION_ID`; empty is normalized to
...
/// `*.json` hook files. Maps 1:1 to [`atelier_hooks::discovery::HookSource`]
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.atelier/hooks/`).
    Directory(PathBuf),
}
