...
//!
//! Moved from `atelier-shell/src/session/worktree.rs` into the workspace
...
// Canonical in atelier-workspace-types; re-exported for existing paths.
...
/// Returns `~/.atelier/worktrees/<repo_slug>` for the given git root.
///
...
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    ...
    atelier_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.atelier/worktrees/<repo_name>`)
/// for a given source path, correctly handling atelier-managed worktrees.
///
/// When `source_path` is already under `~/.atelier/worktrees/<repo>/...`, the
...
/// original repo).
///
/// For paths outside the atelier worktree directory, falls back to
...
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = atelier_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.atelier/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the atelier-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.atelier/worktrees/`.
...
///
/// When the source path is already inside `~/.atelier/worktrees/<repo>/`, the
...
///
/// ACP equivalent of `atelier -w -r <session_id>` (optionally with `--ref`).
...
/// Response from `atelier/git/worktree/resume_session`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Response from `atelier/session/rehydrate`.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Scan `~/.atelier/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/atelier/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/atelier/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/atelier/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/atelier/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `ATELIER_HOME` at an isolated tempdir (`resolve_atelier_home` re-reads
    ...
    /// and restores `ATELIER_HOME` on drop (before releasing the lock), so the
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // can't clobber this row (AtelierHomeFixture is not visible across crates).
