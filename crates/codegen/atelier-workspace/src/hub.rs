...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(atelier_version::VERSION)
            ...
            match transport {
                ...
                None => {
                    tracing::warn!(
                        ...
                        "ATELIER_HITL_PERMISSION_LIVE set but no hub ToolServer; rejecting guarded tool"
                    );
                    return terminal_only(Err(ToolError::new(
                        ToolErrorKind::PermissionDenied,
...
mod tests {
    ...
    use atelier_tools::types::tool_metadata::ToolMetadata as XaiToolMetadata;
    #[derive(Debug)]
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("AtelierBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    fn bg_started_notif(task_id: &str) -> ToolNotification {
        use atelier_tools::notification::types::{BashExecutionBackgrounded, BashNotificationBase};
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use atelier_tools::computer::types::{TaskKind, TaskSnapshot};
