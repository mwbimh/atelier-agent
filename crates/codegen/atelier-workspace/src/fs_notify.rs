...
//! FsNotify adapter functions bridging [`atelier_fsnotify`] events to
...
//! sent to the [`IndexManagerHandle`](atelier_codebase_graph::IndexManagerHandle).
...
/// Convert [`atelier_fsnotify::FsEventKind`] to the wire-type
/// [`atelier_workspace_types::FsEventKind`].
///
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> atelier_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => atelier_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => atelier_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => atelier_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => atelier_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => atelier_workspace_types::FsEventKind::Modified,
    }
}
...
/// [`WorkspaceEvent::FsChanged`](atelier_workspace_types::WorkspaceEvent::FsChanged)
...
pub(crate) fn spawn_fs_event_forwarder(
    ...
    events_tx: tokio::sync::broadcast::Sender<atelier_workspace_types::WorkspaceEvent>,
    cwd: PathBuf,
    cancel: tokio_util::sync::CancellationToken,
    codebase_index: Option<std::sync::Arc<atelier_codebase_graph::IndexManagerHandle>>,
) {
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    atelier_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    ...
    atelier_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(atelier_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            atelier_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: atelier_workspace_types::FsEventKind,
) -> atelier_codebase_graph::FileEvent {
    use atelier_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        atelier_workspace_types::FsEventKind::Created => FileEventKind::Created,
        atelier_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        atelier_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        atelier_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.atelier/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use atelier_workspace_types::FsEventKind as WsKind;
        assert_eq!(
