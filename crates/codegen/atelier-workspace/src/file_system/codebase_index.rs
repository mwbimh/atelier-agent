...
use atelier_codebase_graph::{IndexManager, IndexManagerConfig, IndexManagerHandle};

use atelier_tools::util::atelier_home::atelier_home;

/// Get the cache path for a cwd's index.
///
/// Cache is stored in: `~/.atelier/indexes/{url_encoded_cwd}/goto_index.bin`
...
mod tests {
    ...
    // which maps to `atelier/code/status` reporting `reason: notStarted`.
    ...
    /// In `atelier/code/status`, `resolve_index_handle()` calls
