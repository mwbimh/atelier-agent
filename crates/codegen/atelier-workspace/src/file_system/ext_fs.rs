...
//! for the shell's `atelier/fs/*` ACP extension methods.
//!
//! These mirror the pure functions that previously lived only in the
//! shell (`atelier-shell/src/session/file_system.rs`) so that, in proxy
//! mode, a `atelier/fs/*` request executes on the *remote* workspace server
...
use atelier_workspace_types::rpc::fs::FsReadEncoding;
