...
//! `atelier-agent` and `atelier-tools` rather than duplicating them.
...
//!
//! Re-exports [`AgentsMdTracker`] from `atelier-tools` for external
...
// workspace crate without a direct atelier-tools dependency.
pub use atelier_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use atelier_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use atelier_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use atelier_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`atelier_agent::prompt::skills::list_skills`] with
...
pub async fn discover_skills(root_cwd: &Path, config: &SkillsConfig) -> Vec<Value> {
    ...
    let skills = atelier_agent::prompt::skills::list_skills(
        Some(&cwd_str),
        config,
...
pub async fn discover_agents_md(root_cwd: &Path) -> Vec<Value> {
    let cwd_str = root_cwd.to_string_lossy();
    let files = atelier_agent::prompt::agents_md::read_agents_config_with_paths(
        &cwd_str,
...
///
/// Delegates to [`atelier_agent::plugins::discover_plugins`] with
...
pub fn discover_plugins(
    ...
    let discovered = atelier_agent::plugins::discover_plugins(
        Some(root_cwd),
        config,
        trust_store,
        project_trusted,
...
/// Load the project config from `<root_cwd>/.atelier/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".atelier").join("config.toml");
    match atelier_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.atelier/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".atelier").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".atelier").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes atelier-build's AgentConfigFile and the remote
        ...
        // break discovery.
        let src = atelier_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: atelier_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.atelier`, so fixtures use test-unique names.
    #[tokio::test]
    async fn discover_agents_md_strips_rules_frontmatter() {
        let tmp = tempfile::tempdir().unwrap();
        let rules_dir = tmp.path().join(".atelier").join("rules");
    ...
    // from `~/.atelier/plugins/`, so tests check for specific plugins.
    ...
    // developer machine with Atelier installed it may return non-Null even
