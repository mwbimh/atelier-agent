//! Local workspace environment capture — `workspace_environment.json`.
//!
...
impl WorkspaceIdentity {
    ...
    /// `AtelierAuth.team_id`, and for a Team principal `principal_id` *is* the
    /// team id.
...
/// [`AuthIdentity`](atelier_tool_hub_sdk::AuthIdentity). The two types carry the
...
impl From<atelier_tool_hub_sdk::AuthIdentity> for WorkspaceIdentity {
    fn from(id: atelier_tool_hub_sdk::AuthIdentity) -> Self {
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `atelier-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$ATELIER_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("ATELIER_SANDBOX_PROFILE").ok(),
            atelier_sandbox::is_inside_bwrap(),
    ...
    /// Serialize to pretty JSON bytes for local persistence.
...
/// userinfo, so the raw URL must never be persisted.
