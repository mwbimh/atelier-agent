...
//! [`crate::trust::TrustStore`] (`~/.atelier/trusted_folders.toml`). The
//! consume/gating half (the `DECISIONS` cache, `resolve_and_record`,
//! `project_scope_allowed`, the loader filters) lives in `atelier-shell`.
//!
...
//! provisional and re-checked rather than cached — is a `atelier-shell`
...
pub fn decide_inputs_with_interactive(
    ...
    DecideInputs {
        ...
        // can't persist (Case 2: cwd IS $HOME, incl. the default `~/.atelier`).
...
/// this binary — true on a local/dev build (no `ATELIER_VERSION` release stamp).
///
/// THE single security short-circuit: every explicit trust auto-grant site calls
/// this (greppable via `folder_trust_inert`). When true a self-built atelier never
...
/// (`ATELIER_VERSION` unset at compile time) — i.e. a local/dev build.
///
/// Kept local (not in `atelier-version`) on purpose: adding a symbol to that
...
fn is_local_build() -> bool {
    // Runtime escape hatch: a pinned ATELIER_TEST_VERSION simulates a release build,
    // so tests/CI (which run unstamped, i.e. local-looking) can exercise the gate.
    if std::env::var(atelier_version::TEST_VERSION_ENV).is_ok() {
        return false;
    }
    option_env!("ATELIER_VERSION").is_none()
}

/// Resolve whether the folder-trust gate is enabled.
///
/// On a local/dev build (no `ATELIER_VERSION` release stamp) the feature is OFF
/// regardless of env/config/remote — a self-built atelier auto-trusts (never
...
/// env `ATELIER_FOLDER_TRUST` > `[folder_trust] enabled` (user) > managed >
...
fn feature_enabled_for_build(remote: Option<&LocalRuntimeSettings>, is_local_build: bool) -> bool {
    ...
    let user = atelier_config::load_from_disk().ok();
    let managed = atelier_config::load_managed_config().ok();
    BoolFlag::env("ATELIER_FOLDER_TRUST")
...
/// same `~/.atelier/trusted_folders.toml`. Best-effort; a write failure is logged,
/// not fatal.
...
fn collect_repo_config_kinds(cwd: &Path, first_only: bool) -> Vec<&'static str> {
    ...
    let chain = atelier_agent::repo::RepoDirChain::resolve(cwd);
    ...
    // Project `.atelier/config.toml` declaring repo-controlled code-exec: a
    ...
    for path in crate::project_config::find_project_configs_in(&chain.dirs) {
        let Ok(root) = atelier_config::load_config_file(&path) else {
            continue;
        };
    ...
    // Project `.atelier/lsp.json`.
    if cwd.join(".atelier").join("lsp.json").is_file() {
        hit!("lsp");
    }
    ...
    // must be gated — else a hooks-only clone (e.g. `.atelier/hooks/evil.json`) would
    ...
    if hook_root.join(".atelier").join("hooks").is_dir()
    ...
    // that must be gated — else a plugin clone (e.g. `.atelier/plugins/evil/`, even
    // one in a subdir launched via `cd sub && atelier`) would resolve trusted and
...
pub fn prompt_for_trust(key: &Path) -> bool {
    ...
    let _ = writeln!(
        err,
        "This folder contains repo-local config (.mcp.json / .atelier/lsp.json / hooks) \
...
mod tests {
    ...
    #[test]
    fn repo_configs_present_detects_atelier_config_mcp_servers() {
        let tmp = repo_tmp();
    ...
    #[test]
    fn repo_configs_present_detects_atelier_lsp_json() {
        let tmp = repo_tmp();
        let atelier = tmp.path().join(".atelier");
        std::fs::create_dir_all(&atelier).unwrap();
        std::fs::write(atelier.join("lsp.json"), "{}").unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins() {
        ...
        let tmp = repo_tmp();
        std::fs::create_dir_all(tmp.path().join(".atelier").join("plugins").join("x")).unwrap();
        assert!(repo_configs_present(tmp.path()));
    }
    ...
    fn repo_configs_present_detects_project_plugins_in_subdir() {
        ...
        std::fs::create_dir_all(subdir.join(".atelier").join("plugins").join("evil")).unwrap();
        assert!(repo_configs_present(&subdir));
    }
    ...
    fn repo_configs_present_false_for_empty_mcp_servers_table() {
        ...
        let atelier = tmp.path().join(".atelier");
        std::fs::create_dir_all(&atelier).unwrap();
        std::fs::write(atelier.join("config.toml"), "[mcp_servers]\n").unwrap();
        assert!(!repo_configs_present(tmp.path()));
    }

    #[test]
    fn repo_configs_present_detects_atelier_config_plugins_paths() {
    ...
    // ATELIER_HOME-isolation idiom mirrored from this crate's `permission::claude_compat`
    // tests (the workspace crate has no `serial_test`/`atelier-test-support`
    ...
    fn simulate_release_build() -> EnvVarGuard {
        EnvVarGuard::set(atelier_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"))
    }
    ...
    fn release_build_keeps_gate_when_enabled() {
        ...
        // config nor an ambient env flag can override it: empty ATELIER_HOME (no
        // config.toml/managed_config.toml) + ATELIER_FOLDER_TRUST unset. nextest's
        // process-per-test makes atelier_home()'s OnceLock pick up the temp dir.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvVarGuard::unset("ATELIER_FOLDER_TRUST");
    ...
    fn local_build_ignores_explicit_env_optin() {
        ...
        // ATELIER_FOLDER_TRUST=1 does NOT enable the feature (so a self-built atelier
        // never prompts). ATELIER_HOME isolated so on-disk config can't influence it.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvVarGuard::set("ATELIER_FOLDER_TRUST", Path::new("1"));
    ...
    fn release_build_defaults_on() {
        ...
        // the feature ON. Empty ATELIER_HOME (no config.toml/managed config) +
        // ATELIER_FOLDER_TRUST unset so only the default applies.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _flag = EnvVarGuard::unset("ATELIER_FOLDER_TRUST");
    ...
    fn is_local_build_honors_test_version_override() {
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        // A pinned ATELIER_TEST_VERSION simulates a release build => not a local build.
        {
            let _sim = EnvVarGuard::set(atelier_version::TEST_VERSION_ENV, Path::new("0.0.0-sim"));
            assert!(!is_local_build());
        }
        // With it unset, an unstamped build (no ATELIER_VERSION) is a local build.
        // Guard to the unstamped case so a release-stamped test binary (CI release)
        // doesn't spuriously fail this arm.
        let _unset = EnvVarGuard::unset(atelier_version::TEST_VERSION_ENV);
        if option_env!("ATELIER_VERSION").is_none() {
            assert!(is_local_build());
        }
    }
    ...
    fn store_io_is_noop_on_local_build() {
        ...
        // (mirrors `is_local_build_honors_test_version_override`); ATELIER_HOME-isolated
        // and ENV_LOCK-serialized so toggling ATELIER_TEST_VERSION is race-safe.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _unset = EnvVarGuard::unset(atelier_version::TEST_VERSION_ENV);
    ...
    fn revoke_folder_trust_store_persists_untrust_for_trusted_folder() {
        ...
        // ATELIER_HOME-isolated so the seed/deny hit a temp store, not the real file.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _sim = simulate_release_build();
        let tmp = repo_tmp();
        let key = workspace_key(tmp.path());
    ...
    fn revoke_folder_trust_store_writes_no_deny_for_never_trusted_folder() {
        ...
        // ATELIER_HOME-isolated so the grant writes to a temp store.
        let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let home = tempfile::tempdir().unwrap();
        let _env = EnvVarGuard::set("ATELIER_HOME", home.path());
        let _sim = simulate_release_build();
