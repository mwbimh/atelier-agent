...
//!
//! Reads OIDC credentials from `~/.atelier/auth.json`, connects to a
//! server, exposes workspace tools, and refreshes tokens
//! automatically.
use atelier_workspace::config::WorkspaceServerMetadata;
use atelier_workspace::daemonize;
use atelier_workspace::diag_server;
use atelier_workspace::preview_supervisor::{self, PreviewArgs, PreviewVisibility};
use clap::Parser;
...
const SERVICE_NAME: &str = "atelier_workspace";
...
/// The old hub-connected workspace server is not part of the private build.
/// Keep `--capabilities` as a local probe, but never let normal startup parse
/// credentials or open a remote WebSocket.
fn legacy_remote_workspace_server_enabled() -> bool {
    false
}

fn apply_startup_sandbox(
    mut sandbox: atelier_sandbox::SandboxManager,
    cwd: &std::path::Path,
) -> anyhow::Result<atelier_sandbox::SandboxManager> {
    sandbox
        .apply(cwd)
        .map_err(|error| anyhow::anyhow!("sandbox could not be applied: {error}"))?;
    if sandbox.status() == atelier_sandbox::SandboxStatus::Unavailable
        && !sandbox.backend().is_unsafe()
    {
        return Err(anyhow::anyhow!(
            "sandbox could not be applied: native enforcement is unavailable"
        ));
    }
    Ok(sandbox)
}
...
struct Args {
    ...
    #[arg(long)]
    capabilities: bool,
    /// Legacy hub URL. Normal startup is disabled in the private build; this
    /// option remains only so old launchers receive a structured refusal.
    #[arg(long)]
    ...
    /// Confine `atelier/fs/*` resolution to the workspace root (reject `..`,
    ...
    /// Override with `ATELIER_WORKSPACE_CONFINE_FS_TO_ROOT=false` (e.g. local dev).
    #[arg(
        long,
        env = "ATELIER_WORKSPACE_CONFINE_FS_TO_ROOT",
...
/// `/usr/local/bin/atelier-preview-proxy` child (see `cli.rs` for the proxy's
...
fn main() -> anyhow::Result<()> {
    ...
    if !legacy_remote_workspace_server_enabled() {
        return Err(anyhow::anyhow!(
            "legacy remote workspace server is disabled in the private Atelier build"
        ));
    }
...
async fn run(args: Args, cwd: PathBuf) -> anyhow::Result<()> {
    ...
    let hub_url = args
        .hub_url
        .ok_or_else(|| anyhow::anyhow!("--hub-url is required by the disabled legacy path"))?;
    let url = Url::parse(&hub_url).map_err(|e| anyhow::anyhow!("invalid --hub-url: {e}"))?;
    {
        use atelier_sandbox::{ProfileName, SandboxManager};
        let profile = match std::env::var("ATELIER_SANDBOX_PROFILE").ok() {
            Some(val) => {
                let parsed = val
                    .parse::<ProfileName>()
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(
                        value = % val,
                        "Unrecognized ATELIER_SANDBOX_PROFILE, defaulting to workspace"
                    );
            ...
            None if atelier_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let backend = atelier_sandbox::parse_backend(
            std::env::var("ATELIER_SANDBOX_BACKEND").ok().as_deref(),
        )
        .map_err(|error| anyhow::anyhow!("invalid ATELIER_SANDBOX_BACKEND: {error}"))?;
        let profile_name = profile.to_string();
        if profile == ProfileName::Off && !backend.is_unsafe() {
            return Err(anyhow::anyhow!(
                "sandbox profile 'off' is not allowed with the native backend; set ATELIER_SANDBOX_BACKEND=unsafe to explicitly run without sandboxing"
            ));
        }
        let sandbox = apply_startup_sandbox(
            SandboxManager::new_with_backend(profile, &cwd, backend),
            &cwd,
        )?;
        ...
        tracing::info!(
            ...
            active = atelier_sandbox::is_active(),
            restrict_network = atelier_sandbox::should_restrict_child_network(),
            "Workspace server sandbox configured"
        );
    }
    let auth_provider = atelier_workspace::hub_auth::provider(&url, args.auth_config.as_deref())?;
    tracing::info!(hub_url = % url, cwd = % cwd.display(), "Starting workspace server");
    let cwd_display = cwd.display().to_string();
    let session_id = std::env::var("ATELIER_SESSION_ID").ok();
    ...
    atelier_workspace::init_metrics();
    let ws_handle = atelier_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
        ...
        status_config,
        // Atelier has no remote artifact/session telemetry.
        false,
        project_lsp_trusted,
    ...
    tracing::info!("remote trace, log, and metric export disabled");
    tracing::info!(
    ...
    let grace_budget = atelier_workspace::handle::termination_grace_from_env();
    ws_handle
        .two_phase_drain(
            grace_budget,
            atelier_workspace::handle::DrainReason::Sigterm,
        )
        .await;
...
mod tests {
    ...
    #[test]
    fn explicit_unsafe_startup_sandbox_is_allowed_and_visible() {
        let sandbox = atelier_sandbox::SandboxManager::new_with_backend(
            atelier_sandbox::ProfileName::Workspace,
            std::path::Path::new("."),
            atelier_sandbox::SandboxBackendKind::Unsafe,
        );
        let sandbox = apply_startup_sandbox(sandbox, std::path::Path::new("."))
            .expect("explicit unsafe backend is an intentional escape hatch");
        assert_eq!(sandbox.status(), atelier_sandbox::SandboxStatus::Unsafe);
    }

    #[cfg(target_os = "windows")]
    #[test]
    fn native_startup_sandbox_fails_closed_when_backend_is_unavailable() {
        let sandbox = atelier_sandbox::SandboxManager::new(
            atelier_sandbox::ProfileName::Workspace,
            std::path::Path::new("."),
        );
        match atelier_sandbox::native_sandbox_availability() {
            ...
            atelier_sandbox::NativeSandboxAvailability::Available => {
                let sandbox = apply_startup_sandbox(sandbox, std::path::Path::new("."))
                    .expect("Windows preview helper is available");
                assert_eq!(sandbox.status(), atelier_sandbox::SandboxStatus::Preview);
            }
        }
    }
    ...
    #[test]
    fn legacy_remote_workspace_server_is_disabled() {
        assert!(!legacy_remote_workspace_server_enabled());
    }
    #[test]
    ...
    fn preview_flags_parse_and_lower_to_supervisor_config() {
        let args = Args::try_parse_from([
            ...
            "https://atelier.invalid/preview-auth",
        ...
        assert_eq!(
            ...
            Some("https://atelier.invalid/preview-auth")
        );
