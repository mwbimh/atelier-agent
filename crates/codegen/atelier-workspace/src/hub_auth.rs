//! Hub [`AuthProvider`] from `~/.atelier/auth.json` for the standalone
...
//!
//! The in-leader `atelier workspace` exposure does NOT use this path — it sources
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let atelier = atelier_config::user_atelier_home()
        .ok_or_else(|| anyhow::anyhow!("no user atelier home (set $ATELIER_HOME or $HOME)"))?;
    Ok(atelier.join("auth.json"))
}
...
fn read_auth_entry(path: &Path) -> anyhow::Result<(String, AuthEntry)> {
    if !path.exists() {
        anyhow::bail!(
            "No auth credentials found at {}. Run `atelier login` first.",
            path.display()
        );
    }
    ...
    entries
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "no OIDC auth entry found in {}. Run `atelier login` first.",
                path.display()
            )
        })
}
...
/// the default credential path (`~/.atelier/auth.json`).
pub fn provider(
    hub_url: &Url,
