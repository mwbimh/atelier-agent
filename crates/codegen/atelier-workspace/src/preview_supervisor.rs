...
//! spawns the unchanged `/usr/local/bin/atelier-preview-proxy` binary as a
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/atelier-preview-proxy";
...
/// `atelier_workspace_preview_proxy_restart_total{reason}` — (re)start events the
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "atelier_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    ...
    /// `atelier-preview-proxy/src/cli.rs`). Absent options and a false
...
/// Build the unspawned proxy command. Secrets (`ATELIER_SERVER_KEY` /
/// `ATELIER_SESSION_ID`) reach the proxy by env inheritance — never argv.
...
/// `atelier-preview-proxy`'s `/__control/activity` route).
...
/// the default in `atelier-preview-proxy/src/cli.rs`).
...
mod tests {
    ...
    fn sample_cfg() -> PreviewArgs {
        PreviewArgs {
            ...
            auth_redirect: Some("https://atelier.invalid/preview-auth".to_owned()),
            allow_public: true,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match atelier-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "ATELIER_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
