...
/// environment (`ATELIER_HOME`, `HOME`, …). nextest isolates each test in its own
...
/// can clobber `ATELIER_HOME` mid-test. A single shared lock (used by every
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "atelier_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "atelier_workspace_permission_timeout_total")
        );
    }
}
