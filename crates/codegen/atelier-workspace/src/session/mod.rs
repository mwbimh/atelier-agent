...
impl WorkspaceSession {
    pub(crate) fn new(
        ...
        filesystem: Option<Arc<dyn AsyncFileSystem>>,
    ) -> Self {
        ...
        let async_fs =
            AsyncFsWrapper::new(filesystem.unwrap_or_else(|| Arc::new(LocalFs::new(cwd.clone()))));
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<atelier_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<atelier_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<atelier_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<atelier_hooks::error::HookError>,
    ...
    pub(crate) activity_notify_handle:
        arc_swap::ArcSwap<Option<atelier_tools::notification::types::ToolNotificationHandle>>,
    /// Sink for workspace-originated ext-notifications to the client (e.g.
    /// `atelier/search/fuzzy/status`). Mode-agnostic: the shell wires it to the
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn atelier_tools::implementations::lsp::LspBackend>>,
    ...
    /// enabled (`ATELIER_WORKSPACE_TOOL_DEFS_ENABLED=true`).
    ...
    /// Filesystem used by workspace-session bookkeeping and extension RPCs.
...
impl WorkspaceShared {
    ...
    pub(crate) fn set_session_filesystem(
        &self,
        filesystem: Arc<dyn AsyncFileSystem>,
    ) -> crate::error::WorkspaceResult<()> {
        if !self.sessions.read().is_empty() {
            return Err(crate::error::WorkspaceError::HubError(
                "session filesystem must be configured before creating a workspace session".into(),
            ));
        }
        *self.session_filesystem.write() = Some(filesystem);
        Ok(())
    }
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<atelier_workspace_types::WorkspaceEvent> {
