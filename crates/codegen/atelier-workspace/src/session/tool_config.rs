...
use atelier_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            atelier_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// enabled (`ATELIER_WORKSPACE_TOOL_STATE_ENABLED=true`; any other value keeps
/// legacy behavior).
pub fn tool_state_enabled() -> bool {
    std::env::var("ATELIER_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
/// `ATELIER_WORKSPACE_TOOL_STATE_ENABLED`. Aliased to the crate-wide
...
pub struct WorkspaceSessionContextFactory {
    ...
    /// the full worker boundary opt in before constructing `WorkspaceHandle`.
    worker_fs: Option<Arc<dyn atelier_tools::computer::types::AsyncFileSystem>>,
    /// Resolved `$ATELIER_WORKSPACE_HOME` when tool-state persistence is enabled;
...
impl WorkspaceSessionContextFactory {
    ...
    /// Route built-in file tools through a process-isolated workspace worker.
    ///
    /// The worker client is created by an async caller before the synchronous
    /// `SessionContextFactory` is installed in `WorkspaceConfig`.
    pub fn with_worker_client(
        mut self,
        root: PathBuf,
        client: crate::worker::WorkspaceWorkerClient,
    ) -> Self {
        self.worker_fs = Some(Arc::new(crate::worker::WorkspaceWorkerFs::new(
            root, client,
        )));
        self
    }
    /// Enable session-keyed tool-state persistence rooted at `home`
    /// (`$ATELIER_WORKSPACE_HOME`). Callers should only invoke this when
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = self.worker_fs.clone().unwrap_or_else(|| {
            Arc::new(atelier_tools::computer::local::LocalFs)
                as Arc<dyn atelier_tools::computer::types::AsyncFileSystem>
        });
        let notification_handle = atelier_tools::notification::ToolNotificationHandle::noop();
...
fn build_proxy_headers(base_url: &str) -> indexmap::IndexMap<String, String> {
    ...
    headers.insert(
        ...
        format!("atelier-workspace/{version}"),
    );
    headers.insert("x-atelier-client-version".to_string(), version.to_string());
    if base_url.contains("cli-chat-proxy") || base_url.contains("chat-proxy") {
        headers.insert("X-XAI-Token-Auth".to_string(), "atelier-cli".to_string());
        headers.insert(
...
/// `ATELIER_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config()
-> atelier_tools::implementations::atelier_build::web_fetch::WebFetchConfig {
    ...
    if std::env::var("ATELIER_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("ATELIER_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("ATELIER_WEB_SEARCH_MODEL")
        .unwrap_or_else(|_| "atelier-4.20-multi-agent".to_string())
}
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("AtelierBuild:search_replace", Some(ToolKind::Edit)),
                tc("AtelierBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("AtelierBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("AtelierBuild:search_replace".to_owned(), ToolKind::Edit),
            ("AtelierBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("AtelierBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                test_support::tc("AtelierBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("AtelierBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
        ...
        assert_eq!(
            kind_of("AtelierBuild:read_file"),
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("AtelierBuild:read_file", None),
                test_support::tc("AtelierBuild:grep", None),
                test_support::tc("AtelierBuild:list_dir", None),
                test_support::tc("AtelierBuild:search_replace", None),
                test_support::tc("AtelierBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("AtelierBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"AtelierBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "ATELIER_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use atelier_tools::types::resources::{State, WebCitationCounter};
