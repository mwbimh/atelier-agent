...
use atelier_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
use atelier_tools::computer::types::TaskKind;
use atelier_tools::implementations::atelier_build::scheduler::interval::interval_to_human;
use atelier_tools::implementations::atelier_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use atelier_tools::registry::types::FinalizedToolset;
use atelier_tools::types::resources::Terminal;
use atelier_workspace_types::rpc::workspace::{
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "atelier_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "atelier_workspace_rpc_duration_seconds",
...
) -> Vec<atelier_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use atelier_tools::computer::types::TaskKind;
    use atelier_tools::types::resources::Terminal;
    use atelier_tools::types::tool::ToolKind;
    use atelier_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &atelier_tools::registry::types::FinalizedToolset,
) -> Vec<atelier_workspace_types::rpc::workspace::TodoSummaryWire> {
    use atelier_tools::implementations::atelier_build::todo::{TodoState, TodoStatus};
    use atelier_tools::types::resources::State;
    use atelier_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    pub async fn dispatch(
        ...
        use atelier_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use atelier_workspace_types::rpc::search::FuzzyStatusReq;
        use atelier_workspace_types::rpc::skills::DiscoverPluginsReq;
        use atelier_workspace_types::rpc::workspace::{
        ...
        use atelier_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
    ...
    /// Binary-safe file operations used by the local workspace worker.
    ///
    /// The existing `workspace.fs_*` RPCs intentionally carry text-oriented
    /// payloads for ACP compatibility.  The worker boundary must also support
    /// arbitrary bytes, so these three methods use base64 while reusing the
    /// same root confinement implementation as the regular workspace RPCs.
    pub async fn dispatch_worker_file_method(
        &self,
        method: &str,
        params: Value,
    ) -> WorkspaceResult<Value> {
        ...
        if matches!(method, "atelier.worker.grep" | "atelier.worker.search") {
            let mut request: atelier_tools::computer::types::GrepRequest =
                serde_json::from_value(params).map_err(|error| {
                    WorkspaceError::HubError(format!("invalid worker grep request: {error}"))
                })?;
            let (path, _) = self
                .workspace
                .confine_to_workspace_root(&request.path)
                .await?;
            request.path = path;
            let output =
                atelier_tools::implementations::atelier_build::grep::run_worker_grep(request)
                    .await
                    .map_err(|error| {
                        WorkspaceError::HubError(format!("worker grep failed: {error}"))
                    })?;
            return serde_json::to_value(output)
                .map_err(|error| WorkspaceError::HubError(error.to_string()));
        }

        if method == "atelier.worker.read_dir" {
            let request: atelier_workspace_types::rpc::fs::ClientFsListReq =
                serde_json::from_value(params).map_err(|error| {
                    WorkspaceError::HubError(format!("invalid worker read_dir request: {error}"))
                })?;
            let output = crate::file_system::client_fs::list(&self.workspace, &request).await?;
            return serde_json::to_value(output)
                .map_err(|error| WorkspaceError::HubError(error.to_string()));
        }

        if method == "atelier.worker.stat" {
            let request: atelier_workspace_types::rpc::fs::ClientFsStatReq =
                serde_json::from_value(params).map_err(|error| {
                    WorkspaceError::HubError(format!("invalid worker stat request: {error}"))
                })?;
            let output = crate::file_system::client_fs::stat(&self.workspace, &request).await?;
            return serde_json::to_value(output)
                .map_err(|error| WorkspaceError::HubError(error.to_string()));
        }
        ...
        let path = params
            .get("path")
            .and_then(Value::as_str)
            .ok_or_else(|| WorkspaceError::HubError("worker file method missing path".into()))?;
        let root = self.workspace.root_cwd()?;
        ...
        let (absolute, _) = self.workspace.confine_to_workspace_root(&absolute).await?;
        match method {
            ...
            "atelier.worker.write_file" => {
                let encoded = params
                    ...
                    .and_then(Value::as_str)
                    .ok_or_else(|| {
                        WorkspaceError::HubError("worker write_file missing data_base64".into())
                    })?;
                let data = base64::engine::general_purpose::STANDARD
                    .decode(encoded)
                    .map_err(|e| WorkspaceError::HubError(format!("invalid data_base64: {e}")))?;
                if params
                    .get("create_dirs")
            ...
            _ => Err(WorkspaceError::HubError(format!(
                "unknown worker file method: {method}"
            ))),
        }
    }
}
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<atelier_tool_runtime::SessionContext>();
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use atelier_tool_protocol::turn_hook::{self, TurnHookRequest};
...
mod tests {
    ...
    use atelier_tool_protocol::turn_hook;
    use atelier_tools::implementations::atelier_build::scheduler::types::ScheduledTask;
    /// Helper: consume the first item from a ToolStream.
    async fn next_item(
        stream: &mut ToolStream<TypedToolOutput>,
    ) -> Option<atelier_tool_runtime::ToolStreamItem<TypedToolOutput>> {
    ...
    async fn handle_evict_triggers_two_phase_drain() {
        use atelier_tool_protocol::ToolServerLifecycleStatus;
        let handle = make_handle();
    ...
    async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
        ...
        use atelier_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
        let handle = make_handle();
        ...
        let read_only = ToolServerConfig {
            tools: vec![tc(
                ...
                Some(atelier_tools::types::tool::ToolKind::Read),
            )],
    ...
    async fn handle_call_wraps_in_envelope_with_value() {
        ...
        ctx.extensions
            .insert(atelier_tool_runtime::SessionContext("main".to_owned()));
        ...
        match item {
            atelier_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                let ok_val = typed
                    .value
                    .get("ok")
    ...
    async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
        ...
        let has_bogus_series = prometheus::gather()
            .iter()
            .filter(|mf| mf.name() == "atelier_workspace_rpc_requests_total")
    ...
    async fn handle_hook_cancel_marks_call_completed() {
        use atelier_tool_protocol::ToolCallId;
        let handle = make_handle();
        let handler = WorkspaceRpcHandler::new(handle.clone());
        let tracker = handle.activity_tracker();
