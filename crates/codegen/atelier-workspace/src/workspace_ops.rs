...
use atelier_tool_hub_sdk::ToolHarness;
use atelier_tools::types::output::ToolRunResult;
use atelier_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use atelier_workspace_types::rpc::WorkspaceRpc;
pub use atelier_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use atelier_workspace_types::rpc::code_nav::{
...
pub use atelier_workspace_types::rpc::fs::{
...
pub use atelier_workspace_types::rpc::git::{
...
pub use atelier_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use atelier_workspace_types::rpc::hunks::{
...
pub use atelier_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use atelier_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use atelier_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use atelier_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use atelier_workspace_types::rpc::worktree::{
...
fn hunk_line_info_to_wire(info: &atelier_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: atelier_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use atelier_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &atelier_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: atelier_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use atelier_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: atelier_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(
    entry: atelier_hunk_tracker::FileContentEntry,
...
fn session_stats_to_wire(stats: &atelier_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: atelier_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: atelier_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> atelier_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => atelier_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => atelier_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<atelier_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = atelier_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                atelier_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &atelier_hooks::discovery::HookRegistry,
...
) -> WorkspaceResult<atelier_hooks::discovery::HookRegistry> {
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<atelier_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<atelier_codebase_graph::QueryResult, atelier_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<atelier_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   session's [`FinalizedToolset`](atelier_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    /// Configure the local workspace's session bookkeeping to use the same
    /// filesystem boundary as the agent's built-in file tools. This must be
    /// called before `bind_local_session` creates the first workspace session.
    pub fn set_local_session_filesystem(
        &self,
        filesystem: std::sync::Arc<dyn crate::file_system::AsyncFileSystem>,
    ) -> WorkspaceResult<()> {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: atelier_hunk_tracker::HunkTrackerHandle,
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<atelier_hooks::discovery::HookRegistry> {
    ...
    ///   [`FinalizedToolset`](atelier_tools::registry::types::FinalizedToolset)
    ...
    pub async fn call_tool(
        ...
        match self {
            ...
            Self::Proxy { client } => {
                ...
                let tool_id = atelier_tool_protocol::ToolId::new(name).map_err(|e| {
                    atelier_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                ...
                ctx.call_id = atelier_tool_protocol::ToolCallId::new(call_id.to_owned())
...
mod tests {
    ...
    /// request types are defined in `atelier-workspace-types` (and re-exported
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset =
            std::sync::Arc::new(atelier_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    fn hunk_to_wire_serializes_identically() {
        use atelier_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        ...
        use atelier_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use atelier_hunk_tracker::SessionSummary;
        use atelier_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
        use std::sync::Arc;
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = atelier_hooks::config::HookSpec {
            ...
            event: atelier_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.atelier/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
        };
        let mut registry = atelier_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use atelier_hooks::event::HookEventName as E;
    ...
    /// can't depend on `atelier-hooks`, and `hook_registry_to_wire` only couples
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        use atelier_hooks::config::HookSpec;
