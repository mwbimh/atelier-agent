...
pub struct PermissionEvent {
    ...
    /// sandbox_override_session, persisted_grant, session_grant, static_allowlist, safe_command,
...
pub enum ClientType {
    ...
    #[default]
    #[serde(rename = "generic", alias = "atelier-shell", alias = "atelier_shell")]
    Generic,
    /// Atelier TUI client - show fancy options with interactive bash term selection
    #[serde(rename = "atelier-tui", alias = "atelier_tui")]
    AtelierTUI,
    /// Atelier Web client - identified by clientIdentifier "atelier-web"
    #[serde(rename = "atelier_web")]
    AtelierWeb,
    /// Named client (`"nebula"`) — uses the generic permission UI
    #[serde(rename = "nebula")]
    Nebula,
    /// IDE extension client (VS Code and similar) - identified by clientIdentifier "atelier-code-extension"
    #[serde(rename = "extension")]
    Extension,
    /// Atelier Pager client - TUI-like terminal pager with interactive permission UI.
    /// Treated identically to AtelierTUI for permission options (gets bash highlights +
    /// interactive selection). Reports as "pager" for telemetry attribution.
    ///
    /// Accepts both the hyphenated `"atelier"` (what the pager actually
    /// sends over the wire, matching `PAGER_CLIENT_TYPE`) and the underscored
    /// `"atelier_pager"` form for symmetry with the rest of this enum.
    #[serde(rename = "atelier", alias = "atelier_pager")]
    AtelierPager,
    /// Atelier Desktop (Electron) client - identified by clientIdentifier "atelier-desktop".
    /// Uses TUI-style bash permission options (primary command extraction + prefix matching)
    /// but without interactive `<`/`>` word selection.
    #[serde(rename = "atelier_desktop")]
    Desktop,
}
impl ClientType {
    /// Product token for the `User-Agent` header (e.g. `atelier`).
    pub fn user_agent_label(&self) -> &'static str {
        match self {
            Self::Generic => "atelier-shell",
            Self::AtelierTUI => "atelier-tui",
            Self::AtelierWeb => "atelier-web",
            Self::Nebula => "nebula",
            Self::Extension => "atelier-code-extension",
            Self::AtelierPager => "atelier",
            Self::Desktop => "atelier-desktop",
        }
    }
    /// Resolve from ACP `clientIdentifier` string (e.g. `"atelier-web"`, `"atelier-desktop"`).
    pub fn from_client_identifier(id: Option<&str>) -> Self {
        match id {
            Some("atelier-web") => Self::AtelierWeb,
            Some("nebula") => Self::Nebula,
            Some("atelier-code-extension") => Self::Extension,
            Some("atelier-desktop") => Self::Desktop,
            Some("atelier") => Self::AtelierPager,
            _ => Self::Generic,
        }
    }
    ...
    pub fn feedback_label(&self) -> &'static str {
        match self {
            Self::AtelierTUI | Self::AtelierPager => "tui",
            Self::AtelierWeb => "web",
            Self::Nebula => "nebula",
...
impl From<&atelier_tools::types::ToolInput> for AccessKind {
    fn from(input: &atelier_tools::types::ToolInput) -> Self {
...
pub enum RequirementSource {
    Unknown,
    /// User-writable `~/.atelier/requirements.toml` — untrusted for keeping a
...
mod tests {
    ...
    fn hashline_edit_maps_to_edit_access() {
        use atelier_tools::implementations::atelier_build_hashline::edit::types::HashlineEditInput;
        use atelier_tools::types::ToolInput;
    ...
    fn bash_maps_to_bash_access() {
        use atelier_tools::implementations::atelier_build::bash::BashToolInput;
        use atelier_tools::types::ToolInput;
    ...
    fn use_tool_maps_to_mcp_tool_access() {
        use atelier_tools::implementations::use_tool::UseToolInput;
        use atelier_tools::types::ToolInput;
    ...
    fn monitor_maps_to_bash_access() {
        use atelier_tools::implementations::atelier_build::monitor::types::MonitorInput;
        use atelier_tools::types::ToolInput;
    ...
    fn web_fetch_maps_to_web_fetch_access() {
        use atelier_tools::implementations::atelier_build::web_fetch::WebFetchInput;
        use atelier_tools::types::ToolInput;
    ...
    fn web_search_maps_to_web_search_access() {
        use atelier_tools::implementations::atelier_build::web_search::WebSearchInput;
        use atelier_tools::types::ToolInput;
    ...
    #[test]
    fn client_type_deserializes_atelier_shell_as_generic() {
        assert_eq!(
            serde_json::from_value::<ClientType>("atelier-shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
            serde_json::from_value::<ClientType>("atelier_shell".into()).unwrap(),
            ClientType::Generic,
        );
        assert_eq!(
