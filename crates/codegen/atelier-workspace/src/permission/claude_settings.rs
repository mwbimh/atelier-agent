...
pub struct ClaudeSettings {
    ...
    /// atelier-only root legacy). Recognized values: `acceptEdits`,
...
///     under `permissions.*`. Root-level keys are **atelier legacy only** (not in
...
pub fn load_claude_settings(path: &Path) -> Option<ClaudeSettings> {
    ...
    // Canonical vendor settings store these under `permissions`; root is atelier legacy only.
...
///
/// Root `defaultMode` is atelier-only back-compat for older tests / hand-written
...
pub(crate) fn extract_default_mode(value: &serde_json::Value, path: &Path) -> Option<String> {
    ...
    // Nested key absent — optional atelier legacy root.
        ...
        Some(dm) => match dm.as_str() {
            ...
            None => {
                warn!(
                    path = %path.display(),
                    actual_type = %dm.type_of(),
                    "root defaultMode (atelier legacy): expected string, ignoring"
                );
                None
            }
        },
...
fn extract_additional_directories(value: &serde_json::Value, path: &Path) -> Option<Vec<String>> {
    ...
    // when it is present but the wrong type, do *not* resurrect the atelier-legacy
...
// permissions), consider extracting to a shared helper (e.g., in atelier-hooks
...
fn global_claude_settings_paths() -> Vec<PathBuf> {
    ...
    if let Some(home) = crate::trust::home_dir() {
...
// =============================================================================
//
// `atelier-shell::claude_import` writes the marker. We re-implement a small
...
pub fn is_claude_import_marked() -> bool {
    ...
    // lives in atelier-shell (inaccessible from here at runtime). They also
    // set this env var so the workspace-resident gate honours the override
    // without a cross-crate dependency.
    if std::env::var("_ATELIER_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = atelier_config::user_atelier_home().map(|g| g.join("config.toml"))
    else {
        return false;
    };
