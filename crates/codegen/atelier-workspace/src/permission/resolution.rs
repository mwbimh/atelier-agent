//! Permission resolution engine: merges native `.atelier/config.toml`,
...
/// (fail-safe) with a warn + skip record for `atelier inspect`.
...
/// user `~/.atelier` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    atelier_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Find every `<dir>/.atelier/config.toml` from `cwd` upward to the git repo
/// root (or just `<cwd>/.atelier/config.toml` when there is no git repo).
///
/// Returned paths are ordered from repo root (lowest priority) to `cwd`
/// (highest priority), matching `atelier-shell::config::find_project_configs`.
fn find_project_atelier_configs(cwd: &Path) -> Vec<PathBuf> {
    ...
    if let Some(ref root) = git_root {
        ...
        while let Some(dir) = current {
            let p = dir.join(".atelier").join("config.toml");
            if p.is_file() {
                configs.push(p);
            }
            if dir == *root {
        ...
        let p = cwd.join(".atelier").join("config.toml");
        if p.is_file() {
            configs.push(p);
        }
    }
...
/// Load `[permission]` rules from native Atelier TOML config files:
///
///   * `~/.atelier/config.toml` (lowest priority)
///   * Each `.atelier/config.toml` from the git repo root down to `cwd`
...
fn load_config_toml_permissions(cwd: &Path) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.atelier/config.toml` first (lowest priority within this layer).
    // Gated on user_atelier_home() so a project's .atelier/config.toml is never read as
    // global permissions when neither ATELIER_HOME nor a home dir resolves.
    if let Some(global_path) = atelier_config::user_atelier_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match atelier_config::load_config_file(&global_path) {
    ...
    for path in find_project_atelier_configs(cwd) {
        match atelier_config::load_config_file(&path) {
            Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[atelier_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Atelier and Claude sources.
...
/// blocked `--yolo`); keep admin-tier ones. Records each drop for `atelier inspect`.
    ...
    fn live() -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &atelier_config::managed_config_layers(),
            ),
        }
    }
}
...
/// bypass is pinned off via atelier `requirements.toml`
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `atelier inspect`; zero
...
fn resolve_claude_settings_inner(
    ...
    for path in find_claude_settings_paths(cwd) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // `atelier inspect` (all-invalid allow/deny/ask must not leave
    ...
    // `atelier inspect` via the outer resolver.
...
/// [`atelier_config::claude_managed_settings_path`] (platform path such as
...
fn load_managed_settings() -> ManagedSettings {
    let Some(path) = atelier_config::claude_managed_settings_path() else {
...
/// not consulted: atelier must not inherit a host-wide always-approve lockdown from
/// that file. atelier still honors that file's permission rules / MCP / marketplace
/// allowlists, and the user's own `--yolo` / `[ui] permission_mode` / runtime
/// toggle drive always-approve; to disable it in atelier use a root-owned
/// `requirements.toml`. Fails open on user-writable layers.
pub fn yolo_disabled_by_policy() -> Option<&'static str> {
    let layers = atelier_config::requirements_layers();
...
/// without `~/.atelier`); `path` only names the layer in a non-bool warning.
...
/// Namespace prefix for managed (atelier.invalid-injected) MCP server names. Defined
/// here (shell depends on workspace) and re-exported by shell's `to_managed_name`
/// so the prefix and policy matching never drift.
pub const MANAGED_MCP_PREFIX: &str = "atelier_com_";

/// Max `char` length of a managed runtime name (`atelier_com_` + normalized display
...
///
/// Both sides reduce to one key (strip `atelier_com_`, [`normalize_managed_name`],
...
mod tests {
    ...
    // Shared so `ATELIER_HOME`/`HOME` mutations here also serialize against the
    ...
    fn load_claude_env_merges_with_precedence() {
        // ATELIER_HOME-isolate so the claude-import marker reads clean (an imported
    ...
    fn load_claude_env_empty_when_no_settings() {
        // Isolate ATELIER_HOME (claude-import marker) AND HOME (global `~/.claude`)
    ...
    fn load_claude_env_with_project_drops_repo_env_when_untrusted() {
        ...
        // drop it. Isolate ATELIER_HOME so the claude-import marker reads clean (an
    ...
    fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
        ...
        assert!(mcp_name_matches("foo", "atelier_com_foo"));
        assert!(mcp_name_matches("atelier_com_foo", "foo"));
        assert!(mcp_name_matches("atelier_com_foo", "atelier_com_foo"));
        assert!(!mcp_name_matches("foo", "foobar"));
        assert!(!mcp_name_matches("foo", "atelier_com_foobar"));
    ...
    fn mcp_name_matches_is_case_and_space_insensitive() {
        ...
        assert!(mcp_name_matches("Slack", "atelier_com_slack"));
        assert!(mcp_name_matches("My Server", "atelier_com_my_server"));
        assert!(mcp_name_matches("atelier_com_my_server", "My Server"));
        ...
        assert!(!mcp_name_matches("atelier_com_", "atelier_com_anything"));
    }
    ...
    fn denied_by_server_name_matches_bare_and_managed_prefix() {
        ...
        let managed = http_named("atelier_com_foo", "https://foo.example.com/mcp");
        assert!(al.is_server_denied(&managed));
        assert!(!al.is_server_allowed(&managed));

        // Name match is transport-agnostic.
        let stdio = stdio_named("atelier_com_foo", "npx");
        ...
        for unrelated in ["foobar", "atelier_com_foobar", "barfoo", "bar"] {
    ...
    fn allowed_by_server_name_restricts_across_transports() {
        ...
        assert!(al.is_server_allowed(&http_named("atelier_com_foo", "https://evil.example.com/x")));
        assert!(al.is_server_allowed(&stdio_named("atelier_com_foo", "/usr/bin/whatever")));
    ...
    fn server_name_deny_beats_allow() {
        ...
        for s in [
            ...
            http_named("atelier_com_foo", "https://foo.example.com/x"),
        ] {
    ...
    fn server_name_prefix_edge_cases_vice_versa() {
        ...
        let al = allowlist_from(serde_json::json!({
            "deniedMcpServers": [ { "serverName": "atelier_com_foo" } ]
        }));

        assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
        assert!(al.is_server_denied(&http_named("atelier_com_foo", "https://x.example.com/mcp")));
    ...
    fn non_bool_lock_key_warns_and_does_not_lock() {
        ...
        let p = Path::new("/etc/atelier/requirements.toml");
        let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
        ...
        assert!(
            out.contains("/etc/atelier/requirements.toml"),
    ...
    fn admin_source_trusts_only_root_owned_tiers() {
        ...
        // the user-writable `~/.atelier/requirements.toml` is not, despite its path.
        ...
        assert!(is_admin_source(&RequirementSource::SystemRequirements {
            path: "/etc/atelier/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::Requirements {
            path: "/home/u/.atelier/requirements.toml".into(),
        }));
        assert!(!is_admin_source(&RequirementSource::ManagedConfig {
            path: "/etc/atelier/managed_config.toml".into(),
        }));
    ...
    fn drop_untrusted_catchall_allows_is_source_aware() {
        ...
        let rules = vec![
            ...
            // Managed config: defaults tier, untrusted even from /etc/atelier.
            sourced(
                allow_any(Some("*")),
    ...
    fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
        ...
        assert!(
            ...
            "typo should be recorded for atelier inspect"
        );
    }
    ...
    async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
        ...
        let layers =
            atelier_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
        ...
        .unwrap();
        assert_eq!(
            atelier_config::managed_config_layers_at(Some(system.path()), Some(user.path())).len(),
            1
        );
