...
//! it" buckets.
//!
//! `atelier-sampler` is intentionally decoupled from `atelier-shell`
...
/// tools live in `atelier-tools`
/// (`crates/codegen/atelier-tools/src/implementations/`), have their
/// own HTTP clients that do not flow through `SamplingClient`, and
/// hook into the `atelier_tools::ApiKeyProvider` trait rather than
/// this enum.
...
/// `atelier_shell::auth::token_suffix` (which truncates to 12 chars
...
/// `atelier-shell/src/auth/manager.rs` to match, otherwise the
