...
pub use atelier_sampling_types::ApiBackend;
...
pub(crate) fn extract_model_metadata(
    ...
    let max_completion_tokens = headers
        .get("x-atelier-max-completion-tokens")
        .and_then(|v| v.to_str().ok())
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        // Add x-atelier-client-version header for version gating at the proxy.
        if let Some(client_version) = config.client_version.as_ref()
            && let Ok(header_value) = HeaderValue::from_str(client_version)
        {
            headers.insert(
                HeaderName::from_static("x-atelier-client-version"),
                header_value,
            );
        }
        ...
        {
            headers.insert(
                HeaderName::from_static("x-atelier-deployment-id"),
                header_value,
            );
        }
        ...
        {
            headers.insert(HeaderName::from_static("x-atelier-user-id"), header_value);
        }

        {
            ...
            if let Ok(header_value) = HeaderValue::from_str(&client_id) {
                headers.insert(
                    HeaderName::from_static("x-atelier-client-identifier"),
                    header_value,
                );
            }
        }
    ...
    /// gate that adds the opt-in `x-atelier-doom-loop-check` request header, so
...
mod tests {
    ...
    fn new_applies_extra_headers() {
        ...
        cfg.extra_headers
            .insert("x-XAI-token-auth".to_string(), "atelier-cli".to_string());
        ...
        let make = |ticks: i64| {
            format!(
                ...
                "response": {{
                    ...
                    "model": "atelier-build", "status": "completed", "output": [],
                    "usage": {{
                        "input_tokens": 10,
