//! Filesystem locations for Atelier config files and binaries.
...
/// The default Atelier directory (`~/.atelier`, canonicalized) used when
/// `ATELIER_HOME` is unset.
///
...
/// `~/.atelier/marketplace-cache`. `dunce` strips the prefix whenever the path
/// is safely representable in legacy form; on non-Windows it is identical to
/// `std::fs::canonicalize`.
///
pub fn default_atelier_home() -> PathBuf {
    #[allow(deprecated)]
    let home = std::env::home_dir().unwrap_or_else(|| PathBuf::from("."));
    dunce::canonicalize(&home).unwrap_or(home).join(".atelier")
}

/// Per-user config directory: `$ATELIER_HOME` or `~/.atelier`. Created if needed.
pub fn atelier_home() -> PathBuf {
    ATELIER_HOME
        .get_or_init(|| {
            let atelier_home = if let Ok(v) = std::env::var("ATELIER_HOME") {
                PathBuf::from(v)
            } else {
                default_atelier_home()
            };
...
/// The user-global Atelier home, but only when one genuinely resolves.
pub fn user_atelier_home() -> Option<PathBuf> {
    #[allow(deprecated)]
    let resolvable = std::env::var_os("ATELIER_HOME").is_some() || std::env::home_dir().is_some();
...
/// System-wide config directory: `/etc/atelier/` on Unix, `None` on Windows.
pub fn system_config_dir() -> Option<PathBuf> {
    if cfg!(unix) {
        Some(PathBuf::from("/etc/atelier"))
    } else {
        None
    }
}
...
/// `atelier_home()/sessions/{encode_cwd_dirname(cwd)}`.
///
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ...
    let dir = atelier_home().join("sessions").join(&encoded_name);
...
mod tests {
    ...
    #[test]
    fn default_atelier_home_has_no_verbatim_prefix() {
        ...
        let home = default_atelier_home();
        assert!(!home.to_string_lossy().starts_with(r"\\?\"));
        assert!(home.ends_with(".atelier"));
    }
