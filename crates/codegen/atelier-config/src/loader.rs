...
use crate::paths::{system_config_dir, user_atelier_home};
...
/// atelier config files; use [`load_toml_file`] directly for unrelated TOML.
...
pub fn load_from_disk() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_atelier_home().as_deref(), "config.toml")
}
...
pub fn load_managed_config() -> std::io::Result<toml::Value> {
    load_user_config_layer(user_atelier_home().as_deref(), MANAGED_CONFIG_FILENAME)
}
...
/// `.atelier/<filename>` (the cwd-fallback would silently promote an untrusted
/// project `.atelier` to the user tier).
...
pub struct ManagedConfigLayer {
    ...
    /// `true` for the root-owned system layer (`/etc/atelier`), derived from the
    /// load directory.
    pub is_system: bool,
}
...
impl ConfigLayers {
    ...
    /// wraps this with the `ATELIER_CAMPAIGNS_OVERRIDE` env layer.
...
/// `ATELIER_CAMPAIGNS=0` or `[features] campaigns = false` on pre-campaign base.
pub fn campaigns_application_disabled(base_effective: &toml::Value) -> bool {
    if crate::env_bool("ATELIER_CAMPAIGNS") == Some(false) {
        return true;
    }
    base_effective
        .get("features")
...
/// (`atelier_shell::util::config::load_effective_config`) when remote campaigns
/// or `ATELIER_CAMPAIGNS_OVERRIDE` must be honored. The name mirrors the
...
/// can't be parsed (broken `ATELIER_TEST_VERSION` in dev), silently strips
...
pub fn apply_version_overrides_with_registered(value: &mut toml::Value) -> std::io::Result<()> {
    match atelier_version::installed_semver() {
...
mod tests {
    ...
    fn load_user_config_layer_reads_file_when_home_present() {
        ...
        let dir = std::env::temp_dir().join(format!("atelier-load-layer-{}", std::process::id()));
    ...
    fn parse_error_keeps_kind_but_not_snippet() {
        let dir = std::env::temp_dir().join(format!("atelier-toml-leak-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn kill_switch_env_var_disables() {
        ...
        // crate mutates or reads ATELIER_CAMPAIGNS concurrently.
        unsafe { std::env::set_var("ATELIER_CAMPAIGNS", "0") };
        assert!(campaigns_application_disabled(&empty));

        unsafe { std::env::remove_var("ATELIER_CAMPAIGNS") };
        assert!(!campaigns_application_disabled(&empty));

        match prior {
            Some(v) => unsafe { std::env::set_var("ATELIER_CAMPAIGNS", v) },
            None => unsafe { std::env::remove_var("ATELIER_CAMPAIGNS") },
        }
    }
}
