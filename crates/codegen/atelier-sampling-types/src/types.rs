...
impl ChatCompletionRequest {
    pub fn new(model: impl Into<String>, messages: Vec<ChatRequestMessage>) -> Self {
        Self {
            ...
            x_atelier_conv_id: None,
            x_atelier_req_id: None,
            x_atelier_session_id: None,
            x_atelier_turn_idx: None,
            x_atelier_agent_id: None,
            x_atelier_deployment_id: None,
...
// Re-export ToolDefinition and FunctionTool from atelier-tools.
...
pub use atelier_tools::types::definition::{FunctionTool, ToolDefinition};
...
/// Parse a canonical wire effort token; remapped menu ids need a model catalog.
...
mod tests {
    ...
    #[test]
    fn reasoning_effort_keeps_max_distinct_from_xhigh() {
        assert_eq!(
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(
                json!({ "agentType": "atelier" }).as_object().unwrap()
            ))
            .is_none()
        );
    }
