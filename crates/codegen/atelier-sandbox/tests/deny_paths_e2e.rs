...
fn profile_from_env() -> atelier_sandbox::ProfileName {
    atelier_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    #[cfg(target_os = "linux")]
    {
        if !atelier_sandbox::is_inside_bwrap() {
            ...
            match atelier_sandbox::bwrap_reexec_for_profile(&profile_from_env(), workspace) {
                Some(mut cmd) => {
    ...
    match scenario.as_str() {
        "block_deny" => {
            let mut sandbox = atelier_sandbox::SandboxManager::new(profile_from_env(), workspace);
...
fn run_deny_case(
    ...
    let support = atelier_sandbox::SandboxManager::support_info();
    ...
    let deny_list = deny_entries
        ...
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".atelier")).expect("mkdir .atelier");
    fs::write(
        tmp.join(".atelier").join("sandbox.toml"),
