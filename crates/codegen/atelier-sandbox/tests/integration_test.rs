//! Integration tests for atelier-sandbox.
//!
...
fn test_profile_capability_set_construction() {
    use atelier_sandbox::ProfileName;
...
fn test_sandbox_logger() {
    use atelier_sandbox::{SandboxEvent, SandboxLogger};
...
fn test_should_restrict_child_network_default() {
    ...
    assert!(!atelier_sandbox::should_restrict_child_network());
}
