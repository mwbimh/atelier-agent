...
//! `off`. Custom profiles via `~/.atelier/sandbox.toml` or `.atelier/sandbox.toml`.
...
/// Load sandbox config from `~/.atelier/sandbox.toml` and `.atelier/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.atelier/sandbox.toml
    let global_path = atelier_home().join("sandbox.toml");
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.atelier/sandbox.toml (additive only)
    let project_path = workspace.join(".atelier").join("sandbox.toml");
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&atelier_home().join("sandbox.toml")).unwrap_or_default();
    let project =
        load_config_file(&workspace.join(".atelier").join("sandbox.toml")).unwrap_or_default();
...
impl ProfileName {
    ...
    pub fn to_capability_set_with_config(
        ...
        // directories like ~/.atelier/ that may not exist on first run.
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.atelier/sandbox.toml or .atelier/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
