...
//! OS-level sandboxing for Atelier via [nono](https://crates.io/crates/nono).
//!
...
//!
//! ```rust,no_run
//! use atelier_sandbox::{SandboxManager, ProfileName};
//! use std::path::Path;
//!
...
mod types;
pub use backend::{
    NativeSandboxAvailability, SandboxBackendKind, SandboxBackendParseError, SandboxDiagnostics,
    SandboxError, SandboxStatus, ensure_backend_available, native_sandbox_availability,
    parse_backend, require_backend, resolve_sandbox_status,
};
#[cfg(windows)]
pub use backend::{validate_windows_preview_profile, windows_child_sandbox_mode};
...
/// Return a stable, serializable sandbox diagnostic snapshot for shell/RPC consumers.
...
pub fn sandbox_status_label() -> &'static str {
    SANDBOX
        .get()
        .map(|state| state.diagnostics.status.as_str())
        .unwrap_or(SandboxStatus::Unavailable.as_str())
}
...
impl SandboxManager {
    ...
    pub fn new(profile: ProfileName, workspace: &Path) -> Self {
        Self::new_with_backend(profile, workspace, SandboxBackendKind::Native)
    }
    /// Create a manager with an explicit backend selection.
    pub fn new_with_backend(
        ...
        let diagnostics = if profile == ProfileName::Off {
            ...
            SandboxDiagnostics::new(
                ...
                availability
                    .reason()
                    .unwrap_or("sandbox has not been applied yet"),
            )
        };
        Self {
            profile,
    ...
    /// Parse an optional backend configuration and create a manager.
    pub fn try_new_with_backend_name(
        profile: ProfileName,
        workspace: &Path,
        backend: Option<&str>,
    ) -> Result<Self, SandboxBackendParseError> {
    ...
    fn mark_unsafe_backend(&mut self) {
        ...
        tracing::warn!(
            ...
            "explicit unsafe sandbox backend selected; running without native enforcement"
        );
    }
    fn record_failure(
        &mut self,
        ...
        self.applied = false;
        self.net_restricted = false;
        self.diagnostics = SandboxDiagnostics::unavailable(
            self.backend,
            self.profile.to_string(),
            native_available,
            reason,
    ...
    /// Refuses to continue when the selected native backend cannot be applied.
    #[cfg(all(feature = "enforce", unix))]
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        if self.profile == ProfileName::Off {
            ...
            self.diagnostics = SandboxDiagnostics::disabled(self.backend, self.profile.to_string());
            return Ok(());
        }
        if self.backend.is_unsafe() {
            self.mark_unsafe_backend();
            return Ok(());
        }
        let availability = native_sandbox_availability();
        if let NativeSandboxAvailability::Unavailable { reason } = availability {
            return Err(self.record_failure(
                workspace,
                SandboxError::NativeUnavailable { reason },
                false,
            ));
        }
            ...
            .to_capability_set_with_config(workspace, &config)
        {
            ...
            Err(error) => {
                return Err(self.record_failure(
                    workspace,
                    SandboxError::NativeApplyFailed {
        ...
        let mut resolved = match self.profile.resolve_profile(workspace, &config) {
        ...
        match Sandbox::apply(&caps) {
            Ok(_) => {
                self.applied = true;
                self.diagnostics = SandboxDiagnostics::active(
                    self.backend,
                    self.profile.to_string(),
                    "native sandbox enforced",
                );
    ...
    /// Windows first-stage preview: child commands are routed through the
    /// restricted-token helper. The current process remains unsandboxed so the
    /// model Provider can continue to use the network; terminal adapters must
    /// enforce the preview status before spawning a child.
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        if let Err(reason) = validate_windows_preview_profile(&self.profile) {
            return Err(self.record_failure(
                workspace,
                SandboxError::NativeApplyFailed { reason },
                true,
            ));
        }
        self.applied = false;
        self.net_restricted = false;
        self.diagnostics = SandboxDiagnostics::new(
            self.backend,
            self.profile.to_string(),
            true,
            false,
            "Windows restricted-token child sandbox configured (sandbox-preview)",
        );
        tracing::warn!(
            profile = %self.profile,
            "Windows sandbox configured in preview mode; child processes use the restricted-token helper"
        );
        Ok(())
    }

    /// Stub when native enforcement is unavailable in this build/platform.
    ...
    pub fn apply(&mut self, workspace: &Path) -> anyhow::Result<()> {
        ...
        let reason = native_sandbox_availability()
            .reason()
            .unwrap_or("native sandbox enforcement is unavailable")
            .to_string();
        Err(self.record_failure(workspace, SandboxError::NativeUnavailable { reason }, false))
    }
    ...
    /// The manager's current runtime status.
    pub fn status(&self) -> SandboxStatus {
        self.diagnostics.status
    }
    /// Access a serializable diagnostic snapshot before `install()`.
...
/// Zero-permission placeholder (file or dir) under `atelier_home` used by bwrap bind-over.
///
/// The placeholder name is suffixed with the current PID so concurrent atelier
...
fn bwrap_blocked_placeholder(name: &str, want_dir: bool) -> Option<PathBuf> {
    ...
    let path = paths::atelier_home().join(format!("{name}.{}", std::process::id()));
...
mod tests {
    ...
    /// Create a temp workspace whose `.atelier/sandbox.toml` contains `toml_body`.
    ...
    fn temp_workspace_with_sandbox_toml(tag: &str, toml_body: &str) -> PathBuf {
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let ws = std::env::temp_dir().join(format!("atelier-{tag}-{}-{nanos}", std::process::id()));
        let atelier = ws.join(".atelier");
        std::fs::create_dir_all(&atelier).unwrap();
        std::fs::write(atelier.join("sandbox.toml"), toml_body).unwrap();
        ws
    }
    ...
    fn bwrap_reexec_uses_dir_placeholder_for_directories() {
        let _g = EnvGuard::remove(BWRAP_ENV_VAR);
        let dir = std::env::temp_dir().join(format!("atelier-deny-dir-{}", std::process::id()));
        ...
        let args: Vec<String> = cmd
            ...
            .map(|a| a.to_string_lossy().to_string())
            .collect();
        let blocked_dir = paths::atelier_home()
