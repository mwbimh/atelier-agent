...
//! cargo run -p atelier-sandbox --example sandbox_smoke_test
//!
//! # Test strict profile
//! cargo run -p atelier-sandbox --example sandbox_smoke_test -- strict
//!
//! # Test read-only profile
//! cargo run -p atelier-sandbox --example sandbox_smoke_test -- read-only
//! ```

use atelier_sandbox::{ProfileName, SandboxManager, native_sandbox_availability};
use std::path::Path;
...
fn main() {
    ...
    let support = native_sandbox_availability();
    let is_supported = support.is_available();
    println!(
        "Platform support: {}",
    ...
    println!(
        "Details: {}",
        support.reason().unwrap_or("native sandbox is available")
    );
    ...
    let tmp_test = Path::new("/tmp/.atelier-sandbox-test");
