...
//!
//! Extracted from `atelier-shell/src/agent/subagent/` `normalize_forked_context()`.
...
/// `atelier-shell/src/session/storage/jsonl.rs` (it truncates to the last
...
mod tests {
    use super::*;
    use atelier_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(atelier_sampling_types::synthesized_reasoning_item(text))
    }
