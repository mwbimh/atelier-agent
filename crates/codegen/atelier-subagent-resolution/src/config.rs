...
//! `atelier_shell::config::{SubagentRole, SubagentPersona, PersonaIOField}`.
//!
//! Methods that remain in `atelier-shell` (on `SubagentsConfig`):
...
"#;
        ...
        assert_eq!(role.model.as_deref(), Some("atelier-3"));
        assert_eq!(role.reasoning_effort.as_deref(), Some("high"));
        assert_eq!(
            role.prompt_file.as_deref(),
            Some(".atelier/prompts/researcher.md")
        );
...
"#;
        ...
        assert_eq!(
            ...
            Some(".atelier/personas/concise.md")
        );
        assert_eq!(persona.model.as_deref(), Some("atelier-3-fast"));
