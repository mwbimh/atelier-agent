//! Integration tests for atelier-crash-handler.
//!
...
fn subprocess_entry() {
    ...
    let config = atelier_crash_handler::CrashHandlerConfig {
        app_version: "0.0.0-test".to_string(),
        crash_dir,
    };
    atelier_crash_handler::install(config);
...
fn sigbus_produces_valid_crash_blob() {
    ...
    let blob =
        atelier_crash_handler::format::CrashBlob::parse(&data).expect("crash blob should parse");
    ...
    let report = atelier_crash_handler::check_previous_crash(tmp.path())
