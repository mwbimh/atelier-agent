//! Leader-mode (`atelier agent --leader stdio`) test harness.
//!
...
/// (version-skew) test. Falls back to [`atelier_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "ATELIER_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a
/// two-binary test. Falls back to [`atelier_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "ATELIER_BINARY_CLIENT";
...
/// (`ATELIER_BINARY_LEADER`, else the shared [`atelier_binary`] resolution).
pub fn leader_binary() -> PathBuf {
    role_binary(LEADER_BINARY_ENV)
}

/// Binary for the client side of a version-skew test (`ATELIER_BINARY_CLIENT`,
/// else the shared [`atelier_binary`] resolution).
...
impl acp::Client for LeaderAcpClient {
    ...
    async fn ext_notification(&self, args: acp::ExtNotification) -> acp::Result<()> {
        if &*args.method == "atelier/leader_reconnected" {
            self.capture
                .reconnected_count
                .fetch_add(1, Ordering::SeqCst);
        }
...
/// A `atelier agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderStdioClient {
    pub async fn spawn(server: &MockInferenceServer, cwd: &Path, home: &Path) -> Self {
        Self::spawn_with_binary(&atelier_binary(), server, cwd, home).await
    }
    ...
    pub async fn spawn_with_binary(
        ...
        cmd.args(["agent", "--leader", "stdio"])
            .current_dir(cwd)
            // Hermetic env: the developer's shell may export ATELIER_* vars
            // (e.g. ATELIER_LEADER_SOCKET pointing at a REAL leader on this
            ...
            .env("RUST_LOG", "atelier_shell=debug");
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".atelier").join("leader.lock")
}
...
///
/// The `atelier/leader_reconnected` ext notification is dropped by the typed
/// `ClientSideConnection` (bare `atelier/*` methods are rejected by the ACP
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".atelier").join("leader.log")).unwrap_or_default()
}
