//! ACP stdio clients for testing atelier sessions end-to-end: the typed
//! [`AtelierStdioClient`] (`agent-client-protocol::ClientSideConnection` —
...
/// Spawn `atelier agent stdio` with the canonical hermetic test env: the sandbox
/// from [`test_env_cmd_tokio`] plus the debug-logging kill-list, so the
/// hermeticity setup exists exactly once for the typed ([`AtelierStdioClient`])
...
/// kill-list so a test can still set e.g. `ATELIER_DEBUG_LOG=1` explicitly.
...
/// Drives `atelier agent stdio` with verbatim newline-delimited JSON-RPC lines.
///
/// Exists for wire shapes the typed [`AtelierStdioClient`] (`ClientSideConnection`,
