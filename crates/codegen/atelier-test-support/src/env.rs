...
pub fn atelier_binary() -> PathBuf {
    if let Ok(path) = std::env::var("ATELIER_BINARY") {
        ...
        assert!(p.exists(), "ATELIER_BINARY does not exist: {}", p.display());
        return p;
    }
    ...
    let binary = local_atelier_binary_path();
    ensure_local_atelier_binary(&binary);
    binary
}
...
pub fn test_env_cmd_tokio(
    ...
    cmd.env("HOME", home)
        // HOME alone does not sandbox atelier on Windows: the product resolves
        // `~` via `USERPROFILE`/Known Folders (`std::env::home_dir()`), so
        // without an explicit ATELIER_HOME every spawned child shares the real
        // `%USERPROFILE%\.atelier` — test 1's models_cache.json (which embeds
