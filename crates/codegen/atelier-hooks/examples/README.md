...
Sample hooks for Atelier. Copy to `~/.atelier/hooks/` to enable globally, or to `<project>/.atelier/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.atelier/hooks/bin
cp examples/hooks/safe-shell.json ~/.atelier/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.atelier/hooks/bin/
chmod +x ~/.atelier/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.atelier/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.atelier/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.atelier/hooks/bin/
chmod +x ~/.atelier/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.atelier/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.atelier/hooks/bin
cp examples/hooks/session-log.json ~/.atelier/hooks/
cp examples/hooks/bin/session-log.sh ~/.atelier/hooks/bin/
chmod +x ~/.atelier/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.atelier/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.atelier/hooks/bin
cp examples/hooks/tool-logger.json ~/.atelier/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.atelier/hooks/bin/
chmod +x ~/.atelier/hooks/bin/tool-logger.sh
```
...
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Atelier names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.atelier/hooks/`. The hook stops running on the next session.