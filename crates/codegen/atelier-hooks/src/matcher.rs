use atelier_tools::types::{atelier_names_for, claude_names_for};
use regex::Regex;
...
///   **exact** match against each name (after external→Atelier alias expansion), NOT a regex;
/// - anything else is an **unanchored** regex (also tested against the tool's external
///   alias names, so e.g. `^Bash$` matches the Atelier tool `run_terminal_command`).
///
...
/// plus any Atelier tool names that term aliases (so `"Bash"` also matches
/// `run_terminal_command`), per the shared external-name to Atelier registry in
/// `atelier-tools`. Empty terms and duplicates are dropped.
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for atelier_name in atelier_names_for(term) {
            push(atelier_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_atelier_tool() {
        let m = HookMatcher::new("Bash").unwrap();
        assert!(m.is_match("Bash")); // external alias name
        assert!(m.is_match("run_terminal_command")); // Atelier name
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_edit_write_matches_atelier_tool_exactly() {
        let m = HookMatcher::new("Edit|Write").unwrap();
        assert!(m.is_match("Edit"));
        assert!(m.is_match("Write"));
        assert!(m.is_match("search_replace")); // Atelier equivalent
        assert!(m.is_match("hashline_edit")); // second Atelier alias
        assert!(!m.is_match("read_file"));
    ...
    #[test]
    fn claude_read_matches_atelier_tool() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_atelier_tool() {
        // A regex written against an external alias still matches the Atelier tool
