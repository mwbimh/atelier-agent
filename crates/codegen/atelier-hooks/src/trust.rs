...
// (`~/.atelier/trusted_folders.toml`) is the single authority for whether a repo's
...
/// (`<user_atelier_home>/trusted-hook-projects`), or `None` when no user atelier home
/// resolves. Retained only for the one-time migration into folder-trust.
pub fn legacy_trust_file_path() -> Option<PathBuf> {
    Some(atelier_config::user_atelier_home()?.join("trusted-hook-projects"))
}
...
pub fn disable_hook(hook_name: &str) -> Result<(), String> {
    let file = disabled_hooks_file_path()
        .ok_or_else(|| "no user atelier home (set $ATELIER_HOME or $HOME)".to_string())?;
...
/// Returns the path to `$ATELIER_HOME/disabled-hooks`, or `None` when no user atelier
/// home resolves.
fn disabled_hooks_file_path() -> Option<PathBuf> {
    Some(atelier_config::user_atelier_home()?.join("disabled-hooks"))
}
...
mod tests {
    ...
    fn trust_file_in(dir: &Path) -> PathBuf {
        let atelier_dir = dir.join(".atelier");
        std::fs::create_dir_all(&atelier_dir).unwrap();
        atelier_dir.join("trusted-hook-projects")
    }
