//! Integration tests for atelier-hooks.
//!
...
use atelier_hooks::discovery::load_hooks;
use atelier_hooks::dispatcher;
use atelier_hooks::event::*;
use atelier_hooks::result::HookDecision;
use atelier_hooks::runner::RunContext;
...
/// runner-reserved keys (`ATELIER_HOOK_EVENT`, `ATELIER_HOOK_NAME`,
/// `ATELIER_SESSION_ID`, `ATELIER_WORKSPACE_ROOT`, `CLAUDE_PROJECT_DIR`)
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "ATELIER_HOOK_EVENT": "spoofed_event",
                                "ATELIER_HOOK_NAME": "spoofed_name",
                                "ATELIER_SESSION_ID": "spoofed_session",
                                "ATELIER_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "ATELIER_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed ATELIER_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "ATELIER_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed ATELIER_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "ATELIER_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed ATELIER_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
