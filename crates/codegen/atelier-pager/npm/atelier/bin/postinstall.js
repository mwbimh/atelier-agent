#!/usr/bin/env node
// Runs once after npm install/update. Reads the atelier binary from the
...
//
//   Unix:    atelier-<version>  +  atelier  (symlink)
//   Windows: atelier-<version>.exe  +  atelier.exe  (copy)
//
    ...
    if (IS_WINDOWS) {
        ...
        try {
            ...
            try {
                ...
                console.error('Close all running atelier processes and try again.');
                return false;
            }
        }
    } else {
...
// Uses an exact prefix match + hyphen + digit to avoid atelier-* matching atelier-*.
...
// Persist the npm registry so `atelier update` and the trampoline use the same one.
...
const ATELIER_PATH = path.join(CANONICAL_DIR, `atelier${EXE}`);
if (process.env.ATELIER_INSTALL_COMPLETIONS === '1' && !IS_WINDOWS) {
    try {
        ...
        const bashPath = path.join(completionsDir, 'bash', 'atelier.bash');
        const zshPath = path.join(completionsDir, 'zsh', '_atelier');
