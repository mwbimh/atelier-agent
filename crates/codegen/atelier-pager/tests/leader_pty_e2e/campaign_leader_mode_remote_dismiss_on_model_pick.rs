...
#[ignore = "PTY e2e; run with cargo test -p atelier-pager --test leader_pty_e2e -- --ignored --test-threads=1"]
async fn campaign_leader_mode_remote_dismiss_on_model_pick() {
    ...
    // socket under the shared ATELIER_HOME so every spawn elects/attaches to the
    // same leader (mirrors `LeaderCluster`).
    let atelier_home = content.home().join(".atelier");
    std::fs::create_dir_all(&atelier_home).expect("create ATELIER_HOME");
    std::fs::write(
    ...
    let state_path = atelier_home.join("campaigns_state.json");
    ...
    let config =
        std::fs::read_to_string(atelier_home.join("config.toml")).expect("read config.toml");
    assert!(
