...
async fn reasoning_efforts_from_config_toml_menu() {
    let content = ContentController::start_with_models(vec![
        MockModel::new("atelier-4.5").with_supports_reasoning_effort(true),
    ])
    .await
    .expect("start content");
    content.set_response(format!("{MOCK_RESPONSE_SENTINEL} turn."));

    // Seed `~/.atelier/config.toml` with a per-model reasoning-effort menu.
    let atelier_home = content.home().join(".atelier");
    std::fs::create_dir_all(&atelier_home).expect("create .atelier");
    // Quote the dotted model id: bare `[model.atelier-4.5]` is TOML key-path syntax (model.atelier-4.5), not the id "atelier-4.5".
    std::fs::write(
