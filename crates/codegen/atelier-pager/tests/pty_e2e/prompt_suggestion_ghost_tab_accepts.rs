...
/// turn and the turn-end `atelier/suggestPrompt` call (the mock's fixed mode
...
async fn prompt_suggestion_ghost_tab_accepts() {
    // The suggestion model (`atelier-build-0.1`, the shell's built-in default)
    ...
    let content = ContentController::start_with_models(vec![
        ...
        MockModel::new("atelier-build-0.1"),
    ])
    .await
    ...
    env.retain(|(k, _)| k != "ATELIER_PROMPT_SUGGESTIONS");
    env.push(("ATELIER_PROMPT_SUGGESTIONS".into(), "true".into()));
