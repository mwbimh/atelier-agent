...
use atelier_pager_pty_harness::PtyHarness;
pub(crate) use atelier_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
/// (`atelier-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this
...
/// `atelier-pager-pty-harness/src/scroll_matrix/session.rs` is a port of
...
/// (e.g. `ATELIER_SCROLL_SPEED`) appended to the controller's env.
