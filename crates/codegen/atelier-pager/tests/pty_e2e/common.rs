...
pub(crate) use atelier_pager_pty_harness::{
...
pub(crate) fn contextual_hints_env(content: &ContentController) -> Vec<(String, String)> {
    ...
    env.push(("ATELIER_CONTEXTUAL_HINTS".into(), "1".into()));
    env
}
...
// `atelier_pager_pty_harness::flows` (re-exported above).
...
/// the harness's default credential-free Provider path has no session auth and
/// would never fetch `/v1/settings`. Spawns WITHOUT `ATELIER_ANNOUNCEMENTS_OVERRIDE`
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    env_refs.push(("ATELIER_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// (`ATELIER_TEST_VERSION`) and an explicit `ATELIER_FOLDER_TRUST` — `1` when `feature_on`,
/// else `0` (an explicit opt-out that overrides the now-on default). HOME/ATELIER_HOME
...
pub(crate) fn trust_env(content: &ContentController, feature_on: bool) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    // A self-built (unstamped) atelier auto-trusts and never prompts; simulate a
    ...
    // auto-trust.
    env.push(("ATELIER_TEST_VERSION".into(), "0.0.0-sim".into()));
    // Set ATELIER_FOLDER_TRUST explicitly: the default is on, so `0` is the opt-out
    // that exercises the feature-off path rather than relying on an absent var.
    let folder_trust = if feature_on { "1" } else { "0" };
    env.push(("ATELIER_FOLDER_TRUST".into(), folder_trust.into()));
    env
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".atelier")
        .join(atelier_workspace::trust::TRUST_FILE_NAME);
    let store = atelier_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&atelier_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `atelier_pager_pty_harness::flows`
// (re-exported above).
...
/// Budget for session creation plus the `atelier/mcp/list` round-trip.
...
pub(crate) fn seed_mcp_server_config(content: &ContentController) {
    ...
    std::fs::create_dir_all(&atelier_home).expect("create fake ATELIER_HOME");
    let config = format!(
        "[mcp_servers.{MCP_TEST_SERVER}]\ncommand = \"{command}\"\nargs = []\nstartup_timeout_sec = 2\n"
    );
    std::fs::write(atelier_home.join("config.toml"), config).expect("write config.toml");
}
...
// unit tests in atelier-hooks::discovery::tests. The PTY E2E test requires
...
/// Seed `~/.atelier/config.toml` with a `[ui]` section body (e.g.
/// `"vim_mode = true"`). Same `{ATELIER_HOME|HOME}/.atelier/config.toml` location
...
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let atelier_home = content.home().join(".atelier");
    std::fs::create_dir_all(&atelier_home).expect("create .atelier");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let atelier_home = content.home().join(".atelier");
    std::fs::create_dir_all(&atelier_home).expect("create .atelier");
    // Minimal opt-in only — matches load_config's `{ATELIER_HOME|HOME}/.atelier/config.toml`.
...
pub(crate) fn mouse_toggle_env(content: &ContentController) -> Vec<(String, String)> {
    let mut env = content.env_for_pager();
    env.push(("ATELIER_MOUSE_REPORTING_TOGGLE".into(), "true".into()));
    env
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "ATELIER_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    ...
    env.push((
        ...
        atelier_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string(),
    ));
    let env_refs: Vec<(&str, &str)> = env.iter().map(|(k, v)| (k.as_str(), v.as_str())).collect();
    PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &[], &env_refs).expect("spawn pager")
}
...
/// `atelier wrap` run budget. Same contention math as the requirements-version
...
/// Run `atelier wrap <wrap_args...>` to completion inside a PTY with an isolated
/// `ATELIER_HOME`, returning the exit code (`None` if it never exited within
...
pub(crate) fn run_wrap(wrap_args: &[&str], extra_env: &[(&str, &str)]) -> (Option<u32>, String) {
    ...
    let mut env: Vec<(&str, &str)> = vec![("ATELIER_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness = PtyHarness::new(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env)
        .expect("spawn atelier wrap");
...
/// `atelier wrap` hands to the user's shell without depending on any real
...
/// Dump an asciinema cast of `harness` into `$ATELIER_PTY_CAST_DIR/<file_name>`
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("ATELIER_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
pub(crate) use atelier_pager_pty_harness::host_clipboard::{
...
pub(crate) use atelier_pager_pty_harness::host_clipboard::clipboard_roundtrip_works;
