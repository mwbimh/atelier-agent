...
const PAID_TIER: &str = "SuperAtelierPro";

/// Display name delivered via `/settings` `subscription_tier_display`.
const PAID_TIER_DISPLAY: &str = "SuperAtelier Pro";
...
///
/// Proto `prod_auth.SubscriptionTier`: 5 = `superatelier_heavy` = live
/// `/user` string `SuperAtelierPro`. Must match
...
/// Like [`seed_fake_oauth`], but under the `ATELIER_LOCAL_AUTH` dev issuer
...
/// `ATELIER_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup
/// resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let atelier_home = content.home().join(".atelier");
    std::fs::create_dir_all(&atelier_home).expect("create temp .atelier");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    env_refs.push(("ATELIER_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let content = ContentController::start_with_models(vec![MockModel::new("atelier-3")])
        .await
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[("ATELIER_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn startup_gate_shows_paywall_for_free_user_after_live_check() {
    ...
    content.server().set_settings(json!({
        ...
        "gate_url": "https://atelier.invalid/superatelier?referrer=atelier-build",
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[("ATELIER_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
