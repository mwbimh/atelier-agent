...
const PROMO_URL: &str = "https://atelier/zz-promo-cta";
...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    env.push((
        "ATELIER_ANNOUNCEMENTS_OVERRIDE".into(),
...
async fn critical_announcement_hide_button_click_hides_banner() {
    ...
    let long_msg = format!(
        "{CRIT_MSG} elevated error rates persist across regions check status.example.com for updates and retry your request later"
    );
...
/// safe-open path — observed via the `ATELIER_TEST_OPEN_URL_FILE` seam, never a
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("ATELIER_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `ATELIER_ANNOUNCEMENTS_OVERRIDE`.
...
/// `ATELIER_TEST_OPEN_URL_FILE` seam + a `TERM_PROGRAM` pin for OSC 8).
