...
async fn managed_policy_gate_refusal_reaches_real_terminal() {
    ...
    let mut harness = PtyHarness::new(
        ...
        DEFAULT_COLS,
        &["--no-auto-update"],
        // ATELIER_MANAGED_CONFIG=0 disables the background refetch so the gate decision is deterministic and offline.
        &[
            ("ATELIER_HOME", home_str),
            ("ATELIER_MANAGED_CONFIG", "0"),
            ("NO_COLOR", "1"),
        ],
    )
    .expect("spawn pager");
