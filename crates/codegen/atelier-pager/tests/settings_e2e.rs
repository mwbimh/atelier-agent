...
fn make_state() -> SettingsModalState {
    ...
    atelier_pager::app::set_voice_mode_enabled_for_test(true);
...
fn render_with_filter_active_and_small_viewport_clamps_scroll() {
    ...
    atelier_pager::views::settings_modal::render_settings_modal(
...
fn defaults_round_trip_through_registry() {
    use atelier_pager::settings::{SettingValue, current_value_for};
    let reg = SettingsRegistry::defaults();
    ...
    atelier_pager::appearance::cache::set_keep_text_selection(
        atelier_pager::appearance::TextSelection::Flash,
    );
    atelier_pager::appearance::cache::set_show_thinking_blocks(true);
    atelier_pager::appearance::cache::set_prompt_suggestions(true);
    atelier_pager::appearance::cache::set_group_tool_verbs(true);
    atelier_pager::appearance::cache::set_scroll_mode(atelier_pager::appearance::ScrollMode::Auto);
    atelier_pager::appearance::cache::set_invert_scroll(false);
    // 3 = the registry default shown while the profile is in charge.
    atelier_pager::appearance::cache::set_scroll_lines(3);
    ...
    let expected = |key: &str| -> SettingValue {
        match key {
            ...
            "theme" => SettingValue::Enum("ateliernight"),
            "auto_dark_theme" => SettingValue::Enum("ateliernight"),
            "auto_light_theme" => SettingValue::Enum("atelierday"),
    ...
    for meta in reg.all() {
        ...
        let default_value = atelier_pager::settings::default_value_for(meta);
...
// `crates/codegen/atelier-pager/src/app/dispatch.rs::tests` (next to
...
fn pr4_mouse_click_in_theme_picker_is_no_op() {
    ...
    s.mode = SettingsModalMode::PickingEnum {
        key: "theme",
        choices_idx: 0,
        original_value: SettingValue::Enum("ateliernight"),
        supports_preview: true,
    };
    synth_rects(&mut s);
    let outcome = handle_settings_mouse(
...
fn pr6_permission_mode_picker_enter_dispatches_set_permission_mode_commit() {
    use atelier_pager::app::actions::PermissionModeKind;
    let reg = SettingsRegistry::defaults();
...
fn reset_overlay_dims_all_rows_except_target() {
    use atelier_pager::views::settings_modal::ResetConfirmOverlay;
    use ratatui::buffer::Buffer;
...
fn docs_footer_renders_for_browse_and_picker() {
    ...
    for fixture_label in ["browse", "picker"] {
        ...
        assert!(
            all_text.contains("Ask Atelier"),
            "[{fixture_label}] docs footer (`Ask Atelier`) must appear in the rendered modal:\n\
             {all_text}"
        );
        assert!(
...
fn render_modal_to_string(s: &mut SettingsModalState, width: u16, height: u16) -> String {
    ...
    let mut buf = Buffer::empty(area);
    atelier_pager::views::settings_modal::render_settings_modal(&mut buf, area, s, false, None);
...
fn restart_pill_hidden_when_edited_but_collapsed() {
    use atelier_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use atelier_shell::agent::config::UiConfig;
...
fn reset_confirm_prompt_helper_builds_well_formed_string_for_every_setting() {
    use atelier_pager::settings::{PagerLocalSnapshot, SettingsRegistry};
    use atelier_pager::views::modal::{ActiveModal, ModalConfirmation, reset_confirm_prompt};
    use atelier_shell::agent::config::UiConfig;
    let reg = SettingsRegistry::defaults();
    for meta in reg.all() {
...
fn pr8_default_model_and_max_thoughts_width_defaults_roundtrip() {
    ...
    assert_eq!(
        atelier_pager::settings::default_value_for(mt_meta),
...
fn pr10_plan_mode_picker_enter_dispatches_set_commit() {
    use atelier_pager::app::actions::PlanModeKind;
...
fn render_mermaid_picker_enter_dispatches_set_commit() {
    use atelier_pager::appearance::RenderMermaid;
...
fn keep_text_selection_picker_enter_dispatches_set_commit() {
    use atelier_pager::appearance::TextSelection;
    ...
    atelier_pager::appearance::cache::set_keep_text_selection(TextSelection::Flash);
    let mut s = make_state();
    navigate_to(&mut s, "keep_text_selection");
    let _ = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn keep_text_selection_hold_snapshot_seeds_picker_at_hold() {
    ...
    atelier_pager::appearance::cache::set_keep_text_selection(TextSelection::Hold);
    let ui = UiConfig {
...
fn invert_scroll_enter_dispatches_typed_setter() {
    atelier_pager::appearance::cache::set_invert_scroll(true);
    let mut s = make_state();
    navigate_to(&mut s, "invert_scroll");
    let outcome = handle_settings_key(&mut s, &press(KeyCode::Enter));
...
fn show_thinking_blocks_space_dispatches_typed_setter() {
    ...
    atelier_pager::appearance::cache::set_show_thinking_blocks(false);
    let mut s = make_state();
...
fn prompt_suggestions_space_dispatches_typed_setter() {
    // Pin off so space toggles to true.
    atelier_pager::appearance::cache::set_prompt_suggestions(false);
    let mut s = make_state();
...
fn group_tool_verbs_cache_off_dispatches_on() {
    ...
    atelier_pager::appearance::cache::set_group_tool_verbs(false);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
...
fn collapsed_edit_blocks_space_dispatches_typed_setter() {
    ...
    atelier_pager::appearance::cache::set_collapsed_edit_blocks(false);
    let mut s = make_state();
...
fn collapsed_edit_blocks_cache_on_dispatches_off() {
    ...
    atelier_pager::appearance::cache::set_collapsed_edit_blocks(true);
    let mut s = SettingsModalState::new(
        Arc::new(SettingsRegistry::defaults()),
        UiConfig::default(),
        PagerLocalSnapshot::default(),
