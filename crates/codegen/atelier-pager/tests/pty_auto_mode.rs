...
//!
//! Uses `atelier-pager-pty-harness` (`PtyHarness`) + Shift+Tab (CSI Z,
...
//! Always-Approve.
//!
//! Auth: seeds `HOME/.atelier/auth.json` from `ATELIER_AUTH_JSON` (path) or the
//! developer's `~/.atelier/auth.json` so the pager skips device-login when
...
//!
//! Run with:
//! `cargo test -p atelier-pager --test pty_auto_mode -- --ignored --nocapture`

use std::path::{Path, PathBuf};
use std::time::Duration;

use atelier_pager_pty_harness::{PtyHarness, pager_binary};
...
/// Prefer explicit path, else the user's real `~/.atelier/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("ATELIER_AUTH_JSON") {
    ...
    dirs_next_home()
        .map(|h| h.join(".atelier/auth.json"))
        .filter(|p| p.is_file())
}
...
fn prepare_sandbox(home: &Path, gate_on: bool) -> Vec<(String, String)> {
    ...
    let _ = std::fs::create_dir_all(&atelier);
    if let Some(src) = auth_json_source() {
        let dest = atelier.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.atelier/auth.json — may hit device login");
    }

    let home_s = home.display().to_string();
    let mut env = vec![
        ("HOME".into(), home_s.clone()),
        ("ATELIER_HOME".into(), atelier.display().to_string()),
    ...
    // layer below requirements; "1"/"0" parse to on/off (atelier_config::
    ...
    env.push((
        "ATELIER_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("ATELIER_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness = PtyHarness::new(&binary, ROWS, COLS, &[], &env_refs)
        .expect("spawn pager in PTY (atelier-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "ATELIER_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
