...
//!
//! These tests exercise the real `atelier-pager` binary through a PTY using
...
/// enterprise deploy report: with `ATELIER_GOAL=1`, `/goal` must
...
/// explicitly off (`ATELIER_GOAL=0`; goal mode defaults on), `/goal` must stay
...
/// Full folder-trust session: with `ATELIER_FOLDER_TRUST=1` and a git repo that
...
/// also pins `ATELIER_CONTEXTUAL_HINTS=0` (belt-and-suspenders); the same planning
