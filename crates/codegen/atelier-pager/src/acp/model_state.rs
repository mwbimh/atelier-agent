...
impl ModelState {
    ...
    /// not offer (e.g. `none` on atelier-4.5) are rejected so we fail in the TUI
    ...
    pub(crate) fn resolve_effort_token_for(
        ...
        // Canonical level (for example "high" or the distinct "max") only if the model menu
        // actually offers that value — not free-form power-user aliases that
        // would 400 on the server (e.g. `none` on atelier-4.5).
...
mod tests {
    ...
    fn update_catalog_preserves_user_effort_when_model_unchanged() {
        let id = acp::ModelId::new(Arc::from("atelier-build"));
        let mut state = ModelState::default();
        state.available.insert(
            id.clone(),
            model_with_effort("atelier-build", "Atelier", "high"),
        );
