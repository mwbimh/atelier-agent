...
pub struct AcpConnection {
    ...
    /// Whether the agent is a atelier-shell instance.
    pub is_atelier_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<atelier_announcements::RemoteAnnouncement>>
    /// Whether interactive login is required (deferred auth for `atelier.invalid`).
    ...
    /// disabled feature produces zero `atelier/recap` traffic. Defaults to `false`
    ...
    pub auth_manager: std::sync::Arc<atelier_shell::auth::AuthManager>,
}
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_atelier_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    let auth_manager = std::sync::Arc::new(atelier_shell::auth::AuthManager::new_local(
        &atelier_shell::util::atelier_home::atelier_home(),
    ));

    // Leader has no in-process agent; init this process's product telemetry client.
    atelier_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
fn apply_config_writes(flags: &ConnectFlags) {
    ...
    let config_path = atelier_shell::util::atelier_home::atelier_home().join("config.toml");
...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    serde_json::json!({
        "atelier/incrementalBashOutput": true,
        "atelier/hunkTracker": { "mode": hunk_mode },
        "atelier/bashOutputNoColor": true,
        "atelier/gitHeadChanged": true,
    })
}
...
async fn initialize(
    ...
    // Check if this is a atelier-shell agent
    let is_atelier_shell = resp
        .meta
        .as_ref()
        .and_then(|m| m.get("atelierShell"))
...
/// for a `atelier.invalid` or `oidc` method — these are the ones that can trigger
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "atelierShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "atelierShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    fn client_capabilities_meta_defaults_absent_or_blank_mode_to_agent_only() {
        ...
        assert_eq!(absent["atelier/hunkTracker"]["mode"], "agent_only");
        let blank = client_capabilities_meta(&ConnectFlags {
            hunk_tracker_mode: Some("   ".into()),
            ..Default::default()
        });
        assert_eq!(blank["atelier/hunkTracker"]["mode"], "agent_only");
    }
    ...
    fn client_capabilities_meta_canonicalizes_off_and_mixed_case() {
        ...
        for raw in ["off", "OFF", "Disabled"] {
            ...
            assert_eq!(meta["atelier/hunkTracker"]["mode"], "off", "raw={raw}");
        }
    }
}
