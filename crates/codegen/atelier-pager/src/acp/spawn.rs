...
//!
//! Simplified to only support AtelierShell (in-process) mode.
...
/// Spawn a AtelierShell agent in a background thread.
///
/// Returns the ACP client channel for communication and a cancellation token.
pub async fn spawn_atelier_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<atelier_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    let (agent_config, models_manager) =
        atelier_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
