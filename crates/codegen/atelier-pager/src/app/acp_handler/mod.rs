...
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    match notif.method.as_ref() {
        ...
        "atelier/follow_ups" => handle_follow_ups(notif, app),
        "atelier/task_backgrounded" => handle_task_backgrounded(notif, app),
        "atelier/task_completed" => handle_task_completed(notif, app),
        "atelier/models/update" => handle_models_update(notif, app),
        "atelier/settings/update" => handle_settings_update(notif, app),
        "atelier/sessions/changed" => handle_sessions_changed(notif, app),
        "atelier/queue/changed" => handle_queue_changed(notif, app),
        ...
        "atelier/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        "atelier/scheduled_task_inject_prompt" => handle_scheduled_task_inject_prompt(notif, app),
        "atelier/announcements/update" => handle_announcements_update(notif, app),
        "atelier/git_head_changed" => handle_git_head_changed(notif, app),
        "atelier/mcp/init_progress" => handle_mcp_init_progress(notif, app),
        "atelier/mcp/tools_changed" | "atelier/mcp_initialized" => {
            handle_mcp_tools_changed(notif, app)
        }
        "atelier/mcp/server_status" if push_server_status_enabled() => {
            handle_mcp_server_status(notif, app)
        }
        "atelier/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}

/// Handle `atelier/session/interjection` — the leader broadcasts this
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse atelier/session/interjection");
        return false;
    };
    ...
    // emits the queue-emptying `atelier/queue/changed` right after it).
