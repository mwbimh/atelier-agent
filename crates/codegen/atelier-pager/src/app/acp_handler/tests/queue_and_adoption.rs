#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// `atelier/queue/changed` broadcast, and an empty broadcast clears it.
    #[test]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        ...
        let notif = acp::ExtNotification::new("atelier/queue/changed", raw.into());
    ...
    /// broadcasts `atelier/queue/changed` with `runningPromptId` = that synthetic
    ...
    fn viewer_does_not_enter_turn_running_for_server_initiated_turn() {
        ...
        // the actor and emits NO `atelier/session/prompt_complete`. If a viewer
    ...
    fn viewer_enters_turn_running_for_scheduler_fired_cron_turn() {
        ...
        // `MvpAgent::prompt()` and DOES emit `atelier/session/prompt_complete`. So a
    ...
    fn viewer_prompt_complete_finishes_turn() {
        // A viewer in TurnRunning receives atelier/session/prompt_complete for its
