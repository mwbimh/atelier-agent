#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.atelier` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.atelier` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.atelier` reads).
    #[test]
