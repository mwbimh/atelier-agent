#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression: replay from `updates.jsonl` emits `atelier/session/update` (not
    ...
    fn ext_session_notification_and_update_equivalent_for_subagent_spawned() {
        ...
        let (spawn_notif, finish_notif) =
            run_subagent_lifecycle_via_method("atelier/session_notification", child_sid);
        let (spawn_update, finish_update) =
            run_subagent_lifecycle_via_method("atelier/session/update", child_sid);
    ...
    fn ext_session_update_malformed_params_returns_false() {
        ...
        assert!(
            !affected,
            "malformed atelier/session/update params must not redraw"
        );
        assert!(
