...
pub(super) fn compressed_entry(
    index: usize,
) -> atelier_shell::extensions::notification::ImageCompressedEntry {
    atelier_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn queue_changed_ext(session_id: &str, ids: &[&str]) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "atelier/queue/changed",
        std::sync::Arc::from(serde_json::value::to_raw_value(&params).unwrap()),
    )
}
/// Build a `atelier/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn make_inject_notif(payload: &serde_json::Value) -> acp::ExtNotification {
    ...
    acp::ExtNotification::new(
        "atelier/scheduled_task_inject_prompt",
...
pub(super) fn make_fired_notif(
    ...
    acp::ExtNotification::new("atelier/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn make_created_ext_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
/// Build an `atelier/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("atelier/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `atelier/session/prompt_complete` ext-notification with an explicit
...
/// Build an `atelier/session/prompt_complete` ext-notification carrying a
...
/// Build an `atelier/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `atelier/session/interjection` ext-notification with an optional
...
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/session/interjection", std::sync::Arc::from(raw))
}
...
/// Shared temp `ATELIER_HOME` for disk-replay tests. `atelier_home()` uses a
/// process-wide `OnceLock`, so parallel tests must not each set `ATELIER_HOME`
...
pub(super) fn replay_disk_test_home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
            ...
            unsafe {
                std::env::set_var("ATELIER_HOME", tmp.path());
            }
            tmp
        })
        .path()
}
/// Runs `f` with a thread-local atelier home override so disk replay tests do not
/// depend on process-wide `atelier_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_atelier_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_atelier_home_for_tests(None);
    out
}
...
/// Build an `atelier/session_notification` carrying
...
pub(super) fn interaction_resolved_ext(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/session_notification", std::sync::Arc::from(raw))
}
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = atelier_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn task_completed_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("atelier/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("atelier/models/update", std::sync::Arc::from(raw))
}
/// `atelier/models/update` carrying a single reasoning-capable model whose
...
/// Helper: build an `atelier/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("atelier/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: atelier_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use atelier_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = atelier_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "atelier_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName" : "atelier_com_linear", "tools" : [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("atelier/mcp_initialized", std::sync::Arc::from(raw))
}
