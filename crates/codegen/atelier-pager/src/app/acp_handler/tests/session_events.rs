#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_compaction_started_sets_activity() {
        ...
        let update = ExtensionSessionUpdate::AutoCompactStarted {
            tokens_used: 90000,
    ...
    fn apply_image_dropped_pushes_scrollback_block() {
        ...
        let update = ExtensionSessionUpdate::ImageDropped {
            notes: notes.clone(),
        };
    ...
    fn apply_retry_state_auth_failure_pushes_reauth_prompt() {
        ...
        apply_retry_state(
            &RetryState::Failed {
                ...
                message: "Unauthorized (401) from https://cli-chat-proxy.atelier.com/v1/messages: \
                          no auth context"
                    .into(),
            },
            &mut session,
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `ate logout` /
    /// `ate login` guidance), not the generic re-auth prompt.
    #[test]
    ...
    fn apply_compaction_completed_defers_message_until_turn_end() {
        ...
        let update = ExtensionSessionUpdate::AutoCompactCompleted {
    ...
    fn apply_unhandled_event_returns_false() {
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        let update = ExtensionSessionUpdate::MemoryFlushStarted;
        assert!(!apply_session_event(&update, &mut session, &mut scrollback, false));
    }
    ...
    fn child_compact_started_does_not_reset_context_used() {
        ...
        child_view.context_state = Some(atelier_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
