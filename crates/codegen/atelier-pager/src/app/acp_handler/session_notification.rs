...
/// Handle `atelier/session_notification` and replay-path `atelier/session/update`.
///
    ...
    let matched = match find_session_match(app, &session_notif.session_id) {
        ...
        None => {
            tracing::debug!(
                ...
                "load-race: atelier/session_notification DROPPED — no agent matches session_id"
            );
            return false;
        }
    };
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        atelier_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        atelier_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        atelier_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[atelier_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use atelier_tools::types::SessionMode;
