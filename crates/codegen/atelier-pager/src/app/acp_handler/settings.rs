...
/// Handle `atelier/models/update` — model list changed (etag-triggered refresh).
pub(super) fn handle_models_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    if let Ok(model_state) = serde_json::from_str::<acp::SessionModelState>(notif.params.get()) {
        ...
        tracing::info!(
            ...
            "models updated via atelier/models/update"
        );
        ...
        tracing::warn!("Failed to parse atelier/models/update");
        false
    }
}

/// Handle `atelier/settings/update` — remote settings refreshed on `/new`.
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(update) = serde_json::from_str::<PagerSettingsUpdate>(notif.params.get()) else {
        tracing::warn!("Failed to parse atelier/settings/update");
        return false;
    };
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = atelier_shell::config::load_effective_config().ok();
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("ATELIER_SESSION_PICKER_GROUPED")
            .ok()
    ...
    let (requirements, user_config, managed_config) = (
        atelier_shell::config::load_merged_requirements(),
        atelier_shell::config::load_from_disk().ok(),
        atelier_shell::config::load_managed_config().ok(),
    );
    ...
    let resolved = atelier_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = atelier_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use atelier_shell::util::config::resolve_tips;
    ...
    tracing::info!("settings updated via atelier/settings/update");
    true
}
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = atelier_shell::util::config::resolve_permission_mode(effective_ui, remote);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        atelier_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}

/// Tell live sessions to leave Auto on the mid-session kill-switch: fire the
/// `atelier/yolo_mode_changed` notification the agent maps to
...
/// Handle `atelier/sessions/changed` — the leader broadcasts roster
...
pub(super) fn handle_sessions_changed(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(changed) = serde_json::from_str::<crate::app::roster::RosterChanged>(notif.params.get())
        ...
        tracing::warn!("Failed to parse atelier/sessions/changed");
        return false;
    };
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<atelier_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `ATELIER_ANNOUNCEMENTS_OVERRIDE` first, so a
/// backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[atelier_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = atelier_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = atelier_announcements::filter_expired(merged);
    ...
    if atelier_announcements::prune_hidden_announcement_ids(
...
) -> Option<atelier_announcements::RemoteAnnouncement> {
...
/// Deserialization type for the `atelier/settings/update` notification payload.
///
...
/// `atelier-shell/src/agent/mvp_agent.rs` when adding fields that both sides
/// need.
