...
fn enqueue_permission(
    ...
    let bash_selection_count = bash_highlights
        .as_ref()
        .map(|h| atelier_workspace::permission::default_always_allow_scope(&h.highlighted_words))
        .unwrap_or(0);
    ...
    let mcp_scope = perm
        ...
        .and_then(|m| {
            serde_json::from_value::<atelier_workspace::permission::McpToolPermission>(
...
/// `atelier-tools` for typed access to `command` and `description`.
...
fn build_permission_display(
    ...
    let bash_input = req.tool_call.fields.raw_input.as_ref().and_then(|v| {
        serde_json::from_value::<atelier_tools::implementations::BashToolInput>(v.clone()).ok()
    });
    ...
    let title = if sandbox_override {
        ...
        if let Some(path) = file_path {
            ...
            format!(
                "Allow {}?",
                atelier_workspace::permission::mcp_pretty_name_if_qualified(t)
            )
        } else {
