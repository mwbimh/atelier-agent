...
pub enum SwitchModelError {
    ...
    /// `acp::Error.data`.
    IncompatibleAgent {
        error: atelier_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    /// `prompt_id`. Routed to the agent as `atelier/queue/remove`;
    /// the resulting `atelier/queue/changed` rebroadcast is the source of truth.
    QueueRemoveShared {
        id: String,
        expected_version: u64,
    },
    /// Reorder the server-authoritative (shared) queued prompts to match
    /// `ordered_ids`. Routed as `atelier/queue/reorder`.
    QueueReorderShared {
        ordered_ids: Vec<String>,
    },
    /// Clear the caller's server-authoritative (shared) queued prompts.
    /// Routed as `atelier/queue/clear`.
    QueueClearShared,
    /// Replace the text of a server-authoritative (shared) queued prompt.
    /// Routed to the agent as `atelier/queue/edit`; the rebroadcast of
    /// `atelier/queue/changed` is the source of truth. Last write wins via the
    ...
    /// merges its text into the in-flight turn. Routed as `atelier/queue/interject`;
    /// the `atelier/session/interjection` + `atelier/queue/changed` rebroadcasts are
    ...
    OpenTranscriptPager,
    /// Minimal mode (`atelier --minimal`): re-print the most-recently committed
    ...
    /// Delete an MCP server via atelier/mcp/delete.
    DeleteMcpServer {
        server_name: String,
    },
    ...
    /// Toggle a skill enable/disable via atelier/skills/toggle.
    ToggleSkill {
        skill_name: String,
        enabled: bool,
    },
    ...
    AnnouncementsOpenCta(atelier_telemetry::events::AnnouncementCtaSurface),
    ...
    /// Commit the active theme (canonical name, e.g. `"ateliernight"`, `"auto"`).
    SetTheme(String),
    ...
    /// Open the Agent Dashboard view (`/dashboard`, `Ctrl+\`, `atelier dashboard`).
    OpenDashboard,
...
///
/// Both variants write to `~/.atelier/config.toml` and route ACP
/// `atelier/yolo_mode_changed` notifications. The ACP notification is
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: atelier_workspace::foreign_sessions::EnabledForeignSessionSources,
        atelier_home: std::path::PathBuf,
    ...
    FetchSessionList {
        /// Text search pushed down to `atelier/session/list` as `query` (chat
    ...
    /// `atelier/sessions/list`. Only issued in leader mode while the
    /// dashboard is open.
    FetchRoster,
    /// Fetch the local on-disk session list (dormant/idle sessions) for the
    /// dashboard via `atelier/session/list` — the non-leader fallback for the
    ...
    /// Cancel a subagent via `atelier/subagent/cancel`.
    KillSubagent {
        session_id: acp::SessionId,
        subagent_id: String,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.atelier/config.toml`.
    ...
    /// Persist a typed setting to `~/.atelier/config.toml`. On failure,
    ...
    /// `atelier/queue/remove`. The agent re-broadcasts the authoritative queue.
    QueueRemove {
        session_id: acp::SessionId,
        id: String,
        expected_version: u64,
    },
    /// Reorder server-owned queued prompts: fire-and-forget `atelier/queue/reorder`.
    QueueReorder {
        session_id: acp::SessionId,
        ordered_ids: Vec<String>,
    },
    ...
    /// `atelier/queue/edit`. The session actor's serialized mailbox makes this
    /// last-writer-wins for concurrent edits; the rebroadcast of
    /// `atelier/queue/changed` is the truth signal.
    QueueEdit {
        session_id: acp::SessionId,
        id: String,
        new_text: String,
    ...
    /// fire-and-forget `atelier/queue/interject`. The session actor atomically
    ...
    /// Resolve the running agent name for a session (`atelier/session/info`).
    ...
    /// Fetch MCP server list from the shell (atelier/mcp/list).
    FetchMcpsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
        cache: bool,
    },
    /// Trigger MCP OAuth for a server (atelier/mcp/auth_trigger).
    McpAuthTrigger {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    },
    /// Fetch hooks list from the shell (atelier/hooks/list).
    FetchHooksList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Fetch plugins list from the shell (atelier/plugins/list).
    FetchPluginsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Fetch skills list from the shell (atelier/skills/list).
    FetchSkillsList {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    /// Toggle a skill via atelier/skills/toggle (enable/disable without restart).
    ToggleSkill {
        agent_id: AgentId,
        session_id: acp::SessionId,
        skill_name: String,
    ...
    /// Reload plugins after a CTA install via `atelier/plugins/action`
    ...
    /// Read the MCP server list after a CTA install via `atelier/mcp/list`,
    ...
    /// the same `atelier/mcp/list` fetch as `FetchPluginCtaMcps`, reported back via
    ...
    /// Upsert an MCP server via atelier/mcp/upsert.
    UpsertMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        name: String,
    ...
    /// Live-toggle an MCP server via atelier/mcp/toggle (no restart needed).
    ToggleMcpServer {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Toggle a single MCP tool via atelier/mcp/toggle_tool.
    ToggleMcpTool {
        agent_id: AgentId,
        session_id: acp::SessionId,
        server_name: String,
    ...
    /// Fetch and display detailed context usage via atelier/session/info.
    ShowContextInfo {
        agent_id: AgentId,
        session_id: acp::SessionId,
    },
    ...
    /// Send raw note to atelier/memory/rewrite for LLM-powered reformatting.
    ...
    /// Fire a /btw side question via atelier/btw ext method.
    SendBtw {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    /// Send a mid-turn interjection via atelier/interject ext method.
    SendInterject {
        agent_id: AgentId,
        session_id: acp::SessionId,
        text: String,
        /// Client-minted id echoed back on the `atelier/session/interjection`
    ...
    /// registry (`~/.atelier/active_sessions.json`).
    ...
    /// Call `atelier/session/fork` to create a peer session that resumes
    ...
    /// Send an ACP `atelier/suggest` request to the shell. `agent_id` is echoed
    ...
    /// Send an ACP `atelier/suggestPrompt` request to the shell — predict the
    /// user's likely next prompt after a completed turn (tab autocomplete
    /// ghost text).
    FetchPromptSuggestion {
        agent_id: AgentId,
        generation: u64,
        /// Suggestion model resolved by the pager (`atelier-build-0.1` when the
...
/// Outcome of an `atelier/subagent/cancel` request, telling dispatch whether the
...
pub enum TaskResult {
    ...
    WorktreeForked {
        ...
        restore_degree: Option<atelier_workspace::session::git::RestoreDegree>,
    },
    ...
    SessionLoaded {
        ...
        /// `_meta["atelier/runningPromptId"]`), present only when the session was
    ...
    SessionListLoaded {
        ...
        /// Degraded conversations lane (`_meta["atelier/partial"]`), surfaced
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<atelier_workspace::foreign_sessions::RecentForeignSession>,
    },
    ...
    /// Leader session roster loaded via `atelier/sessions/list`.
    RosterLoaded {
    ...
    CancelComplete,
    /// Response to `atelier/subagent/cancel`; see [`SubagentKillOutcome`].
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<atelier_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<atelier_shell::util::changelog::ChangelogEntry>,
    },
    ...
    AuthUrlReady {
        ...
        external: bool,
        /// Presentation mode from `atelier/auth/get_url`; `None` on older agents.
        mode: Option<String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<atelier_tools::implementations::skills::types::SkillInfo>, String>,
    },
    ...
    AuthCopiedTimeout,
    DeepSearchResults {
        results: Vec<atelier_shell::extensions::session_search::SearchSessionHit>,
        seq: u64,
    },
    /// `atelier/session/fork` completed (no-worktree path). The pager adopts
    ...
    /// `atelier/session/fork` failed. The placeholder agent stays in
    ...
    /// Shell suggestions loaded from ACP `atelier/suggest`. `request_text` /
    ...
    /// Predicted next prompt loaded from ACP `atelier/suggestPrompt`.
