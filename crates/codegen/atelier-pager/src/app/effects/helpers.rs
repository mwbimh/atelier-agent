...
/// Prefer a Provider-supplied error detail when available.
/// All errors are sanitized to remove internal service names and jargon.
...
pub(crate) struct SessionFlags {
    ...
    /// Active auth marker retained in the shared session-flags shape.
    /// Error display no longer varies by auth method.
