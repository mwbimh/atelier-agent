...
/// Run the post-CTA-install `atelier/mcp/list` read (uncached, which also nudges
...
pub(super) fn format_acp_error(err: &acp::Error, is_api_key_auth: bool) -> String {
    ...
    if err.code == acp::ErrorCode::InvalidParams && let Some(data) = &err.data
        && let Some(msg) = atelier_shell::sampling::error::error_detail_from_data(data)
        && !msg.is_empty()
    {
        return msg;
    }
...
) -> (bool, Option<String>, Option<atelier_workspace::session::git::RestoreDegree>) {
...
/// CANONICAL wire parser for `LoadSessionResponse._meta["atelier/runningPromptId"]`.
///
...
pub(crate) fn parse_session_load_running_prompt_id(
    ...
    resp_meta
        .and_then(|m| m.get("atelier/runningPromptId"))
        .and_then(|v| v.as_str())
        .map(String::from)
}
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `atelier-build` (default)         | `false`            |
/// | false | true      | false    | `atelier-build` (default)         | `false`            |
/// | false | false     | true     | `atelier-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `atelier-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `atelier-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `atelier-build-plan`              | `false`            |
/// | true  | false     | true     | `atelier-build-plan-no-subagents` | omitted (shell gate) |
/// | true  | true      | true     | `atelier-build-plan`              | omitted (shell gate) |
///
/// When [`Self::chat_mode`] is set (gateway light-frontend / `--chat`), Build
/// `agentProfile` injection is omitted (K12) and `_meta["atelier/session"].kind`
...
pub(crate) struct SessionFlags {
    ...
    /// Injected as `atelier/restore_code` into `LoadSession` meta, or passed
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `atelier-build` profile (no `_meta`
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("atelier-build-plan"),
            (true, false, _) => Some("atelier-build-plan-no-subagents"),
            (false, _, true) => Some("atelier-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    /// `ATELIER_AGENT` — the env var chooses the *agent*, but the tool-strip is
    /// independent. Chat mode additionally stamps `atelier/session.kind`.
    pub(super) fn to_meta(&self) -> Option<acp::Meta> {
        let mut meta = serde_json::Map::new();
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("ATELIER_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("ATELIER_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
        {} else if let Some(profile) = self.agent_profile() {
            meta.insert("agentProfile".into(), serde_json::json!(profile));
        }
        if self.chat_mode {
            meta.insert("atelier/session".into(), serde_json::json!({ "kind" : "chat" }));
        }
        if !self.ask_user {
...
pub(super) const CHAT_FORBIDDEN_WORKSPACE_BIND_KEYS: &[&str] = &[
    "envId",
    "atelier/cloud_server_id",
    "atelier/cloud_existing_workspace",
];
/// Stamp `_meta["atelier/session"].kind = "chat"` and strip Build `agentProfile` (K12).
pub(super) fn apply_chat_kind_meta(meta: &mut Option<acp::Meta>) {
    let obj = meta.get_or_insert_with(acp::Meta::new);
    obj.insert("atelier/session".into(), serde_json::json!({ "kind" : "chat" }));
...
pub(super) fn extract_first_user_prompt(
    info: &atelier_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = atelier_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use atelier_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
/// Degraded conversations lane on `atelier/session/list`, parsed from the
/// response's `_meta["atelier/partial"]` envelope.
...
/// Read `_meta["atelier/partial"]` from a session-list payload. `None` when the
...
pub(super) fn parse_session_list_partial(
    ...
    let partial = payload.get("_meta")?.get("atelier/partial")?;
...
/// Parse the `atelier/session/list` response payload (the unwrapped
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            let is_conversation = v
                .get("_meta")
                .and_then(|m| m.get("atelier/session"))
            ...
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = atelier_shell::session::info::Info {
        ...
        .filter_map(|mut e| {
            ...
            if e.source == "remote"
                && atelier_shell::session::resolve_local_session_any_cwd(&e.id)
                    .is_some()
            {
...
pub(super) async fn send_check_subscription(
    ...
    let req = acp::ExtRequest::new(
        "atelier/auth/check_subscription",
        serde_json::value::to_raw_value(&serde_json::json!({}))
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            atelier_shell::util::config::set_compact_mode(b)
                .await
        ...
        "show_timestamps" => {
            ...
            atelier_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            atelier_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            atelier_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            atelier_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            atelier_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            atelier_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            atelier_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            atelier_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            atelier_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        "theme" => {
            ...
            atelier_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            atelier_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            atelier_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            atelier_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            atelier_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            atelier_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            atelier_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            atelier_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            atelier_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            atelier_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            atelier_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            atelier_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            atelier_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            atelier_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            atelier_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            atelier_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            atelier_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            atelier_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            atelier_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            atelier_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            atelier_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            atelier_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            atelier_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            atelier_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            atelier_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            atelier_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            atelier_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            atelier_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
/// 2. Fire ACP `atelier/yolo_mode_changed` (gated on disk success for
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = atelier_shell::util::config::update_config(|cfg| {
...
/// Whether to fire the ACP `atelier/yolo_mode_changed` notification.
...
/// serializes (`atelier_shell::extensions::task::KillTaskResponse` +
/// `atelier_shell::session::result::ExtMethodResult`) so the contract stays
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<atelier_tools::types::KillOutcome> {
    use atelier_shell::extensions::task::KillTaskResponse;
    use atelier_shell::session::result::ExtMethodResult;
    serde_json::from_str::<ExtMethodResult<KillTaskResponse>>(resp)
        .ok()
        .and_then(|envelope| envelope.result)
        .map(|payload| payload.outcome)
}
/// Map an `atelier/subagent/cancel` response (payload under `result`) to a kill
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use atelier_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: atelier_shell::extensions::billing::BillingConfig,
...
/// Fetch the user's auto top-up rule via the `atelier/auto-topup-rule` extension.
...
/// Map an `atelier/auto-topup-rule` payload to an [`AutoTopupFetch`]. A body that
...
pub(super) fn parse_auto_topup_response(
    ...
    use atelier_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.atelier` lock must never
...
pub(super) fn unregister_active_session_best_effort(session_id: &acp::SessionId) {
    unregister_active_session_best_effort_in(
        &atelier_shell::util::atelier_home::atelier_home(),
        session_id,
    );
}
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match atelier_shell::active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
