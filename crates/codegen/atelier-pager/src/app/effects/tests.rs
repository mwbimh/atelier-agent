...
/// legacy `atelier/interject` wire shape stays byte-identical.
#[test]
...
fn picker_keeps_conversation_with_empty_cwd_and_missing_updated_at() {
    let payload = serde_json::json!(
        ...
        "Compare GPU vendors", "source" : "conversation", "_meta" : { "atelier/session" : {
        "kind" : "chat" } } }] }
    );
...
fn picker_keeps_old_conversation_past_cutoff() {
    let payload = serde_json::json!(
        ...
        "_meta" : { "atelier/session" : { "kind" : "chat" } } }] }
    );
    let entries = parse_session_picker_entries(&payload);
...
/// Untitled atelier.invalid chats must stay listed, rendered as "Untitled".
#[test]
...
fn session_list_partial_parses_reasons() {
    let payload = |reason: &str| {
        serde_json::json!(
            { "sessions" : [], "_meta" : { "atelier/partial" : { "conversations" : true,
            "reason" : reason } } }
        )
    };
    assert_eq!(
...
fn session_list_partial_absent_for_healthy_or_meta_less_responses() {
    let healthy = serde_json::json!(
        { "sessions" : [], "_meta" : { "atelier/partial" : { "conversations" : false } } }
    );
...
fn parse_kill_outcome_reads_result_envelope() {
    use atelier_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use atelier_workspace::session::git::RestoreDegree;
...
/// Redirect `ATELIER_HOME` to a tempdir for test isolation.
fn setup_atelier_home_in_tempdir() -> tempfile::TempDir {
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use atelier_shell::active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
        atelier_shell::active_sessions::list_in(dir.path()).expect("list").is_empty(),
...
fn unregister_best_effort_is_nonblocking_under_lock_contention() {
    ...
    assert_eq!(
        atelier_shell::active_sessions::list_in(dir.path()).expect("list").len(), 1,
...
async fn persist_permission_mode_acp_notification_fires_once_on_best_effort() {
    ...
    let _guard = setup_atelier_home_in_tempdir();
    ...
    assert_eq!(
        ...
        "ACP `atelier/yolo_mode_changed` notification must fire exactly once \
        ...
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                match args.request.method.as_ref() {
                    ...
                    "atelier/plugins/notify-updates" => {
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: atelier_workspace::foreign_sessions::EnabledForeignSessionSources::default(),
            atelier_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn fetch_session_list_pushes_query_and_echoes_seq() {
    ...
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            if let AcpAgentMessage::ExtMethod(args) = msg {
                assert_eq!(args.request.method.as_ref(), "atelier/session/list");
...
fn agent_profile_names_are_valid_builtins() {
    use std::str::FromStr;
    use atelier_agent::config::BuiltinAgentName;
    let test_cases: &[(SessionFlags, &str)] = &[
        ...
        (
            ...
            "atelier-build-plan-no-subagents",
        ),
        (
            SessionFlags {
                plan_mode: true,
...
/// Default flags produce no agent profile (uses atelier-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (atelier-build already has TaskTool).
#[test]
...
/// Neutralize `ATELIER_AGENT` for the profile-matrix tests below: agent-driven
...
/// Callers must be `#[serial_test::serial(ATELIER_AGENT)]` (process-global env).
fn without_atelier_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("ATELIER_AGENT", "")
}
...
#[serial_test::serial(ATELIER_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_atelier_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "atelier-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "atelier-build-plan-no-subagents");
...
/// --ask-user alone selects the atelier-build-ask-user profile.
#[serial_test::serial(ATELIER_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "atelier-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// does not need an `agentProfile` (default `atelier-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce atelier-build-plan
...
fn to_meta_chat_mode_stamps_kind_and_omits_agent_profile() {
    ...
    assert_eq!(meta["atelier/session"] ["kind"], "chat");
    assert!(
...
fn chat_load_meta_never_includes_workspace_bind_keys() {
    ...
    apply_chat_kind_meta(&mut meta);
    {
        ...
        obj.insert("atelier/cloud_server_id".into(), serde_json::json!("srv-poison"));
        obj.insert(
            "atelier/cloud_existing_workspace".into(),
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> atelier_shell::session::SessionInfoResponse {
    use atelier_shell::session::acp_types::{ContextInfo, SessionInfoData};
    atelier_shell::session::SessionInfoResponse {
...
fn format_session_info_shows_resolved_when_enabled_and_different() {
    let info = make_session_info("atelier-4.5", Some("atelier-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: atelier-4.5 (atelier-4.3)"));
}
#[test]
fn format_session_info_hides_resolved_when_disabled() {
    let info = make_session_info("atelier-4.5", Some("atelier-4.3"), 1000, 10000);
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model: atelier-4.5"));
    assert!(! text.contains("atelier-4.3"));
}
#[test]
fn format_session_info_no_parens_when_resolved_matches_requested() {
    let info = make_session_info("atelier-4.5", Some("atelier-4.5"), 1000, 10000);
    let text = format_session_info(&info, None, true);
    assert!(text.contains("Model: atelier-4.5"));
    assert!(! text.contains("(atelier-4.5)"));
}
#[test]
...
fn format_session_info_shows_model_hash_for_coding_slug_without_flag() {
    let mut info = make_session_info("atelier-build", None, 1000, 10000);
    info.data.model_fingerprint = Some("abc123".into());
    info.data.show_model_fingerprint = false;
    let text = format_session_info(&info, None, false);
    assert!(text.contains("Model Hash: abc123"));
...
fn session_picker_entry_maps_to_dormant_roster_row() {
    ...
    let entry = SessionPickerEntry {
        ...
        model_id: Some("atelier-4".to_string()),
        num_messages: 3,
    ...
    assert_eq!(roster.model_id.as_deref(), Some("atelier-4"));
