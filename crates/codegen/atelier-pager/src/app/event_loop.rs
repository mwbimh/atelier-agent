...
struct AgentLoadOutcome {
    ...
    success: bool,
    /// `atelier/runningPromptId` from the reload response: the turn another
...
/// [`decide`](atelier_workspace::folder_trust::decide) precedence.
///
...
fn seed_trust_state(
    ...
    use atelier_workspace::trust::workspace_key;
...
pub(crate) async fn run(
    ...
    if args.log_sampling {
        ...
        unsafe { std::env::set_var("ATELIER_LOG_SAMPLING", "1") };
    }
    ...
    let launch_effective_ui = atelier_shell::config::load_effective_config()
        .ok()
    ...
    app.require_plan_approval = atelier_shell::util::config::load_require_plan_approval();
    ...
    app.session_picker_grouped = std::env::var("ATELIER_SESSION_PICKER_GROUPED")
        ...
        .or_else(|| {
            atelier_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            // --force-login: find the atelier.invalid method from the advertised list
            let atelier_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "atelier.invalid");
            if let Some(method) = atelier_com {
                ...
                // No atelier.invalid method available, use the first method as fallback
        ...
        // fail-closed: do not invent atelier.invalid / auto-start OIDC.
        tracing::info!(
    ...
    let mut post_render_effects = if needs_interactive_login {
        if connection.auth_methods.is_empty() {
            ...
            app.auth_state = super::app_view::AuthState::Pending {
                error: Some(
                    atelier_shell::agent::auth_method::PREFERRED_API_KEY_UNAVAILABLE.to_string(),
                ),
            };
            vec![]
        } else {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<atelier_shell::auth::AuthMeta>(meta.clone()) {
    ...
    let requirements = atelier_shell::config::load_merged_requirements();
    ...
    let effective_config = match atelier_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            app.voice_config = atelier_voice::VoiceConfig::from_config_table(table);
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage
    // to atelier-cli server-side (mirrors sampler / imagine). Done after
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        atelier_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        atelier_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        atelier_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
...

    {
        ...
        app.active_announcements = atelier_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = atelier_shell::util::tips::pick_and_advance(&app.tips, &atelier_home);
        }
    }

    let hints = atelier_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.atelier/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        atelier_shell::util::config::clamped_display_permission_mode(
            atelier_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        atelier_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = atelier_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = atelier_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // ATELIER_SCROLL_SPEED/_MODE/_LINES + ATELIER_INVERT_SCROLL env overrides
    // apply on first load).
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<atelier_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`atelier "fix the bug"`). When
    ...
    // `atelier dashboard` startup: open the dashboard view immediately. The
    // CLI subcommand wrote a `ATELIER_OPEN_DASHBOARD_AT_STARTUP=1` env var
    // so we don't have to thread a flag through every arg struct.
    if std::env::var("ATELIER_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(atelier_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice pipeline could not start — restart atelier");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.atelier/config.toml [ui]`
                // rather than `~/.atelier/pager.toml`. The watcher only knows
                        ...
                        let join_handle = tokio::spawn(async move {
                            ...
                            let ok = tokio::time::timeout(timeout, async {
                                ...
                                for (agent_id, plan) in load_plans {
                                    ...
                                    let mcp_servers = atelier_shell::util::config::load_mcp_servers(
                                        &plan.cwd,
...
pub(crate) fn load_initial_ui_config() -> atelier_shell::agent::config::UiConfig {
    use atelier_shell::agent::config::UiConfig;
    let Ok(root) = atelier_shell::config::load_effective_config() else {
