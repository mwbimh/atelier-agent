...
fn effective_cancel_subagents_preference(
    ...
    ui: &atelier_shell::agent::config::UiConfig,
) -> Option<bool> {
...
pub(super) fn do_cancel_turn(app: &mut AppView, cancel_subagents: bool) -> Vec<Effect> {
    ...
    // FRONT queued prompt as the next turn — its `atelier/queue/changed`
...
/// Grace window between a driver-side `atelier/session/prompt_complete`
...
/// Finish turns whose end was announced by `atelier/session/prompt_complete`
...
// TODO: Add dispatch_cancel_command() once atelier-shell supports proper
