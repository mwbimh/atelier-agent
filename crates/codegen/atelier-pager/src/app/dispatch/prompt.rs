...
/// Start (if needed) and submit the initial prompt from `atelier "<prompt>"`.
///
...
pub(super) fn dispatch_show_undo_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(atelier_telemetry::events::ContextualTip {
            tip: atelier_telemetry::events::ContextualTipKind::Undo,
...
pub(in crate::app) fn show_small_screen_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(atelier_telemetry::events::ContextualTip {
            tip: atelier_telemetry::events::ContextualTipKind::SmallScreen,
            action: atelier_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_show_plan_nudge(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(atelier_telemetry::events::ContextualTip {
            tip: atelier_telemetry::events::ContextualTipKind::PlanMode,
            action: atelier_telemetry::events::ContextualTipAction::Shown,
        });
    }
    vec![]
...
pub(super) fn dispatch_show_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(atelier_telemetry::events::ContextualTip {
            tip: atelier_telemetry::events::ContextualTipKind::WordSelect,
            action: atelier_telemetry::events::ContextualTipAction::Shown,
        });
    }
...
pub(super) fn dispatch_accept_word_select_tip(app: &mut AppView) -> Vec<Effect> {
    ...
    log_event(atelier_telemetry::events::ContextualTip {
        tip: atelier_telemetry::events::ContextualTipKind::WordSelect,
        action: atelier_telemetry::events::ContextualTipAction::Accepted,
    });
...
fn maybe_show_send_now_tip(app: &mut AppView) {
    ...
    if agent.show_ephemeral_tip(
        ...
        log_event(atelier_telemetry::events::ContextualTip {
            tip: atelier_telemetry::events::ContextualTipKind::SendNow,
            action: atelier_telemetry::events::ContextualTipAction::Shown,
        });
    }
}
...
pub(super) fn dispatch_send_prompt_inner(
    ...
    let global_models_for_slash = if !literal && text.trim().starts_with('/') {
        Some(app.models.clone())
    } else {
        None
    };
    ...
    // Keep slash-command validation aligned with the refreshed global model
    // catalog. Sessions created before `/provider refresh` may not have
    // received a session-local model snapshot yet.
    if let Some(global_models) = global_models_for_slash
        && agent.session.models.is_empty()
        && !global_models.is_empty()
    {
        agent
            .session
            .models
    ...
    if !literal && trimmed.starts_with('/') {
        ...
        let exec_result = {
            ...
            if let Some(invocation) = parse_invocation(trimmed) {
                ...
                {
                    use atelier_telemetry::events::{PagerCommandSource, PagerSlashCommand};
                    use atelier_telemetry::session_ctx::log_event;
        ...
        // `atelier/queue/changed`. We render an optimistic echo into the shared
    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
            ...
            && agent.session.current_prompt_id.as_deref() != Some(response_pid)
        {
                ...
                && crate::app::acp_handler::is_server_initiated_prompt(response_pid)
            {
                ...
                // later `atelier/queue/changed` broadcast can't re-pin a
