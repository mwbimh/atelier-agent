...
use atelier_telemetry::events::{SuperAtelierUpsell, SuperAtelierUpsellClicked};
...
/// Whether the user is at the highest subscription tier (SuperAtelier Heavy).
///
...
pub(super) fn is_max_tier(subscription_tier: Option<&str>) -> bool {
    ...
    // keys ("superatelier_heavy") and CCP display names ("SuperAtelier Heavy").
    t.to_ascii_lowercase().replace(' ', "_") == "superatelier_heavy"
}

/// URL for upgrading the subscription tier.
pub(crate) const UPSELL_URL_UPGRADE: &str =
    "https://atelier.invalid/superatelier?referrer=atelier-build";

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) const UPSELL_URL_PAYG: &str = "https://atelier.invalid?_s=usage";
...
pub(crate) const FREE_USAGE_USER_MESSAGE: &str = "You\u{2019}ve reached your free Atelier usage limit for now. Get SuperAtelier for much higher limits, or try again later: https://atelier.invalid/superatelier?referrer=atelier-build";
...
///
/// **`max_tier = true`** (positively identified as SuperAtelier Heavy):
...
pub(super) fn open_credit_limit_upsell(
    ...
    let (
        ...
        atelier_telemetry::events::CreditLimitChoice,
        bool,
    ) = match mode {
        CreditLimitUpsellMode::UnifiedCredits => (
            ...
            "Buy more credits",
            "Purchase credits to keep using Atelier",
    ...
    if max_tier {
        use crate::scrollback::block::RenderBlock;
        log_event(atelier_telemetry::events::CreditLimitUpsellShown {
    ...
    .with_local_kind(LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            atelier_telemetry::events::CreditLimitChoice::UpgradeTier,
            second_choice,
        ],
    })
    .with_no_freeform();
...
/// `SuperAtelierUpsellShown` funnel event.
pub(super) fn open_free_usage_upsell(agent: &mut AgentView, auth_method: Option<String>) {
    open_superatelier_upsell(agent, UpsellReason::FreeUsageLimit, auth_method);
}

/// Open the SuperAtelier upsell for a tier-restricted slash command
...
pub(super) fn open_restricted_command_upsell(
    ...
    open_superatelier_upsell(agent, UpsellReason::RestrictedCommand, auth_method)
}

/// Which situation opened the SuperAtelier upsell modal. Controls the heading
...
fn open_superatelier_upsell(
    ...
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit => (
            ...
            SuperAtelierUpsell::FreeUsagePaywall,
            "free-usage-upsell",
        ),
        UpsellReason::RestrictedCommand => (
            "Unlock all features with SuperAtelier.",
            SuperAtelierUpsell::RestrictedCommand,
            "restricted-command-upsell",
        ),
    };

    log_event(atelier_telemetry::events::SuperAtelierUpsellShown {
        source,
        auth_method,
    });
    ...
    let options = vec![
        QuestionOption {
            label: "Upgrade to SuperAtelier".into(),
            description: "For everyday coding and productivity tasks".into(),
            preview: None,
            id: Some(UPSELL_URL_UPGRADE.into()),
        },
        QuestionOption {
            label: "Upgrade to SuperAtelier Heavy".into(),
            description: "Get the most out of Atelier. Highest usage limits.".into(),
            preview: None,
            // No Heavy-specific URL exists; the /superatelier page lists
...
/// `atelier/auth/check_subscription` completed. Meta is authoritative
...
pub(super) fn handle_check_subscription_complete(
    ...
    let applied = match meta {
        Some(meta_val) => {
            match serde_json::from_value::<atelier_shell::auth::AuthMeta>(meta_val) {
                Ok(auth_meta) => {
...
pub(super) fn handle_credit_limit_recheck_complete(
    ...
    if let Some(meta_val) = meta
        && let Ok(auth_meta) = serde_json::from_value::<atelier_shell::auth::AuthMeta>(meta_val)
    {
        app.apply_auth_meta(&auth_meta);
    }
...
pub(super) fn dispatch_open_superatelier_url(app: &mut AppView) -> Vec<Effect> {
    log_event(SuperAtelierUpsellClicked {
        source: SuperAtelierUpsell::WelcomeScreen,
    ...
    // Funnel attribution: tag CLI-originated SuperAtelier upsell clicks
    // with `referrer=atelier-build`, matching the OAuth consent flow and
    // atelier/cli marketing links. Applied even when the URL came from
    ...
    let url = crate::app::link_opener::ensure_query_param(url, "referrer", "atelier-build");
