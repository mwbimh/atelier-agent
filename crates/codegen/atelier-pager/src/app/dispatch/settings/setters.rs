...
pub(super) fn set_voice_stt_language_inner(app: &mut AppView, canonical: &str) {
    ...
    if language_changed && let Some(tx) = app.voice_cmd_tx.take() {
        app.voice_reset();
        let _ = tx.try_send(atelier_voice::VoiceCommand::Shutdown);
    }
}
...
pub(in crate::app::dispatch) fn set_voice_stt_language(
    ...
    refresh_open_settings_modals(app);
    let effective = atelier_voice::language_for_api(canonical);
    tracing::info!(
        target: "settings",
    ...
    let toast = if canonical == atelier_voice::STT_LANGUAGE_AUTO {
        format!("\u{2713} Voice language: System ({effective})")
    } else {
        let name = atelier_voice::stt_language_by_code(canonical).map_or(canonical, |l| l.name);
...
/// `ATELIER_PROMPT_SUGGESTIONS` env var overrides the effective value.
...
pub(super) fn set_contextual_hint_inner(
    app: &mut AppView,
    write: fn(&mut atelier_shell::agent::config::ContextualHints, Option<bool>),
    new: bool,
) {
    write(&mut app.current_ui.contextual_hints, Some(new));
    let resolved = atelier_shell::util::config::resolve_contextual_hints(
...
pub(in crate::app::dispatch) fn set_auto_dark_theme(app: &mut AppView, new: String) -> Vec<Effect> {
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        // No prior config: fall back to AtelierNight (the default).
        .unwrap_or_else(|| crate::theme::ThemeKind::AtelierNight.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
...
pub(in crate::app::dispatch) fn set_auto_light_theme(
    ...
    let prev_canonical: &'static str = app
        ...
        .and_then(crate::theme::canonical_name)
        .filter(|s| *s != "auto")
        .unwrap_or_else(|| crate::theme::ThemeKind::AtelierDay.display_name());
    let new_canonical = match crate::theme::canonical_name(&new) {
        Some(c) if c != crate::theme::ThemeKind::Auto.display_name() => c,
        _ => {
            tracing::error!(
...
pub(in crate::app::dispatch) fn set_default_model_inner(
    ...
    // the next `atelier/models/update` roundtrip.
...
pub(in crate::app::dispatch) fn clear_fork_secondary_model(app: &mut AppView) -> Vec<Effect> {
    let baseline = atelier_shell::models::default_model().to_string();
    let prev_id_str = app.current_ui.fork_secondary_model.clone();
