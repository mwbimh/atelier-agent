...
//! `atelier/interject` effect, and prompt-history recording. Split out of
...
/// prompt, and fires the `atelier/interject` ext method carrying a client-minted
/// id.
///
/// The shell broadcasts `atelier/session/interjection` to every attached pane so
...
pub(super) fn dispatch_interject(
    ...
    // record its id so the broadcast echo (`atelier/session/interjection`) is
