...
/// `atelier dashboard` before login: the startup hook consumes the
/// `ATELIER_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `atelier dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(atelier_shell::sampling::types::ReasoningEffort::High),
            display: "Atelier 4.5".to_string(),
        });
    ...
    assert_eq!(
        ...
        Some((
            model_id,
            Some(atelier_shell::sampling::types::ReasoningEffort::High)
        )),
...
/// `atelier dashboard`, and the startup hook.
#[serial_test::serial(ATELIER_AGENT_DASHBOARD)]
#[test]
...
fn dashboard_open_reseeds_pending_model_and_mode() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: acp::ModelId::new(std::sync::Arc::from("atelier-4.5")),
            effort: None,
            display: "Atelier 4.5".to_string(),
        });
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_atelier_home_for_tests(Some(home.path().to_path_buf()));
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("ATELIER_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
