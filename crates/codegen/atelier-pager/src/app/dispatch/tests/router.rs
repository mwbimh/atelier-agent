...
fn critical_announcement(id: &str) -> atelier_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> atelier_announcements::RemoteAnnouncement {
    atelier_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(atelier_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: Some(format!("https://atelier/{id}")),
            caption: None,
        }),
        ..Default::default()
    }
...
/// the promo url (observed via the `ATELIER_TEST_OPEN_URL_FILE` seam, from every
...
#[serial_test::serial(ATELIER_TEST_OPEN_URL_FILE)]
#[test]
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    use atelier_telemetry::events::AnnouncementCtaSurface;
    let url_file =
        std::env::temp_dir().join(format!("atelier-cta-open-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&url_file);
    unsafe { std::env::set_var("ATELIER_TEST_OPEN_URL_FILE", &url_file) };
    ...
    for surface in [
        ...
        assert!(
            opened().lines().any(|l| l == "https://atelier/promo-open"),
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!(
        { "scope" : "user", "path" : "/home/user/.atelier/skills/pick-best/SKILL.md", }
    );
