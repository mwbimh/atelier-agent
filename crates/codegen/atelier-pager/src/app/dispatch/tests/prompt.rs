...
fn turn_end_drains_next_queued_prompt() {
    ...
    // No re-send: the prompt was already sent before the adoption handoff.
    assert!(effects.is_empty());
    assert!(app.agents[&id].session.state.is_turn_running());
...
fn multiple_queued_prompts_drain_one_per_turn() {
    ...
    assert!(matches!(effects.as_slice(), [Effect::SendPrompt { text, .. }] if text == "b"));
    assert_eq!(app.agents[&id].session.queue_len(), 1);

    // Turn end → drain "c".
    let effects = dispatch(end_turn(), &mut app);
    assert!(matches!(effects.as_slice(), [Effect::SendPrompt { text, .. }] if text == "c"));
    assert_eq!(app.agents[&id].session.queue_len(), 0);

    // Turn end with an empty queue has no side effects.
    let effects = dispatch(end_turn(), &mut app);
    assert!(effects.is_empty());
    assert!(app.agents[&id].session.state.is_idle());
}
...
/// Turn end with prompt suggestions enabled fires the `atelier/suggestPrompt`
...
fn turn_end_fetches_prompt_suggestion_when_enabled() {
    ...
    // No `atelier-build-0.1` in the test catalog and no env override →
    // `None` on the wire; the shell then uses its own `atelier-build-0.1`
...
fn turn_complete_notification_suppressed_when_queue_non_empty() {
    ...
    // No re-send. The second prompt is adopted without an extra effect.
    assert!(effects.is_empty());
    assert!(app.agents[&id].session.state.is_turn_running());
    assert!(
...
/// `atelier/queue/changed` rebroadcast — not client-side prediction — updates
...
fn cancel_with_queued_prompt_drains_on_completion() {
    ...
    assert!(matches!(effects.as_slice(), [Effect::SendPrompt { text, .. }] if text == "queued"));
    assert!(app.agents[&id].session.state.is_turn_running());
    assert_eq!(app.agents[&id].session.queue_len(), 0);
}
...
fn cancel_with_multiple_queued_prompts_drains_only_front_prompt() {
    ...
    assert!(matches!(effects.as_slice(), [Effect::SendPrompt { text, .. }] if text == "queued-1"));
    assert!(app.agents[&id].session.state.is_turn_running());
    assert_eq!(app.agents[&id].session.queue_len(), 1);
...
fn cancel_drain_is_blocked_when_editing_front_prompt() {
    ...
    assert!(effects.is_empty(), "drain must stay blocked while editing");
    assert!(app.agents[&id].session.state.is_idle());
    assert_eq!(app.agents[&id].session.queue_len(), 2);
...
/// The always-on pipeline end to end, with `ATELIER_SUGGESTIONS` semantics
