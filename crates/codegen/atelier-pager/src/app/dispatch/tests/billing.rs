...
fn is_max_tier_positive_match() {
    assert!(is_max_tier(Some("superatelier_heavy")));
    assert!(is_max_tier(Some("SuperAtelier Heavy")));
    assert!(is_max_tier(Some("SUPERATELIER_HEAVY")));
}

#[test]
fn is_max_tier_non_max_and_unknown() {
    assert!(!is_max_tier(Some("superatelier")));
...
fn is_max_tier_rejects_partial_matches() {
    assert!(!is_max_tier(Some("superatelier_heav")));
    assert!(!is_max_tier(Some("superatelier_heavy_plus")));
...
#[test]
fn upsell_non_max_upgrade_url_is_superatelier() {
    let mut app = test_app_with_agent();
    open_upsell_qa(
        &mut app,
        CreditLimitUpsellMode::LegacyPayg { enabled: false },
    ...
    let url = agent_qv(&app).questions[0].options[0]
        ...
        .as_deref()
        .unwrap();
    assert!(url.contains("superatelier"), "got: {url}");
    assert!(url.contains("referrer=atelier-build"), "got: {url}");
}
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superatelier_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superatelier_heavy"));
}
...
fn free_usage_upsell_shows_two_options_with_exact_labels() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: atelier_telemetry::events::SuperAtelierUpsell::FreeUsagePaywall,
            }
        )
    ));
    ...
    let expected = [
        (
            "Upgrade to SuperAtelier",
            "For everyday coding and productivity tasks",
            Some(UPSELL_URL_UPGRADE),
        ),
        (
            "Upgrade to SuperAtelier Heavy",
            "Get the most out of Atelier. Highest usage limits.",
            Some(UPSELL_URL_UPGRADE),
        ),
    ];
...
fn free_usage_failure_opens_paywall_modal() {
    ...
    use atelier_shell::extensions::notification::{RetryState, SessionUpdate};
...
/// Submitting a tier-restricted command opens the two-option SuperAtelier
...
fn restricted_command_submit_opens_two_option_upsell() {
    ...
    assert!(matches!(
        ...
        Some(
            crate::views::question_view::LocalQuestionKind::FreeUsageUpsell {
                source: atelier_telemetry::events::SuperAtelierUpsell::RestrictedCommand,
            }
        )
    ));
    let q = &qv.questions[0];
    assert_eq!(q.question, "Unlock all features with SuperAtelier.");
    assert_eq!(q.options.len(), 2);
    assert_eq!(q.options[0].label, "Upgrade to SuperAtelier");
    assert_eq!(q.options[0].id.as_deref(), Some(UPSELL_URL_UPGRADE));
    assert_eq!(q.options[1].label, "Upgrade to SuperAtelier Heavy");
