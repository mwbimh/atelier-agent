...
fn cancel_turn_leaves_shared_queue_for_agent_to_drain() {
    ...
    // mutate the queue locally — the `atelier/queue/changed` rebroadcast is the
    // source of truth.
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    {
...
fn bg_task_killed_not_found_removes_task_from_inactive_agent() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            session_id: "sess-B".into(),
            task_id: "task-B-1".into(),
            outcome: Some(atelier_tools::types::KillOutcome::NotFound),
        }),
        &mut app,
    );
...
fn bg_task_killed_keeps_pending_kill_on_killed_outcome() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::BgTaskKilled {
            session_id: "sess-B".into(),
            task_id: "task-B-1".into(),
            outcome: Some(atelier_tools::types::KillOutcome::Killed),
        }),
        &mut app,
    );
...
fn subagent_label_strips_control_characters() {
    ...
    // Inject an ANSI escape into the fixed Role — this is what flows
    // through `format_subagent_label` → row builder sanitisation.
    info.role = Some(Arc::from("a\x1b[31mevil\x1b[0m"));
    agent
        .subagent_sessions
        .insert(info.child_session_id.to_string(), info);
    let rows = build_rows(
