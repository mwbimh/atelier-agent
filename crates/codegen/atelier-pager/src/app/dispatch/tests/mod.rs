...
fn test_app() -> AppView {
    ...
    AppView {
        ...
        auth_methods: vec![acp::AuthMethod::Agent(acp::AuthMethodAgent::new(
            acp::AuthMethodId::new("atelier.invalid"),
            "Atelier".to_string(),
        ))],
...
/// Plant a Build session under the process `atelier_home()` (OnceLock-cached;
/// do not rely on setting `ATELIER_HOME` mid-process). Caller must remove `sess_dir`.
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    let home = atelier_shell::util::atelier_home::atelier_home();
    let encoded = atelier_shell::util::atelier_home::encode_cwd_dirname(&cwd.to_string_lossy());
    let sess_dir = home.join("sessions").join(encoded).join(session_id);
    std::fs::create_dir_all(&sess_dir)
        .unwrap_or_else(|error| panic!("plant session dir {sess_dir:?}: {error}"));
...
/// `atelier/ask_user_question` ext-method request. Returns the args
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: atelier_tools::implementations::atelier_build::ask_user_question::AskUserQuestionMode::Default,
        questions: vec![
        ...
        vec![
            acp::PermissionOption::new(
                acp::PermissionOptionId::new(Arc::from(
                    atelier_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID,
                )),
...
const POLICY_WARNING: &str =
    atelier_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
