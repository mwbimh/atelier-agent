...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Atelier session whose first prompt resumes a foreign tool session.
    ForeignResume {
...
/// Build `atelier/session/fork` params shared by TUI effects and headless.
///
...
pub fn fork_session_params(
    ...
    let source_cwd = atelier_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = atelier_shell::util::atelier_home::atelier_home().join("sessions");
    let encoded = atelier_shell::util::atelier_home::encode_cwd_dirname(&cwd_str);
...
/// Parse `newSessionId` from an `atelier/session/fork` ACP response body.
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    atelier_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub struct MaterializeCtx {
    ...
    pub has_worktree: bool,
    /// Legacy compatibility field. Atelier never performs remote session
    /// restore, even when a caller sets this flag to `true`.
    pub allow_remote_restore: bool,
    /// Process-wide flag for the removed hosted conversation mode.
    pub chat_mode: bool,
}
impl MaterializeCtx {
...
async fn most_recent_session_id(cwd: &str) -> anyhow::Result<(String, Option<String>)> {
    let summaries = atelier_shell::session::persistence::list_summaries(Some(cwd)).await?;
    let first = summaries.first().ok_or_else(|| {
        anyhow::anyhow!(
            "No session found for current directory. \
             Use 'atelier' to start a new session."
        )
    })?;
    Ok((first.info.id.to_string(), first.display_title_opt()))
}
/// Return a local-only auth manager for callers that still need an auth handle
/// while the removed remote-session code is being phased out. It deliberately
/// does not load credentials or install a refresher.
pub(crate) fn pre_acp_auth_manager(
    _agent_config: &atelier_shell::agent::config::Config,
) -> std::sync::Arc<atelier_shell::auth::AuthManager> {
    std::sync::Arc::new(atelier_shell::auth::AuthManager::new_local(
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if atelier_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = atelier_shell::session::resolve_local_session(session_id, cwd) {
        tracing::info!(
    ...
    if let Some(original_cwd) = atelier_shell::session::resolve_local_session_any_cwd(session_id) {
        tracing::info!(
...
mod tests {
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["atelier"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["atelier", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_session_id_alone_is_new_with_id() {
        assert_eq!(
            parse(&["atelier", "--session-id", "my-id"])
                .session_startup_intent()
                .unwrap(),
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["atelier", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["atelier", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["atelier", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["atelier", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["atelier", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["atelier"])).chat_mode);
    }
    ...
    #[tokio::test]
    async fn remote_restore_request_is_rejected_even_when_flag_is_true() {
        let ctx = MaterializeCtx {
            has_worktree: false,
            allow_remote_restore: true,
            chat_mode: false,
        ...
        let err = materialize_startup_for_cwd(
            ...
            "/nonexistent/cwd/for/vendorless-remote-restore-test",
        )
        .await
        .unwrap_err();
        assert!(
            err.to_string()
                .contains("remote session restore is disabled"),
            "unexpected error: {err}"
        );
    }
    #[tokio::test]
