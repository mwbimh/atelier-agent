...
//!
//! Env sandboxing follows this crate's `serial(ATELIER_HOME)` idiom; note
//! `atelier_home()` is process-cached (OnceLock), so disk assertions always go
...
//!
//! ```bash
//! cargo test -p atelier-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The atelier home the agent actually persisted under: `atelier_home()` is
/// process-cached, so an earlier test in this binary may have pinned it.
fn effective_atelier_home() -> PathBuf {
...
struct PagerLeaderCluster {
    ...
    /// agent on the same ATELIER_HOME (two agents on one updates.jsonl is the
    ...
    _atelier_home: TempDir,
}

impl PagerLeaderCluster {
    /// Stand up the cluster. Callers MUST be `#[serial_test::serial(ATELIER_HOME)]`
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (atelier_version), not
        ...
        // atelier-shell/src/agent/app.rs (inside its LocalSet body) — a
        // deliberate copy so production stays untouched. Second copy of the
        // same wiring: atelier-shell/tests/test_leader_soak.rs ("Real agent
    ...
    async fn kill_leader(&mut self) {
        ...
        // be running against the same ATELIER_HOME when the next generation's
    ...
    /// `LeaderReconnector` (socket pinned via `ATELIER_LEADER_SOCKET`, flock held
