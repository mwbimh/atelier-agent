...
pub enum Command {
    /// Run Atelier without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Atelier discovers for this directory
    Inspect {
...
")]
    ...
    /// `~/.atelier/config.toml` or when the `ATELIER_AGENT_DASHBOARD=0` env
    /// var is set.
    Dashboard,
}
    ...
    pub fn canonical_plugin_dirs(&self) -> Vec<PathBuf> {
        self.plugin_dirs
            ...
            .filter_map(|p| match dunce::canonicalize(p) {
                ...
                Ok(_) => {
                    eprintln!(
                        "atelier: --plugin-dir {}: not a directory; skipping",
                        p.display()
                    );
                    None
                }
                Err(e) => {
                    eprintln!("atelier: --plugin-dir {}: {e}; skipping", p.display());
                    None
                }
            })
            .collect()
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Atelier WebSocket relay
...
pub struct HeadlessArgs {
    #[arg(long = "atelier-ws-origin")]
    pub atelier_ws_origin: Option<String>,
    #[arg(long = "atelier-ws-url")]
    pub atelier_ws_url: Option<String>,
}
...
pub struct ServeArgs {
    ...
    #[arg(long, env = "ATELIER_AGENT_SECRET")]
...
pub struct LeaderArgs {
    ...
    /// Defer the atelier.invalid relay WebSocket until the first headless IPC client
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.atelier/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// persists per-segment markdown to grep. Sets `ATELIER_COMPACTION_MODE`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            atelier_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    #[arg(long, env = "ATELIER_SANDBOX", value_name = "PROFILE")]
    ...
    pub fullscreen: bool,
    /// Write sampling events to ~/.atelier/logs/sampling.jsonl.
    #[arg(long = "log-sampling", env = "ATELIER_LOG_SAMPLING", hide = true)]
...
impl PagerArgs {
    ...
    /// explicit `--sandbox`/`ATELIER_SANDBOX` that differs from the saved profile is
    ...
    pub fn saved_resume_profile(&self) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                atelier_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
                atelier_shell::session::persistence::resumed_session_sandbox_profile(None, cwd)
            }
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<atelier_sandbox::ProfileName>().ok()
                    != s.parse::<atelier_sandbox::ProfileName>().ok() =>
            {
    ...
    /// whitespace. This is the `atelier "<prompt>"` launch form; the headless
...
mod tests {
    ...
    fn version_short_flag_exits_zero() {
        let err = PagerArgs::try_parse_from(["atelier", "-v"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::DisplayVersion);
        assert!(
            err.exit_code() == 0,
            "-v must exit 0; got {}",
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["atelier"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["atelier", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["atelier", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["atelier", "--minimal", "--fullscreen"]).unwrap_err();
        assert_eq!(err.kind(), clap::error::ErrorKind::ArgumentConflict);
    }
    #[test]
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["atelier", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["atelier", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["atelier"]).expect("bare atelier parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["atelier", "leader", "list", "--json"])
            .expect("atelier leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["atelier", "leader", "info", "--pid", "42"])
            .expect("atelier leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill =
            PagerArgs::try_parse_from(["atelier", "leader", "kill"]).expect("atelier leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["atelier", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["atelier", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["atelier", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args = PagerArgs::try_parse_from(["atelier", "fix the bug"])
    ...
    #[test]
    fn bare_atelier_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["atelier"]).expect("bare atelier parses");
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args =
            PagerArgs::try_parse_from(["atelier", "  spaced  "]).expect("padded prompt parses");
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["atelier", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["atelier"]).expect("bare atelier parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["atelier", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias = PagerArgs::try_parse_from(["atelier", "--trust-folder"])
            .expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["atelier", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias = PagerArgs::try_parse_from(["atelier", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["atelier", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep = PagerArgs::try_parse_from(["atelier", "--reasoning-effort", "deep"])
            .expect("deep parses");
    ...
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["atelier", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["atelier", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["atelier", "agent", "--effort", "max", "stdio"])
