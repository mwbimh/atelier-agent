...
impl AgentView {
    ...
    pub(crate) fn log_prompt_suggestion_shown_if_visible(&mut self) {
        ...
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::PromptSuggestion {
            action: atelier_telemetry::events::PromptSuggestionAction::Shown,
            chars,
            words,
        });
    }
    ...
    /// Apply an `atelier/follow_ups` notification, keyed by `response_id`
    ...
    /// on each `atelier/follow_ups` notification (the same `promptId` it stamps on
    ...
    /// - A buffer-replayed `atelier/follow_ups` for a PRIOR turn's `response_id`
    ...
    /// Buffer a stamped `atelier/follow_ups` for a turn that is not yet current,
    ...
    /// Flush a buffered `atelier/follow_ups` for `prompt_id` (a turn that has just
    ...
    /// the running turn's `atelier/follow_ups` arrive on the ext channel DURING
    ...
    pub(in crate::app) fn connect_matched_plugin(&mut self) {
        ...
        atelier_telemetry::session_ctx::log_event(
            atelier_telemetry::events::PluginCtaConnectClicked {
