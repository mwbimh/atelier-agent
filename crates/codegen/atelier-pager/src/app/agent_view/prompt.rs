    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    atelier_telemetry::session_ctx::log_event(
                        atelier_telemetry::events::PromptSuggestion {
                            action: atelier_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                        },
                    );
                ...
                && self.prompt.text().is_empty()
            {
                ...
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::PromptSuggestion {
                        action: atelier_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                    },
                );
            ...
            match self.prompt.handle_key(key) {
                PromptEvent::Edited => {
                    if undo_tip_accepted {
                        atelier_telemetry::session_ctx::log_event(
                            atelier_telemetry::events::ContextualTip {
...
mod prompt_suggestion_key_tests {
    ...
    /// right after a turn ends with `atelier/suggestPrompt` resolved. Pins the
