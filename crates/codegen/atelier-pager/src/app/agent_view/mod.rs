...
impl McpInitProgress {
    ...
    ///   `atelier/mcp_initialized` clears the progress.
...
/// (`atelier/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    /// (optimistic echo). The shell broadcasts `atelier/session/interjection` to
    ...
    pub(crate) deferred_session_mode: Option<atelier_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `atelier/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`atelier/session/info` `agentName`).
    ...
    /// Armed when an `atelier/session/prompt_complete` broadcast arrives for the
    ...
    /// `atelier/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `atelier/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `atelier/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`atelier/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `atelier/follow_ups` that arrived for a turn that is NOT yet the
...
fn translate_project_select(
    ...
    use atelier_telemetry::events::{ProjectPickerOutcome, ProjectPickerSelected};
    let project_dir_options = resolved_paths.len().saturating_sub(1);
    let emit = |outcome: ProjectPickerOutcome| {
        atelier_telemetry::session_ctx::log_event(ProjectPickerSelected {
            outcome,
...
fn remember_mode_enabled() -> bool {
    let path = atelier_tools::util::atelier_home::atelier_home().join("config.toml");
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &atelier_tools::implementations::atelier_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(super) mod test_fixtures {
    ...
    /// FIX 4 (b): after adopting a NEW turn, a buffer-replayed `atelier/follow_ups`
    ...
    /// FINDING B (stamped path): a LATE FIRST-TIME (never-seen) `atelier/follow_ups`
    ...
    /// upstream by the `_meta["atelier/replayed"]` gate); it is pinned here so the
    ...
    /// FIX (buffer-before-adoption): a stamped `atelier/follow_ups` for a turn that
