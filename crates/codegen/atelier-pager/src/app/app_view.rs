...
/// [`atelier_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<atelier_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: atelier_shell::util::config::WorktreeHintMode) -> Self {
        use atelier_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        atelier_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
    ...
    pub fn resolve_from_hints_value(hints: Option<&toml::Value>) -> (Self, Self) {
        let (new_session, fork) =
            atelier_shell::util::config::WorktreeHintMode::resolve_pair(hints);
...
/// [`atelier_workspace::folder_trust::decide`] verdict; when the feature flag
...
/// Cap for the `ATELIER_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets
/// exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `ATELIER_ESC_DOUBLE_PRESS_MS`-overridable
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("ATELIER_ESC_DOUBLE_PRESS_MS").ok()))
}
...
/// (toolset) gate via [`atelier_shell::tier::is_restricted_tier_name`] so
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => atelier_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    /// stays sans-IO.
    pub current_ui: atelier_shell::agent::config::UiConfig,
    /// Working directory.
    pub cwd: PathBuf,
    /// Whether the project picker question has already been shown this session.
    pub project_picker_shown: bool,
    /// "Don't ask me again" opt-out from [`atelier_shell::util::config::resolve_hints`];
    ...
    /// Current appearance config (hot-reloadable from ~/.atelier/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`ATELIER_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `ATELIER_FPS` env on release
    /// builds, where the dev overlay is compiled out) — see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<atelier_announcements::RemoteAnnouncement>,
    ...
    pub announcement: Option<atelier_announcements::RemoteAnnouncement>,
    ...
    /// `atelier/sessions/list` polls and `atelier/sessions/changed` broadcasts.
    ...
    /// poll outside leader mode, so we fetch the same `atelier/session/list` the
    ...
    /// Reconciled from `atelier/queue/changed` broadcasts so
    ...
    /// confirming `atelier/queue/changed` broadcast has not yet arrived. Keyed by
    ...
    /// non-selectable headers. Gated by `ATELIER_SESSION_PICKER_GROUPED` env var
    ...
    /// back into the input box. Gated by `ATELIER_CANCEL_REWIND` env /
    ...
    /// When false, the pager must not request recaps (zero `atelier/recap` traffic).
    ...
    /// empty and inert outside `--minimal`; the `atelier-pager-minimal` crate
    ...
    pub session_picker_content_results:
        Option<Vec<atelier_shell::extensions::session_search::SearchSessionHit>>,
    ...
    /// `atelier_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Stamps `_meta["atelier/session"].kind = "chat"` and omits Build agent
    ...
    /// at startup and on settings toggles from `ATELIER_CONTEXTUAL_HINTS` (master)
    /// > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: atelier_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<atelier_shell::util::config::ContextualHintsRemote>,
    ...
    /// `atelier_build_usage_redirect_url`, targeted at personal-team users).
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, atelier_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `atelier_build_access_gate`. `Some` = blocked.
    pub gate: Option<atelier_shell::auth::GateInfo>,
    /// User-friendly subscription tier name (e.g. "SuperAtelier", "Free").
    ...
    pub pending_gate_verification: Option<atelier_shell::auth::GateInfo>,
    ...
    /// `~/.atelier/config.toml`. `None` when the file/section is absent
    ...
    /// kill switch or `ATELIER_VOICE_MODE=0` — the STT pipeline is not started and
    ...
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<atelier_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &atelier_shell::util::config::RemoteSettings,
    ) -> Option<atelier_shell::auth::GateInfo> {
        let msg = rs.gate_message.as_ref()?;
        if msg.is_empty() {
            return None;
        }
        Some(atelier_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    pub fn apply_auth_meta(&mut self, meta: &atelier_shell::auth::AuthMeta) {
        ...
        if was_gated && self.gate.is_none() {
            ...
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::events::SubscriptionActivated {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: atelier_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: atelier_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: atelier_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn deferred_model_switch_from_cli(
        ...
        acp::ModelId,
        Option<atelier_shell::sampling::types::ReasoningEffort>,
    )> {
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && atelier_voice::AUDIO_SUPPORTED
    }
    ...
    /// `atelier/settings/update` handler when the subscription tier changes, so
    ...
    fn voice_send(&self, cmd: atelier_voice::VoiceCommand) {
    ...
    pub(crate) fn voice_begin_recording(&mut self, target: VoiceTarget, hold: bool) {
        self.voice_send(atelier_voice::VoiceCommand::PttPress);
    ...
    pub(crate) fn voice_stop_keeping_final(&mut self) {
        ...
        self.voice_send(atelier_voice::VoiceCommand::PttRelease);
    ...
    /// `atelier/queue/changed` broadcast. The broadcast is
    ...
    /// `atelier/queue/changed` broadcast replaces it (matched by `id`) rather than
    ...
    /// Rows the dev `ATELIER_FPS` overlay occupies (0 in non-dev builds), so
...
struct WelcomeInputCtx<'a> {
    ...
    sp_content_results:
        &'a Option<Vec<atelier_shell::extensions::session_search::SearchSessionHit>>,
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    atelier_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(rect) = ctx.gate_url_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::OpenSuperatelierUrl);
                }
                if let Some(rect) = ctx.upgrade_cta_rect
                    && rect.contains(ratatui::layout::Position::new(mouse.column, mouse.row))
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        atelier_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
fn dispatch_access_gate_menu_action(index: usize) -> InputOutcome {
    match index {
        0 => InputOutcome::Action(Action::OpenSuperatelierUrl),
...
impl AppView {
                ...
                match *active_view {
                    ActiveView::Welcome => {
                        ...
                        if !has_access && !self.access_gate_shown_logged {
                            ...
                            atelier_telemetry::session_ctx::log_event(
                                atelier_telemetry::events::SuperAtelierUpsellShown {
                                    source:
                                        atelier_telemetry::events::SuperAtelierUpsell::WelcomeScreen,
                                    auth_method: self
                                        .login_method_id
                                        .as_ref()
    ...
    /// Log [`atelier_telemetry::events::AnnouncementCtaShown`] for each
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        use atelier_telemetry::events::AnnouncementCtaSurface;
        ...
        let key = atelier_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: atelier_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::ContextualTip {
                tip: atelier_telemetry::events::ContextualTipKind::ImageInput,
                action: atelier_telemetry::events::ContextualTipAction::Shown,
            });
            return true;
        }
        false
...
pub(crate) mod tests {
    ...
    fn slash_gate_resyncs_when_critical_expires_between_pushes() {
        ...
        app.active_announcements = vec![atelier_announcements::RemoteAnnouncement {
    ...
    fn needs_animation_gates_todo_badge_flash() {
        ...
        app.agents
            ...
            .unwrap()
            .todo
            .update_todos(vec![atelier_shell::tools::TodoItem {
                content: "do the thing".into(),
                priority: Default::default(),
                status: atelier_shell::tools::TodoStatus::InProgress,
                meta: None,
            }]);
        assert!(
    ...
    fn apply_auth_meta_hides_usage_for_team_users() {
        ...
        let meta = atelier_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_shows_usage_for_personal_users() {
        ...
        let meta = atelier_shell::auth::AuthMeta::default();
        app.apply_auth_meta(&meta);
        assert!(app.usage_visible);
    }
    #[test]
    fn apply_auth_meta_clears_api_key_flag_and_shows_usage_on_personal_login() {
        ...
        app.usage_visible = false;
        app.apply_auth_meta(&atelier_shell::auth::AuthMeta::default());
    ...
    fn apply_auth_meta_api_key_enables_voice_and_skips_tier_gate() {
        ...
        app.apply_auth_meta(&atelier_shell::auth::AuthMeta {
    ...
    fn apply_auth_meta_lifts_restrictions_for_paid_tiers_and_teams() {
        ...
        advertise_media_tools(&mut app);
        let meta = atelier_shell::auth::AuthMeta {
            subscription_tier: Some("SuperAtelier".into()),
            ..Default::default()
        };
        app.apply_auth_meta(&meta);
        assert!(app.tier_restricted_commands.is_empty());
        ...
        app.apply_auth_meta(&atelier_shell::auth::AuthMeta::default());
        assert!(!app.tier_restricted_commands.is_empty());
        app.subscription_tier = Some("SuperAtelier".into());
    ...
    fn is_restricted_tier_classification() {
        ...
        assert!(!is_restricted_tier(Some("SuperAtelier")));
        assert!(!is_restricted_tier(Some("SuperAtelier Heavy")));
    ...
    fn apply_auth_meta_clears_gate_on_subscription() {
        ...
        app.gate = Some(atelier_shell::auth::GateInfo {
            message: "Subscribe to use Atelier".into(),
            url: Some("https://atelier.invalid/superatelier?referrer=atelier-build".into()),
            label: None,
        });
    ...
    fn apply_auth_meta_gate_unchanged_when_still_gated() {
        let mut app = test_app();
        let gate = atelier_shell::auth::GateInfo {
    ...
    fn welcome_ctrl_u_update_keeps_priority_over_foreign_resume() {
        ...
        app.apply_foreign_resume_detection(
            launch_token,
            &canonical_cwd,
            Some(atelier_workspace::foreign_sessions::RecentForeignSession {
                tool: atelier_workspace::foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn overlay_left_arrow_file_search_open_does_not_exit() {
        let (mut app, id) = neutral_overlay_app();
        {
            ...
            agent.prompt.file_search.set_test_state(
                ctx,
                vec![atelier_workspace::file_system::FuzzyMatchResult {
