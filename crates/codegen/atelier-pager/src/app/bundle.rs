...
//!
//! Pager-side cache of what `atelier-shell` reports from
//! `atelier/bundle/status`. The shell now performs the actual bundle download in
...
///
/// Populated from `atelier/bundle/status` ACP responses.
...
/// Deserialized response from `atelier/bundle/entry/get`.
#[derive(Debug, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
