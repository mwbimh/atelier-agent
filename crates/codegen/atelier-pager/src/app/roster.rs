...
//!
//! - Request/response `atelier/sessions/list` → [`RosterListResponse`].
//! - Broadcast notification `atelier/sessions/changed` → [`RosterChanged`].
//!
...
/// Response to `atelier/sessions/list`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Broadcast payload for `atelier/sessions/changed`.
#[derive(Debug, Clone, Default, Deserialize)]
...
/// Parse an `atelier/sessions/list` ext-response body into a [`RosterListResponse`].
///
/// The agent serializes the response through
/// `ExtMethodResult::success(..).to_ext_response()` (see
/// `atelier-shell/src/agent/handlers/session.rs::handle_roster_list`), which
...
/// `atelier/sessions/changed` broadcast. Mirrors how `Effect::FetchSessionList`
/// unwraps `result` for `atelier/session/list`.
...
mod tests {
    ...
    fn agent_entry() -> atelier_shell::agent::roster::RosterEntry {
        use atelier_shell::agent::roster as agent;
        agent::RosterEntry {
    ...
    fn roster_list_response_survives_result_envelope() {
        ...
        // Exact wire bytes the agent emits for `atelier/sessions/list`.
        ...
        assert_eq!(e.model_id.as_deref(), Some("atelier-4"));
        assert!(e.yolo);
    ...
    /// `atelier/sessions/changed` round-trip: serialize the agent's `RosterChanged`
