...
mod tests {
    ...
    fn model_unavailable_with_switch() {
        let event = SessionEvent::ModelUnavailable {
            previous_model_id: "atelier-4.5".into(),
            new_model_id: "atelier-build".into(),
            reason: "Model \"atelier-4.5\" is no longer available.".into(),
        };
        assert_eq!(
            event.message(),
            "Model \"atelier-4.5\" is no longer available. Switched to \"atelier-build\"."
        );
    }
