...
//! the generic [`CodeBlockSpan`](atelier_markdown::CodeBlockSpan) API) and
...
use atelier_markdown::MarkdownRenderView;
...
pub struct MermaidBlock {
    ...
    /// from [`CodeBlockSpan::body`](atelier_markdown::CodeBlockSpan::body). For
    ...
    /// [`CodeBlockSpan::output_line_range`](atelier_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a atelier_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `AtelierDay` is the only light theme; every other concrete theme (and the
/// `AtelierNight` default that `Auto` resolves to before it reaches the cache) is
/// dark. The render worker maps this to `atelier_mermaid::MermaidTheme`; it
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::AtelierDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_atelierday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::AtelierDay),
            "AtelierDay is the light theme"
        );
        for dark in [
            ThemeKind::AtelierNight,
