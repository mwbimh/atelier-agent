...
mod tests {
    ...
    /// Pins AtelierNight via the shared test-lock guard; hold it for the whole
    /// test so a concurrent theme flip can't skew the compared highlighter walks.
    fn pin_ateliernight_syntect() -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::theme::cache::pin_theme();
        assert!(
            !Theme::ateliernight().diff_uses_line_fg(),
            "AtelierNight must be banded for Equal-line syntect path in render"
        );
        guard
    }
    ...
    /// Caller pins the theme via `pin_ateliernight_syntect` (lock is not reentrant).
...
"
    ...
    fn syntax_highlight_splits_keyword_and_string_fg() {
        let _guard = pin_ateliernight_syntect();
    ...
    fn file_scoped_stale_theme_falls_back_to_hunk_only() {
        ...
        let stale = ThemeKind::AtelierDay;
