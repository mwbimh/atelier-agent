...
pub(crate) fn map_hyperlinks_to_overlay(
    hyperlinks: &[atelier_markdown::HyperlinkTarget],
...
mod tests {
    ...
    fn rendered_verb_group_folds_subagent_row_and_expansion_keeps_activity() {
        ...
        let mut block = SubagentBlock::started(
            "task", "child-A", "explore", None, None, /*is_background=*/ false,
        );
    ...
    fn make_hyperlink(
        ...
        url: &str,
        id: u32,
    ) -> atelier_markdown::HyperlinkTarget {
        atelier_markdown::HyperlinkTarget {
            line_index: line,
            column_range: cols,
            url: url.to_string(),
    ...
    fn overlay_empty_hyperlinks_produces_nothing() {
        ...
        let links: &[atelier_markdown::HyperlinkTarget] = &[];
        let mut overlay = LinkOverlay::new();
        map_hyperlinks_to_overlay(links, &output, 0, 0, 10, 0, 0, &[], &mut overlay);
