...
use atelier_shell::session::repo_changes::UploadMethod;
...
pub struct TraceArgs {
    ...
    #[arg(long)]
    pub local: bool,
    /// Output path (default: $ATELIER_HOME/trace-exports/<session-id>.tar.gz)
    #[arg(short, long)]
...
#[cfg(test)]
mod tests {
    use super::trace_uploads_removed;

    #[test]
    fn trace_command_is_local_only() {
        assert!(trace_uploads_removed());
    }
}
...
pub async fn run(args: TraceArgs, agent_config: &AgentConfig) -> Result<()> {
    // The `--local` flag is retained in the CLI shape for now, but every
    // Atelier trace export is local-only. There is no upload fallback or
    // provider-owned trace destination.
    let _ = args.local;
    run_export(
        &args.session_id,
...
/// Compile-time contract for privacy-sensitive callers and tests: trace
/// exports never upload session data in Atelier.
pub const fn trace_uploads_removed() -> bool {
    true
}
...
pub fn build_session_tar(
    ...
    let mut file_count: u32 = 0;
    {
        ...
        let metadata = ExportMetadata {
            ...
            atelier_version: env!("VERSION_WITH_COMMIT").to_owned(),
...
pub(crate) fn find_session_dir(session_id: &str) -> Result<PathBuf> {
    atelier_shell::session::persistence::find_session_dir_by_id(session_id).with_context(|| {
        format!(
            "Session '{session_id}' not found under {}",
            crate::util::display_user_atelier_path("sessions")
        )
    })
}

pub fn trace_exports_dir() -> PathBuf {
    atelier_home().join("trace-exports")
}
...
// ---------------------------------------------------------------------------
// Remote trace upload was intentionally removed from Atelier.
//
// Session traces are exported to a local archive only. Keeping this boundary
// explicit prevents future callers from accidentally reintroducing a network
// sink through the old trace command.
