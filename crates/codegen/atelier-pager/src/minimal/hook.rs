...
//! render mode.
//!
//! `atelier-pager` cannot depend on `atelier-pager-minimal`: that crate reads
...
//!
//! The composition-root binary (`atelier-pager-bin`) wires it up once at
//! startup by calling `atelier_pager_minimal::install()`. When nothing has
