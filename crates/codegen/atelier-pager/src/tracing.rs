...
/// firehose (`ATELIER_DEBUG_LOG` / `ATELIER_LOG_FILE`).
pub use atelier_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids,
/// status, payload sizes). Cheap to format at streaming rate.
///
/// Defined in `atelier-telemetry` so the firehose directives and the pager
/// filter share one constant (re-exported here for callsites).
pub use atelier_telemetry::debug_log::ACP_UPDATE_TARGET;
