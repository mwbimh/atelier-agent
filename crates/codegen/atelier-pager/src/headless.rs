//! Headless single-turn mode (`atelier -p "prompt"`).
//!
//! Runs the agent in-process via
//! `spawn_atelier_shell`, sends the ACP lifecycle (init → auth → session → prompt),
...
pub fn parse_permission_rules_strict(
    allow: &[String],
    deny: &[String],
) -> anyhow::Result<Vec<atelier_workspace::permission::types::PermissionRule>> {
...
pub(crate) fn parse_permission_rules_inner(
    ...
    use atelier_workspace::permission::types::RuleAction;
...
fn attach_result_usage(result: &mut serde_json::Value, usage: &serde_json::Value) {
    atelier_shell::extensions::notification::attach_result_usage_fail_closed(result, usage);
}
...
async fn authenticate(
    ...
    let method_id = crate::acp::select_eager_auth_method(auths, default_auth_method_id)
        .ok_or_else(|| {
            ...
            let interactive = std::io::stdin().is_terminal()
                && !atelier_shell::util::clipboard::is_remote_session();
    ...
    if kind.needs_interactive_login() {
        use std::io::IsTerminal;
        let interactive =
            std::io::stdin().is_terminal() && !atelier_shell::util::clipboard::is_remote_session();
...
async fn open_session(
    ...
    let mcp_servers =
        cli_config::load_mcp_servers(cwd, &atelier_tools::types::compat::CompatConfig::default());
            ...
            acp::LoadSessionRequest::new(acp::SessionId::new(sid.to_string()), cwd.to_path_buf())
                ...
                .meta({
                    ...
                    if let Some(true) = restore_code {
                        m.insert("atelier/restore_code".into(), serde_json::Value::Bool(true));
                    }
                    Some(m)
                }),
            acp_tx,
...
pub async fn run_single_turn(
    ...
    atelier_shell::http::set_process_client_mode_headless();
    ...
    agent_config.mode = atelier_shell::agent::config::AgentMode::Headless;
    ...
    if options.trust {
        atelier_shell::agent::folder_trust::grant_folder_trust(&cwd);
    }

    let cancel = CancellationToken::new();
    let memory_config = agent_config.memory_config.clone();
    let spawned = match spawn_atelier_shell(agent_config, &cancel, memory_config).await {
        Ok(s) => s,
        Err(e) => {
    ...
    let track_active = std::env::var("ATELIER_TRACK_HEADLESS").is_ok();
    if track_active {
        let _ = atelier_shell::active_sessions::register(
            atelier_shell::active_sessions::ActiveSession {
    ...
    if options.self_verify {
        prompt_blocks.push(acp::ContentBlock::Text(acp::TextContent::new(
            skill_body(atelier_shell::builtin::CHECK_SKILL_MD).to_string(),
        )));
    }
        ...
        let n = n.clamp(2, 10);
        {
            prompt_blocks.insert(
                ...
                acp::ContentBlock::Text(acp::TextContent::new(format!(
                    ...
                    skill_body(atelier_shell::builtin::BEST_OF_N_SKILL_MD)
                ))),
            );
        }
    }
    ...
    // Pending background work: bash/monitor via atelier/task_backgrounded +
    // task_completed; background subagents via SubagentSpawned + SubagentFinished
    // on atelier/session_notification (prefixed `subagent:{id}` in pending_bg).
    ...
    // Handle result
    if track_active {
        // Non-blocking flock so a slow/network ~/.atelier can't hang exit.
        let _ = atelier_shell::active_sessions::try_unregister(&session_id);
    }
    cancel.cancel();
    match prompt_result {
        Some(Ok(resp)) => {
...
fn handle_ext_notification(
    ...
    if method == "atelier/task_backgrounded" {
    ...
    if method == "atelier/task_completed" {
        #[derive(serde::Deserialize)]
    ...
    if method == "atelier/monitor_event" {
        return ExtEvent::MonitorEvent;
    }

    match method {
        "atelier/session_notification" | "atelier/session/update" => {}
...
mod tests {
    ...
    fn reap_request_for_task_kills_with_session_scope() {
        ...
        assert_eq!(request.method.as_ref(), "atelier/task/kill");
    ...
    fn reap_request_for_subagent_cancels_with_stripped_id() {
        ...
        assert_eq!(request.method.as_ref(), "atelier/subagent/cancel");
        let params: serde_json::Value = serde_json::from_str(request.params.get()).unwrap();
        assert_eq!(params["subagentId"], "sub-7");
    }

    use super::*;
    use atelier_workspace::permission::types::{RuleAction, ToolFilter};
    ...
    fn domain_mode_web_fetch() {
        ...
        assert_eq!(
            ...
            atelier_workspace::permission::types::PatternMode::Domain
        );
    ...
    fn headless_task_backgrounded_parses_task_id() {
        ...
        // the inner update object (matching the real `atelier/task_backgrounded`
        // wire shape: `{ "update": { "sessionUpdate": ..., "task_id": ... } }`).
        let notif = make_ext_notif(
            "atelier/task_backgrounded",
            serde_json::json!({
