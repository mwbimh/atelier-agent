...
//! `atelier/suggest` request through the Effect pipeline. Stale responses
...
/// Parsed ghost suggestion from an ACP `atelier/suggest` response.
#[derive(Debug, Clone)]
...
/// A single completion item from an ACP `atelier/suggest` response.
...
// (e.g. atelier-pager-minimal's) compiling when optional fields are added.
#[derive(Debug, Clone, Default)]
...
/// Parsed response from an ACP `atelier/suggest` request.
#[derive(Debug, Clone)]
...
impl SuggestResponseParsed {
    /// Parse a raw JSON value from an ACP `atelier/suggest` response.
...
/// Wire `limit` for `atelier/suggest` fetches. Matches the shell file
...
pub struct SuggestionController {
    ...
    /// `ATELIER_SUGGESTIONS` env var. Tab-triggered completion in bash mode
    ...
    /// Resolved at construction from `ATELIER_SUGGESTIONS_AI` env var.
    pub ai_enabled: bool,
    /// Model to use for AI suggestions. Sent in the `atelier/suggest` request.
    /// Resolved at construction from `ATELIER_SUGGESTIONS_AI_MODEL` env var.
...
impl SuggestionController {
    pub fn new() -> Self {
        Self {
            ...
            tab_pending: None,
            enabled: atelier_config::env_bool("ATELIER_SUGGESTIONS").unwrap_or(false),
            dropdown: CompletionDropdownState::default(),
            ai_enabled: atelier_config::env_bool("ATELIER_SUGGESTIONS_AI").unwrap_or(false),
            ai_model: std::env::var("ATELIER_SUGGESTIONS_AI_MODEL")
                .ok()
                .filter(|s| !s.is_empty()),
        }
    }
    ...
    /// Called when an ACP `atelier/suggest` response arrives, with the text and
