...
use atelier_announcements::visible_announcements;
...
fn is_critical(a: &atelier_announcements::RemoteAnnouncement) -> bool {
    a.severity.as_deref() == Some("critical")
}

fn is_promo(a: &atelier_announcements::RemoteAnnouncement) -> bool {
...
fn is_live_critical(
    a: &atelier_announcements::RemoteAnnouncement,
    now: chrono::DateTime<chrono::Utc>,
) -> bool {
    is_critical(a) && !atelier_announcements::is_expired_at(a, now)
}
...
fn is_live_promo(
    ...
    is_promo(a) && !atelier_announcements::is_expired_at(a, now)
}
...
pub fn is_dismissible(a: &atelier_announcements::RemoteAnnouncement) -> bool {
...
fn is_hidden(a: &atelier_announcements::RemoteAnnouncement, hidden_ids: &BTreeSet<String>) -> bool {
    is_dismissible(a) && hidden_ids.contains(&atelier_announcements::announcement_hide_key(a))
}
...
fn usable_cta(a: &atelier_announcements::RemoteAnnouncement) -> Option<(&str, &str)> {
...
pub(crate) fn usable_cta_caption(a: &atelier_announcements::RemoteAnnouncement) -> Option<&str> {
...
fn first_critical_session_announcement<'a>(
    announcements: &'a [atelier_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<&'a atelier_announcements::RemoteAnnouncement> {
...
pub fn promo_cta_target<'a>(
    announcements: &'a [atelier_announcements::RemoteAnnouncement],
    hidden_ids: &BTreeSet<String>,
) -> Option<(&'a atelier_announcements::RemoteAnnouncement, &'a str)> {
...
pub fn session_announcement_hide_keys_at(
    ...
    visible_announcements(announcements)
        ...
        .map(atelier_announcements::announcement_hide_key)
        .collect()
}
...
fn render_critical_rows(
    area: Rect,
    buf: &mut Buffer,
    ann: &atelier_announcements::RemoteAnnouncement,
    hide_hovered: bool,
) -> BannerHits {
    use unicode_width::UnicodeWidthStr;
...
mod tests {
    ...
    fn promo(id: &str, message: &str, cta: Option<(&str, &str)>) -> RemoteAnnouncement {
        RemoteAnnouncement {
            ...
            cta: cta.map(|(label, url)| atelier_announcements::AnnouncementCta {
    ...
    fn promo_cta_target_requires_usable_pair() {
        let full = vec![promo("p", "msg", Some(("Go", " https://atelier/promo ")))];
        let (a, url) = promo_cta_target(&full, &no_hidden()).expect("usable target");
        assert_eq!(a.id.as_deref(), Some("p"));
        assert_eq!(url, "https://atelier/promo");

        let mut label_only = promo("p", "msg", None);
        label_only.cta = Some(atelier_announcements::AnnouncementCta {
            label: Some("Go".into()),
            url: None,
            caption: None,
        });
    ...
    fn promo_cta_returns_label_and_pinned_flag() {
        let mut pinned = promo(
            "p",
            "msg",
            Some(("Upgrade Account", "https://atelier/promo")),
        );
    ...
    fn promo_cta_target_yields_to_critical_slot_owner() {
        let promo_ann = promo("p", "upsell", Some(("Go", "https://atelier/promo")));
        ...
        assert_eq!(
            ...
            Some((Some("p"), "https://atelier/promo"))
        );
    ...
    fn usable_cta_rejects_unsafe_schemes() {
        ...
        for good in ["https://atelier/promo", "http://atelier/promo"] {
    ...
    fn render_promo_row_button_and_hide_affordances() {
        let mut ann = promo(
            "p",
            "New promo",
            Some(("Get SuperAtelier", "https://atelier/atelier")),
        );
        ann.cta.as_mut().unwrap().caption = Some("or use Ctrl+O".into());
        let anns = [ann];
        ...
        let row0 = buf_row(&buf, area, 0);
        assert!(row0.starts_with("[Get SuperAtelier]"), "row0={row0:?}");
        assert!(
