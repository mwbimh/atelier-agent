#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/atelier-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/atelier-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/atelier-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/atelier-test-image.png"),
    ...
    fn accept_inside_command_with_args_absorbs_existing_separator() {
        ...
        pw.textarea.insert_str("/mod atelier-4");
        pw.textarea.set_cursor(3);
        pw.refresh_slash(&models);
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: atelier_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> atelier_workspace::file_system::FuzzyMatchResult {
        atelier_workspace::file_system::FuzzyMatchResult {
    ...
    /// after a turn ends with `atelier/suggestPrompt` resolved.
