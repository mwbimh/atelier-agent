...
//!  atelier-3 · yolo                             ← info line (optional)
//! ```
//!
        ...
        let applied = if snap.cursor_in_command {
            ...
            if let Some(ref range) = snap.command_range {
                    ...
                    && self.textarea.text().is_char_boundary(range.end)
                {
                    ...
                    // `/mod atelier-4` yields `/model atelier-4`, not `/model  atelier-4`
            ...
            // Replace the args range (e.g., "gr" → "Atelier 4 Fast").
    ...
    fn handle_key_inner(&mut self, key: &KeyEvent) -> PromptEvent {
        ...
        if changed {
            ...
            if is_backspace_key && !old_text.is_empty() {
                use atelier_telemetry::events::BackspaceNoEffect;
...
impl PromptWidget {
    ...
    pub fn draw(
        ...
        // Bottom divider: ╰──────────atelier-3 · flags──╯
