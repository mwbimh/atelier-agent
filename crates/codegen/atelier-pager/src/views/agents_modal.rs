...
use atelier_agent::config::{AgentDefinition, AgentScope, BuiltinAgentName};
use atelier_shell::agent::config::AgentSelectionConfig;
use atelier_tools::implementations::skills::discovery::extract_first_paragraph;
...
/// User-level vs project-level config files (`~/.atelier` vs `{cwd}/.atelier`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as shell: `[agent]`, `ATELIER_AGENT`,
    /// model `agentType`, then `atelier-build`).
...
/// Skips internal variants (AtelierBuildConcise, AtelierBuildPlan,
/// AtelierBuildPlanNoSubagents, AtelierBuildAskUser, Codex, Opencode,
/// CursorExtended, AtelierBuildOrchestrator).
...
pub fn build_agent_list(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<AgentListEntry> {
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = atelier_agent::discovery::discover(cwd);
...
/// Union bundled personas with local `~/.atelier/personas` and `{cwd}/.atelier/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = atelier_home.join("bundled").join("personas");
    ...
    let dirs = [
        (
            ...
            cwd.join(".atelier").join("personas"),
        ),
        (ConfigFileScope::User, atelier_home.join("personas")),
    ];
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match atelier_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => atelier_config::atelier_home().join("personas"),
        ConfigFileScope::Project => cwd.join(".atelier").join("personas"),
    }
}
...
/// True when `path` is a deletable local persona file (user or project `.atelier/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.atelier/{subdir}` or `{cwd}/.atelier/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let atelier_home = atelier_config::atelier_home();
    let in_user = dunce::canonicalize(atelier_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".atelier").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    atelier_shell::config::load_effective_config()
        .ok()
        .and_then(|root| atelier_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// `MvpAgent::resolve_agent_definition` in atelier-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    atelier_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        atelier_agent::config::PromptMode::Extend => "extend",
        atelier_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    use super::*;
    use atelier_shell::agent::config::DEFAULT_AGENT_TYPE;
    #[test]
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".atelier").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".atelier").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.atelier/bundled/personas/b.toml".into()),
            scope_label: None,
        };
