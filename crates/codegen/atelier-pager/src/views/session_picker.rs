...
/// only one normal component (e.g., `/repo`), returns that component alone.
/// Does not perform tilde expansion — callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` → `"fw-1"`, `/repo` → `"repo"`, `/` → `"/"`.
///
...
impl SourceFilter {
    ...
    ///
    /// atelier.invalid conversations carry `source == "conversation"` and live remotely,
...
pub(crate) fn capture_picker_selection(
    ...
    content_results: Option<&[atelier_shell::extensions::session_search::SearchSessionHit]>,
    state: &PickerState,
    query: &str,
    grouped: bool,
...
pub(crate) fn build_content_entry_data(
    hits: &[atelier_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/repo"), "user-repo");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/repo"), "repo");
    }
    ...
    fn source_filter_matches() {
        ...
        // atelier.invalid conversations are remote: visible under All + Remote, not Local.
