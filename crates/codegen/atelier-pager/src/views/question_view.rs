...
/// driven by an ACP `atelier/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    CreditLimitUpsell {
        choices: Vec<atelier_telemetry::events::CreditLimitChoice>,
    },
    /// SuperAtelier upsell modal: the free-usage paywall (429 +
    ...
    /// `id`.
    FreeUsageUpsell {
        /// Telemetry source for `SuperAtelierUpsellClicked` — distinguishes
        /// the paywall from the restricted-command upsell.
        source: atelier_telemetry::events::SuperAtelierUpsell,
    },
...
///
/// Created when an `atelier/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// instead of by an ACP `atelier/ask_user_question` request. `None` for
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `atelier/ask_user_question`
...
impl QuestionViewState {
    ...
    /// questions (e.g. the SuperAtelier upsell) have no freeform row, so
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> atelier_tools::implementations::atelier_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: atelier_tools::implementations::atelier_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    /// `no_freeform` questions (e.g. the SuperAtelier upsell) have no "Other"
