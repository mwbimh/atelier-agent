...
fn render_docs_footer(buf: &mut Buffer, area: Rect, theme: &Theme) {
    const LONG: &str =
        "Tip · Ask Atelier: \"change theme to atelierday\" or \"what does compact mode do?\"";
    const SHORT: &str = "Tip · Ask Atelier to change a setting";
...
/// Atelier STT language list (25 codes + client-only `auto` = 26) with headroom.
...
mod tests {
    ...
    fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
        ...
        let snapshot = PagerLocalSnapshot {
            available_models: vec![(
                "Atelier Test".to_string(),
                acp::ModelId::new(Arc::from("atelier-test")),
            )],
            ..PagerLocalSnapshot::default()
        };
    ...
    fn render_editing_value_cursor_at_logical_position_when_buffer_fits() {
        let mut s = editor_render_fixture("Atelier Test", 4); // cursor between "Atelier" and " Test"
        let area = Rect {
            x: 0,
            y: 0,
            width: 60,
    ...
    fn picking_enum_esc_returns_to_browse() {
        ...
        match outcome {
            ...
            other => {
                panic!("expected Action::PreviewTheme(\"ateliernight\") on Esc, got {other:?}")
            }
        }
        assert!(matches!(s.mode, SettingsModalMode::Browse));
    }
    ...
    fn editing_value_chars_mutate_buffer_and_invalid_enter_is_noop() {
            ...
            SettingsModalMode::EditingValue {
                ...
                assert!(
                    ...
                    "validation_error must be Some for unknown model 'a' \
                     (catalog has 'Atelier 4 Fast' only)",
                );
            }
        ...
        // (catalog has 'Atelier 4 Fast'; "a" doesn't match) is
    ...
    //    the "Tip · Ask Atelier…" docs footer and the keybindings hints.
    //
    ...
    fn docs_footer_tip_is_centered() {
        ...
        // SHORT = "Tip · Ask Atelier to change a setting" (34 cells);
        ...
        assert!(
            !row_short.contains("atelierday"),
            "width=40 must NOT render LONG path (no `atelierday`): {row_short:?}",
        );
        assert!(
    ...
    fn tip_line_has_blank_row_above() {
        ...
        for y in 0..area.height {
            let txt = buf_row_text(&buf, y, area.x, area.width);
            if txt.contains("Tip") && txt.contains("Ask Atelier") {
                tip_y = Some(y);
                break;
            }
        }
    ...
    fn click_settings_breadcrumb_collapses_picker_to_browse() {
        ...
        // `"ateliernight"`. Tightened from the previous `Action(_) |
        ...
        match outcome {
            ...
            other => panic!(
                "expected Action(PreviewTheme(\"ateliernight\")) — the keyboard \
    ...
    fn click_settings_breadcrumb_after_nav_reverts_to_original() {
        ...
        // Pick a different index. The default theme is `ateliernight`
    ...
    fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
        ...
        match outcome {
            SettingsKeyOutcome::ActionPair(
                ...
                // Default theme is `ateliernight`; entering the picker
                // captures `original_value = current value = ateliernight`,
    ...
    /// misleading: on dark themes (AtelierNight, TokyoNight, RosePine
    /// Moon) `bg_visual` is actually *lighter* than `bg_highlight`;
    /// only on the AtelierDay light theme is title darker. The
    ...
    fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
        ...
        let raw_theme = match crate::theme::Theme::current_kind() {
            crate::theme::ThemeKind::AtelierNight => crate::theme::Theme::ateliernight(),
            crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
            crate::theme::ThemeKind::AtelierDay => crate::theme::Theme::atelierday(),
            ...
            crate::theme::ThemeKind::Auto => crate::theme::Theme::ateliernight(),
        };
        assert_ne!(
