...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a atelier_announcements::RemoteAnnouncement>,
...
pub(super) fn render_version_badge(
    ...
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", atelier_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
            spans.push(Span::styled(
                " Beta",
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a atelier_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a atelier_workspace::foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [atelier_shell::extensions::session_search::SearchSessionHit]>,
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = params.login_label.unwrap_or("atelier.invalid");
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Atelier is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// multi-line block showing the workspace path and the warning that Atelier
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "Atelier may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &atelier_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
mod tests {
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use atelier_workspace::foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = atelier_workspace::foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn extract_user_code_parses_verification_url() {
        assert_eq!(
            extract_user_code("https://accounts.atelier/oauth2/device?user_code=ABCD-EFGH"),
            Some("ABCD-EFGH"),
        );
        // Trailing params after the code are ignored.
        assert_eq!(
            extract_user_code("https://atelier/oauth2/device?user_code=WXYZ-1234&foo=bar"),
            Some("WXYZ-1234"),
        );
        // A param whose name merely ends in `user_code` must not be matched.
        assert_eq!(
            extract_user_code("https://atelier/d?foo_user_code=BAD&user_code=GOOD"),
            Some("GOOD"),
        );
        // No code param, empty code, and unexpected characters all yield None.
        assert_eq!(extract_user_code("https://atelier/oauth2/device"), None);
        assert_eq!(extract_user_code("https://atelier/d?user_code="), None);
        assert_eq!(
            extract_user_code("https://atelier/d?user_code=AB%20CD"),
            None
        );
    }
    ...
    fn device_auth_arm_shows_url_and_no_paste_box() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.atelier/oauth2/device?user_code=ABCD-EFGH";
    ...
    fn device_auth_arm_raw_url_mode_shows_full_url() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.atelier/oauth2/device?user_code=WXYZ-1234";
    ...
    fn raw_url_mode_uses_full_width_for_long_urls() {
        ...
        let url = "https://accounts.atelier/oauth2/device?user_code=WXYZ-1234&extra=0123456789";
    ...
    fn command_auth_arm_shows_url_and_waiting() {
        ...
        let mut buf = Buffer::empty(area);
        let theme = Theme::current();
        let url = "https://accounts.atelier/oauth2/authorize?client_id=atelier";
    ...
    fn long_ann() -> atelier_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = atelier_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
