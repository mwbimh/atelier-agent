//! `atelier wrap` — run any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `atelier ssh` wrapper: spawns an arbitrary command inside a
...
//! run — a single shell-quoted string (`atelier wrap "mycli ssh host"`) or a shell
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("atelier wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = atelier_config::shell::is_command_available(program);
        (
    ...
    if should_wrap() {
        match crate::pty_wrap::run_wrapped_command(&wrapped.program, &wrapped.args) {
            ...
            Err(e) => {
                ...
                eprintln!("atelier wrap: wrapped mode failed, running without PTY wrapping: {e}");
...
/// The program and argv `atelier wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // (`atelier wrap "mycli ssh host"`), not a program name: hand it to the shell
    ...
    // name, and an empty one (`atelier wrap "$PROG" ...` with `$PROG` unset)
...
/// deliberately not `atelier_config::shell::unix_shell_path`, which coerces
...
///
/// Unlike `atelier ssh`, `atelier wrap` does not gate on the terminal brand: the user
