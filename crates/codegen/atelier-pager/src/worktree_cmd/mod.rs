...
pub async fn run(args: WorktreeArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    let spawned =
        crate::acp::spawn::spawn_atelier_shell(agent_config.clone(), &cancel, None).await?;
    ...
    let _init: acp::InitializeResponse = acp_send(
        acp::InitializeRequest::new(acp::ProtocolVersion::V1)
            ...
            .meta(
                serde_json::json!({
                    "clientType": crate::runtime_identity::HEADLESS_CLIENT_TYPE,
                    "clientVersion": crate::runtime_identity::PAGER_CLIENT_VERSION
                })
                .as_object()
                .cloned(),
            ),
...
async fn cmd_db(tx: &atelier_acp_runtime::AcpAgentTx, command: WorktreeDbCommand) -> Result<()> {
    match command {
        ...
        WorktreeDbCommand::Path => {
            ...
            let resp: PathResp = ext_call(tx, "atelier/git/worktree/db/path", &()).await?;
            println!("{}", resp.path);
            Ok(())
        }
        WorktreeDbCommand::Rebuild => {
            let report: RebuildReport =
                ext_call(tx, "atelier/git/worktree/db/rebuild", &()).await?;
...
mod tests {
    ...
    fn ext_request_builds_db_stats_empty_params() {
        let req = ext_request("atelier/git/worktree/db/stats", &()).unwrap();
        assert_eq!(req.method.as_ref(), "atelier/git/worktree/db/stats");
    }
    ...
    fn ext_envelope_unwraps_success_result() {
        let json = r#"{"result": {"path": "/home/user/.atelier/worktrees.db"}, "error": null}"#;
        #[derive(serde::Deserialize)]
        struct PathResp {
            path: String,
        }
        ...
        assert_eq!(inner.path, "/home/user/.atelier/worktrees.db");
    }
