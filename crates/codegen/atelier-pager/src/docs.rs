...
/// Extract user-guide docs to `<atelier_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(atelier_home: &std::path::Path) {
    let docs_dir = atelier_home.join("docs").join("user-guide");
