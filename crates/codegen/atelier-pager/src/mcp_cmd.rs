//! `atelier mcp` — manage MCP server configurations from the command line.
...
use crate::util::display_user_atelier_path;
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.atelier/config.toml`, available in all your projects
    User,
    /// `./.atelier/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// flags such as `-y` are passed to the server instead of atelier.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.atelier/config.toml) or project (./.atelier/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = atelier_shell::util::config::load_mcp_server_configs_with_project(&cwd);
...
async fn run_add(args: AddArgs) -> Result<()> {
    ...
    atelier_shell::util::config::save_mcp_server_config_at(&path, name, &config).await?;
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => atelier_shell::util::config::user_config_path(),
        McpScope::Project => {
            atelier_shell::util::config::project_config_path(&current_dir_or_exit())
        }
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_atelier_path("config.toml"),
...
fn select_remove_site(
    ...
    use atelier_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    atelier_shell::util::config::user_config_path(),
                )
            })
        })
}
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        atelier_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
    ...
    // ancestor .atelier/config.toml, where it still resolves for sessions.
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = atelier_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        atelier_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["atelier", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        ...
        let add = parse_add(&["atelier", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "atelier", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "atelier", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a atelier scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "atelier", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        ...
        let add = parse_add(&["atelier", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["atelier", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["atelier", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["atelier", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["atelier", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = atelier_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.atelier/config.toml");
