...
pub async fn run(args: SessionsArgs, _agent_config: &AgentConfig) -> Result<()> {
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions =
                atelier_shell::session::merge::fetch_merged(None, cwd.to_str(), None, limit).await;
            print_sessions_grouped(&sessions);
        }
        SessionsCommand::Search { query, limit } => {
            use atelier_shell::session::storage::search::{SessionSearchRequest, execute_search};
            ...
            let root = atelier_home();

            let local_resp = execute_search(&root, &req).await;
            let resp = local_resp?;
