//! `atelier plugin` CLI subcommand — manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `atelier_shell::plugin` (shared orchestration)
//! and lower crates (`atelier-agent`, `atelier-plugin-marketplace`). This
...
use atelier_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use atelier_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use atelier_plugin_marketplace::SourceKind;
use atelier_plugin_marketplace::git::SourceCacheLease;
use atelier_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `atelier plugin list --json`. The `status` field acts as a
...
/// Typed entry for `atelier plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `atelier plugin list`).
        name: String,
...
fn installed_plugins(
    repos: &[(
        &str,
        &atelier_agent::plugins::install_registry::InstalledRepo,
    )],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = atelier_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = atelier_plugin_marketplace::load_sources(&config);
    sources.extend(atelier_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    for source in &sources {
        ...
        for plugin in atelier_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                atelier_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &atelier_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = atelier_plugin_marketplace::git::default_cache_root();
            atelier_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
...
fn install_kind(is_git: bool) -> atelier_telemetry::events::InstallKind {
    if is_git {
        atelier_telemetry::events::InstallKind::Git
    } else {
        atelier_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: atelier_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = atelier_plugin_marketplace::install_resolve::parse_marketplace_ref(source) {
    ...
    if !trust {
        use atelier_agent::plugins::git_install::{self, InstallSource};
...
fn cmd_install_marketplace(
    source: &str,
    mref: &atelier_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
...
fn cmd_enable(name: &str) -> Result<()> {
    ...
    atelier_shell::config::add_enabled_plugin(name)
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    atelier_shell::config::add_disabled_plugin(name)
...
fn cmd_validate(path: &str) -> Result<()> {
    ...
    match load_manifest(&root) {
        ...
        Ok(ManifestLoadResult::NotFound) => {
            println!(
                "No plugin.json found. Atelier discovers skills, agents, and hooks \
...
fn marketplace_add(
    ...
    use atelier_shell::plugin::MarketplaceAddInput;
...
fn marketplace_remove(
    ...
    let local_input = match plugin::classify_marketplace_add_input(url, &cwd) {
        atelier_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &atelier_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match atelier_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use atelier_plugin_marketplace::MarketplaceSource;
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = atelier_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
    ...
    fn current_head(repo: &Path) -> String {
        let output = atelier_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
