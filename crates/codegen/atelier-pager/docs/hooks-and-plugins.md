...
Atelier supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.atelier/hooks/`
- **Project hooks** — from `.atelier/hooks/` in your repo
...
Plugin hooks automatically receive `ATELIER_PLUGIN_ROOT` and `ATELIER_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.atelier/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `atelier plugin marketplace add <source>`)
...
Sources land in `~/.atelier/config.toml`:
...
Or in `~/.atelier/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../atelier-hooks/examples/README.md) — ready-to-use sample hooks
