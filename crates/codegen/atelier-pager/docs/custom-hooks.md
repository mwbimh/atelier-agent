...
Hooks let you run custom scripts or HTTP requests at key moments during a Atelier session — for example, before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.atelier/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.atelier/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: Open the hooks modal (`Ctrl+L` on non–VS Code family, or `/hooks` on any terminal including VS Code family) or run `/hooks-trust` (the same folder-trust gate as `--trust`, recorded in `~/.atelier/trusted_folders.toml`) the first time you open a project with hooks. This prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Atelier's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
Atelier injects the following variables into every hook process:

- `ATELIER_HOOK_EVENT` — the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`)
- `ATELIER_HOOK_NAME` — the full configured name of this hook
- `ATELIER_SESSION_ID` — the current session identifier
- `ATELIER_WORKSPACE_ROOT` — absolute path to the workspace root

For hooks provided by plugins, the following are also set:

- `ATELIER_PLUGIN_ROOT` — absolute path to the plugin's installation directory
- `ATELIER_PLUGIN_DATA` — absolute path to the plugin's writable data directory

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `ATELIER_PLUGIN_ROOT` and `ATELIER_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`ATELIER_PLUGIN_ROOT` and `ATELIER_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/atelier-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env Atelier itself sees).
...
`${ATELIER_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.atelier/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.atelier/...`) run with your user permissions — treat them like shell scripts.
...
- **See errors?** → Check the pager logs (usually in the tracing pane or `~/.atelier/logs`).
