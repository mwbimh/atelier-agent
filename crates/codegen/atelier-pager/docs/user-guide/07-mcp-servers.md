...
MCP (Model Context Protocol) servers extend Atelier with external tool integrations. They let Atelier interact with any service that implements the MCP standard.

---

## What Are MCP Servers?

An MCP server is a process that exposes tools to Atelier over a standardized protocol. When you configure an MCP server, its tools become available to the model alongside Atelier's built-in tools. The model can discover and call these tools during a session.
...
MCP servers are configured in `~/.atelier/config.toml` under `[mcp_servers.<name>]` sections.

### stdio Transport (Local Process)

Atelier spawns a local process and communicates over stdin/stdout:
...
> `ATELIER_MCP_STARTUP_TIMEOUT_SECS` (seconds). A per-server `startup_timeout_sec`
...
>
> - env `ATELIER_MAX_MCP_OUTPUT_BYTES` or `MAX_MCP_OUTPUT_BYTES` (bytes; Atelier-native
>   wins if both set; Claude-style name, but we bound by **bytes** not tokens)
> - `config.toml` — user-level (`~/.atelier/config.toml`) **or repo-level**
>   (`.atelier/config.toml` anywhere on the cwd → git-root chain; the deepest
...
MCP servers can be configured per-project by placing a `.atelier/config.toml` in your repository:
...
When a server exposes a native HTTP/SSE endpoint, prefer the `url` form over wrapping it in a stdio proxy such as `npx mcp-remote <url>`. Atelier handles HTTP/SSE and OAuth directly, so the native form avoids an extra subprocess per session. It also registers Atelier's own OAuth client with the provider.

Atelier walks from the current directory up to the git repo root, loading `.atelier/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.atelier/config.toml` | All projects | Lowest |
| `<repo-root>/.atelier/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Atelier reads most other config sections only from `~/.atelier/config.toml`.
...
You can enable or disable MCP servers during a session without restarting Atelier.
...
Atelier loads MCP server configurations from multiple sources for compatibility:

| Source | Format | Location | Configurable |
|--------|--------|----------|-------------|
| `config.toml` | Native Atelier config | `~/.atelier/config.toml`, `.atelier/config.toml` | Always on |
...
For MCP servers that require OAuth authentication, Atelier handles the credential flow automatically. When an MCP server requests OAuth credentials, Atelier opens a browser-based authorization flow and stores the resulting tokens for future use.
...
You must authenticate OAuth-based MCP servers before you can use them. Atelier stores the resulting tokens under `~/.atelier/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
To avoid putting secrets in the config file, reference an environment variable with `${VAR}` (or `${VAR:-default}`). Atelier expands string fields in `[mcp_servers.*]` — `url`, `command`, `args`, and the values in `env` and `headers` — at load time:
...
For stdio servers, Atelier captures the process's standard error to `~/.atelier/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.atelier/logs/mcp/filesystem.stderr.log
```
