...
Skills are reusable prompt packages that extend Atelier with task-specific instructions. They let you capture a repeatable procedure once, instead of re-explaining it each session.

---

## What Are Skills?

A skill is a directory that contains a `SKILL.md` file. Its markdown body tells Atelier how to handle a specific type of task: step-by-step instructions, conventions, and tool-usage patterns.

Use a skill for a repeatable procedure that's too specific for AGENTS.md but too long to retype. Atelier activates a skill only when it applies to your current task.
...
|----------|-------|----------|-------|
| `./.atelier/skills/`, `./.atelier/commands/` | Local (CWD) | Highest | Current directory skills / legacy command markdown |
| `<repo_root>/.atelier/skills/`, `…/commands/` | Repo | Medium | Shared across the repo |
| `~/.atelier/skills/`, `~/.atelier/commands/` | User | Lowest | Personal skills for all projects |
...
Atelier deduplicates skills by name -- a higher-priority location overrides a lower one. Atelier also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.atelier/`) and walks every directory between your working directory and the repo root.
...
Skill and command discovery does **not** use `.gitignore`. Paths under known skill roots (`.atelier/`, `.agents/`, `.claude/`, `.cursor/`) always load when present on disk — teams often ignore `.claude/**` as local-only config while still expecting `/frontend`-style project commands to work. To hide a skill, use `[skills] ignore` in config (not repo ignore rules).

Atelier scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.atelier/config.toml`, or set the `ATELIER_CURSOR_SKILLS_ENABLED` or `ATELIER_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Atelier always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.

### Additional Skill Directories

Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.atelier/config.toml`:
...
Each entry in `paths` is a `SKILL.md` file or a directory that Atelier walks recursively. `ignore` hides a skill completely; `disabled` keeps it in the list but excludes it from the system prompt and from invocation. `paths` and `ignore` take filesystem paths and support `~` expansion; `disabled` takes skill names.
...
| Field | Description |
|-------|-------------|
| `name` | Skill identifier. Use lowercase letters, digits, and hyphens, up to 64 characters. Atelier normalizes spaces and underscores to hyphens. If you omit `name`, Atelier uses the skill's directory name. |
| `description` | What the skill does and when to use it. Atelier reads this to decide whether to invoke the skill. If you omit it, Atelier uses the first paragraph of the body. |

Write a specific `description`. It determines when Atelier invokes the skill automatically. Name the trigger phrases and use cases.
...
| `metadata` | Arbitrary string key-value pairs. Atelier promotes `metadata.author` and `metadata.short-description` for display. |

---

## Creating Skills with /create-skill

The `/create-skill` command walks you through building a new skill interactively. Atelier asks what you want, drafts the files, and writes them to disk.

### How It Works

When you run `/create-skill`, Atelier:

1. **Gathers requirements.** Atelier asks for the skill name, the scope to save it under, and a description of the workflow you want to capture. Use a name with lowercase letters, digits, and hyphens (2–64 characters, starting and ending with a letter or digit).

2. **Drafts the description.** Atelier writes a `description` that states what the skill does, the phrases that trigger it, and the slash command name. You approve or edit the draft before continuing.

3. **Creates the skill directory.** Atelier creates the `<scope>/.atelier/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.

4. **Writes SKILL.md.** Atelier writes the frontmatter (`name` and `description`) and a markdown body of instructions, along with any supporting files.

5. **Verifies and confirms.** Atelier reads the file back, confirms it wrote correctly, and tells you how to run the skill.

### Choosing a Scope

Atelier asks where to save the skill:

- **Project** (`<repo_root>/.atelier/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Atelier recommends this scope inside a git repository.
- **User** (`~/.atelier/skills/<name>/`) -- available across all your projects.

The new skill appears in the slash menu within a few seconds, because Atelier reloads skills when files change on disk.
...
When a skill's name collides with another skill or a built-in command, Atelier advertises a qualified name prefixed by the skill's scope -- `local:`, `repo:`, `user:`, or the plugin name. Use the qualified form to choose a specific skill:

```
/local:commit        # The "commit" skill from ./.atelier/skills/
/user:commit         # The "commit" skill from ~/.atelier/skills/
```

### Automatic Invocation

Atelier can invoke a skill on its own when it recognizes a relevant task. Atelier matches your prompt against the skill's `description` and `when-to-use` fields, so write both to describe the triggering situation.
...
2. **Include concrete steps.** Skills work best when they give Atelier a clear, ordered procedure to follow.
...
5. **Version-control project skills.** Commit `.atelier/skills/` to your repository so the whole team benefits. User skills in `~/.atelier/skills/` stay personal and unshared.
