...
Atelier connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.

---

## Default Models

By default, Atelier uses models hosted by SpaceXAI, and new sessions start with `atelier-build`. Default models require no configuration. Authenticate with `atelier login` or an API key, then start a session.
...
```bash
atelier -p "Hello" -m atelier-build
```
...
Set a persistent default in `~/.atelier/config.toml`:
...
Atelier supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
When you omit `api_backend`, Atelier uses `chat_completions`.

To send provider-specific authentication or version headers -- for example, Anthropic's `x-api-key` -- use the `extra_headers` field described below. Atelier sends those headers verbatim with every request to the endpoint.

---

## Configuring Custom Models

Add custom model endpoints in `~/.atelier/config.toml` under `[model.<name>]` sections:
...
Atelier resolves the API key in this order:
...
3. Your signed-in session token (from `atelier login`), for a model with no `api_key`/`env_key` of its own
4. The `XAI_API_KEY` environment variable (global fallback; Atelier also accepts `ATELIER_CODE_XAI_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells Atelier when to trigger auto-compaction. When you override a known model, Atelier inherits that model's context window. When you define a new model and omit `context_window`, Atelier defaults to 200,000 tokens, so set it explicitly to match your provider.
...
When you override a built-in model, Atelier starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
The `messages` backend uses the Anthropic Messages protocol. Anthropic authenticates with an `x-api-key` header rather than `Authorization: Bearer`, so pass your key through `extra_headers`, which Atelier sends verbatim.
...
Point Atelier at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `ATELIER_MODELS_BASE_URL` | Yes | Base URL for inference. Atelier fetches the model list from `{base_url}/models`. |
| `XAI_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Atelier also accepts `ATELIER_CODE_XAI_API_KEY`. |
| `ATELIER_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Atelier inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Atelier uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `atelier login` -- the API key is enough.
...
```toml
[models]
web_search = "atelier-4.20-multi-agent"
```

Or via environment variable:

```bash
export ATELIER_WEB_SEARCH_MODEL="atelier-4.20-multi-agent"
```

If you point web search at a custom model, you also need a `[model.*]` entry so Atelier can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
