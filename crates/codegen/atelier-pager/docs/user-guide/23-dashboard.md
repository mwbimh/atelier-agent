...
- **Ctrl+\\** — same as the slash command, two keystrokes. Configurable
  in `~/.atelier/config.toml` under `[keybindings]` like every other shortcut.
...
```
 Atelier · Dashboard — 4 agents · 2 awaiting
...
The state icon matches Atelier's sibling views (
`tasks_pane`):
...
Set `ATELIER_AGENT_DASHBOARD=0` to force-disable the feature for a single
