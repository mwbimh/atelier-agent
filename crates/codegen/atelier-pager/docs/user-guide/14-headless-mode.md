...
Headless mode runs Atelier non-interactively from the command line. It accepts a single prompt, executes it with full tool access, and returns the result. Use it to automate tasks, script workflows, build integrations, and parse output programmatically.
...
| `--always-approve`       | Auto-approve ordinary tool executions; preserve the configured sandbox |
| `--yolo`                | Alias for `--dangerously-bypass-approvals-and-sandbox`: approval Never + DangerFullAccess |
...
On failure, Atelier emits an error object (process exit non-zero). Prompt-level
...
Atelier may also emit `max_turns_reached` and `auto_compact_*` events; treat the list as non-exhaustive and switch on `type`.
...
Atelier's headless mode can be wrapped as an OpenAI-compatible chat completion API:
...
The `--yolo` flag is an alias for `--dangerously-bypass-approvals-and-sandbox`, matching Codex: approval policy Never plus DangerFullAccess for the entire process. It disables Atelier's application sandbox and suppresses ordinary Tool approval prompts. Use `--always-approve` instead when automation should retain the configured sandbox. Explicit `deny` rules and `PreToolUse` hooks still apply, and administrators can disable bypass mode via `requirements.toml` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
**Use `--yolo` with extreme care.** It runs commands as the user who launched Atelier without the application sandbox. It does not grant Administrator rights or automatically trigger UAC.
...
When Atelier starts, it discovers the project root by walking upward from `--cwd`
...
Atelier discovers that repository as the project root and scopes its discovery (AGENTS.md, skills, git history) to it, which can make
...
Atelier stores data in `~/.atelier` (override with `ATELIER_HOME`; see [Environment Variables for Headless](#environment-variables-for-headless)):

| Path                     | Contents                              |
| ------------------------ | ------------------------------------- |
| `config.toml`            | User configuration                    |
| `providers.toml`         | Provider connection/OAuth registry only; no models or Roles |
| `roles.toml`             | User-configured fixed Role to Provider/model assignments |
| `request-agents.toml`    | Selectable outbound identities with explicit User-Agent strings |
| `models/default/`        | Exact model-ID defaults; family wildcards are rejected |
| `models/providers/`      | Provider-specific model overrides and experimental endpoints |
| `cache/providers/`       | Provider discovery results |
| `contexts/`              | Editable generic, Role, Goal, and compaction prompt presets |
