...
- **Shell builtins** -- handled by the agent backend (atelier-shell)
- **Pager builtins** -- handled by the TUI frontend (atelier-pager)
...
When the context window fills up, Atelier auto-compacts at 85% usage (configurable via `[session] auto_compact_threshold_percent` in config.toml).
...
`--fullscreen` CLI flags are likewise session-scoped. To make plain `atelier` open
...
The `/flush`, `/dream`, and `/memory` commands require `--experimental-memory` or `ATELIER_MEMORY=1`. `/remember` is always available.
...
Run a prompt on a recurring interval. Specify the interval as `30m`, `1 hour`, or `every 2 days`. If you omit it, Atelier prompts you.
...
Set, manage, or check an autonomous goal. Atelier works toward the objective across turns and reports progress.
...
- `/docs web` opens https://docs.atelier/build/overview in the browser
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter appears as a slash command. (A skill turned off via `/skills` is not advertised.) For example, if you have a skill at `~/.atelier/skills/commit/SKILL.md`, you can invoke it with:
