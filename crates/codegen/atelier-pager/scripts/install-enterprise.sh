#!/bin/bash
#
# Atelier CLI installer (enterprise channel) — https://atelier/cli/enterprise-install.sh
#
...
#
# Usage:
#   curl -fsSL https://atelier/cli/enterprise-install.sh | bash            # latest enterprise
#   curl -fsSL https://atelier/cli/enterprise-install.sh | bash -s 0.1.42  # specific version
#   ATELIER_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://atelier/cli/enterprise-install.sh)
#
...
if [ "$os" = "windows" ]; then
    ...
    chmod +x "$binary_path"
    ln -sf "$binary_path" "$BIN_DIR/atelier"
