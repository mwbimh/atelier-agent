...
use atelier_tools::types::memory_backend::{MemoryBackend, MemorySearchResult};
...
pub struct MemoryBackendParams {
    ...
    pub embed_config: Option<atelier_config_types::MemoryEmbeddingConfig>,
    ...
    pub search_config: atelier_config_types::MemorySearchConfig,
    ...
    pub auth_credentials: Option<Arc<dyn atelier_auth::AuthCredentialProvider>>,
}
...
async fn build_embedding_provider(
    config: Option<&atelier_config_types::MemoryEmbeddingConfig>,
    auth_credentials: Option<&Arc<dyn atelier_auth::AuthCredentialProvider>>,
...
pub struct MemoryBackendImpl {
    ...
    embed_config: Option<atelier_config_types::MemoryEmbeddingConfig>,
    ...
    search_config: atelier_config_types::MemorySearchConfig,
...
impl MemoryBackendImpl {
    ...
    pub fn new(db_path: PathBuf, storage: MemoryStorage) -> Self {
        Self {
            ...
            embed_api_key: None,
            search_config: atelier_config_types::MemorySearchConfig::default(),
            watcher: None,
            stale_claim_secs: 60,
    ...
    pub fn with_embedding(
        mut self,
        config: atelier_config_types::MemoryEmbeddingConfig,
        base_url: String,
    ...
    pub fn with_search_config(mut self, config: atelier_config_types::MemorySearchConfig) -> Self {
...
impl MemoryBackendImpl {
    ...
    pub fn search_config_for_test(&self) -> &atelier_config_types::MemorySearchConfig {
        &self.search_config
    }
}
...
impl MemoryBackend for MemoryBackendImpl {
    ...
    async fn search(
        ...
        let mut index = super::index::MemoryIndex::open_or_create(
            &self.db_path,
            self.storage.clone(),
            atelier_config_types::MemoryIndexConfig::default(),
            embed_dims,
        )
        ...
        if needs_release {
            ...
            if let Some((dirty_count, reindexed_count, sync_start)) = watcher_sync_stats {
                atelier_telemetry::session_ctx::log_event(
                    atelier_telemetry::memory_telemetry::MemoryWatcherSync {
        ...
        if results.is_empty() {
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::memory_telemetry::MemorySearchEmpty {
                    session_id: self.session_id.clone(),
            ...
            atelier_telemetry::session_ctx::log_event(
                atelier_telemetry::memory_telemetry::MemorySearch {
                    session_id: self.session_id.clone(),
                    query_length: query.len(),
                    keyword_count,
...
mod factory_tests {
    ...
    use atelier_config_types::{MemoryEmbeddingConfig, MemorySearchConfig};
    ...
    fn test_factory_wires_non_overridable_search_config_fields() {
        ...
        let custom_search = MemorySearchConfig {
            max_results: 7,
            mmr: atelier_config_types::MmrConfig {
                enabled: true,
                lambda: 0.42,
            },
            temporal_decay: atelier_config_types::TemporalDecayConfig {
                enabled: true,
                half_life_days: 14.0,
            },
            ..Default::default()
    ...
    async fn make_embedding_provider_uses_async_api_key_resolution() {
        ...
        let probe: atelier_tools::types::SharedApiKeyProvider = Arc::new(AsyncProbe {
...
mod tests {
    use super::*;
    use crate::index::{MemoryIndex, init_sqlite_vec};
    use atelier_config_types::MemoryIndexConfig;
    use tempfile::TempDir;
