...
//! `~/.atelier/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.

use std::path::{Path, PathBuf};

use atelier_tools::util::atelier_home::atelier_home;

/// Scope for a memory write operation.
/// Write-operation scope. Distinct from `atelier_agent::config::MemoryScope` (agent memory dir).
...
pub struct MemoryStorage {
    /// `~/.atelier/memory/`
    global_dir: PathBuf,
    /// `~/.atelier/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.atelier/memory/`.
    ///
    ...
    fn new_inner(cwd: &Path, root_override: Option<&Path>, use_workspace_hash: bool) -> Self {
        let global_dir = root_override
            .map(|p| p.to_path_buf())
            .unwrap_or_else(|| atelier_home().join("memory"));
    ...
    ///
    /// File path: `~/.atelier/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
    ...
    pub fn ensure_initialized(&self) -> std::io::Result<()> {
        ...
        if !global_file.exists() {
            std::fs::write(
                ...
                "# Global Memory\n\
                 \n\
                 > This file is automatically managed by Atelier's memory system.\n\
