...
//! are published per-version to the CDN at `atelier/cli/changelogs/`.
//!
...
/// Retained only for deterministic unit tests that exercise the cache helper.
...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$ATELIER_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `atelier_home()` OnceLock). A seeded `$ATELIER_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("ATELIER_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::atelier_home::atelier_home);
        Self {
    ...
    /// Load both formats from the local disk cache. Atelier never contacts a
    /// CDN or other vendor service for release notes.
    ...
    /// without mutating process-global env (`ATELIER_HOME` /
    /// `ATELIER_CHANGELOG_OFFLINE`), which races across the parallel test
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$ATELIER_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$ATELIER_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("atelier-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
