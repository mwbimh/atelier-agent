pub mod atelier_home;
pub mod changelog;
pub mod event_id;
pub mod secure_file;
pub mod tips;
pub mod uname;
pub use atelier_shared::clipboard;
pub use atelier_shared::stderr::{stderr_lock, with_locked_stderr};
...
/// True if `pid` is a atelier process; pairs with [`kill_process_by_pid`] to avoid killing a recycled PID.
/// Best-effort on macOS/BSD (liveness-only via `kill -0`), exact on Linux (/proc cmdline) and Windows (image path).
pub fn is_atelier_process(pid: u32) -> bool {
    #[cfg(target_os = "linux")]
    {
        let cmdline_path = format!("/proc/{pid}/cmdline");
        match std::fs::read(&cmdline_path) {
            Ok(data) => String::from_utf8_lossy(&data).contains("atelier"),
            Err(_) => false,
        }
    }
    #[cfg(windows)]
...
mod tests {
    ...
    fn test_is_cli_chat_proxy_url_rejects_public_api() {
        assert!(!is_cli_chat_proxy_url("https://api.atelier/v1"));
    }
    #[test]
    fn test_is_cli_chat_proxy_url_rejects_spoofed_hostname() {
        assert!(!is_cli_chat_proxy_url(
            "https://cli-chat-proxy.atelier.com.evil.example/v1"
        ));
    }
    #[test]
    ...
    #[test]
    fn is_atelier_process_self_true_impossible_pid_false() {
        assert!(is_atelier_process(std::process::id()));
        assert!(!is_atelier_process(u32::MAX));
    }
}
