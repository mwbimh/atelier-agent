...
mod tests {
    ...
    fn no_match_returns_borrowed() {
        ...
        assert!(matches!(
            redact_secrets("model=atelier-3"),
            Cow::Borrowed(_)
        ));
    }
    ...
    fn redacts_sensitive_url_query_params() {
        let out = redact_secrets("callback https://atelier/cb?code=ABC123XYZ&state=xyz789 failed");
    ...
    fn url_regex_excludes_trailing_punctuation() {
        let out = redact_secrets("see `https://atelier/cb?code=ABCD12345`");
