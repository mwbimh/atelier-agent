...
// ---------------------------------------------------------------------------
// Pager events (called from atelier-pager via log_event)
// ---------------------------------------------------------------------------
...
/// first prompt when Atelier is launched from a non-project directory).
...
#[derive(Serialize)]
pub struct SuperAtelierUpsellShown {
    pub source: SuperAtelierUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}

#[derive(Serialize)]
pub struct SuperAtelierUpsellClicked {
    pub source: SuperAtelierUpsell,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub auth_method: Option<String>,
}
...
/// Modeled on [`SuperAtelierUpsell`]; lets the funnel attribute the click to the
...
pub struct SubscriptionActivated {
    ...
    /// is strongly attributable to Atelier's upsell surface.
...
telemetry_event!(SuperAtelierUpsellShown, "superatelier_upsell_shown");
telemetry_event!(SuperAtelierUpsellClicked, "superatelier_upsell_clicked");
