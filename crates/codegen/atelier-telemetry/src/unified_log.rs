...
//! over ACP (`atelier/log` notifications); shell receives them in
...
pub const LOG_METHOD: &str = "atelier/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "atelier")]
    #[serde(rename = "atelier")]
    AtelierPager,
    #[strum(serialize = "atelier-desktop")]
    #[serde(rename = "atelier-desktop")]
    AtelierDesktop,
}
...
/// Wire format for the `atelier/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
fn log_path() -> PathBuf {
    atelier_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Called by the `atelier/log` notification handler. Entries from
