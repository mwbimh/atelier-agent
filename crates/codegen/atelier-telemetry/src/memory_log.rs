...
//! ATELIER_MEMORY_LOG=0 atelier                # disable even when enabled
//! tail -f ~/.atelier/logs/memory.log      # watch in another terminal
//! ```
...
mod inner {
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || atelier_home().join("logs").join("memory.log");
