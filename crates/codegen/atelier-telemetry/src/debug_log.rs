...
//! - PerSession (`ATELIER_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.atelier/debug/<session_id>.txt` (one file per session), with a
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::AtelierLogFile => "ATELIER_LOG_FILE",
            Self::AtelierDebugLog => "ATELIER_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `atelier-pager`) so the firehose directives below and
...
/// safe. See `atelier-pager/src/tracing.rs` for the consumer side.
...
// Broad firehose filter for the routing/ATELIER_DEBUG_LOG sources: capture our
...
// ATELIER_DEBUG_LOG single-file source (mirrors `default_file_filter`).
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the ATELIER_LOG_FILE source: DEBUG default, honor
// RUST_LOG, silence sampling_log (preserves ATELIER_LOG_FILE back-compat).
    ...
    match resolve_debug_target() {
        ...
        Some(DebugTarget::SingleFile { path, src }) => {
            let filter = match src {
                DebugSource::AtelierLogFile => default_file_filter(),
                DebugSource::AtelierDebugLog => firehose_filter(),
            };
...
pub(crate) enum DebugTarget {
    /// `ATELIER_DEBUG_LOG=1` → route per session into `<dir>` (`~/.atelier/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path → one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit ATELIER_LOG_FILE wins
/// (single file, RUST_LOG filter); else ATELIER_DEBUG_LOG — a truthy bool routes
/// per session into `~/.atelier/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let atelier_log_file = std::env::var_os("ATELIER_LOG_FILE");
    let atelier_debug_log = std::env::var_os("ATELIER_DEBUG_LOG");
    resolve_debug_target_inner(
        atelier_log_file.as_deref(),
        atelier_debug_log.as_deref(),
        &atelier_home().join("debug"),
    )
}
...
fn resolve_debug_target_inner(
    atelier_log_file: Option<&OsStr>,
    atelier_debug_log: Option<&OsStr>,
    debug_dir: &Path,
) -> Option<DebugTarget> {
    if let Some(raw) = atelier_log_file
        && !is_blank(raw)
    {
        return Some(DebugTarget::SingleFile {
            path: os_path(raw),
            src: DebugSource::AtelierLogFile,
        });
    }
    let raw = atelier_debug_log?;
    match raw.to_str().map(str::trim) {
        ...
        _ => Some(DebugTarget::SingleFile {
            path: os_path(raw),
            src: DebugSource::AtelierDebugLog,
        }),
    }
}
...
/// `~/.atelier/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
pub(crate) fn sweep_old_logs() {
    prune_old_logs(&atelier_home().join("debug"), LOG_RETENTION);
}
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for ATELIER_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for ATELIER_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty / whitespace ATELIER_LOG_FILE is treated as unset (mirrors ATELIER_DEBUG_LOG).
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 ATELIER_DEBUG_LOG value is a path, not a bool keyword, and its
        ...
        match target {
            DebugTarget::SingleFile { path, src } => {
                assert_eq!(src, DebugSource::AtelierDebugLog);
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "atelier_shell", "inside session");
            });
            tracing::info!(target: "atelier_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // `atelier_telemetry::session_ctx`) survives the real filter so
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "atelier_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "atelier_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "atelier_shell", "one first");
                tracing::info!(target: "atelier_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "atelier_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn prune_old_logs_missing_dir_is_noop() {
        ...
        prune_old_logs(
            Path::new("/no/such/atelier/debug/dir"),
