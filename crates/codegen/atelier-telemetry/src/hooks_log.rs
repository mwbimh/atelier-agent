...
//! file logger that writes to `~/.atelier/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with
//! targets `atelier_hooks` or `atelier_agent::plugins` at key lifecycle
//! points — discovery, dispatch, execution, errors.
//!
//! ## Enabling
//!
//! ```bash
//! ATELIER_HOOKS_LOG=1 atelier              # enable, write to ~/.atelier/logs/hooks.log
//! ATELIER_HOOKS_LOG=/tmp/h.log atelier     # write to custom path
//! ATELIER_HOOKS_LOG=0 atelier              # explicitly disable
//! tail -f ~/.atelier/logs/hooks.log     # watch in another terminal
//! ```
...
///
/// Writes to `~/.atelier/logs/hooks.log` (or custom path via `ATELIER_HOOKS_LOG`).
/// Filters to hooks (`atelier_hooks`) and plugins (`atelier_agent::plugins`) targets.
/// Set `ATELIER_HOOKS_LOG=0` to disable, `ATELIER_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "atelier_hooks=debug,atelier_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || atelier_home().join("logs").join("hooks.log");
