//! Headless markdown analysis sharing Atelier's exact `pulldown-cmark` config.
//!
...
//! feature set, shared with `atelier-markdown` so analysis matches what Atelier
//! Build actually renders 1:1.
//!
//! After parsing, Atelier applies [`offset_events`]: only `~~…~~` is strikethrough.
...
/// The exact `pulldown-cmark` option set Atelier uses to render markdown.
///
...
/// Offset event stream from Atelier's parser, with single-tilde strikethrough demoted.
///
...
/// Parse `text` with Atelier's options; count elements and flag structural issues.
