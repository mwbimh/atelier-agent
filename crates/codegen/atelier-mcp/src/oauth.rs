...
const MCP_OAUTH_CLIENT_NAME: &str = "Atelier";
...
// mode disabled, multiple `atelier` invocations).
//
// Layer 1 (cross-process): filesystem lock at $ATELIER_HOME/mcp_auth_{safe_name}.lock
...
fn auth_lock_path(server_name: &str) -> std::path::PathBuf {
    let safe: String = server_name
        ...
        .collect();
    atelier_config::atelier_home().join(format!("mcp_auth_{safe}.lock"))
}
