...
//!
//! Credentials are stored in `$ATELIER_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$ATELIER_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$ATELIER_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$ATELIER_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user atelier home (set $ATELIER_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$ATELIER_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(atelier_config::user_atelier_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$ATELIER_HOME/mcp_credentials.json`. Existing credential files must keep
    ...
    fn save_and_load_from_file() {
        let dir = std::env::temp_dir().join("atelier-mcp-credentials-test");
