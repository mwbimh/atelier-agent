...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"atelier_com_linear"`).
...
/// SDK-side id echoed back in `atelier/mcp/sdk_call`. A named struct (rather than a
/// `(String, String)` tuple) so callers can't transpose the two strings.
///
/// `Deserialize`d straight from a `_meta["atelier/mcp/servers"]` entry, so the
...
/// The session's in-process SDK MCP servers (declared via `_meta["atelier/mcp/servers"]`,
...
struct AcpMcpRegistry {
    ...
    /// `atelier/mcp/sdk_call` over the ACP connection).
...
pub struct McpState {
    ...
    /// Persisted to `~/.atelier/config.toml` under `[mcp_servers.<name>].disabled_tools`.
    ...
    /// `atelier/mcp/server_status` notifications.
    ///
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: atelier_runtime_events::events::EventWriter::noop(),
    ...
    /// `AtelierClientHandler` cloned during `try_handshake` reads
    ...
    pub fn set_event_writer(&mut self, writer: atelier_runtime_events::events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &atelier_runtime_events::events::EventWriter {
        &self.event_writer
    }
...
/// MCP initialization strategy. Defined in `atelier-telemetry`; re-exported
/// here so existing call sites continue to work.
pub use atelier_telemetry::enums::McpInitStrategy;
...
impl McpError {
    ...
    pub fn error_category(&self) -> atelier_runtime_events::events::McpErrorCategory {
        use atelier_runtime_events::events::McpErrorCategory;
        match self {
...
///   the frontend via `atelier/mcp/tools_changed` notifications and callable via
///   `atelier/mcp/call`.
...
/// `atelier_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl atelier_tool_runtime::Tool for McpErasedTool {
    ...
    fn id(&self) -> atelier_tool_protocol::ToolId {
        ...
        atelier_tool_protocol::ToolId::new(&qualified)
            .unwrap_or_else(|_| atelier_tool_protocol::ToolId::new("mcp_tool").expect("valid"))
    }
    ...
    fn description(
        ...
        atelier_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
        ...
        event_writer.emit(atelier_runtime_events::events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(atelier_runtime_events::events::Event::McpAuthRetry {
                    server_name: server.clone(),
        ...
        atelier_telemetry::session_ctx::log_event(atelier_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &atelier_runtime_events::events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, atelier_tool_runtime::ToolError> {
        ...
        match result {
            ...
            Ok(Err(e)) => Err(atelier_tool_runtime::ToolError::custom(
                "process_manager",
                e.to_string(),
            )),
            Err(_) => {
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(atelier_runtime_events::events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(
                    atelier_runtime_events::events::Event::McpTransportReconnect {
                        server_name: self.tool.server_name.clone(),
                        success: true,
                        error: None,
                    },
        ...
        match tokio::time::timeout(timeout_duration, mcp_service.call_tool(params)).await {
            Ok(Ok(call_result)) => Ok(call_result),
            Ok(Err(retry_err)) => Err(atelier_tool_runtime::ToolError::custom(
                "process_manager",
                retry_err.to_string(),
            )),
            Err(_) => {
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer.emit(
            atelier_runtime_events::events::Event::McpTransportDecodeError {
...
enum PendingTransport {
    ...
    /// (`atelier/mcp/sdk_call`). Rebuildable from its `server_id` + invoker, so handshake
...
/// Uses [`AtelierClientHandler`] rather than rmcp's default `ClientInfo`
...
pub type McpService = Arc<RunningService<RoleClient, AtelierClientHandler>>;
...
///    (`TransportClosed`).
/// 2. [`AtelierClientHandler`] when the server pushes a notification we
...
///
/// Consumers fan these out to ACP `atelier/mcp/server_status` after 50 ms
...
pub struct McpClient {
    ...
    /// [`AtelierClientHandler`]** constructed by
...
/// and the [`AtelierClientHandler`] it constructs during
...
/// External-config overrides for an MCP server, surfaced to atelier-mcp
...
impl McpClient {
    ...
    /// channel. `server_id` is the id the agent echoes back in `atelier/mcp/sdk_call`; the
    ...
    pub async fn ensure_initialized(&self) -> Result<McpService, McpError> {
        ...
        tracing::info!(target: atelier_telemetry::instrumentation::TARGET, event = "timing", name = "mcp_try_handshake", elapsed_us = handshake_elapsed);
    ...
    async fn try_handshake(
        &self,
        pending: PendingTransport,
    ) -> Result<rmcp::service::RunningService<RoleClient, AtelierClientHandler>, McpError> {
        ...
        match pending {
            ...
            PendingTransport::Acp { server_id, invoker } => {
                // Per-reverse-call backstop on `atelier/mcp/sdk_call`: the larger of the
    ...
    fn make_client_info(server_name: &str) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("atelier-shell-{server_name}"),
    ...
    /// Build the [`AtelierClientHandler`] that drives `client.serve(...)`.
    ///
    ...
    fn make_client_handler(&self) -> AtelierClientHandler {
        AtelierClientHandler {
    ...
    /// references (the [`AtelierClientHandler`] handed to
    ...
    pub async fn get_tool_registrations(
        ...
        let _ensure_init_timer =
            atelier_telemetry::instrumentation::timer("mcp_ensure_initialized");
        let mcp_service = self.ensure_initialized().await?;

        let mut all_tools = Vec::new();
        let mut cursor: Option<String> = None;

        let _list_tools_timer = atelier_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
/// Copy an MCP server's stderr to `~/.atelier/logs/mcp/<server>.stderr.log`
/// in a background task. Truncated per spawn.
fn drain_mcp_stderr_to_log(server_name: &str, mut stderr: tokio::process::ChildStderr) {
    let log_dir = atelier_config::atelier_home().join("logs").join("mcp");
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`atelier_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "atelier_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("atelier-cli"),
    );
}
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = atelier_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
        acp::McpServer::Stdio(acp::McpServerStdio {
            ...
            let _stdio_spawn_timer = atelier_telemetry::instrumentation::timer("mcp_stdio_spawn");
                ...
                SafeTokioChildProcess::spawn(cmd, name.clone(), event_writer.clone()).map_err(
                    |e| {
                        ...
                        atelier_telemetry::session_ctx::log_event(
                            atelier_telemetry::events::McpServerFailed {
                                server_name: name.clone(),
                                error_type: atelier_telemetry::events::McpErrorType::SpawnFailed,
        ...
        | acp::McpServer::Sse(acp::McpServerSse {
            ...
            let auth_prep = if has_existing_auth {
                ...
                let _auth_discovery_timer =
                    atelier_telemetry::instrumentation::timer("mcp_http_auth_discovery");
                ...
                .await
                {
                    ...
                    Err(_) => {
                        ...
                        event_writer.emit(
                            atelier_runtime_events::events::Event::McpOAuthDiscoveryTimeout {
                                server_name: name.clone(),
                                url: url.clone(),
                            },
                        );
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = atelier_telemetry::instrumentation::timer("mcp_start_servers");
...
/// `atelier/mcp/server_status` events.
///
...
#[derive(Debug)]
pub struct AtelierClientHandler {
...
impl ClientHandler for AtelierClientHandler {
    ...
    // See the [`AtelierClientHandler`] doc-comment for the full RPIT
    // contract.
...
mod tests {
    ...
    async fn resilient_transport_skips_undecodable_line_and_keeps_stream_alive() {
        ...
        let mut transport = ResilientRwTransport::new(
            ...
            tokio::io::sink(),
            "fwbuild".to_string(),
            atelier_runtime_events::events::EventWriter::noop(),
        );
    ...
    fn is_figma_mcp_matches_name_and_host() {
        ...
        assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("atelier_com_figma", "https://example.com/mcp"));
        assert!(is_figma_mcp("ATELIER_COM_FIGMA", "https://example.com/mcp"));
        assert!(is_figma_mcp("atelier_com_FIGMA", "https://example.com/mcp"));
    ...
    #[test]
    fn ensure_figma_user_agent_sets_atelier_cli_when_missing() {
        let mut headers = reqwest::header::HeaderMap::new();
    ...
    fn test_same_raw_name_different_servers_no_local_registry_collision() {
        use atelier_tool_hub_sdk::LocalRegistry;
        use atelier_tool_runtime::Tool;
    ...
    async fn recover_and_retry_surfaces_original_error_when_recover_fails() {
        ...
        let mut reconnect_attempted = false;
        let mut is_timeout = false;
        let ew = atelier_runtime_events::events::EventWriter::noop();
    ...
    async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
        ...
        let ew = atelier_runtime_events::events::EventWriter::open(tmp.path());
    ...
    async fn try_call_tool_reconnects_then_succeeds_after_retriable_transport_error() {
        ...
        async fn dead_service() -> McpService {
            ...
            let handler = AtelierClientHandler {
    ...
    fn is_auth_rejection_message_matches_auth_signals() {
        ...
        assert!(is_auth_rejection_message(
            "MCP server 'atelier_com_notion' handshake failed: Auth required, when send initialize request"
        ));
    ...
    fn auth_required_records_as_auth_not_init_failed_and_maps_category() {
        ...
        assert!(matches!(
            err.error_category(),
            atelier_runtime_events::events::McpErrorCategory::AuthRequired
        ));
    }
