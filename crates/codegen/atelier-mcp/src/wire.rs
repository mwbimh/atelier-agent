//! Single source of truth for the `atelier/mcp/*` ACP wire strings.
//!
...
/// Forward tool-invocation method (client -> agent): `atelier/mcp/call`.
///
...
pub const MCP_CALL: &str = "atelier/mcp/call";

/// Reverse zero-IPC tool-invocation method (agent -> client): `atelier/mcp/sdk_call`.
///
...
pub const MCP_SDK_CALL: &str = "atelier/mcp/sdk_call";

/// `session/new` `_meta` key listing in-process SDK MCP servers: `atelier/mcp/servers`.
pub const MCP_SERVERS: &str = "atelier/mcp/servers";

/// `initialize` `_meta` capability flag advertising in-process SDK MCP support
/// (enables the SDK's `transport="acp"`): `atelier/mcp/sdk`.
pub const MCP_SDK: &str = "atelier/mcp/sdk";
