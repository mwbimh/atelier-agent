...
//! `atelier_sampler::shared_http` (one HTTP/2 pooled client plus
//! a pool-less HTTP/1.1 fallback shared across every
//! `SamplingClient`). The sampler reads `ATELIER_POOL_*` /
//! `ATELIER_CONNECT_TIMEOUT_SECS` once, when its shared client is
//! first built, and `ATELIER_SAMPLER_SHARED_CLIENT=0` falls back to
...
// `OriginClientInfo` is owned by `atelier-sampler` so `SamplerConfig` can use
// it without taking a circular dependency on `atelier-shell`. Re-exported
// under the same path (`crate::http::OriginClientInfo`) so existing call-sites
// compile unchanged. The telemetry engine in `atelier-telemetry` consumes
// the same type via `atelier_sampler::OriginClientInfo`. The shell-specific
// constructors that depended on `ClientType` (a shell-only type) are free
// functions below.
pub use atelier_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from `ATELIER_CLIENT_NAME` /
/// `ATELIER_CLIENT_VERSION` env vars. Returns `None` when
/// `ATELIER_CLIENT_NAME` is unset.
pub fn origin_client_info_from_env() -> Option<OriginClientInfo> {
    std::env::var("ATELIER_CLIENT_NAME")
        .ok()
        .map(|product| OriginClientInfo {
            product,
            version: std::env::var("ATELIER_CLIENT_VERSION").ok(),
        })
}
...
/// Process-level client identifier (`ATELIER_CLIENT_NAME` env var, default `"atelier-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("ATELIER_CLIENT_NAME").unwrap_or_else(|_| "atelier-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt
/// (`atelier -p`) run or an interactive session; feeds the `client_mode`
/// metric label.
pub const CLIENT_MODE_HEADER: &str = "x-atelier-client-mode";

/// One-way latch: set to `"headless"` at startup by the non-TUI entry points
/// (`run_single_turn` for `atelier -p`, `run_headless_inner` for
/// `atelier agent [headless]`), `"interactive"` otherwise.
...
pub fn with_auth_retry(
    ...
    credentials: std::sync::Arc<dyn atelier_auth::AuthCredentialProvider>,
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        let meta = serde_json::json!({
            "clientIdentifier": "atelier-desktop",
            "clientVersion": "1.2.3",
        })
        .as_object()
        .cloned()
