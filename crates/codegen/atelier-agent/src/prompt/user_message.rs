...
//! - `Default` -- the legacy Atelier prefix (built by the shell layer).
...
use atelier_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Atelier prefix: `<user_info>` + optional `<git_status>`.
...
/// One discovered rule file (AGENTS.md / Claude.md / .atelier/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// string should use `atelier_shell::util::uname::os_kernel_and_release`
    /// (or equivalent).
    ...
    /// User-scoped rule files (~/.atelier/, ~/.claude/).
    ...
    /// (`~/.atelier/projects/<encoded-cwd>/mcps`). Surfaced in
