...
//!
//! Searches from cwd to repo root, plus `~/.atelier/`. Also discovers
//! `*.md` files in `.atelier/rules/` and `.claude/rules/` directories.
...
/// Find `*.md` files in `.atelier/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.atelier/, git repo root, and session cwd.
...
async fn read_agents_config_with_options(
    ...
    let global_dir = atelier_tools::util::atelier_home::atelier_home();
    ...
    // Always include atelier_home (~/.atelier/) first, then ~/.claude/ and ~/.cursor/
...
/// Used by `atelier-shell` to structurally detect legacy untagged AGENTS.md
...
fn render_agents_md(configs: &[AgentConfigFile]) -> Option<String> {
    ...
    for config in configs {
        ...
        // .atelier/rules/*.md) so globs/paths metadata doesn't leak into the
        // system prompt as raw YAML.
        let is_rules_file = config.file_path.contains("/.atelier/rules/")
            || config.file_path.contains("/.claude/rules/");
        let content = if is_rules_file {
            atelier_tools::implementations::skills::skill::extract_skill_body(&config.content)
        } else {
