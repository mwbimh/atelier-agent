...
//!
//! Plugins from project directories (`.atelier/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.atelier/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.atelier/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.atelier/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_atelier_home() so a project's `.atelier/trusted-plugins` is never
        // read as the user trust store when neither ATELIER_HOME nor a home dir resolves.
        let Some(atelier) = atelier_config::user_atelier_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = atelier.join(TRUST_FILE_NAME);
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.atelier/trusted-plugins`.
    ...
    /// rewrites `~/.atelier/trusted-plugins` without the revoked entry.
