...
///
/// Replaces `${ATELIER_PLUGIN_ROOT}`, `${CLAUDE_PLUGIN_ROOT}`,
/// `${ATELIER_PLUGIN_DATA}`, and `${CLAUDE_PLUGIN_DATA}` with the provided values.
///
/// Delegates to [`atelier_tools::util::substitute_plugin_tokens`], the single
...
pub fn substitute_env_vars(s: &str, plugin_root: &str, plugin_data: &str) -> String {
    atelier_tools::util::substitute_plugin_tokens(s, Some(plugin_root), Some(plugin_data))
}
...
mod tests {
    ...
    fn load_manifest_fallback_paths() {
        ...
        std::fs::create_dir_all(plugin_root.join(".atelier-plugin")).unwrap();

        // Write manifest in .atelier-plugin/ fallback location
        std::fs::write(
            plugin_root.join(".atelier-plugin/plugin.json"),
    ...
    fn substitute_env_vars_replaces_all() {
        let input =
            "${ATELIER_PLUGIN_ROOT}/bin:${CLAUDE_PLUGIN_ROOT}/lib:${ATELIER_PLUGIN_DATA}/cache";
