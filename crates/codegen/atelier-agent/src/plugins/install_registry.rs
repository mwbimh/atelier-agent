...
/// Default install directory name under `~/.atelier/`.
...
/// Lives here (not in atelier-plugin-marketplace) to keep dependency direction sane.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl InstallRegistry {
    ...
    /// 2. Default: `~/.atelier/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        atelier_config::atelier_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
