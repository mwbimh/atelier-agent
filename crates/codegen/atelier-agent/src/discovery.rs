...
//!
//! Searches `.atelier/agents/` and `.claude/agents/` from cwd to repo root,
//! then `~/.atelier/agents/`, then `~/.claude/agents/`. Name-based dedup keeps
//! highest priority.
...
/// Project-level agent directories to scan (`.atelier/agents/` + `.claude/agents/` compat).
const PROJECT_AGENT_SUBDIRS: &[&str] = &[".atelier/agents", ".claude/agents"];

/// Existing project-level agent dirs (`.atelier/agents` / `.claude/agents`), walked
...
/// Existing project agent dirs (`.atelier/agents` / `.claude/agents`) under each
...
pub fn all_subagents(cwd: &Path, toggle: &HashMap<String, bool>) -> Vec<SubagentEntry> {
    ...
    all_subagents_with_home(cwd, toggle, dirs::home_dir().as_deref(), atelier.as_deref())
}
...
fn all_subagents_with_home(
    ...
    let discovered = discover_with_home(cwd, home, atelier_home);
...
fn merge_subagents(
    ...
    //
    // A user-level ~/.atelier/agents/explore.md does NOT shadow built-in explore
...
/// 1. `.atelier/agents/` walking from `cwd` up to repo root
/// 2. `~/.atelier/agents/` (user-level)
/// 3. `~/.claude/agents/` (compat user-level)
/// 4. `~/.atelier/bundled/agents/` (bundled, lowest priority)
///
/// Deduplicates by name — higher-priority definitions win.
/// User-level agent directories in priority order: user atelier agents, `.claude`
/// compat agents, then bundled. `.atelier` dirs resolve from `atelier_home`
/// (ATELIER_HOME-aware) plus the legacy literal `~/.atelier` when ATELIER_HOME points
/// elsewhere; `.claude` resolves from `home`.
pub(crate) fn user_agent_dirs(
    home: Option<&Path>,
    atelier_home: Option<&Path>,
) -> Vec<(std::path::PathBuf, AgentScope)> {
    // Legacy literal ~/.atelier, included only when it differs from atelier_home
    // (i.e. ATELIER_HOME points elsewhere) so agents left in the old location are
    // still discovered and stay consistent with scope_from_path classification.
    let legacy_atelier = home
        .map(|h| h.join(".atelier"))
        .filter(|legacy| atelier_home != Some(legacy.as_path()));

    let mut dirs = Vec::new();
    if let Some(g) = atelier_home {
        dirs.push((g.join("agents"), AgentScope::User));
    }
    if let Some(l) = &legacy_atelier {
...
pub fn discover(cwd: &Path) -> Vec<AgentDefinition> {
    let atelier = atelier_config::user_atelier_home();
    discover_with_home(cwd, dirs::home_dir().as_deref(), atelier.as_deref())
}
...
fn discover_with_home(
    ...
    for (dir, scope) in user_agent_dirs(home, atelier_home) {
        if dir.is_dir() {
...
pub fn by_name(name: &str) -> Option<AgentDefinition> {
    let atelier = atelier_config::user_atelier_home();
    by_name_with_home(name, dirs::home_dir().as_deref(), atelier.as_deref())
}
...
fn by_name_with_home(
    ...
    {
        let home_dirs = user_agent_dirs(home, atelier_home);
...
///
/// Project-level `.atelier/agents/` has highest priority, then falls back
/// to built-ins, user-level, and finally bundled definitions.
pub fn by_name_in_cwd(name: &str, cwd: &Path) -> Option<AgentDefinition> {
    let atelier = atelier_config::user_atelier_home();
    by_name_in_cwd_with_home(name, cwd, dirs::home_dir().as_deref(), atelier.as_deref())
}
...
fn by_name_in_cwd_with_home(
    ...
    by_name_with_home(name, home, atelier_home)
}
...
fn by_name_in_cwd_with_plugins_and_home(
    ...
    if let Some(def) = by_name_in_cwd_with_home(name, cwd, home, atelier_home) {
        return Some(def);
    }
...
/// Expand `${CLAUDE_PLUGIN_ROOT}` / `${CLAUDE_PLUGIN_DATA}` (and the Atelier
...
/// Load project agent definitions from every `.atelier/agents` / `.claude/agents`
...
mod tests {
    ...
    fn test_origin_for_scope(scope: PluginScope) -> crate::plugins::PluginOrigin {
        ...
        match scope {
            ...
            PluginScope::Project => PluginOrigin::ProjectAtelier,
            PluginScope::User => PluginOrigin::UserAtelier,
    ...
    #[test]
    fn user_agent_dirs_includes_legacy_atelier_when_atelier_home_differs() {
        let home = Path::new("/home/u");
        let atelier = Path::new("/custom/atelierhome");
        let paths: Vec<_> = user_agent_dirs(Some(home), Some(atelier))
            .into_iter()
            .map(|(p, _)| p)
            .collect();
        assert!(paths.contains(&atelier.join("agents")));
        assert!(paths.contains(&home.join(".atelier").join("agents")));
        assert!(paths.contains(&home.join(".claude").join("agents")));
        assert!(paths.contains(&atelier.join("bundled").join("agents")));
        assert!(paths.contains(&home.join(".atelier").join("bundled").join("agents")));
    }

    #[test]
    fn user_agent_dirs_dedups_legacy_when_atelier_home_is_dot_atelier() {
        let home = Path::new("/home/u");
        let atelier = home.join(".atelier");
        let count = user_agent_dirs(Some(home), Some(&atelier))
            .into_iter()
            .filter(|(p, _)| *p == atelier.join("agents"))
            .count();
        assert_eq!(
            count, 1,
            "no duplicate ~/.atelier/agents when atelier_home == ~/.atelier"
        );
    }
    ...
    #[test]
    fn test_by_name_builtin_atelier_build() {
        let def = by_name("atelier-build");
        assert!(def.is_some());
        assert_eq!(def.unwrap().name, "atelier-build");
    }
    ...
    fn test_discover_dedup_by_name() {
        ...
        let agents_dir_1 = tmp.path().join(".atelier").join("agents");
        let agents_dir_2 = inner_dir.join(".atelier").join("agents");
    ...
    fn test_discover_includes_bundled_agents_at_lowest_priority() {
        ...
        let bundled_dir = home.join(".atelier").join("bundled").join("agents");
        ...
        let defs = discover_with_home(&cwd, Some(&home), Some(&home.join(".atelier")));
        assert_eq!(defs.len(), 1);
    ...
    fn test_by_name_in_cwd_user_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let user_dir = home.join(".atelier").join("agents");
        let bundled_dir = home.join(".atelier").join("bundled").join("agents");
        fs::create_dir_all(&cwd).unwrap();
        ...
        let def =
            by_name_in_cwd_with_home("reviewer", &cwd, Some(&home), Some(&home.join(".atelier")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_builtin_beats_bundled_for_builtin_names() {
        ...
        let def =
            by_name_in_cwd_with_home("explore", &cwd, Some(&home), Some(&home.join(".atelier")))
                .unwrap();
    ...
    fn test_by_name_in_cwd_project_beats_bundled() {
        ...
        let cwd = tmp.path().join("workspace");
        let home = tmp.path().join("home");
        let project_dir = cwd.join(".atelier").join("agents");
        let bundled_dir = home.join(".atelier").join("bundled").join("agents");
    ...
    fn test_by_name_in_cwd_project_shadows_builtin() {
        ...
        // Create a project-level "atelier-build" that shadows the built-in
        write_agent_file(
            &agents_dir,
            "atelier-build.md",
            "atelier-build",
        ...
        let def = by_name_in_cwd("atelier-build", tmp.path());
        assert!(def.is_some());
        let def = def.unwrap();
        assert_eq!(def.name, "atelier-build");
        assert_eq!(def.description, "Custom atelier-build");
    }

    #[test]
    fn test_by_name_in_cwd_falls_back_to_builtin() {
        let tmp = tempfile::tempdir().unwrap();
        // No .atelier/agents/ directory — should fall back to built-in
    ...
    fn test_orchestrator_from_str_resolves() {
        use std::str::FromStr;
        let variant = BuiltinAgentName::from_str("atelier-build-orchestrator")
            .expect("from_str must resolve atelier-build-orchestrator");
        assert_eq!(variant, BuiltinAgentName::AtelierBuildOrchestrator);
        let def = variant.definition();
        assert_eq!(def.name, "atelier-build-orchestrator");
        assert!(
    ...
    fn test_orchestrator_by_name_in_cwd() {
        let tmp = tempfile::tempdir().unwrap();
        let def = by_name_in_cwd("atelier-build-orchestrator", tmp.path())
            .expect("by_name_in_cwd must find atelier-build-orchestrator");
        assert_eq!(def.name, "atelier-build-orchestrator");
    ...
    fn test_merge_user_level_builtin_name_is_skipped() {
        // A user-level (~/.atelier/agents/) agent named "explore" should NOT shadow
