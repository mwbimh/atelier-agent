...
use atelier_tools::computer::types::{AsyncFileSystem, TerminalBackend};
...
use std::sync::Arc;
/// The Atelier [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so
/// a plugin's upstream allowlist still binds. Backed by the shared vendor-to-Atelier
/// tool registry in `atelier-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    atelier_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.atelier/worktrees/project/fork-...-overlay`) that must stay
    ...
    parent_scheduler_handle:
        Option<atelier_tools::implementations::atelier_build::scheduler::types::SchedulerHandle>,
    ...
    image_gen_config: atelier_tools::implementations::atelier_build::image_gen::ImageGenConfig,
    video_gen_config: atelier_tools::implementations::atelier_build::video_gen::VideoGenConfig,
    ...
    compat: atelier_tools::types::compat::CompatConfig,
    ...
    attribution_callback: Option<atelier_tools::SharedAttributionCallback>,
    ...
    /// `resolve_max_mcp_output_bytes_for_cwd` in atelier-shell).
    ...
    preloaded_skills: Option<Vec<atelier_tools::implementations::skills::types::SkillInfo>>,
}
/// Ensure plan mode tools (`enter_plan_mode`, `exit_plan_mode`,
/// `ask_user_question`) are present in the tool config.
fn ensure_plan_mode_tools(tool_config: &mut atelier_tools::registry::types::ToolServerConfig) {
    use atelier_tools::implementations::atelier_build;
    let existing: std::collections::HashSet<&str> =
        tool_config.tools.iter().map(|tc| tc.id.as_str()).collect();
    let missing_enter = !existing.contains("AtelierBuild:enter_plan_mode");
    let missing_exit = !existing.contains("AtelierBuild:exit_plan_mode");
    let missing_ask = !existing.contains("AtelierBuild:ask_user_question");
    drop(existing);
    if missing_enter {
        tool_config
            .tools
            .push((&atelier_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&atelier_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&atelier_build::AskUserQuestionTool).into());
    }
}
...
fn merge_tool_params(
    tool_config: &mut atelier_tools::registry::types::ToolServerConfig,
    ids: &[&str],
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(atelier_tools::computer::local::LocalFs),
            notification_handle,
            ...
            system_reminder_tag: atelier_tools::reminders::DEFAULT_REMINDER_TAG,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<atelier_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn atelier_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: atelier_tools::implementations::atelier_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: atelier_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// `ATELIER_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: atelier_tools::implementations::atelier_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn atelier_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    pub fn with_image_gen_config(
        mut self,
        config: atelier_tools::implementations::atelier_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    pub fn with_video_gen_config(
        mut self,
        config: atelier_tools::implementations::atelier_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: atelier_tools::implementations::atelier_build::deploy_app::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: atelier_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the AtelierBuild Task description.
    ...
    ///
    /// When disabled, `AtelierBuild:ask_user_question` is stripped from the
    ...
    /// Without this, only auto-discovered skills (cwd/.atelier/skills, ~/.atelier/skills)
    /// are included — custom paths added via `atelier/skills/add` would be ignored.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                ...
                tool_config
                    .tools
                    .push((&atelier_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use atelier_tools::implementations::atelier_build;
                tool_config
                    .tools
                    .push((&atelier_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
            ...
            if self.write_file_enabled && !has_write_tool {
                tool_config
                    .tools
                    .push((&atelier_tools::implementations::opencode::OpenCodeWriteTool).into());
            }
            ensure_plan_mode_tools(&mut tool_config);
        }
        if self.memory_backend.is_none() {
            let atelier_build_ns =
                atelier_tools::types::tool::ToolNamespace::AtelierBuild.to_string();
            let mem_search_id = format!(
                "{atelier_build_ns}:{}",
                atelier_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{atelier_build_ns}:{}",
                atelier_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if !self.ask_user_question_enabled {
            let ask_user_id = format!(
                "{}:ask_user_question",
                atelier_tools::types::tool::ToolNamespace::AtelierBuild,
            );
        ...
        if task_stripped {
            use atelier_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::AtelierBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::AtelierBuildConcise, "run_terminal_cmd", true)
        ...
        if let atelier_tools::implementations::atelier_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["AtelierBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "AtelierBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        if let Some(ref ask_params) = self.ask_user_question_params_json {
            merge_tool_params(
                &mut tool_config,
                &["AtelierBuild:ask_user_question"],
                ask_params,
            );
        }
        ...
        if !definition.tools.is_empty() {
            ...
            if unresolved.is_empty() {
                ...
                tracing::warn!(
                    ...
                    definition.tools,
                    "tools allowlist had unmappable entries; keeping full atelier toolset"
                );
            }
        }
        tool_config
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                atelier_tools::types::resources::TruncationCfg(
                    atelier_tools::types::context::TruncationConfig {
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(atelier_sampling_types::HostedTool::WebSearch {
...
mod tests {
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: atelier_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_lists_public_model_slugs() {
        ...
        assert!(!desc.contains(concat!("atelier", " models")));
    }
    #[test]
    ...
    fn task_model_guidance_resolves_model_param_override() {
        use atelier_tools::types::template_renderer::TemplateRenderer;
        use atelier_tools::types::tool::ToolKind;
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "atelier-build / subagents+ask_user",
                profile: AgentDefinition::default_atelier_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build / subagents / no-ask-user",
                profile: AgentDefinition::default_atelier_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "atelier-build / no-subagents / ask_user",
                profile: AgentDefinition::default_atelier_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_atelier_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "atelier-build-ask-user / subagents",
                profile: AgentDefinition::atelier_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build-ask-user / no-subagents",
                profile: AgentDefinition::atelier_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build-plan",
                profile: AgentDefinition::atelier_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build-plan / no-ask-user",
                profile: AgentDefinition::atelier_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "atelier-build-plan-no-subagents",
                profile: AgentDefinition::atelier_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "atelier-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::atelier_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use atelier_tools::computer::local::LocalTerminalBackend;
        use atelier_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_atelier_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        use atelier_tools::computer::local::LocalTerminalBackend;
        use atelier_tools::implementations::atelier_build::ask_user_question::AskUserQuestionParams;
        use atelier_tools::notification::ToolNotificationHandle;
        use atelier_tools::types::resources::Params;
        let profile = crate::config::AgentDefinition::default_atelier_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "AtelierBuild:ask_user_question"),
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use atelier_tools::computer::local::LocalTerminalBackend;
        use atelier_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_atelier_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("AtelierBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use atelier_tools::computer::local::LocalTerminalBackend;
        use atelier_tools::implementations::atelier_build::bash::BashParams;
        use atelier_tools::notification::ToolNotificationHandle;
        use atelier_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_atelier_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Atelier
    /// equivalents by `ToolKind` — a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_atelier_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their atelier
    ...
    /// treat `mcp__*` as always-on, so atelier keeps the MCP meta-tools
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        ...
        use atelier_tools::implementations::web_search::WebSearchConfig;
        use atelier_tools::notification::ToolNotificationHandle;
        let mut definition = crate::config::AgentDefinition::default_atelier_build();
        ...
        .with_web_search_config(WebSearchConfig::Enabled {
            ...
            base_url: "https://api.atelier/v1".into(),
    ...
    /// atelier-build toolsets have no Skill tool — skills are read from
    ...
    /// Compat `ToolSearch` meta-tool maps to atelier's `search_tool` (MCP
