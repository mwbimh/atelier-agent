...
This crate extracts a first-class `Agent` type from `atelier-shell`.
...
`atelier-shell`, another in-process host, or a headless batch runner.
...
in `.atelier/agents/` (project-level) or `~/.atelier/agents/` (user-level).

```rust
use atelier_agent::{AgentDefinition, AgentBuilder};
use atelier_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".atelier/agents/code-reviewer.md")?;
...
```rust
use atelier_agent::discovery;

// Find all .md files in .atelier/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  atelier-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`atelier-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `atelier-agent` depends on it for tool setup.
- **`atelier-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `atelier-agent` (AGENTS.md
...
|---|---|---|
| `atelier-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p atelier-agent

# Test
cargo test -p atelier-agent

# Clippy
cargo clippy -p atelier-agent --fix --allow-dirty
