//! Memory-system configuration value types, extracted from atelier-shell
...
//! `atelier-shell` — `resolve()` depends on `toml` and on shell-internal
...
///
/// When enabled, watches `~/.atelier/memory/` for `.md` file changes (create,
...
///
/// On session init, directories under `~/.atelier/memory/` are scanned:
