...
pub use permission::*;
mod pool;
use atelier_announcements::RemoteAnnouncement;
pub use pool::*;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-atelier-doom-loop-check` header and parse the reported
...
pub struct LocalRuntimeSettings {
    ...
    /// `app::subscription` module). Forwarded from the `atelier_build_settings`
    ...
    /// default (on). Sits below env `ATELIER_FOLDER_TRUST`, user
    ...
    /// remote settings `atelier_build_settings.max_mcp_output_bytes` — global default
    ...
    /// `atelier_build_settings` remote settings key) to enable it. See
    ...
    /// `ATELIER_TWO_PASS_COMPACTION` / default (off).
    #[serde(default)]
    ...
    /// Controlled via remote settings (`non_git_warning` in `atelier_build_settings`).
    ...
    /// ghost text), from the `atelier_build_settings` remote settings flag. Sits below
    /// env (`ATELIER_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion`
    /// in config.toml, above the client hint and the built-in
    /// `atelier-build-0.1` default. The effective model is catalog-guarded: when
    ...
    /// (e.g. `atelier-imagine-image`) instead of the default quality model
    /// (`atelier-imagine-image-quality`). Absent/empty → default model.
    #[serde(default)]
    ...
    /// Overridable locally via `ATELIER_VOICE_MODE`. Free-tier SuperAtelier upsell
    ...
    /// User's subscription tier from remote settings `atelier_build_access_gate`.
    /// E.g. "free", "premium", "superatelier", "superatelier_heavy".
    ...
    /// Whether the user is allowed to use Atelier. Set by remote settings
    /// `atelier_build_access_gate` targeting rules. `None` = no server response
    ...
    /// Server-controlled via the remote settings `atelier_build_usage_redirect_url`
    ...
    /// `atelier_build_settings`. Per-model override on `ModelInfo`
    /// (`atelier_build_models`) takes precedence; user config and env var
    ...
    /// `0` disables. Env (`ATELIER_COMPACTION_WALL_CLOCK_SECS`) overrides it.
    ...
    /// Env (`ATELIER_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none` | `minimal` | `balanced` | `verbose`)
    /// from remote settings. Env (`ATELIER_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`ATELIER_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// remote settings gate for the `atelier workspace` CLI command (Computer Hub
    /// workspace exposure), from `atelier_build_settings.workspace_command_enabled`.
...
/// only guaranteed to work with a compatible harness (cursor vs atelier-build).
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct GoalRoleModel {
    /// Model id, e.g. "atelier-4". Resolved against available models at
    /// spawn time; unknown/unauthorized ⇒ fail-open to current model.
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "atelier-build-plan") whose
    /// `AgentDefinition` decides the role subagent's harness flavor (system
    /// prompt + cursor-vs-atelier-build toolset), applied REGARDLESS of the
