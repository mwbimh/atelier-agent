...
//!
//! ```
//! use atelier_mermaid::{default_engine, render_checked, RenderLimits, RenderParams};
//!
...
pub enum MermaidTheme {
    /// Light surfaces with dark text (e.g. `AtelierDay`).
    #[default]
    Light,
    /// Dark surfaces with light text (e.g. `AtelierNight`, `TokyoNight`).
    Dark,
}
