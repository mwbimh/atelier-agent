...
pub use atelier_config::atelier_home;

/// Path to `$ATELIER_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    atelier_home().join("pager.toml")
}

/// User-facing label for the user atelier directory (``~/.atelier`` or ``$ATELIER_HOME``).
///
/// Derived from resolved [`atelier_home()`] vs `atelier_config::default_atelier_home()`,
/// not from whether `ATELIER_HOME` is set in the environment.
pub fn display_atelier_home_prefix() -> String {
    if atelier_home() == atelier_config::default_atelier_home() {
        "~/.atelier".to_string()
    } else {
        "$ATELIER_HOME".to_string()
    }
}

/// User-facing path under [`atelier_home()`], e.g. ``~/.atelier/config.toml``.
pub fn display_user_atelier_path(relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_atelier_home_prefix();
...
/// Abbreviate an absolute path for display: prefer [`atelier_home()`], then `$HOME`.
pub fn abbreviate_path(path: &str) -> Cow<'_, str> {
    let path_buf = Path::new(path);
    let atelier = atelier_home();
    if let Ok(rest) = path_buf.strip_prefix(&atelier) {
        let prefix = display_atelier_home_prefix();
...
/// True when `path` is under user [`atelier_home()`] (not project `{cwd}/.atelier`).
pub fn is_under_user_atelier_home(path: &Path) -> bool {
    path.starts_with(atelier_home())
}
...
mod tests {
    ...
    #[test]
    fn display_atelier_home_prefix_default_install() {
        if std::env::var("ATELIER_HOME").is_ok() {
            return;
        }
        assert_eq!(display_atelier_home_prefix(), "~/.atelier");
    }

    #[test]
    fn display_user_atelier_path_joins_relative() {
        let path = display_user_atelier_path("config.toml");
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".atelier") || path.contains("$ATELIER_HOME"));
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_atelier() {
        if let Ok(home) = std::env::var("HOME") {
            if home.is_empty() {
                return;
            }
            let full = format!("{home}/.atelier/memory/MEMORY.md");
