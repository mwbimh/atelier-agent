...
/// in `config.toml`. When `None`, defaults to `AtelierNight` / `AtelierDay`.
///
...
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::AtelierDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::AtelierNight),
    }
}
