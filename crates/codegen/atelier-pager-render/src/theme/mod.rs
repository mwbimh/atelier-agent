...
//! The default theme is AtelierNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! AtelierNight is defined in `Color::Rgb` (truecolor). At startup,
...
impl ThemeKind {
    ...
    pub const ALL: &[ThemeKind] = &[
        ThemeKind::AtelierNight,
        ThemeKind::AtelierDay,
    ...
    pub fn available() -> &'static [ThemeKind] {
        ...
        const NO_TRUECOLOR: &[ThemeKind] = &[ThemeKind::AtelierNight, ThemeKind::AtelierDay];
    ...
    pub fn display_name(self) -> &'static str {
        match self {
            Self::AtelierNight => "ateliernight",
            Self::TokyoNight => "tokyonight",
            Self::AtelierDay => "atelierday",
    ...
    /// when quantized to 256 or 16 colors. AtelierNight uses neutral grays
    /// that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::AtelierNight => false,
            Self::TokyoNight => true,
            Self::AtelierDay => false,
    ...
    pub fn from_name(name: &str) -> Option<Self> {
        ...
        match lower.as_str() {
            ...
            "ateliernight" | "atelier-night" | "dark" => Some(Self::AtelierNight),
            "tokyonight" | "tokyo-night" | "tokyo" => Some(Self::TokyoNight),
            "atelierday" | "atelier-day" | "light" | "day" => Some(Self::AtelierDay),
...
/// `"ateliernight"` → `"Atelier Night"`). Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "ateliernight" => "Atelier Night",
        "atelierday" => "Atelier Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.atelier/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        let base = match cache::current_kind() {
            ThemeKind::AtelierNight => Self::ateliernight(),
            ThemeKind::TokyoNight => Self::tokyonight(),
            ThemeKind::AtelierDay => Self::atelierday(),
            ...
            // if reached, fall back to AtelierNight.
            ThemeKind::Auto => Self::ateliernight(),
        };
        ...
        //      (e.g., `TERM=xterm`, `TERM=ansi`, or `ATELIER_FORCE_COLOR_LEVEL=basic`),
    ...
    ///    on ateliernight, erasing every state signal). We pin each
    ...
    /// theme (e.g., AtelierNight on a white terminal would show white
    ...
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // AtelierNight bg field quantizes to Color::Black and these
            // become invisible.
...
pub fn apply_cursor_color() {
    ...
    atelier_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::AtelierNight.is_auto());
        assert!(!ThemeKind::AtelierDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::atelierday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::ateliernight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::atelierday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // the theme disagrees with the terminal profile (e.g. AtelierNight
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::AtelierNight => Theme::ateliernight(),
                ThemeKind::AtelierDay => Theme::atelierday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_ateliernight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`:
        // naive `Basic` quantization maps every dark AtelierNight bg field
        ...
        use ratatui::style::Color;
        let q = Theme::ateliernight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("ateliernight"),
            Some(ThemeKind::AtelierNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::AtelierNight));
        assert_eq!(
            ThemeKind::from_name("atelierday"),
            Some(ThemeKind::AtelierDay)
        );
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::AtelierDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("ateliernight", ThemeKind::AtelierNight),
            ("atelier-night", ThemeKind::AtelierNight),
            ("dark", ThemeKind::AtelierNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("atelierday", ThemeKind::AtelierDay),
            ("atelier-day", ThemeKind::AtelierDay),
            ("light", ThemeKind::AtelierDay),
            ("day", ThemeKind::AtelierDay),
