...
//!
//! Extracted from the `OpenSuperatelierUrl` dispatch handler so that any
...
pub fn open_url(url: &str) {
    ...
    if let Ok(path) = std::env::var("ATELIER_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "ATELIER_TEST_OPEN_URL_FILE write failed");
        }
        return;
    }
    ...
    atelier_tools::util::detach_std_command(&mut command);
...
///
/// Used by the SuperAtelier upsell flow to attribute clicks to `referrer=atelier-build`,
/// matching the OAuth consent screen and atelier/cli marketing links regardless of
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/atelier session/image 1.jpg");
    ...
    fn ensure_query_param_appends_when_missing() {
        let out = ensure_query_param(
            "https://atelier.invalid/superatelier",
            "referrer",
            "atelier-build",
        );
        assert_eq!(
    ...
    fn ensure_query_param_preserves_existing_value() {
        let out = ensure_query_param(
            "https://atelier.invalid/superatelier?referrer=other",
            "referrer",
            "atelier-build",
        );
        assert_eq!(out, "https://atelier.invalid/superatelier?referrer=other");
    }

    #[test]
    fn ensure_query_param_keeps_other_query_pairs() {
        let out = ensure_query_param(
            "https://atelier.invalid/superatelier?heavy=1",
            "referrer",
            "atelier-build",
        );
        assert_eq!(
            out,
            "https://atelier.invalid/superatelier?heavy=1&referrer=atelier-build"
        );
    }
    ...
    fn ensure_query_param_preserves_fragment() {
        ...
        // routing (`atelier.invalid/#superatelier`); we still want the referrer attached.
        let out = ensure_query_param(
            "https://atelier.invalid/#superatelier",
            "referrer",
            "atelier-build",
        ...
        assert_eq!(
            out,
            "https://atelier.invalid/?referrer=atelier-build#superatelier"
        );
    }

    #[test]
    fn ensure_query_param_returns_unchanged_on_parse_failure() {
        let out = ensure_query_param("not a url", "referrer", "atelier-build");
