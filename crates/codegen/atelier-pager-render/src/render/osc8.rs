...
/// One path segment without spaces (`main.rs`, `.atelier`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments — atelier
/// session media lives under `~/.atelier/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.atelier/x.txt`) — one or more `/`-joined
...
/// media lives at `~/.atelier/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    fn scan_detects_atelier_session_media_path() {
        ...
        let line =
            make_line("Saved to /Users/alice/.atelier/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.atelier/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.atelier/sessions/%2FUsers%2Fali");
        ...
        let expected_url = "file:///Users/alice/.atelier/sessions/%252FUsers%252Fali\
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.atelier/sessions/%2FUsers%2Fali"
            ) as u16
        );
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.atelier/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*link.url,
                "file:///Users/me/.atelier/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        ...
        assert_eq!(
            &*overlay.links()[0].url,
            "file:///tmp/atelier-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/atelier-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/atelier-pager/src/render` should
        // NOT produce a link for the `/atelier-pager/src/render` substring.
        let line = make_line("find crates/codegen/atelier-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
