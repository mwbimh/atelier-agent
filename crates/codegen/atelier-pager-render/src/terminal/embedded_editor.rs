//! Detects whether atelier is running inside an editor's embedded `:terminal`
...
/// Which embedded editor `:terminal` atelier is running inside.
...
/// `HOST_TERMINAL_ENV_VARS` in `atelier-pager-pty-harness/src/pty.rs`
/// (test-env hygiene).
