...
pub enum TerminalName {
    ...
    JetBrains,
    /// Atelier Desktop (Electron app).
    #[strum(to_string = "Atelier Desktop")]
    AtelierDesktop,
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` atelier is running inside, if any.
...
impl TerminalContext {
    ...
    /// keyed off this only fires when a FocusGained actually reaches atelier, which
    ...
    pub fn telemetry_snapshot(&self) -> atelier_telemetry::events::TerminalTelemetry {
        ...
        atelier_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: atelier_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> atelier_shared::session::FeedbackTerminalInfo {
        use atelier_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(atelier_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // first client, not the current one). Revisit when `atelier ssh` can
    // forward env vars.
...
/// `atelier-pager-pty-harness/src/pty.rs` (test-env hygiene — the PTY
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "atelierdesktop" => Some(TerminalName::AtelierDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.atelier/pager.toml` and
