...
/// minimal mode (`atelier --minimal`) sets this once at startup: it never runs the
...
pub fn kitty_format_from_bytes(image_data: &[u8]) -> Option<KittyImageFormat> {
    match atelier_shared::clipboard::mime_from_bytes(image_data) {
...
fn convert_via_sips(image_data: &[u8]) -> Option<Vec<u8>> {
    ...
    let ts = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let src = tmp_dir.join(format!("atelier-sips-{id}-{ts}.dat"));
    let dst = tmp_dir.join(format!("atelier-sips-{id}-{ts}.png"));
