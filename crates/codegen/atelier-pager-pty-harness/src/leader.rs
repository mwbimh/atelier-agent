...
//! elected leader) plus a fixed leader socket beneath its `ATELIER_HOME`; clients
...
impl LeaderCluster {
    ...
    /// one leader) and a fixed leader socket under its `ATELIER_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared ATELIER_HOME => one leader; the socket lives beneath it so
        // every client (sharing the same env) elects/attaches to the same one.
        let atelier_home = content.home().join(".atelier");
        std::fs::create_dir_all(&atelier_home).context("create atelier home")?;
        let socket = atelier_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `ATELIER_HOME/sessions` (layout below is
    /// `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".atelier").join("sessions")
    }
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_atelier/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
    ...
    fn parse_update_payloads_unwraps_and_tolerates_torn_trailing_line() {
        let body = format!(
            "{}\n{}\n{{\"timestamp\":2,\"method\":\"_atelier/sess",
            envelope(
