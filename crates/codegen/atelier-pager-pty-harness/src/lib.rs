//! Unified PTY harness for atelier-pager.
//!
...
//!    `atelier-pager/tests/scenarios/`) — assert screen contents and
...
//! - **`scroll_matrix`**  — `ATELIER_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    ...
    /// # use atelier_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// through atelier's stdout. Used to reproduce the doubled-line class of bugs
    /// where atelier's diff renderer never re-asserts a region it didn't write
