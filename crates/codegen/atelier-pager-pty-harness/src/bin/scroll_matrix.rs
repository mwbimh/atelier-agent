//! `scroll-matrix` — scroll validation matrix sweep for atelier-pager.
//!
...
use atelier_pager_pty_harness::pager_binary;
use atelier_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against atelier-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary. Defaults to PAGER_BINARY, CARGO_BIN_EXE_atelier-pager,
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves ATELIER_SCROLL_LOG against ITS cwd (the
