//! `pty-bench` — PTY benchmark CLI for `atelier-pager`.
//!
...
//! ```bash
//! cargo bench -p atelier-pager-pty-harness \
...
//! ```bash
//! PAGER_BINARY=./artifacts/atelier-${VERSION}-linux-x86_64 \
//!   cargo bench -p atelier-pager-pty-harness \
//!   --bench pty_bench -- --all \
...
#[command(
    name = "pty-bench",
    about = "PTY benchmark harness for atelier-pager",
    long_about = None,
)]
struct Cli {
