...
fn apply_headless_args_to_config(args: &HeadlessArgs, config: &mut AgentConfig) {
    if let Some(v) = &args.atelier_ws_origin {
        config.atelier_com_config.atelier_ws_origin = v.clone();
    }
    if let Some(v) = &args.atelier_ws_url {
        config.atelier_com_config.atelier_ws_url = v.clone();
    }
}
/// Apply global endpoint CLI args to an existing config.
fn apply_agent_endpoint_args(agent_args: &atelier_pager::app::AgentArgs, config: &mut AgentConfig) {
...
fn print_serve_startup_info(bind_addr: SocketAddr, secret: &str) {
    eprintln!();
    eprintln!("   Atelier agent server starting...");
    eprintln!();
...
/// Entrypoint tag for `atelier -p`; keys the quiet stderr default in `init_tracing_simple`.
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    use atelier_telemetry::debug_log::RMCP_SSE_NOISE_TARGET;
...
async fn run_leader_mgmt(args: LeaderMgmtArgs) -> Result<()> {
    match args.command {
        ...
        LeaderMgmtCommand::List { json } => {
            let leaders = atelier_shell::leader::discover_leaders().await;
            if json {
...
async fn kill_leaders() -> Result<()> {
    ...
    for d in &leaders {
        ...
        if !atelier_shell::util::is_atelier_process(pid) {
            if let Some(ref lock) = d.lock_path {
                eprintln!("  PID {pid} is not an Atelier process, removing stale lock");
        ...
        if let Err(e) = atelier_shell::util::kill_process_by_pid(pid) {
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(atelier_shell::env::AtelierBuildEnvironment::Production),
    }
}
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, atelier_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = atelier_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = atelier_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&atelier_shell::leader::ControlPayload>,
...
async fn workspace_control(
    ...
    let raw_config = atelier_shell::config::load_effective_config_disk_only()
...
struct StdioReplayState {
    ...
    /// `atelier/leader_reconnected` as the primary restored session.
...
fn cache_outgoing_acp_state(msg: &str, state: &std::sync::Mutex<StdioReplayState>) {
    ...
    match method {
        ...
        "atelier/session/close" | "_atelier/session/close" => {
...
const REPLAY_LOAD_REQUEST_ID: &str = "atelier/leader-replay/session-load";
...
/// `atelier/leader_reconnected` with empty params in that case, signalling the
...
/// can't drift.
const PLUGIN_DIR_LEADER_WARNING: &str = "atelier: --plugin-dir is ignored in leader mode; run with --no-leader to \
...
async fn run_agent_command(
    ...
    if trust {
        match std::env::current_dir() {
            Ok(cwd) => atelier_shell::agent::folder_trust::grant_folder_trust(&cwd),
            Err(e) => {
                tracing::warn!(
                                    ...
                                    .await
                                {
                                    Ok((new_tx, mut new_rx, _disconnect_rx)) => {
                                        ...
                                        let notification = format!(
                                            r#"{{"jsonrpc":"2.0","method":"atelier/leader_reconnected","params":{params}}}"#
                                        );
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = atelier_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            atelier_shell::agent::run_agent_server(server_config, agent_config).await
        }
...
/// (`[dashboard].enabled = false` / `ATELIER_AGENT_DASHBOARD=0`) is a CLI
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !atelier_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.atelier/config.toml and \
             unsetting ATELIER_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("ATELIER_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<atelier_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(atelier_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<atelier_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(atelier_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    atelier_shell::heap_profile::install(atelier_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    {
        atelier_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        atelier_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    install_heap_profile_hooks();
    if let Some(code) = atelier_pager::app::mermaid_worker::maybe_run_render_subprocess() {
        std::process::exit(code);
    }
    atelier_pager::memory_trace::start(
        atelier_shell::util::atelier_home::atelier_home().join("memtrace"),
    );
    raise_fd_limit();
    if let Err(e) = atelier_config::validate_requirements() {
        eprintln!("Couldn't start Atelier: {e}");
        eprintln!();
        eprintln!(
            "Update Atelier to a version the policy allows, or ask your administrator \
    ...
    let crashed = atelier_shell::active_sessions::collect_crashed().unwrap_or_default();
    if !crashed.is_empty() {
        tracing::info!(
            count = crashed.len(),
...
async fn async_main(mut args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("ATELIER_DEBUG_LOG", path);
            std::env::remove_var("ATELIER_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("ATELIER_DEBUG_LOG", "1");
        set_if_unset("ATELIER_HOOKS_LOG", "1");
    }
    if let Some(Command::Completions { shell }) = &args.command {
        atelier_pager::completions_cmd::run(*shell);
        return Ok(());
    }
...
fn run_async_command_loop(mut args: PagerArgs) -> Pin<Box<dyn Future<Output = Result<()>>>> {
    ...
    let Some(Command::Agent(agent_args)) = args.command.take() else {
        unreachable!("command was checked to be Agent");
    };
    let invalid_leader_flag = if args.leader {
        Some("--leader")
    } else if args.no_leader {
        Some("--no-leader")
    } else {
        None
    };
...
async fn legacy_async_command_loop(mut args: PagerArgs) -> Result<()> {
        ...
        match command {
            ...
            Command::Models => {
                ...
                let agent_config = AgentConfig::new_from_toml_cfg(&config)
                    .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
                return atelier_pager::models::list_available_models(&agent_config).await;
            }
    ...
    let headless_prompt = atelier_pager::headless::HeadlessPrompt::from_args(
    ...
    if let Some(prompt) = headless_prompt {
        ...
        let json_schema = args
            .json_schema
            .as_deref()
            .map(atelier_pager::headless::parse_json_schema)
            .transpose()?;
        if json_schema.is_some() {
            if args.output_format == atelier_pager::headless::OutputFormat::Plain {
                args.output_format = atelier_pager::headless::OutputFormat::Json;
            }
            if args.self_verify {
                anyhow::bail!(
        ...
        return atelier_pager::headless::run_single_turn(
            prompt,
            args.verbatim,
            atelier_pager::headless::HeadlessOptions {
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in atelier_shell::leader::discover_leaders().await {
        if d.classification != atelier_shell::leader::LeaderDiscoveryState::Reachable {
            continue;
        }
        ...
        let client = match atelier_shell::leader::LeaderClient::connect(
            socket_path,
            "atelier-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(atelier_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
            ...
            Ok(Ok(atelier_shell::leader::ControlPayload::RelaunchDeclined { reason })) => {
...
mod tests {
    ...
    fn require_opt_prof() -> bool {
        ...
        eprintln!(
            ...
            atelier_shell::heap_profile::LG_PROF_SAMPLE
        );
        false
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: atelier_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/atelier-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(atelier_shell::heap_profile::stats().expect("shell stats after install"));
        if !require_opt_prof() {
            assert!(!atelier_shell::heap_profile::prof_available());
            return;
        }
        assert!(atelier_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(atelier_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(atelier_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["atelier", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("ATELIER_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
    ...
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["atelier", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    /// `ATELIER_AGENT_DASHBOARD=0` disables the feature — the subcommand
    /// must error visibly before the TUI starts.
    #[serial_test::serial(ATELIER_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_errors_when_disabled() {
        unsafe { std::env::set_var("ATELIER_AGENT_DASHBOARD", "0") };
        let mut args = PagerArgs::try_parse_from(["atelier", "dashboard"]).unwrap();
        let result = flag_dashboard_at_startup_if_requested(&mut args);
        unsafe { std::env::remove_var("ATELIER_AGENT_DASHBOARD") };
        let err = result.expect_err("disabled dashboard must error");
        assert!(err.to_string().contains("disabled"), "got: {err}");
        assert!(
            std::env::var("ATELIER_OPEN_DASHBOARD_AT_STARTUP").is_err(),
    ...
    #[serial_test::serial(ATELIER_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("ATELIER_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("ATELIER_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("ATELIER_WORKSPACE_COMMAND", "off") };
    ...
    fn cache_session_close_stops_replaying_it() {
        ...
        cache_outgoing_acp_state(
            r#"{"jsonrpc":"2.0","id":3,"method":"_atelier/session/close","params":{"sessionId":"s1"}}"#,
            &state,
        );
        let s = state.lock().unwrap();
    ...
    async fn replay_waits_for_load_response_through_notifications() {
        ...
        let responder = tokio::spawn(async move {
            ...
            response_tx
                .send(
                    r#"{"jsonrpc":"2.0","method":"atelier/leader/version_mismatch","params":{}}"#
                        .to_string(),
                )
                .unwrap();
            response_tx
    ...
    /// `atelier/leader_reconnected` with empty params and the external client
