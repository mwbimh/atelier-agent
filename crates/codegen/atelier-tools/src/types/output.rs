...
use crate::implementations::atelier_build::todo::{TodoItem, TodoState};
...
/// protocol boundary in `atelier-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the atelier_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the atelier_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when AtelierBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::atelier_build::monitor::types::MonitorOutput),
    SchedulerCreate(
        crate::implementations::atelier_build::scheduler::create::SchedulerCreateOutput,
    ),
    SchedulerDelete(
        crate::implementations::atelier_build::scheduler::delete::SchedulerDeleteOutput,
    ),
    SchedulerList(crate::implementations::atelier_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::atelier_build::update_goal::UpdateGoalOutput),
...
mod tests {
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/atelier-worktree/wt-agent</worktree_path>"),
