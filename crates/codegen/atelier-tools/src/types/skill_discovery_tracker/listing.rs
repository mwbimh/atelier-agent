...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (atelier build harness) ─────────────
...
///
/// `"/path/.atelier/skills/my-skill/SKILL.md"` -> `"/path/.atelier/skills/"`
...
/// Used by the budgeted (atelier build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Atelier build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the atelier
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // atelier-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: atelier build harness (budgeted XML) ───────────
