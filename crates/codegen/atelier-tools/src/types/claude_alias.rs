//! Canonical external-settings tool name ↔ Atelier tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`atelier-hooks`) needs the
//! Atelier tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`atelier-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `atelier` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Atelier, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Atelier [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Atelier tool names this Claude tool maps to (empty when there is no direct
    /// Atelier tool — the entry then only contributes a `kind`).
    atelier: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, atelier: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        atelier,
...
const fn match_only(claude: &'static str, atelier: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        atelier,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (atelier-build reads SKILL.md)
...
/// The Atelier [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Atelier tool names a Claude matcher term fires on.
pub fn atelier_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.atelier)
...
/// The Claude names that map to `atelier_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(atelier_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.atelier.contains(&atelier_name))
        .map(|t| t.claude)
}

/// Every distinct Atelier name the table references, for the `atelier-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn atelier_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.atelier.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Atelier name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.atelier.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
