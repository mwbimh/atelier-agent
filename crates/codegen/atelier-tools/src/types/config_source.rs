...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.atelier/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.atelier/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.atelier/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.atelier/ or ~/.claude/.
    User { path: PathBuf },
