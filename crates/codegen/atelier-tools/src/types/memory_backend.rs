...
//!
//! `MemoryBackend` is defined in `atelier-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `atelier-tools`.
/// Mirrors `atelier_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "atelier_memory";
