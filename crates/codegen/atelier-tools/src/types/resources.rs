...
/// `"namespace.Name"` (e.g., `"atelier_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"atelier_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("atelier_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "atelier_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"atelier_build.ReadFile"`).
    id: String,
...
/// tool falls back to `Cwd/.atelier/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".atelier/plan.md";
...
/// [`Cwd`]`/.atelier/plan.md`, else the bare relative `.atelier/plan.md`.
...
register_resource!("atelier_build", "WebCitation", WebCitationCounter);
...
/// depending on `atelier-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("atelier_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("atelier_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("atelier_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "atelier_build.ReadFile".to_string(),
            serde_json::json!({ "files_read" : ["loaded.rs"] }),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "atelier_build.Edit".to_string(),
            serde_json::json!(
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "atelier_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "atelier_build.Edit").is_none());
    }
    #[test]
