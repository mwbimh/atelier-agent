...
//!
//! `ToolSearchIndex` is defined in `atelier-tools` to keep the tool crate
//! backend-agnostic. The concrete implementation lives in `atelier-shell`
...
/// in `atelier-shell` maps `mcp_initialized` to `is_ready`.
...
/// serialized), injected by `atelier-shell` after MCP initialization.
#[derive(Clone)]
