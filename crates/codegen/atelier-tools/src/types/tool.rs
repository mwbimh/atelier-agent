...
//!
//! The tool runtime contract (`Tool` trait) lives in `atelier_tool_runtime`.
...
///
/// Serializes to snake_case (`atelier_build`, `mcp`, …) for the
...
/// (e.g. `"AtelierBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "AtelierBuild")]
    AtelierBuild,
    #[serde(alias = "AtelierBuildConcise")]
    AtelierBuildConcise,
    #[serde(alias = "AtelierBuildHashline")]
    AtelierBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
