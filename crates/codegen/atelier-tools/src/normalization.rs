...
/// Resolve `wire_name` in `toolset` and merge the canonical `atelier/tool` object
...
/// the `atelier/tool` `_meta` object. Equivalent tools across toolsets emit the
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let atelier = parse(serde_json::json!({ "variant" : "ReadFile", "target_file" : "/a" }));
        let g = canonical_input(&atelier).unwrap();
