...
//! after every tool call to check for SKILL.md files in `.atelier/skills/`,
...
/// Directories that contain skill definitions (`.atelier/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".atelier", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.atelier/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.atelier/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.atelier/) are outside the git root so the walk breaks early.
