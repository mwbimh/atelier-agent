...
///
/// `skills` is the standard layout (`.atelier/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Atelier sessions.
...
/// name (e.g. `~/.atelier/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Atelier.
...
/// `cwd` (exclusive). At each directory, checks for `.atelier/skills/`,
...
/// visits known config roots (`.atelier`, `.agents`, `.claude`, …); those are
...
pub fn discover_skills_for_paths(
    ...
    // `.atelier` and `.agents` are always scanned; `.claude` is gated on the
    // claude-vendor skills cell. (`.cursor` is excluded here by design — see fn docs.)
    let mut config_dir_names: Vec<&str> = vec![".atelier", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.atelier is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.atelier/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.atelier/ → kept (user content).
        let atelier_shell = tmp.path().join(".atelier").join("skills").join("shell");
        std::fs::create_dir_all(&atelier_shell).unwrap();
        std::fs::write(
            atelier_shell.join("SKILL.md"),
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .atelier skill in an intermediate dir.
        ...
        .unwrap();
        let atelier_skill = sub.join(".atelier").join("skills").join("atelier-dyn");
        std::fs::create_dir_all(&atelier_skill).unwrap();
        std::fs::write(
            atelier_skill.join("SKILL.md"),
            "---\nname: atelier-dyn\n---\n",
        )
        .unwrap();
        ...
        assert!(
            names_on.contains(&"atelier-dyn"),
            "atelier-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only atelier-dyn discovered.
        ...
        assert!(
            names_off.contains(&"atelier-dyn"),
            "atelier-dyn missing: {names_off:?}"
        );
        assert!(
