//! LSP server configuration from `.atelier/lsp.json`.
...
/// This is the canonical merge function — both session startup and `atelier inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::atelier_home::atelier_home().join("lsp.json");
    let project_path = cwd.join(".atelier").join("lsp.json");
...
/// Load LSP server configs from `~/.atelier/lsp.json` and `<cwd>/.atelier/lsp.json`.
