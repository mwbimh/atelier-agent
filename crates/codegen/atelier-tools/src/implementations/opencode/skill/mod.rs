...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.atelier/skills/ or .atelier/skills/)
${%- endif %}
</available_skills>"#;
...
impl atelier_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> atelier_tool_protocol::ToolId {
        atelier_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, atelier_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(atelier_tool_runtime::Tool::id(&tool).as_str(), "skill");
