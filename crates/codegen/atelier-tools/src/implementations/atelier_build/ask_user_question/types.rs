...
//!
//! - **`atelier-tools`** — tool blocks on a oneshot, formats the result.
//! - **`atelier-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`atelier-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `atelier-tools`.
...
register_resource!("atelier_build", "UserQuestionSender", UserQuestionSender);
