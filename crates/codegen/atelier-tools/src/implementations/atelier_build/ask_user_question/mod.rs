...
/// params are injected, `ATELIER_ASK_USER_QUESTION_TIMEOUT_SECS` (positive
/// integer seconds) still overrides this default directly —
/// e.g. `ATELIER_ASK_USER_QUESTION_TIMEOUT_SECS=8` for tests / TUI repro.
...
pub const RESPONSE_TIMEOUT_ENV: &str = "ATELIER_ASK_USER_QUESTION_TIMEOUT_SECS";
...
crate::register_resource!("atelier_build", "AskUserQuestion", AskUserQuestionParams);
...
pub struct QuestionOption {
    ...
    /// Opaque id; hidden from the model. Atelier callers leave it `None`.
...
/// session-owned coordinator (in atelier-shell), which performs an ACP
