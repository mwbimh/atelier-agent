...
use crate::computer::types::{GrepOutputMode, GrepRequest};
...
// Re-export the shared AtelierIntegerSchema from types module
pub use crate::types::AtelierIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("atelier_build", "Grep", GrepParams);
    ...
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Backend(out)) => {
                // A worker returns the already-formatted card atomically. The
                // worker boundary is more important than pretending this path
...
async fn prepare_grep(
    ...
    let (display_cwd, hints_enabled, deny_read_globs, filesystem) = {
        ...
        (
            ...
            res.get::<DenyReadGlobs>()
                .map(|d| d.0.clone())
                .unwrap_or_default(),
            res.get::<FileSystem>().map(|fs| fs.0.clone()),
        )
    };
    ...
    let request = GrepRequest {
        ...
        before_context: input.before_context.filter(|value| *value > 0),
        after_context: input.after_context.filter(|value| *value > 0),
        context: input.context.filter(|value| *value > 0),
        multiline: input.multiline.unwrap_or(false),
        deny_read_globs: deny_read_globs.clone(),
        head_limit: effective_head_limit,
        max_output_bytes,
        max_chars_per_line,
        display_path: cwd_display.clone(),
    };
...
/// Execute one complete grep inside a workspace worker.
///
/// The worker calls this function after the request path has been confined to
/// the worker root. Keeping command construction and result formatting here
/// makes the worker path use the same `rg` flags, output limits, and model
/// facing card as the in-process test path.
pub async fn run_worker_grep(request: GrepRequest) -> anyhow::Result<GrepSearchOutput> {
    let output_mode = match request.output_mode {
    ...
    let config = GrepFormatConfig {
        output_mode,
        effective_head_limit: request.head_limit,
        max_chars_per_line: request.max_chars_per_line,
        max_output_bytes: request.max_output_bytes,
        cwd_display: request.display_path.clone(),
    };
    ...
    for deny in &request.deny_read_globs {
        cmd.arg("--glob").arg(format!("!{deny}"));
    }
    ...
    if let Some(before_context) = request.before_context {
        cmd.arg("-B").arg(before_context.to_string());
    }
    if let Some(after_context) = request.after_context {
    ...
    let read_result = tokio::time::timeout(timeout, async {
        let (stdout_buf, stdout_truncated) = match stdout_pipe {
            Some(stdout_pipe) => {
                read_rg_stdout_capped(stdout_pipe, request.head_limit.saturating_add(1)).await
            }
            None => (Vec::new(), false),
        };
        if stdout_truncated {
    ...
    let (stdout_buf, stdout_truncated, stderr_buf) = match read_result {
        Ok(result) => result,
        Err(_) => {
            let _ = child.start_kill();
            let _ = child.wait().await;
...
mod tests {
    ...
    #[derive(Clone)]
    struct BackendGrepFs {
        calls: std::sync::Arc<std::sync::atomic::AtomicUsize>,
        output: GrepSearchOutput,
    }

    #[async_trait::async_trait]
    impl crate::computer::types::AsyncFileSystem for BackendGrepFs {
        async fn read_file(
            &self,
            _path: &std::path::Path,
        ) -> Result<Vec<u8>, crate::computer::types::ComputerError> {
            Err(crate::computer::types::ComputerError::io("unused in test"))
        }
        ...
        async fn grep(
            &self,
            request: crate::computer::types::GrepRequest,
        ) -> Result<Option<GrepSearchOutput>, crate::computer::types::ComputerError> {
            assert_eq!(request.pattern, "backend_only");
    ...
    #[tokio::test]
    async fn tool_grep_uses_filesystem_search_backend() {
        let calls = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
        let expected = GrepSearchOutput {
            stdout: b"worker-backed grep".to_vec(),
            stderr: Vec::new(),
            exit_code: 0,
            match_count: 1,
            file_matches: Vec::new(),
        ...
        let mut resources = Resources::new();
        resources.insert(Cwd(std::path::PathBuf::from("C:\\atelier-test")));
        resources.insert(crate::types::resources::FileSystem(std::sync::Arc::new(
            BackendGrepFs {
                calls: calls.clone(),
