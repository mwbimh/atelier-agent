...
const RG_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-rg/rg-",
    env!("ATELIER_TOOLS_RG_VER"),
    "-",
    env!("ATELIER_TOOLS_RG_TARGET"),
    ".bin"
));
...
fn resolve_bundled_rg() -> std::io::Result<PathBuf> {
    ...
    let p = crate::util::atelier_home().join("vendor").join(concat!(
        "rg-",
        env!("ATELIER_TOOLS_RG_VER"),
        "-",
        env!("ATELIER_TOOLS_RG_TARGET")
...
/// binary to ~/.atelier/vendor/ and returns that path.
