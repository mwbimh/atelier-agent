...
use crate::implementations::atelier_build::task_output::{
...
use atelier_tool_types::{MultiTaskOutputResult, TaskOutputOutput, WaitMode, WaitTasksToolInput};
...
impl crate::types::tool_metadata::ToolMetadata for WaitTasksTool {
    ...
    fn description_template(&self) -> &str {
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            atelier_tool_types::build_wait_tasks_description(
                &atelier_tool_types::WaitTasksToolNaming {
...
/// [`atelier_tool_types::build_wait_tasks_description`] builder so the CLI and
...
fn wait_tasks_description(
    ...
    atelier_tool_types::build_wait_tasks_description(&atelier_tool_types::WaitTasksToolNaming {
...
impl atelier_tool_runtime::Tool for WaitTasksTool {
    type Args = WaitTasksToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> atelier_tool_protocol::ToolId {
        atelier_tool_protocol::ToolId::new("wait_tasks").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let timeout = crate::implementations::atelier_build::task_output::capped_wait_timeout(
            input.timeout_ms,
        );
