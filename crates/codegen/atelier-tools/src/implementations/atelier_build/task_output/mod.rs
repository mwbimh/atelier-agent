...
use crate::implementations::atelier_build::task::TaskTool;
use crate::implementations::atelier_build::task::backend::SubagentBackendResource;
use crate::implementations::atelier_build::task::types::{
    SubagentSnapshot, SubagentSnapshotStatus,
};
use crate::implementations::atelier_build_concise::BashConciseTool;
...
/// `TaskOutput` cap. Env override: `ATELIER_MAX_WAIT_BLOCK_MS`.
const MAX_WAIT_BLOCK: Duration = Duration::from_secs(600);

fn max_wait_block() -> Duration {
    std::env::var("ATELIER_MAX_WAIT_BLOCK_MS")
        .ok()
...
mod tests {
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::atelier_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::atelier_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::atelier_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::atelier_build::task::types::SubagentEvent,
    ) -> crate::implementations::atelier_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::atelier_build::task::types::SubagentEvent::Query(q) => q,
