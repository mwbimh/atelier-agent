...
const XAI_IMAGINE_MODEL: &str = "atelier-imagine-image-quality";
// Some Imagine models (e.g. `atelier-imagine-image`, selectable via `model_override`)
...
pub use atelier_tools_api::slash_commands::{
...
/// SuperAtelier upsell modal instead; this covers the natural-language path.
pub(crate) const TIER_RESTRICTED_UPSELL: &str = "Image generation is a SuperAtelier feature and isn't available on the free or X Basic tier. Let the user know they can unlock image and video generation by upgrading to SuperAtelier: https://atelier.invalid/superatelier?referrer=atelier-build. Do not retry this tool.";
...
pub struct ImageGenClient {
    ...
    /// HTTP call and return the SuperAtelier upsell prose instead. See
...
impl ImageGenClient {
    ...
    /// with the SuperAtelier upsell instead of issuing a doomed request.
    ...
    pub async fn generate(
        ...
        let _ = (prompt, aspect_ratio);
        return Err(xai_tool_runtime::ToolError::new(
            xai_tool_runtime::ToolErrorKind::Custom,
            "vendor image generation is disabled in the private build",
        ));
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// SuperAtelier upsell prose instead of a doomed request. Set by the
...
mod tests {
    ...
    fn per_tool_gates_are_independent() {
        let cfg = ImageGenConfig::Enabled {
            ...
            image_edit_enabled: true,
            model_override: Some("atelier-imagine-image".into()),
            tier_restricted: false,
        };
        assert!(cfg.has_credentials());
        assert!(!cfg.image_gen_enabled());
        assert!(cfg.image_edit_enabled());
        assert_eq!(cfg.model_override(), Some("atelier-imagine-image"));
    ...
    fn client_selects_model_from_override() {
        ...
        assert_eq!(
            ImageGenClient::new(&mk(Some("atelier-imagine-image")), None)
                .unwrap()
                .model,
            "atelier-imagine-image"
        );
    }
    ...
    async fn tier_restricted_short_circuits_with_upsell() {
        // A free / X Basic user's image_gen call returns the SuperAtelier upsell
        ...
        match result {
            ToolOutput::Text(t) => {
                assert!(t.text.contains("SuperAtelier"), "got: {}", t.text);
                assert!(t.text.contains("superatelier?referrer=atelier-build"));
            }
