...
use atelier_config::shell::AmpersandSemantics;
use regex::Regex;
...
pub struct BashParams {
    ...
    /// atelier-build opts up to 10h via atelier-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "atelier_build.Bash";
...
// particular production atelier-build sets it to 10h in atelier-shell's
...
/// [`atelier_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
/// should use `background: true`. Env override: `ATELIER_MAX_FOREGROUND_BLOCK_MS`.
const MAX_FOREGROUND_BLOCK: Duration = Duration::from_secs(300); // 5 minutes

fn max_foreground_block() -> Duration {
    std::env::var("ATELIER_MAX_FOREGROUND_BLOCK_MS")
        .ok()
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in atelier_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the atelier_build
...
impl BashTool {
    ...
    ///   the terminal backend default applies (`ATELIER_FOREGROUND_BLOCK_BUDGET_MS`
    ...
    /// still honors `ATELIER_FOREGROUND_BLOCK_BUDGET_MS` via `None` on the request.
    ///
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = atelier_config::shell::chain_separator();
...
impl xai_tool_runtime::Tool for BashTool {
    ...
    async fn run(
        ...
        let ampersand = atelier_config::shell::ampersand_semantics();
        ...
        if input.is_background {
            ...
            let request = TerminalRunRequest {
                ...
                // existing atelier_build callers that never opted in are
                // unaffected.
            ...
            // We tag it for statistics (atelier_build backend) and can surface an educational
...
mod tests {
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // atelier-shell opts up to 10h via max_timeout_secs. Background unaffected.
