...
//! (in `atelier-tools`) and the subagent coordinator (in `atelier-shell`).
//!
//! ## Resource types
//!
...
/// defense-in-depth check in `atelier-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "AtelierBuild:run_terminal_cmd" | "AtelierBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
/// the cross-crate drain loop in `atelier-shell` relies on
...
register_resource!("atelier_build", "SubagentEventSender", SubagentEventSender);
...
pub struct MonitorEventNotification {
    ...
    /// events inside another session's turn. `None` for legacy / non-atelier-build
...
register_resource!("atelier_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("atelier_build", "SessionIdResource", SessionIdResource);

/// Carries the current parent prompt/turn ID for TaskTool subagent scoping.
///
/// Set by atelier-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by atelier-shell at turn start.
...
register_resource!("atelier_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("AtelierBuild:run_terminal_cmd", ToolKind::Execute),
                tc("AtelierBuild:read_file", ToolKind::Read),
                tc("AtelierBuild:list_dir", ToolKind::List),
                tc("AtelierBuild:grep", ToolKind::Search),
                tc("AtelierBuild:kill_task", ToolKind::KillTaskAction),
                tc(
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("AtelierBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("AtelierBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
