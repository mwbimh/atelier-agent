...
/// object (mirroring `atelier/mcp_tool`). Consumers deserialize it into
/// [`CanonicalToolMeta`].
pub const TOOL_META_KEY: &str = "atelier/tool";
...
impl ToolKind {
    ...
    /// `atelier/tool`. Exhaustive, so a new `ToolKind` must add a label to compile.
...
///   share it (atelier `read_file` → `"Read"`).
...
///   should read `namespace` loosely (as a string) and, on any `atelier/tool`
...
impl CanonicalToolMeta {
    ...
    /// (`bash_mode`, `backend`, `atelier/mcp_tool`, …).
...
mod tests {
    ...
    fn identity(kind: ToolKind) -> ToolIdentity {
        ToolIdentity {
            tool_kind: kind,
            namespace: ToolNamespace::AtelierBuild,
    ...
    fn namespace_round_trips_snake_case_with_pascal_aliases() {
        ...
        fn wire_and_pascal(ns: ToolNamespace) -> (&'static str, &'static str) {
            match ns {
                ToolNamespace::AtelierBuild => ("atelier_build", "AtelierBuild"),
                ToolNamespace::AtelierBuildConcise => {
                    ("atelier_build_concise", "AtelierBuildConcise")
                }
                ToolNamespace::AtelierBuildHashline => {
                    ("atelier_build_hashline", "AtelierBuildHashline")
                }
