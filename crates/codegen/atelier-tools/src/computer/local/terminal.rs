...
impl LocalTerminalActor {
    ...
    async fn spawn_persistent_command(
        ...
        // Apply SHELL_ENV_OVERRIDES (TERM=dumb, NO_COLOR, ATELIER_AGENT=1, etc.)
        ...
        crate::util::apply_atelier_agent_marker(&mut cmd);
        ...
        #[cfg(target_os = "linux")]
        if atelier_sandbox::should_restrict_child_network() {
            unsafe {
                cmd.pre_exec(|| atelier_sandbox::child_net::install_child_network_filter());
            }
        }
                ...
                && process.exit_status.is_none()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::atelier_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
fn spawn_shell_command(
    ...
    let mut cmd = {
        ...
        // The parent (atelier) process retains network for the LLM API.
    ...
    #[cfg(not(unix))]
    let build_cmd = |with_breakaway: bool| -> std::io::Result<tokio::process::Command> {
        ...
        //     have JOB_OBJECT_LIMIT_BREAKAWAY_OK set — common when atelier-agent
...
mod tests {
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/atelier-sentinel-tty".to_string());
