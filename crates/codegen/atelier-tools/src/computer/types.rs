...
pub trait AsyncFileSystem: Send + Sync {
    ...
    /// Run a content search in the filesystem's execution boundary.
    ///
    /// Local test filesystems may return `None` so callers can exercise the
    /// in-process implementation. A production filesystem that owns a
    /// sandboxed worker should return `Some` and must not delegate the search
    /// back to the host process.
    async fn grep(
        &self,
        _request: GrepRequest,
    ) -> Result<Option<crate::types::output::GrepSearchOutput>, ComputerError> {
        Ok(None)
    }
}

/// Complete request for a sandbox-backed grep operation.
///
/// This is intentionally independent from the model-facing tool input: paths
/// are already resolved by the caller, policy deny globs are already merged,
/// and output budgets are explicit before the request crosses a process
/// boundary.
...
///
/// Serialized over the wire in the `atelier/task/kill` ext response
/// (`atelier-shell::extensions::task::KillTaskResponse`) and deserialized
/// by clients (atelier-pager), so it derives both serde directions.
...
///
/// Implemented by:
/// - `LocalTerminalBackend` (in atelier-tools, spawns processes)
/// - `AcpTerminalBackend` (in atelier-shell, calls ACP protocol)
#[async_trait::async_trait]
