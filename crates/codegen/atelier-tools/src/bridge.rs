//! ToolBridge: adapter that wraps `atelier-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the atelier_build agent and `None` for
...
mod tests {
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + atelier_build's snake_case in one registry
