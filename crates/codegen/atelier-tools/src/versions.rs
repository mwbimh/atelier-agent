...
pub struct ToolVersionEntry {
    /// Fully-qualified tool ID (e.g. `"AtelierBuild:run_terminal_cmd"`).
...
pub struct PresetEntry {
    ...
    /// Keys are fully-qualified tool IDs (e.g. `"AtelierBuild:run_terminal_cmd"`).
...
/// between namespaces (e.g. `AtelierBuild:run_terminal_cmd` vs.
...
pub const TOOL_VERSION_REGISTRY: &[ToolVersionEntry] = &[
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:run_terminal_cmd",
        versions: &[V_CURRENT, V_LEGACY_BASH],
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:read_file",
        versions: &[V_CURRENT, V_LEGACY_READ_FILE],
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:search_replace",
        versions: &[V_CURRENT, V_LEGACY_SEARCH_REPLACE],
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:list_dir",
        versions: &[V_CURRENT, V_LEGACY_LIST_DIR],
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:grep",
        versions: &[V_CURRENT], // Managed but no legacy implementation.
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:kill_task",
        versions: &[V_CURRENT, V_LEGACY_KILL_TASK],
    },
    ToolVersionEntry {
        fq_tool_id: "AtelierBuild:get_task_output",
...
pub const PRESETS: &[PresetEntry] = &[
    ...
    PresetEntry {
        ...
        lifecycle: BehaviorLifecycle::Active,
        tool_defaults: &[
            ("AtelierBuild:run_terminal_cmd", "legacy-0.4.10"),
            ("AtelierBuild:read_file", "legacy-0.4.10"),
            ("AtelierBuild:search_replace", "legacy-0.4.10"),
            ("AtelierBuild:get_task_output", "legacy-0.4.10"),
            ("AtelierBuild:kill_task", "legacy-0.4.10"),
            ("AtelierBuild:list_dir", "legacy-0.4.10"),
        ],
    },
    ...
    PresetEntry {
        ...
        tool_defaults: &[
            ("AtelierBuild:run_terminal_cmd", "current"),
            ("AtelierBuild:read_file", "current"),
            ("AtelierBuild:search_replace", "current"),
            ("AtelierBuild:list_dir", "current"),
            ("AtelierBuild:grep", "current"),
            ("AtelierBuild:kill_task", "current"),
            ("AtelierBuild:get_task_output", "current"),
        ],
    },
];
...
mod tests {
    ...
    fn current_preset_resolves_to_current() {
        let v = resolve_version("current", "AtelierBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_resolves_ported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "AtelierBuild:run_terminal_cmd", None).unwrap();
    ...
    fn legacy_preset_falls_back_to_current_for_unported_tool() {
        ...
        let v = resolve_version("legacy-0.4.10", "AtelierBuild:grep", None).unwrap();
        assert_eq!(v, Some("current".to_string()));
    }
    ...
    fn unmanaged_tool_returns_none() {
        let v = resolve_version("current", "AtelierBuild:web_search", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn override_on_unmanaged_tool_errors() {
        let err = resolve_version("current", "AtelierBuild:web_search", Some("legacy-0.4.10"))
            .unwrap_err();
    ...
    fn unknown_preset_errors() {
        let err =
            resolve_version("nonexistent", "AtelierBuild:run_terminal_cmd", None).unwrap_err();
    ...
    fn unknown_override_version_errors() {
        let err =
            resolve_version("current", "AtelierBuild:run_terminal_cmd", Some("v999")).unwrap_err();
        assert!(
    ...
    fn concise_namespace_not_managed() {
        // AtelierBuildConcise tools should NOT be version-managed.
        assert!(!is_version_managed("AtelierBuildConcise:run_terminal_cmd"));
        let v = resolve_version("current", "AtelierBuildConcise:run_terminal_cmd", None).unwrap();
        assert_eq!(v, None);
    }

    #[test]
    fn is_version_managed_matches_catalog() {
        assert!(is_version_managed("AtelierBuild:run_terminal_cmd"));
        assert!(is_version_managed("AtelierBuild:read_file"));
        assert!(is_version_managed("AtelierBuild:search_replace"));
        assert!(is_version_managed("AtelierBuild:list_dir"));
        assert!(is_version_managed("AtelierBuild:grep"));
        assert!(is_version_managed("AtelierBuild:kill_task"));
        // Not managed:
        assert!(!is_version_managed("AtelierBuild:todo_write"));
    }
    ...
    fn active_version_produces_no_warning() {
        let res = resolve_version_with_warnings("current", "AtelierBuild:run_terminal_cmd", None)
            .unwrap();
        assert!(
    ...
    fn active_bundle_produces_no_bundle_level_warning() {
        let res =
            resolve_version_with_warnings("legacy-0.4.10", "AtelierBuild:run_terminal_cmd", None)
                .unwrap();
        assert!(
    ...
    fn deprecated_version_produces_tool_warning() {
        ...
        let (version, warnings) =
            validate_and_resolve("AtelierBuild:run_terminal_cmd", "current").unwrap();
        ...
        let err = validate_and_resolve("AtelierBuild:grep", "legacy-0.4.10").unwrap_err();
    ...
    fn resolve_with_warnings_unmanaged_returns_none_no_warnings() {
        let res =
            resolve_version_with_warnings("current", "AtelierBuild:web_search", None).unwrap();
    ...
    fn resolve_with_warnings_unsupported_version_errors_with_list() {
        let err =
            resolve_version_with_warnings("current", "AtelierBuild:grep", Some("legacy-0.4.10"))
                .unwrap_err();
        assert!(
            err.contains("is not supported for tool"),
        ...
        assert!(
            err.contains("AtelierBuild:grep"),
