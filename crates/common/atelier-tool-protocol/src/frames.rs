...
pub struct ToolCallResult {
    ...
    /// this crate must not depend on `atelier-tool-runtime`. Sampler-side wire
...
/// `atelier_tool_hub_core::registry::ToolSessionBindOutcome`, with one
...
pub struct ToolsListResult {
    pub tools: Vec<atelier_tool_types::ToolDescription>,
}
...
mod tests {
    ...
    /// Drain and turn-state fields remain on the wire after remote upload
    /// queue fields are removed.
    #[test]
    fn tool_server_status_payload_carries_local_drain_fields_only() {
        let payload = super::ToolServerStatusPayload {
