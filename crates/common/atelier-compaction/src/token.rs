...
//!
//! - Atelier chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `AtelierTurn::get_num_tokens`.
//! - atelier-build estimates with `bytes / 4`.
//!
...
//! (Atelier chat: `ItemTokenCounter<Arc<AtelierTurn>>`), so exactly one mechanism
//! is in play.
