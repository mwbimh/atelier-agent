...
// The `Shared` summarizer reuses atelier-build's full-replace summarization core
...
/// `FullReplace` strategy (default): atelier-build's full-replace — summarize the
...
/// `<atelier_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching atelier-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize the whole conversation through atelier-build's shared core.
    ...
    //     Shared with Atelier chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<atelier_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<atelier_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<atelier_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): atelier-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<atelier_user_queries>` preamble so
...
/// — atelier-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and atelier-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // atelier-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // atelier-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Atelier chat's
...
/// the literal error messages produced by the Atelier chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Atelier chat sampler when the
    ...
    /// — guards the forwarding that Atelier chat (`Arc<AtelierTurn>`) relies on.
    #[test]
