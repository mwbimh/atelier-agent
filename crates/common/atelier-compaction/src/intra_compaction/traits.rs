...
pub enum CompactionTarget {
    ...
    /// with a single summary — atelier-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Atelier chat's
/// `StreamProcessor` (`Item = Arc<AtelierTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<AtelierTurn>`) already satisfies it.
