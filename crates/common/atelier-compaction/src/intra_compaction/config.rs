...
///
/// - `FullReplace` (default): atelier-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from atelier-shell's
...
/// time (token threshold only, matching atelier-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as atelier-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<atelier_user_queries>` preamble prepended to the history compaction
...
pub const DEFAULT_COMPACTION_MODEL_NAME: &str = "atelier-4.20";
