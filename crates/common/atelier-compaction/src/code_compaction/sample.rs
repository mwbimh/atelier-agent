...
//!
//! The canonical `sample → classify → retry` loop, used by **both** atelier-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Atelier chat's intra `Shared` summarizer
...
//! the winning summary (atelier-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the atelier-build host uses to step down its input size).
