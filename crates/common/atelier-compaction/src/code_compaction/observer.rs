//! Observability seam for the full-replace (atelier-build) pass.
//!
//! The shared orchestrator reports per-attempt and terminal outcomes through
//! this trait so each harness can emit its own telemetry (atelier-build:
...
//!
//! Emission points are part of the behavior contract: the atelier-build observer
