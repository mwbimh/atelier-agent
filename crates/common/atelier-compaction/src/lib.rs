...
//! stay in each product host (for example `atelier-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `atelier-sampling-types`. It is decoupled from both Atelier chat and
//! atelier-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — atelier-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Atelier chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Atelier chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Atelier chat and
//! atelier-build; hosts still own snapshotting and host-only sections).
...
// atelier-build's full-replace subsystem now lives under `code_compaction`;
