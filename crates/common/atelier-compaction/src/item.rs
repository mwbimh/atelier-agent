...
//! harness implements [`CompactionItem`] for its `AtelierTurn`;
//! atelier-build implements it for `atelier_sampling_types::ConversationItem`.
//!
...
//! `<atelier_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `AtelierRole` (Atelier chat) and the
/// `ConversationItem` variants (atelier-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Atelier chat) — maps to System on harnesses without a
...
/// extraction (`<atelier_file id=".." name=".." />` lines in the
/// `<atelier_user_queries>` preamble).
...
///
/// Implementors:
/// - Atelier chat: `AtelierTurn`
/// - atelier-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Atelier chat's
    /// `AtelierTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Atelier chat: a
    ...
    /// strips their `<atelier_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<atelier_file>` lines in the
    /// `<atelier_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Atelier chat: a `Developer` turn with category
    ...
    /// requests/results (Atelier chat: keep only `Channel` contents of an
...
/// constructs the typed harness items that make up atelier-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Atelier chat's
/// `AtelierTurn`; adding these constructors to it as required methods would break
/// that impl. They are also atelier-build-specific (Atelier chat's tail-keep path
...
///
/// The atelier-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Atelier chat stores turns as `Arc<AtelierTurn>`).
