...
//!
//! Behavior is byte-for-byte identical for Atelier chat (`T = Arc<AtelierTurn>`).
...
///   summaries so their `<atelier_user_queries>` sections can be split out.
...
/// Extracts **all** `<atelier_user_queries>...</atelier_user_queries>` blocks
...
pub fn split_prior_compaction_text(text: &str) -> (Option<String>, String) {
    let start_tag = "<atelier_user_queries>";
    let end_tag = "</atelier_user_queries>";
...
/// Extract a `<atelier_user_queries>` XML block from `User` items in `turns`.
///
/// Walks `turns`, finds `User` items, and formats each as a `<atelier_query>`
/// element with text content (from [`CompactionItem::text`]) and any
/// `<atelier_file id="..." name="..." />` lines for the item's attachment
...
pub fn extract_user_queries_from_turns<T: CompactionItem>(
    ...
    let mut result = String::from("<atelier_user_queries>\n");
    ...
    result.push_str("</atelier_user_queries>");
    Some(result)
}

/// Walk `turns`, find any prior compaction summary items, extract their
/// `<atelier_user_queries>` blocks via [`split_prior_compaction_text`], and
/// concatenate them.
///
...
pub struct SeparatedHistoryTurns<T> {
    /// `turns` with the `<atelier_user_queries>` block stripped from every
    ...
    /// Concatenation of every `<atelier_user_queries>` block found (in
    ...
    /// regardless of whether it contained a `<atelier_user_queries>` block.
...
/// `<atelier_user_queries>` block (preserved verbatim for the next summary)
...
///   loop, so the LLM never sees `<atelier_user_queries>` from earlier rounds.
...
///   would see the prior `<atelier_user_queries>` and tend to copy it into the
...
pub fn separate_prior_user_queries<T: CompactionItemBuilder>(
    ...
    for turn in turns {
        if turn.is_compaction_summary() {
            ...
            // `<atelier_user_queries>` block (and therefore stripped to an empty
...
mod tests {
    ...
    fn extract_user_queries_wraps_single_user_turn() {
        ...
        assert!(out.starts_with("<atelier_user_queries>"));
        assert!(out.ends_with("</atelier_user_queries>"));
        assert!(out.contains("<atelier_query>hello world</atelier_query>"));
    }
    ...
    fn extract_prior_user_queries_concatenates_blocks() {
        let inner =
            "<atelier_user_queries>\n<atelier_query>first</atelier_query>\n</atelier_user_queries>";
        let inner2 = "<atelier_user_queries>\n<atelier_query>second</atelier_query>\n</atelier_user_queries>";
    ...
    fn separate_strips_user_queries_from_summary_item() {
        let prior = "<atelier_user_queries>\n<atelier_query>Q1</atelier_query>\n</atelier_user_queries>\n\n<chunk_summary index=\"0\">S1</chunk_summary>";
        ...
        assert!(prior.contains("<atelier_user_queries>"));
        ...
        match &sep.turns_for_llm[0] {
            MockItem::Developer { text, summary } => {
                ...
                assert!(!text.contains("<atelier_user_queries>"));
