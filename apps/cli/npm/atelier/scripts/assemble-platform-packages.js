...
//   1. Brotli-compresses the built binary into `../atelier-<platform>/bin/<bin>.br`
...
// (`@atelier/atelier`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw atelier binary is
...
const atelierRoot = process.env.ATELIER_ROOT
    || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.join(atelierRoot, 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `atelier-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] atelier-${platform}-${arch}@${VERSION}: ` +
...
async function main() {
    const targets = [
        {
            platform: 'darwin', arch: 'arm64', binName: 'ate',
            envVar: 'ATELIER_DARWIN_ARM64',
            defaultSource: path.join(atelierRoot, 'target', 'aarch64-apple-darwin', 'release-dist', 'ate'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'ate',
            envVar: 'ATELIER_DARWIN_X64',
            defaultSource: path.join(atelierRoot, 'target', 'x86_64-apple-darwin', 'release-dist', 'ate'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'ate',
            envVar: 'ATELIER_LINUX_X64',
            defaultSource: path.join(atelierRoot, 'target', 'x86_64-unknown-linux-gnu', 'release-dist', 'ate'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'ate',
            envVar: 'ATELIER_LINUX_ARM64',
            defaultSource: path.join(atelierRoot, 'target', 'aarch64-unknown-linux-gnu', 'release-dist', 'ate'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'ate.exe',
            envVar: 'ATELIER_WIN32_X64',
            defaultSource: path.join(atelierRoot, 'target', 'x86_64-pc-windows-msvc', 'release-dist', 'ate.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'ate.exe',
            envVar: 'ATELIER_WIN32_ARM64',
            defaultSource: path.join(atelierRoot, 'target', 'aarch64-pc-windows-msvc', 'release-dist', 'ate.exe'),
        },
    ];
